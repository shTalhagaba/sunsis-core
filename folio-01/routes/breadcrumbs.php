...
Breadcrumbs::for('students.training.index', function ($trail) {
    $trail->parent('home');
    $trail->push('Training Records', route('students.training.index'), ['icon' => '<i class="ace-icon fa fa-graduation-cap"></i>']);
});

// Home > Students > [Student Name] > Training Record
Breadcrumbs::for('students.training.show', function ($trail, $student, $training_record) {
    $trail->parent('students.show', $student);
    $trail->push('Training Record', route('students.training.show', [$student, $training_record]), ['icon' => '<i class="ace-icon fa fa-graduation-cap"></i>']);
});

// Home > Students > [Student Name] > Training Record > Create Evidence
Breadcrumbs::for('students.training.evidences.create', function ($trail, $student, $training_record) {
    $trail->parent('students.training.show', $student, $training_record);
    $trail->push('Create Evidence', route('students.training.evidences.create', [$student, $training_record]), ['icon' => '<i class="ace-icon fa fa-file-text"></i>']);
});

// Home > Students > [Student Name] > Training Record > Resubmit Evidence
Breadcrumbs::for('students.training.evidences.resubmit', function ($trail, $student, $training_record, $evidence) {
    $trail->parent('students.training.show', $student, $training_record);
    $trail->push('Resubmit Evidence', route('students.training.evidences.resubmit', [$student, $training_record, $evidence]), ['icon' => '<i class="ace-icon fa fa-file-text"></i>']);
});

// Home > Students > [Student Name] > Training Record > Assess Evidence
Breadcrumbs::for('students.training.evidences.assess', function ($trail, $student, $training_record, $evidence) {
    $trail->parent('students.training.show', $student, $training_record);
    $trail->push('Assess Evidence', route('students.training.evidences.assess', [$student, $training_record, $evidence]), ['icon' => '<i class="ace-icon fa fa-file-text"></i>']);
});

// Home > Students > [Student Name] > Training Record > IQA Evidence
Breadcrumbs::for('students.training.evidences.iqa', function ($trail, $student, $training_record, $evidence) {
    $trail->parent('students.training.show', $student, $training_record);
    $trail->push('IQA Evidence', route('students.training.evidences.iqa', [$student, $training_record, $evidence]), ['icon' => '<i class="ace-icon fa fa-file-text"></i>']);
});

// Home > Students > [Student Name] > Training Record > Map Evidence
Breadcrumbs::for('students.training.evidences.mapping', function ($trail, $student, $training_record, $evidence) {
    $trail->parent('students.training.show', $student, $training_record);
    $trail->push('Map Evidence', route('students.training.evidences.mapping', [$student, $training_record, $evidence]), ['icon' => '<i class="ace-icon fa fa-file-text"></i>']);
});

// Home > Students > [Student Name] > Training Record > Signoff Progress
Breadcrumbs::for('students.training.signoffProgress', function ($trail, $student, $training_record, $portfolio) {
    $trail->parent('students.training.show', $student, $training_record);
    $trail->push('Signoff Progress', route('students.training.signoffProgress', [$student, $training_record, $portfolio]));
});

// Home > Students > [Student Name] > Training Record > Add Portfolio
Breadcrumbs::for('students.training.portfolios.show', function ($trail, $student, $training_record) {
    $trail->parent('students.training.show', $student, $training_record);
    $trail->push('Add Portfolio', route('students.training.portfolios.show', [$student, $training_record]));
});

// Home > Students > [Student Name] > Training Record > Training plans
Breadcrumbs::for('students.training.training_plans.edit', function ($trail, $student, $training_record) {
    $trail->parent('students.training.show', $student, $training_record);
    $trail->push('Training Plans', route('students.training.training_plans.edit', [$student, $training_record]));
});

// Home > Students > [Student Name] > Training Record > Edit
Breadcrumbs::for('students.training.edit', function ($trail, $student, $training_record) {
    $trail->parent('students.training.show', $student, $training_record);
    $trail->push('Edit Training Record', route('students.training.edit', [$student, $training_record]));
});
...
Breadcrumbs::for('organisations.employers.show', function ($trail, $organisation) {
    $trail->parent('employers.index');
    $trail->push($organisation->legal_name, route('organisations.employers.show', $organisation));
});

// Home > Employers > [Employer Name] > Edit
Breadcrumbs::for('employers.edit', function ($trail, $organisation) {
    $trail->parent('organisations.employers.show', $organisation);
...
// Home > Support
Breadcrumbs::for('support.tickets.index', function ($trail) {
    $trail->parent('home');
    $trail->push('Support Tickets', route('support.tickets.index'));
});

// Home > Support > Create
Breadcrumbs::for('support.tickets.create', function ($trail) {
    $trail->parent('support.tickets.index');
    $trail->push('Create', route('support.tickets.create'));
});

// Home > Support > [Ticket ID]
Breadcrumbs::for('support.tickets.show', function ($trail, $ticket) {
    $trail->parent('support.tickets.index');
    $trail->push($ticket->id, route('support.tickets.show', $ticket));
});
...
Breadcrumbs::for('students.training.otj.create', function ($trail, $student, $training_record) {
    $trail->parent('students.training.show', $student, $training_record);
    $trail->push('Create OTJH entry', route('students.training.otj.create', [$student, $training_record]), ['icon' => '<i class="ace-icon fa fa-file-text"></i>']);
});

// Home > Students > [Student Name] > Training Record > Create Review
Breadcrumbs::for('students.training.reviews.create', function ($trail, $student, $training_record) {
    $trail->parent('students.training.show', $student, $training_record);
    $trail->push('Create Review', route('students.training.reviews.create', [$student, $training_record]), ['icon' => '<i class="ace-icon fa fa-file-text"></i>']);
});
