{!! Form::open(['url' => route('students.index'), 'class' => 'form-horizontal', 'method' => 'GET']) !!}
...
<div class="row">
	<div style="float: none; padding-top: 5px;" class="col-sm-12 center-block">
		<div class="row">
			...
			<div class="col-sm-4">
				<div class="form-group row {{ $errors->has('employer') ? 'has-error' : ''}}">
					{!! Form::label('employer', 'Employer', ['class' => 'col-sm-4 control-label no-padding-right']) !!}
					<div class="col-sm-8">
						{!! Form::select('employer', \App\Models\LookupManager::getEmployersDDL(\App\Models\Organisations\Organisation::TYPE_EMPLOYER, true), $filters->employer, ['class' => 'form-control']) !!}
						{!! $errors->first('employer', '<p class="text-danger">:message</p>') !!}
					</div>
				</div>
			</div>
        </div>
        <div class="row">
            ...
            <div class="col-sm-4">
                <div class="form-group row {{ $errors->has('ethnicity') ? 'has-error' : ''}}">
					{!! Form::label('ethnicity', 'Ethnicity', ['class' => 'col-sm-4 control-label no-padding-right']) !!}
					<div class="col-sm-8">
						{!! Form::select('ethnicity', \App\Models\LookupManager::getEthnicities(), $filters->ethnicity, ['class' => 'form-control', 'placeholder' => '']) !!}
