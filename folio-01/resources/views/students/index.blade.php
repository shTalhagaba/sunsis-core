...
<div class="page-header"><h1>Students</h1></div><!-- /.page-header -->
    ...
    <div class="col-xs-12">
        ...
        <div class="table-responsive">
            <table id="tblUsers" class="table table-striped table-bordered table-hover">
                <thead>
                    <tr>
                        <th>Name</th><th>Employer</th><th>System Access</th><th>Last Login</th><th>Training Records</th><th>ULN</th><th>NI</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr class="" onclick="window.location.href='{{ route('students.show', $student) }}';"
                        ...
                        <td>
                            {{ $student->surname }}, {{ $student->firstnames }}
                            <br><span class="ace-icon fa fa-user"></span> {{ $student->email }}
                            @if ($student->isOnline())
                        ...
                        <td>{{ $student->employer->legal_name }}<br>{{ $student->location->address_line_1 }},
                            {{ $student->location->postcode }}</td>
                        <td align="center">{!! $student->web_access == '1' ? '<i class="fa fa-check green fa-lg"></i>' : '<i class="fa fa-remove red fa-lg"></i>' !!}</td>
                        <td>
                            {{ $student->lastLoginAt() }}<br>
                            {{ $student->lastLoginIp() }}
                        </td>
