<?php
class view_wellbeing_assessment_form_learner implements IUnauthenticatedAction
{
    public function execute(PDO $link)
    {
        ...
        if(trim($id) != '' && trim($key) != '')
        {
            if(!OnboardingHelper::isValidWellbeingAssessmentFormUrl($link, $id, $key))
            {
        ...
        $assessment = DAO::getObject($link, "SELECT * FROM ob_learner_wellbeing_assessment WHERE tr_id = '{$tr->id}'");
        if(!isset($assessment->tr_id))
        {
            $assessment = new stdClass();
            $records = DAO::getSingleColumn($link, "SHOW COLUMNS FROM ob_learner_wellbeing_assessment");
        ...
        include_once('tpl_view_wellbeing_assessment_form_learner.php');
    }
}