<?php
class read_contractholder implements IAction
{
    public $id = NULL;
    public function execute(PDO $link)
    {
        ...
        if($id == '')
            throw new Exception('Missing querystring argument: contract holder id');

        $vo = ContractHolder::loadFromDatabase($link, $id);
        if(is_null($vo))
            throw new Exception('Contract Holder record not found.');

        $this->id = $id;

        $_SESSION['bc']->add($link, "do.php?_action=read_contractholder&id={$vo->id}", "View Contract Holder");

        $locations_count = DAO::getSingleValue($link, "SELECT COUNT(*) FROM locations WHERE locations.organisations_id = '{$vo->id}'");
        $learners_count = 0;//DAO::getSingleValue($link, "SELECT COUNT(*) FROM tr WHERE tr.contract_id = '{$vo->id}'");
        ...
        $repository = Repository::getRoot().'/contractholders/'.$vo->id;
        ...
        include_once('tpl_read_contractholder.php');
    }
...
HEREDOC;
    ...
    private function renderHS(PDO $link, ContractHolder $vo)
    {
        $sql = <<<HEREDOC
SELECT
  health_safety.*,
...
HEREDOC;
        ...
        if($st)
        {
            ...
            while($row = $st->fetch())
            {
                echo HTML::viewrow_opening_tag('do.php?_action=edit_health_and_safety&id=' . $row['location_id'] . '&back=read_contractholder&organisation_id=' . $row['organisations_id']);
                echo '<td align="left">';
...
HTML;
    ...
    public function renderFileRepository(PDO $link, ContractHolder $vo)
    {
        $repository = Repository::getRoot().'/contractholders/'.$vo->id;
