		...
		if($vo->funding_stream == Framework::FUNDING_STREAM_SFA)
		{
			...
			if($st)
			{
				...
				while($row = $st->fetch())
				{
					...
					$frame[$LearnAimRef]['24+'] = $value24;
					if($Non_App == '')
						$Non_App = DAO::getSingleValue($link, "SELECT RateWeighted FROM lars201516.`Core_LARS_Funding` WHERE FundingCategory = 'Matrix' AND LearnAimRef = '$LearnAimRef' ORDER BY EffectiveFrom DESC LIMIT 0,1;");
					$frame[$LearnAimRef]['ER_Other'] = $Non_App;
