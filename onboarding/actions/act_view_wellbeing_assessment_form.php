<?php
class view_wellbeing_assessment_form implements IAction
{
	public function execute(PDO $link)
	{
        ...
        $assessment = DAO::getObject($link, "SELECT * FROM ob_learner_wellbeing_assessment WHERE tr_id = '{$id}'");
		...
		$_SESSION['bc']->add($link, "do.php?_action=view_wellbeing_assessment_form&id={$id}", "View Wellbeing Assessment");
		...
		require_once('tpl_view_wellbeing_assessment_form.php');
	}
    ...
    public function save_sign_form(PDO $link)
    {
        ...
        $assessment = DAO::getObject($link, "SELECT * FROM ob_learner_wellbeing_assessment WHERE tr_id = '{$tr_id}'");
        if(!isset($assessment->tr_id))
        {
            throw new Exception("Invalid ID");
        }
        ...
        try
        {
            ...
            DAO::saveObjectToTable($link, "ob_learner_wellbeing_assessment", $save_object);

            $provider_signatures_log = (object)[
                'entity_id' => $save_object->tr_id,
                'entity_type' => 'ob_learner_wellbeing_assessment',
