<?php
class view_pre_iag_form_learner implements IAction
{
	public function execute(PDO $link)
	{
        ...
        $assessment = DAO::getObject($link, "SELECT * FROM ob_learner_pre_iag_form WHERE tr_id = '{$id}'");
	    ...
	    $_SESSION['bc']->add($link, "do.php?_action=view_pre_iag_form_learner&id={$id}", "View Pre IAG Form");
		...
		require_once('tpl_view_pre_iag_form_learner.php');
	}
	...
	$assessment = DAO::getObject($link, "SELECT * FROM ob_learner_pre_iag_form WHERE tr_id = '{$tr_id}'");
        if(!isset($assessment->tr_id))
        {
            throw new Exception("Invalid ID");
        }
	    ...
	    // users are completing and not signing
            ...
            DAO::saveObjectToTable($link, "ob_learner_pre_iag_form", $save_object);

            $provider_signatures_log = (object)[
                'entity_id' => $save_object->tr_id,
                'entity_type' => 'ob_learner_pre_iag_form',
