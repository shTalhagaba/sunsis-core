<?php
class read_department implements IAction
{
    public $id = NULL;
    public function execute(PDO $link)
    {
        ...
        if($id == '')
            throw new Exception('Missing querystring argument: Department id');

        $vo = Department::loadFromDatabase($link, $id);
        if(is_null($vo))
            throw new Exception('Department record not found.');

        $this->id = $id;

        $_SESSION['bc']->add($link, "do.php?_action=read_department&id={$vo->id}", "View Department");

        $subdepts_count = DAO::getSingleValue($link, "SELECT COUNT(*) FROM sub_departments WHERE linked_dept_id = '{$vo->id}'");
        ...
        $location = $vo->getMainLocation($link);

        $repository = Repository::getRoot().'/departments/'.$vo->id;
        ...
        include_once('tpl_read_department.php');
    }
...
SELECT
	...
	CASE
		...
		ELSE "<img src='/images/notstarted.gif' border='0'> </img>"
	END AS `compliant`,
	(SELECT COUNT(*) FROM tr WHERE tr.provider_location_id = locations.id) AS learners
FROM
	locations
WHERE
...
FROM
	tr INNER JOIN users ON tr.username = users.username
WHERE tr.contract_id='$this->id'
...
HEREDOC;
    ...
    private function renderHS(PDO $link, Department $vo)
    {
        $sql = <<<HEREDOC
SELECT
  health_safety.*,
...
HEREDOC;
        ...
        if($st)
        {
            ...
            while($row = $st->fetch())
            {
                echo HTML::viewrow_opening_tag('do.php?_action=edit_health_and_safety&id=' . $row['location_id'] . '&back=read_department&organisation_id=' . $row['organisations_id']);
                echo '<td align="left">';
...
HTML;
    ...
    private function renderSubDepartments(PDO $link)
    {
        $sql = <<<HEREDOC
SELECT * FROM sub_departments WHERE linked_dept_id = '{$this->id}' ORDER BY id;
HEREDOC;
        /* @var $result pdo_result */
        $st = $link->query($sql);
        if($st)
        {
            ...
            while($row = $st->fetch())
            {
                ...
                $delete = '';
                if($_SESSION['user']->isAdmin() || in_array($_SESSION['user']->type, [User::TYPE_ADMIN]))
                    $edit = '<span class="btn btn-primary btn-xs" onclick="window.location.href=\'do.php?_action=edit_sub_department&organisations_id='.$row['linked_dept_id'].'&id='.$row['id'].'\'"><i class="fa fa-edit"></i> Edit</span>';
                if(($_SESSION['user']->isAdmin() || in_array($_SESSION['user']->type, [User::TYPE_ADMIN])))
                    $delete = '<span class="btn btn-danger btn-xs pull-right" onclick="deleteRecord(\'sub_department\', \'\', \''.$row['id'].'\', \''.$row['dept_code'].'\');"><i class="fa fa-trash"></i> Delete</span>';
                $code = $row['dept_code'];
                $name = $row['dept_name'];
                $pm_name = $row['pm_name'];
                $pm_telephone = $row['pm_telephone'];
	...
	<div class="box box-primary">
		<div class="box-body box-profile">
		    <dl class="dl-vertical">
                ...
                <dt>Programme Manager Name:</dt><dd>{$pm_name}</dd>
                <dt>Programme Manager Telephone:</dt><dd>{$pm_telephone}</dd>
                <dt>Programme Manager Email:</dt><dd>{$pm_email}</dd>
            </dl>   
		</div>
		<div class="box-footer">
			$edit
...
HTML;
    ...
    public function renderFileRepository(PDO $link, Department $vo)
    {
        $repository = Repository::getRoot().'/departments/'.$vo->id;
