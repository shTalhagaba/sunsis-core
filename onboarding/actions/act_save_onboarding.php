...
class save_onboarding implements IUnauthenticatedAction
{
    public function execute(PDO $link)
    {
        ...
        if(trim($id) != '' && trim($key) != '')
        {
            if(!OnboardingHelper::isValidOnboardingUrl($link, $id, $key))
            {
...
SQL;
    ...
    private function saveExtraFields(PDO $link, TrainingRecord $tr, $data)
    {
        ...
        $extra_info->employment_contract = isset($data['employment_contract']) ? $data['employment_contract'] : null;
        $extra_info->employment_start_date = isset($data['employment_start_date']) ? $data['employment_start_date'] : null;
