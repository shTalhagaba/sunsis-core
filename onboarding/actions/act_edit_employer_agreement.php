...
class edit_employer_agreement implements IAction
{
    public function execute(PDO $link)
    {
        ...
        $employerRepsDDL = DAO::getResultset($link, "SELECT DISTINCT contact_id, CONCAT(
            contact_name, 
            IF(
            ...
            ) AS contact_name, null FROM organisation_contacts WHERE org_id = '{$agreement->employer_id}' ORDER BY contact_name");
        $financeContactsDDL = DAO::getResultset($link, "SELECT DISTINCT contact_id, CONCAT(
            contact_name, 
            IF(
                job_role IS NOT NULL, 
                (SELECT CONCAT(' - ', description) FROM lookup_crm_contact_job_roles WHERE lookup_crm_contact_job_roles.id = organisation_contacts.`job_role`),
            ...
            ) AS contact_name, null FROM organisation_contacts WHERE org_id = '{$agreement->employer_id}' AND job_role = '4' ORDER BY contact_name");
        $levyContactsDDL = DAO::getResultset($link, "SELECT DISTINCT contact_id, CONCAT(
            contact_name, 
            IF(
                job_role IS NOT NULL, 
                (SELECT CONCAT(' - ', description) FROM lookup_crm_contact_job_roles WHERE lookup_crm_contact_job_roles.id = organisation_contacts.`job_role`),
            ...
            ) AS contact_name, null FROM organisation_contacts WHERE org_id = '{$agreement->employer_id}' AND job_role = '5' ORDER BY contact_name");
