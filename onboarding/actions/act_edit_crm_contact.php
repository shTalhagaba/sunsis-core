...
class edit_crm_contact implements IAction
{
	public function execute(PDO $link)
	{
		...
		$job_roles_ddl = DAO::getResultset($link, "SELECT id, description, null FROM lookup_crm_contact_job_roles ORDER BY description");

		if($contact_id == '')
		{
			$vo = new OrganisationContact($org_id);
		}
		else
		{
			$vo = OrganisationContact::loadFromDatabase($link, $contact_id);
		}

		$organisation = Organisation::loadFromDatabase($link, $vo->org_id);
		$org_main_location = $organisation->getMainLocation($link);
        ...
        $departments_ddl = DAO::getResultset($link, "SELECT contact_department, contact_department, null FROM organisation_contacts WHERE org_id = '{$organisation->id}' ORDER BY contact_department");
