<?php
class read_systemowner implements IAction
{
    public $id = NULL;
	public function execute(PDO $link)
	{
		$vo = SystemOwner::loadFromDatabase($link);
		if(!$vo) {
			throw new Exception("Could not locate the record for the System Owner ");
		}

		$_SESSION['bc']->index=0;
		$_SESSION['bc']->add($link, "do.php?_action=read_systemowner", "View System Owner");
        ...
        $repository = Repository::getRoot().'/systemowner/'.$vo->id;
        ...
        // Presentation
		include('tpl_read_systemowner.php');
	}
...
HEREDOC;
    ...
    public function renderFileRepository(PDO $link, SystemOwner $vo)
    {
        $repository = Repository::getRoot().'/systemowner/'.$vo->id;
