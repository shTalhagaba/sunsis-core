...
class ajax_lrs extends ActionController
{
    ...
    public function indexAction(PDO $link)
    {
        ...
        $this->certificate = DB_NAME == 'am_ela' ? WEBROOT . '/lib/LRS/ElaLrs.pem' : WEBROOT . '/lib/LRS/superdrug.pem';
        $this->vendorID = '01';
        $this->language = 'ENG';
    }
    ...
    public function getLearnerLearningEventsAction(PDO $link)
    {
        ...
        if(isset($response_ls['status']) && ($response_ls['status'] == 'WSRC0004' || $response_ls['status'] ==  'WSRC0003'))
        {
		...
		if(count($learningEvents) == 0)
                ...
                foreach($learningEvents AS $learningEvent)
                {
                    $events[] = [
                        ...
                        'Subject' => preg_replace('/[^\x20-\x7E]/', '', $learningEvent->getSubject()),
...
//$options = stream_context_get_options($service->_stream_context);
//pre($options);
            $fault = [
                'SOAP_faultcode' => $e->faultcode,
                'SOAP_faultstring' => $e->faultstring,
    ...
    {
        DAO::execute($link, "DELETE FROM lrs_learner_learning_events WHERE tr_id = '{$tr_id}' AND sunesis_core = 0");
