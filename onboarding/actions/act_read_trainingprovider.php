...
class read_trainingprovider implements IAction
{
    ...
    public function execute(PDO $link)
    {
        ...
        if($id == '')
            throw new Exception('Missing querystring argument: training provider id');
        ...
        $_SESSION['bc']->add($link, "do.php?_action=read_trainingprovider&id={$vo->id}", "View Training Provider");

        $locations_count = DAO::getSingleValue($link, "SELECT COUNT(*) FROM locations WHERE locations.organisations_id = '{$vo->id}'");
        $learners_count = DAO::getSingleValue($link, "SELECT COUNT(*) FROM ob_tr WHERE ob_tr.provider_id = '{$vo->id}'");
        ...
        $repository = Repository::getRoot().'/trainingproviders/'.$vo->id;
        ...
        include_once('tpl_read_trainingprovider.php');
    }
...
HTML;
    ...
    private function renderLearners(PDO $link)
    {
        $sql = <<<HEREDOC
SELECT
	users.id, users.username, (SELECT COUNT(*) FROM ob_tr WHERE ob_tr.username = users.username) AS trs
FROM
    ob_tr INNER JOIN users ON ob_tr.username = users.username
WHERE ob_tr.provider_id='$this->id'
ORDER BY ob_tr.firstnames;
HEREDOC;
        $st = $link->query($sql);
        if($st)
        {
...
SELECT
	...
	(SELECT organisations.`legal_name` FROM organisations INNER JOIN locations ON organisations.`id` = locations.`organisations_id` WHERE locations.id = users.`department` ) AS department,
	(SELECT COUNT(*) FROM logins WHERE logins.username = users.username) AS logins, (SELECT DATE_FORMAT(logins.date, '%d/%m/%Y %H:%i:%s') FROM logins WHERE logins.username = users.username ORDER BY logins.date DESC LIMIT 1) AS last_login
FROM
...
HEREDOC;
        ...
        if($st)
        {
            ...
            while($row = $st->fetch())
            {
                ...
                $address .= $user->work_postcode != '' ? '<i class="fa fa-map-marker text-green"></i> ' . $user->work_postcode . '<br>' : '<br>';
                ...
                $department_name = $row['department'] != '' ? '<span class="text-bold">Department: </span>' . $row['department'] : '';
...
HEREDOC;
    ...
    private function renderHS(PDO $link, TrainingProvider $vo)
    {
        $sql = <<<HEREDOC
SELECT
  health_safety.*,
...
HEREDOC;
        ...
        if($st)
        {
            ...
            while($row = $st->fetch())
            {
                echo HTML::viewrow_opening_tag('do.php?_action=edit_health_and_safety&id=' . $row['location_id'] . '&back=read_trainingprovider&organisation_id=' . $row['organisations_id']);
                echo '<td align="left">';
...
HTML;
    ...
    public function renderFileRepository(PDO $link, TrainingProvider $vo)
    {
        $repository = Repository::getRoot().'/trainingproviders/'.$vo->id;
