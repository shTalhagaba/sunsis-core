	...
	public function execute(PDO $link) {
		...
		if ( $_SESSION['user']->isAdmin() || (DB_NAME=="am_lead" && $_SESSION['user']->type == User::TYPE_MANAGER))
		{
			if($filter_username != '')
			{
				...
				$case = $this->client->get_entry_list($this->session_id, 'Cases',' `cases`.`created_by` = "ad7f7f13-2dcf-21c8-b931-4c8e1f495088" and `cases`.`name` = "Support request: '.$filter_user_complete_name.' of '.$_SESSION['user']->org->legal_name.'" ', ' `cases`.`status` desc, `cases`.`date_entered` desc ',0,'',1500);
			}
			else
				$case = $this->client->get_entry_list($this->session_id, 'Cases',' `cases`.`created_by` = "ad7f7f13-2dcf-21c8-b931-4c8e1f495088" and `cases`.`name` like "Support request: % of '.$_SESSION['user']->org->legal_name.'" ', ' `cases`.`status` desc, `cases`.`date_entered` desc ',0,'',1500);
		}
		else
			$case = $this->client->get_entry_list($this->session_id, 'Cases',' `cases`.`created_by` = "ad7f7f13-2dcf-21c8-b931-4c8e1f495088" and `cases`.`name` = "Support request: '.$_SESSION['user']->firstnames.' '.$_SESSION['user']->surname.' of '.$_SESSION['user']->org->legal_name.'" ', ' `cases`.`status` desc, `cases`.`date_entered` desc ',0,'',1500);
		...
		foreach( $case->entry_list as $field ) {
			...
			if ( array_key_exists($this_client_case{'status'}, $case_type_groups) ) {
				...
				if ( $case_type_groups{$this_client_case{'status'}} == 'Requiring Feedback' || $case_type_groups{$this_client_case{'status'}} == 'Requiring Confirmation' ) {
