...
class Address
{
	...
	private function _formatEditBs7666($compulsory = false)
	{
		...
		$paon_start_number = htmlspecialchars($this->paon_start_number);
		$paon_start_suffix = htmlspecialchars($this->paon_start_suffix);
		$paon_end_number = htmlspecialchars($this->paon_end_number);
		$paon_end_suffix = htmlspecialchars($this->paon_end_suffix);
		$paon_description = htmlspecialchars($this->paon_description);

		$saon_start_number = htmlspecialchars($this->saon_start_number);
		$saon_start_suffix = htmlspecialchars($this->saon_start_suffix);
		$saon_end_number = htmlspecialchars($this->saon_end_number);
		$saon_end_suffix = htmlspecialchars($this->saon_end_suffix);
		$saon_description = htmlspecialchars($this->saon_description);

		$street_description = htmlspecialchars($this->street_description);
		$locality = htmlspecialchars($this->locality);
		$town = htmlspecialchars($this->town);
		$county = htmlspecialchars($this->county);
		$postcode = htmlspecialchars($this->postcode);
...
HEREDOC;
	...
	{
		$class = $compulsory ? 'compulsory':'optional';
		$address_line_1 = htmlspecialchars($this->address_line_1);
		$address_line_2 = htmlspecialchars($this->address_line_2);
		$address_line_3 = htmlspecialchars($this->address_line_3);
		$address_line_4 = htmlspecialchars($this->address_line_4);
		$postcode = htmlspecialchars($this->postcode);
...
<div class="Address4Line">
	<div><span class="fieldLabel_$class">Building No./Name &amp; Street</span></div>
	<input class="$class" type="text" name="{$this->prefix}address_line_1" value="$address_line_1" size="40" maxlength="100">
</div>
<div class="Address4Line">
...
<div class="Address4Line">
	<div><span class="fieldLabel_$class">Town / City / Borough</span></div>
	<input class="$class" type="text" name="{$this->prefix}address_line_3" value="$address_line_3" size="40" maxlength="100">
	<span class="button" id="btn_fill_borough" onclick="fillBorough();">Auto-Fill</span>
</div>
<div class="Address4Line">
...
HTML;
	...
	private function _formatReadBs7666()
	{
		...
		if ($this->postcode != '') {
			...
			$addr .= '<a href="http://maps.google.co.uk/maps?f=q&hl=en&q=' . urlencode($this->postcode) . '" target="_blank">'
				. htmlspecialchars($this->postcode) . '</a>';
		}
	...
	{
		return '<address>'
			. '<saon_start_number>'.htmlspecialchars($this->saon_start_number).'</saon_start_number>'
			. '<saon_start_suffix>'.htmlspecialchars($this->saon_start_suffix).'</saon_start_suffix>'
			. '<saon_end_number>'.htmlspecialchars($this->saon_end_number).'</saon_end_number>'
			. '<saon_end_suffix>'.htmlspecialchars($this->saon_end_suffix).'</saon_end_suffix>'
			. '<saon_description>'.htmlspecialchars($this->saon_description).'</saon_description>'
			. '<paon_start_number>'.htmlspecialchars($this->paon_start_number).'</paon_start_number>'
			. '<paon_start_suffix>'.htmlspecialchars($this->paon_start_suffix).'</paon_start_suffix>'
			. '<paon_end_number>'.htmlspecialchars($this->paon_end_number).'</paon_end_number>'
			. '<paon_end_suffix>'.htmlspecialchars($this->paon_end_suffix).'</paon_end_suffix>'
			. '<paon_description>'.htmlspecialchars($this->paon_description).'</paon_description>'
			. '<street_description>'.htmlspecialchars($this->street_description).'</street_description>'
			. '<locality>'.htmlspecialchars($this->locality).'</locality>'
			. '<town>'.htmlspecialchars($this->town).'</town>'
			. '<county>'.htmlspecialchars($this->county).'</county>'
			. '<postcode>'.htmlspecialchars($this->postcode).'</postcode>'
			. '</address>';
	}
	...
	{
		return '<address>'
			. '<address_line_1>' . htmlspecialchars($this->address_line_1) . '</address_line_1>'
			. '<address_line_2>' . htmlspecialchars($this->address_line_2) . '</address_line_2>'
			. '<address_line_3>' . htmlspecialchars($this->address_line_3) . '</address_line_3>'
			. '<address_line_4>' . htmlspecialchars($this->address_line_4) . '</address_line_4>'
			. '<postcode>' . htmlspecialchars($this->postcode) . '</postcode>'
			. '</address>';
	}
