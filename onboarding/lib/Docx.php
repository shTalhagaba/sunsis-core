...
class Docx {
	...
	public function extract_docx() {
		...
		// does the file exist
		$full_filepath = $this->read_location."/".$this->filename;
	...
	public function update_docx($tag = '', $replacement = '' ) {
		...
		if ( $tag != '' ) {
			$this->docx_xml = str_replace($tag, $replacement, $this->docx_xml);
			return 1;
		}
		return;
	}
	...
	public function bundle_docx() {
		...
		foreach ($this->fileList as $f) {
			...
			$zip_pathname = str_replace($this->temp_location."/", "", $f);
	...
	static function load_learner_templates() {
		...
		$handle = @opendir($learner_location);
 		while ( false !== ($file = readdir($handle)) ) {
        	if ( $file == '.' || $file == '..' || is_dir("$learner_location/$file") ) continue;
