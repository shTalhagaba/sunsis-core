...
class FrameworkQualification extends Entity
{
	public static function loadFromDatabase(PDO $link, $qualification_id, $framework_id, $internaltitle)
	{
		...
		$sql = "SELECT * FROM framework_qualifications WHERE id = '$qualification_id' and framework_id='$framework_id' and internaltitle='".addslashes($internaltitle)."';";
		$st = $link->query($sql);
		if($st)
		{
			if($row = $st->fetch())
...
SQL;
	...
	{
		$qualification_id = addslashes($this->id);
		$framework_id = addslashes($this->framework_id);
...
HEREDOC;
	...
	public function toXML($prefix = null, $namespace = null)
	{
		...
		$xml = "<{$p}qualification $xmlns "
			...
			.$p.'proportion="'.htmlspecialchars($this->proportion).'" '
