...
class Repository
{
	 ...
	 * @throws Exception
	 */
	public static function processFileUploads($form_field_name, $target_directory, array $valid_extensions = array(), $max_individual_file_size = 0, $system_file_name = '')
	{
		...
		for($i = 0; $i < count($field['error']); $i++)
		{
			...
			switch($field['error'][$i])
			{
				...
				case UPLOAD_ERR_FORM_SIZE:
					throw new Exception("$filename exceeded the maximum upload size" . $_FILES[$form_field_name]['size']);
					break;
