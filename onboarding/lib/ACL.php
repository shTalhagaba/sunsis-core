...
class ACL
{
	...
	public static function loadFromDatabase(PDO $link, $resource_category, $resource_id)
	{
		...
		$rcat = addslashes($resource_category ?? '');
		$rid = addslashes($resource_id ?? '');
		$sql = <<<HEREDOC
SELECT
	privilege, ident
FROM
...
HEREDOC;
	...
	public function save(PDO $link)
	{
		...
		$rcat = addslashes($this->resource_category);
		$rid = addslashes($this->resource_id);
		...
		if(count($this->identities) > 0)
		{
			...
			foreach($this->identities as $privilege=>$entries)
			{
				foreach($entries as $entry)
				{
					...
					$sql .= " ('$rcat', '$rid', '".addslashes($privilege)."', '".addslashes($entry)."') ";
				}
			}
			if($sql != '')
			{
	...
	public function isAuthorised(User $user, $privilege)
	{
		...
		if( ($this->resource_id == '') || ($this->resource_category == '') || $user->isAdmin() || $user->username == 'perspective')
		{
			return true;
		}
...
HEREDOC;
			...
			$this->employees = array_filter($this->employees_and_wildcards, create_function('$var', 'return strpos($var[0],\'*\')===false;'));
			$this->employee_wildcards = array_filter($this->employees_and_wildcards, create_function('$var', 'return strpos($var[0],\'*\')!==false;'));
...
HEREDOC;
		...
		if($filter_include != '')
		{
			...
			// Trim array
			$filter_include = array_filter($filter_include, create_function('&$var', '$var=trim($var);return strlen($var) > 0;'));
		...
		foreach($this->identities as $privilege=>$entries)
		{
			foreach($entries as $id)
			{
				...
				if(strpos($id, '*') !== false)
				{
					...
					{
						$loc = addslashes($matches[1]);
						$org = addslashes($matches[2]);
...
HEREDOC;
				...
				else
				{
					if(preg_match('#^([^/]+)/([^/]+)/([^/]+)$#', $id, $matches))
					{
						...
						$user = addslashes($matches[1]);
						$loc = addslashes($matches[2]);
						$org = addslashes($matches[3]);
...
HEREDOC;
	...
	public static function expandWildcards(PDO $link, $identities)
	{
		...
		// Strip empty strings
		$identities = array_filter($identities, create_function('&$var', '$var=trim($var);return strlen($var) > 0;'));
...
HEREDOC;
	...
	public static function filterIdentities($identities, $filters)
	{
		...
		// Strip empty strings
		$callback = create_function('&$var', '$var=trim($var);return strlen($var) > 0;');
