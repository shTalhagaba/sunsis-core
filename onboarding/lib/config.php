...
// location constants
define('SOURCE_BLYTHE_VALLEY', in_array($_SERVER['REMOTE_ADDR'], array('80.195.116.88','80.195.116.89','80.195.116.90','92.30.151.1','94.125.128.205','92.236.163.70', '185.39.250.38', '92.238.144.108')));
...
define('SOURCE_HOME', in_array($_SERVER['REMOTE_ADDR'], ['::1', '185.39.248.38', '82.47.154.162']));
...
if (isset($_SERVER['PERSPECTIVE_DATA_ROOT'])) {
	...
	define('DATA_ROOT', "/var/www/html/data");
}
...
/*			if (isset($GLOBALS['link'])) {
			...
			$usr = isset($_SESSION['user']) ? "'".addslashes($_SESSION['user']->username)."'" : "NULL";
	...
	if(IS_AJAX)
	{
		if(headers_sent())
		{
			...
			echo '<php:error xmlns:php="http://php.net">'
				.htmlspecialchars($file.'('.$line.'): '.$message)
				.'</php:error>';
			exit(0);		
		}
		else
		{
			...
			echo "<line>" . htmlspecialchars($line) . "</line>";
			echo "<code>" . htmlspecialchars($code) . "</code>";
			echo "<trace>" . htmlspecialchars($trace) . "</trace>";
			if (SOURCE_LOCAL || SOURCE_BLYTHE_VALLEY) {
				echo "<extra_info>" . htmlspecialchars($extra_info) . "</extra_info>";
			}
			echo "</error>";
			exit(0);
		}
	...
	else
	{
		if(headers_sent())
		{
			...
			echo '<div style="border:3px red solid;color:red;padding:5px;">'
				. '<font size="3"><b>' . htmlspecialchars($message) . '</b></font>';
			if(SOURCE_LOCAL || SOURCE_BLYTHE_VALLEY){
				echo "<br/><font size=\"3\">@$file($line)</font><br/>";
				echo '<font size="1">' . wordwrap(htmlspecialchars($trace), 60, "&#8203;", 1) . '</font>'; // &#8203; is a zero-length space (helps mozilla break long words)
			}
			echo '</div>';
			exit(0);
		}
		else
		{
			...
			$trace = preg_replace('/\((\d+)\)/', '(<span style="font-weight:bold;">\1</span>)', htmlspecialchars($trace));
...
function email_support($code, $message, $file, $line, $trace, $extra_info = '')
{
	...
	$to = "k.khan@sunesis.co,inaam.azmat@perspective-uk.com";
...
HEREDOC;
    ...
    $full_message = htmlspecialchars($full_message);
    $partial_message = htmlspecialchars($partial_message);
    ...
    if(strlen($extra_info) < 5120)
    {
    	...
    	$extra_info = str_replace("\t", "&nbsp;&nbsp;&nbsp;", nl2br(htmlspecialchars($extra_info)));
    }
    else
    {
