...
HEREDOC;
            ...
            $options = array(
                ...
                0=>array(1, '', null, 'ORDER BY student_qualifications.title'),
...
//			echo $this->getViewNavigator('left');
            ...
            /*			echo '<thead><tr><th>&nbsp;</th><th>Title</th><th>Start Date</th><th>End Date</th><th>Total Units</th><th>Not Started</th><th>Behind</th><th>On Track</th><th>Under Assessment</th><th>Completed</th><th>No Status</th></tr></thead>';
                        ...
                        while($row = $fetch())
                        {
                            ...
                            echo HTML::viewrow_opening_tag('do.php?_action=view_student_qualifications&id=' . rawurlencode($row['id']) . '&tr_id=' . rawurlencode($this->tr_id));
            ...
            elseif(DB_NAME=="am_lcurve")
                ...
                echo '<thead><tr><th colspan=8>Qualifications</th><th class="unit_status" style="display: none;" colspan=5>Unit Status</th><th class="additional_dates" style="display: none;" colspan=4>Additional Dates<span class="button" button type="button" style="display: none;" onclick="saveAdditionalDates();">Save</span></th><th colspan=4>Progress</th><th colspan=3>Navigate to</th><th colspan=1>Action</th></tr>';
   ...
   */
            ...
            while($row = $st->fetch())
            {
                ...
                //echo HTML::viewrow_opening_tag('do.php?_action=read_student_qualification&qualification_id=' . rawurlencode($row['id']).'&internaltitle='.rawurlencode($row['internaltitle']).'&framework_id='.rawurlencode($row['framework_id']).'&tr_id='.rawurlencode($this->tr_id));
                if($row['aptitude']==1)
                ...
                echo '<td align="center">' . htmlspecialchars($row['level']) . "</td>";
                ...
                if($row['actual_end_date']!='')
                    echo '<td align="center">' . HTML::cell(htmlspecialchars(date_format(date_create($row['actual_end_date']),"d/m/Y"))) . "</td>";
                else
                    echo '<td align="center">' . htmlspecialchars('') . "</td>";
                if($row['achievement_date']!='')
                    echo '<td align="center">' . HTML::cell(htmlspecialchars(date_format(date_create($row['achievement_date']),"d/m/Y"))) . "</td>";
                else
                    echo '<td align="center">' . htmlspecialchars('') . "</td>";

//				echo '<td align="left">' . htmlspecialchars($row['lsc_learning_aim']) . "</td>";
                echo '<td class="unit_status" style="display: none;" align="center">' . htmlspecialchars($row['units']) . "</td>";
                echo '<td class="unit_status" style="display: none;" align="center">' . htmlspecialchars($unitsnostatus) . "</td>";
                echo '<td class="unit_status" style="display: none;" align="center">' . htmlspecialchars($row['unitsBehind']) . "</td>";
                echo '<td class="unit_status" style="display: none;" align="center">' . htmlspecialchars($row['unitsOnTrack']) . "</td>";
//				echo '<td align="center">' . htmlspecialchars($row['unitsUnderAssessment']) . "</td>";
                echo '<td class="unit_status" style="display: none;" align="center">' . htmlspecialchars($row['unitsCompleted']) . "</td>";
...
//				echo '<td id = "percentageAchievedInput" style="display:none"></td>';
                ...
                if(DB_NAME=='am_doncaster' || DB_NAME=='am_southampton' || DB_NAME=='am_lewisham')
                    ...
                    if($row['aptitude']==1)
                        echo "<td align='center' style='border-right-style: solid;'> <img style='width:35px; height:35px;' src=\"/images/exempt.gif\" border=\"0\" /></td>";
                    else
                        if((int)$target>0 || (int)$row['unitsUnderAssessment']>0)
                            if((int)$row['unitsUnderAssessment']<(int)$target)
                                echo "<td align='center' style='border-right-style: solid;'> <img src=\"/images/red-cross.gif\" border=\"0\" /></td>";
                ...
                $tutors = explode("," , $row['tutor_username']);
                ...
                elseif($_SESSION['user']->isAdmin() || $_SESSION['user']->isOrgAdmin() || $this->isGroupTutor($link, $row['tr_id']) || $this->isGroupVerifier($link, $row['tr_id']) || $this->isGroupAssessor($link, $row['tr_id']) || in_array($_SESSION['user']->id, $tutors) || $_SESSION['user']->id == $row['assessor'] || $_SESSION['user']->id == $row['tutor'] || $_SESSION['user']->type==15 || $_SESSION['user']->type==4 || $_SESSION['user']->type==20 || $_SESSION['user']->type==8 || $_SESSION['user']->type==9 || $_SESSION['user']->type==21)
                {
                    ...
                    echo '<td align=center><a href="do.php?_action=view_tr_qualification_tabular&qualification_id=' . rawurlencode($row['id']).'&internaltitle='.rawurlencode($row['internaltitle']).'&view=COMPACT&framework_id='.rawurlencode($row['framework_id']).'&tr_id='.rawurlencode($this->tr_id).'&target='.rawurlencode($target).'&achieved='.rawurlencode($row['unitsUnderAssessment']) . '"><img src="/images/tabular.jpg" border="0" title="Mark progress through data table" height="25px" width="25px" /></a></td>';
                    if(DB_NAME=="am_ray_recruit" || DB_NAME=="am_demo")
                        echo '<td align=center><a href="do.php?_action=view_tr_qual_portfolio&qualification_id=' . rawurlencode($row['id']).'&internaltitle='.rawurlencode($row['internaltitle']).'&framework_id='.rawurlencode($row['framework_id']).'&tr_id='.rawurlencode($this->tr_id). '"><img src="/images/portfolio_icon.gif" border="0" title="Portfolio" height="25px" width="25px" /></a></td>';
                }
                else
                {
                    //		echo '<td align=center><img src="/images/read.jpg" title="View qualification tree" border="0" height="25px" width="25px" /></a></td>';
                ...
                //echo '<td align=center><a href="do.php?_action=view_tr_qualification_tabular2&qualification_id=' . rawurlencode($row['id']).'&internaltitle='.rawurlencode($row['internaltitle']).'&framework_id='.rawurlencode($row['framework_id']).'&tr_id='.rawurlencode($this->tr_id).'&target='.rawurlencode($target).'&achieved='.rawurlencode($row['unitsUnderAssessment']) . '"><img src="/images/tabular.jpg" border="0" title="Mark progress through data table" height="25px" width="25px" /></a></td>';
                ...
                if(DB_NAME != "am_reed_demo" && DB_NAME != "am_reed")
                {
                    if($_SESSION['user']->isAdmin() || $_SESSION['user']->type==8)
                    {
                        if($flag=="NF")
                            if(DB_NAME!='am_doncaster' && DB_NAME!='am_southampton' && DB_NAME!='am_lewisham')
                                echo '<td align=center><a href="do.php?_action=add_aim_to_ilr&qualification_id=' . rawurlencode($row['id']).'&submission='.rawurlencode($submission).'&contract_id='.rawurlencode($contract_id).'&tr_id='.rawurlencode($this->tr_id).'&target='.rawurlencode($target).'&achieved='.rawurlencode($row['unitsUnderAssessment']) . '">Add to ILR</a></td>';
                ...
                else
                {
                    if($_SESSION['user']->isAdmin() || $_SESSION['user']->type == User::TYPE_MANAGER)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if($row['actual_end_date'] == '')
                                    echo '<td align=center><img title="Close aim with learning aim end information" style="cursor: pointer;" width="30" height="30" src="images/stop_aim.png" onclick="closeAim(\'' . $row['id'] . '\', \'' . rawurlencode($submission) . '\', \'' . rawurlencode($contract_id) . '\', \'' . rawurlencode($this->tr_id) . '\', \'' . rawurlencode($target) . '\', \'' . rawurlencode($row['unitsUnderAssessment']) . '\', \''. $row['start_date']. '\');" /></td>';
                                elseif($row['actual_end_date'] != '' && strtolower(substr( str_replace('/', '', $row['id']), 0, 4)) != "z000")
                                {
                                    if($row['achievement_date'] == '' && $row['compstatus'] != '3' && $row['compstatus'] != '6')
                                        echo '<td align=center><img title="Enter aim achievement information" style="cursor: pointer;" src="images/rosette.gif" onclick="achieveAim(\'' . $row['id'] . '\', \'' . rawurlencode($submission) . '\', \'' . rawurlencode($contract_id) . '\', \'' . rawurlencode($this->tr_id) . '\', \'' . rawurlencode($target) . '\', \'' . rawurlencode($row['unitsUnderAssessment']) . '\', \''. $row['actual_end_date']. '\');" /></td>';
                                    else
                                        echo '<td>&nbsp;</td>';
                                }
                                else
