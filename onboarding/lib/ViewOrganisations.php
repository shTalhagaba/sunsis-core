<?php
class ViewOrganisations extends View
{

	public static function getInstance($organisation_type)
	{
		$key = 'view_'.__CLASS__.$organisation_type;
	...
	where organisations.organisation_type like '%$organisation_type%';
HEREDOC;
	
			$view = $_SESSION[$key] = new ViewOrganisations();
			$view->setSQL($sql);
	...
	public function render(PDO $link,$organisation_type)
	{
		...
		if($st) 
		{
			...
			while($row = $st->fetch())
			{
				echo HTML::viewrow_opening_tag('/do.php?_action=read_organisation&id=' . $row['id'] . '&organisation_type=' . $organisation_type);
			...
			/*	$org_type = $row['org_type_id'];
				$que = "select org_type from lookup_org_type where id='$org_type'";
				$organisation_type = trim(DAO::getSingleValue($link, $que));
				echo '<td align="left">' . HTML::cell($organisation_type) . '</td>';
			*/
