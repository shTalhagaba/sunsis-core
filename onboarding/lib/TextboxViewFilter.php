<?php
class TextboxViewFilter implements IViewFilter
{
	...
	public function toHTML()
	{
		...
		$html = <<<HEREDOC
<input type="text" id="$html_id" name="$html_id" value="$value" $attr />
<script type="text/javascript">document.getElementById('$html_id').resetToDefault = function() { this.value='$def';}</script>
HEREDOC;
	...
	public function getSQLStatement()
	{
		if($this->value != '')
		{
			...
			// 10/08/2011 - re 
			//		- need to monitor as there is no history why it had been commented out
			
			return new SQLStatement(sprintf($this->sql_format, addslashes($this->value)));				
			// return new SQLStatement(sprintf($this->sql_format, $this->value));
		}
		else
		{
			return null;
