	...
	public function toXML()
	{
		...
		foreach($objectProperties as $property)
		{
			if($property->isPublic())
			{
				...
				{
					$xml .= '<value>'.htmlspecialchars($value).'</value>';
				}
				$xml .= '</property>';
			}
		}
	...
	public function toXFDF($pdfURL,$extra=null)
	{
		...
		if ( $extra )
		{
			...
			foreach( $extra as $key => $value)
			{
				...
				$xml .= '<field name="'.$key.'">';
				$xml .= '<value>'.utf8_encode(htmlspecialchars($value)).'</value>';
				$xml .= '</field>';
			}
		...
		foreach($objectProperties as $property)
		{
			if($property->isPublic())
			{
				...
				if(is_array($value))
				{
					foreach($value as $v)
					{
						$xml .= '<value>'.utf8_encode(htmlspecialchars($v)).'</value>';
					}
				}
				elseif(is_object($value))
				{
