@php
    $dynamicAcceptableAnswersCount = old('dynamic_acceptable_answers_count', $question->acceptable_answers ? count($question->acceptable_answers): 1);
    $dynamicMcqOptionsCount = old('dynamic_mcq_options_count', count($options));
    $type = old('type', 'descriptive');
    $showMcq = (isset($question) && $question->type === 'multiple_choice') || $type === 'multiple_choice' ? true : false;
@endphp
{!! Form::hidden('dynamic_acceptable_answers_count', $dynamicAcceptableAnswersCount) !!}
{!! Form::hidden('dynamic_mcq_options_count', $dynamicMcqOptionsCount) !!}

<div class="widget-box">
    <div class="widget-header">
        <h4 class="smaller">Edit Course Details</h4>
    </div>
    <div class="widget-body">
        <div class="widget-main">
            ...
            <div class="alert alert-warning text-center bolder">
                ...
                This question has already been used in learners' tests. Editing its details may cause inconsistencies in existing test records. 
                Please proceed with caution. You can always deactivate the question and create a new one.
            </div>
            @endif
            ...
            <div class="form-group row required {{ $errors->has('question_order') ? 'has-error' : '' }}">
                ...
                <div class="col-sm-8">
                    {!! Form::number('question_order', null, [
            ...
            <div class="form-group row {{ $errors->has('question_image') ? 'has-error' : '' }}">
                ...
                <div class="col-sm-3">
                    ...
                    @include('partials.file_media_well', ['fileMedia' => $question->media()->first()])
                    <br>
                    @endif
                </div>
            </div>
            <div id="correctAnswerContainer"
                ...
                <div class="form-group row {{ $errors->has('acceptable_answers') ? 'has-error' : '' }}">
                    ...
                    <div class="col-sm-8">
                        {!! Form::text('acceptable_answers[]', $question->acceptable_answers[0] ?? '', ['class' => 'form-control', 'maxlength' => 255]) !!}                        
                    </div>
                </div>
                        ...
                        <div class="form-group row ">
                            ...
                            <div class="col-sm-8">
                                <input class="form-control" maxlength="255" name="acceptable_answers[]"
                                    ...
                                    value="{{ old('acceptable_answers.' . $i) ?? $question->acceptable_answers[$i] }}">
                            </div>
                        </div>
                    @endfor
                </div>
                        ...
                        <table class="table table-bordered" id="tblMcqOtions">
                            ...
                            <tbody>
                                @for ($i = 1; $i <= $dynamicMcqOptionsCount; $i++)
                                    <tr>
                                        <td>
                                            {!! Form::text("options[{$i}][text]", $options[$i]["option_text"] ?? null, ['class' => 'form-control', 'maxlength' => 255]) !!}
                                            {!! Form::hidden("options[{$i}][id]", $options[$i]["id"] ?? null) !!}
                                        </td>
                                        <td class="text-center">
                                            <div class="radio">
                                                <label>
                                                    <input type="radio" name="correct_option" class="ace input-lg" 
                                                        value="{{ $i }}" {{ old('correct_option') == $i || (isset($options[$i]) && $options[$i]["is_correct"]) ? 'checked' : '' }}>
                                                    <span class="lbl"> </span>
                                                </label>
                                            </div>
                                        </td>
