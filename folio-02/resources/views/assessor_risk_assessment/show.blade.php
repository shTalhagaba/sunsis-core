...
@section('title', 'View Assessor Risk Assessment')

@section('page-content')
    <div class="page-header">
        <h1>
            View Assessor Risk Assessment
        </h1>
    </div><!-- /.page-header -->
    <div class="row">
        <div class="col-xs-12">
            ...
            <button class="btn btn-sm btn-white btn-default btn-round" type="button"
                onclick="window.location.href='{{ route('assessor_risk_assessment.index') }}'">
                <i class="ace-icon fa fa-times bigger-110"></i> Close
            </button>
            @if (!$riskAssessment->isCompleted() && (auth()->user()->isAdmin() || auth()->user()->created_by == $riskAssessment->created_by))
            <button class="btn btn-sm btn-primary btn-round" type="button"
                onclick="window.location.href='{{ route('assessor_risk_assessment.edit', ['riskAssessment' => $riskAssessment]) }}'">
                <i class="ace-icon fa fa-edit bigger-110"></i> Edit
            </button>
            {!! Form::open([
                'method' => 'DELETE',
                'url' => route('assessor_risk_assessment.destroy', $riskAssessment),
                'id' => 'frmDeleteRiskAssessment',
                'style' => 'display: inline;',
                'class' => 'form-inline',
            ]) !!}
            {!! Form::hidden('risk_assessment_id_to_del', $riskAssessment->id) !!}
            {!! Form::button('<i class="ace-icon fa fa-trash-o bigger-110"></i> Delete', [
                'data-rel' => 'tooltip',
                'class' => 'btn btn-danger btn-xs btn-round',
                'type' => 'click',
                'id' => 'btnDeleteRiskAssessment',
            ]) !!}
            {!! Form::close() !!}
            <button class="btn btn-sm btn-primary btn-round" type="button"
                onclick="window.location.href='{{ route('assessor_risk_assessment.editForm', ['riskAssessment' => $riskAssessment]) }}'">
                <i class="ace-icon fa fa-edit bigger-110"></i> Assessment Form
            </button>
            @endif
                        ...
                        <div class="widget-body">
                            <div class="widget-main">
                                ...
                                <div class="form-group row {{ $errors->has('completed') ? 'has-error' : '' }}">
                                    ...
                                    <div class="col-sm-8">
                                        {{ (isset($form['completed']) && $form['completed'] == '1') ? 'Yes' : 'No' }}
                                    </div>
                                </div>
                            </div>
                        </div>
...
@push('after-scripts')
    <script>
        $("button#btnDeleteRiskAssessment").on('click', function(e) {
            e.preventDefault();
