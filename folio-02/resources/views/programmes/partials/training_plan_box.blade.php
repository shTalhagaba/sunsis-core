            ...
            <div class="dd" id="nestable{{ $_plan->plan_number }}">
                ...
                <ol class="dd-list">
                    @php
                        ...
                        $units = \App\Models\Programmes\ProgrammeQualificationUnit::with('qualification')->whereIn('id', $unit_ids)->get();
                    @endphp
                    ...
                    <li class="dd-item" data-id="{{ $unit->id }}" style="cursor: pointer;">
                        <div class="dd-handle"
                         data-rel="popover"
                         data-trigger="hover"
                         data-original-title="QAN: {{ $unit->qualification->qan }}, Unit: {{ $unit->unique_ref_number }}"
                         data-content="<small><strong>Q:</strong> {{ $unit->qualification->title }}<br><strong>U:</strong> {{ $unit->title }}</small>"
                         data-placement="auto">
                            QAN: {{ $unit->qualification->qan }}, Unit: {{ $unit->unique_ref_number }}
