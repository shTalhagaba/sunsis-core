        ...
        <div class="col-xs-12">
            ...
            <div class="row">
                <div class="col-sm-8">
                    ...
                    {!! Form::model($plan, [
                        'method' => 'PATCH',
                        'url' => route('iqa_sample_plans.update', $plan),
                        ...
                        <div class="widget-body">
                            <div class="widget-main">
                                ...
                                <div class="form-group row">
                                    {!! Form::label('selectedQuals', 'Selected Qualifications', ['class' => 'col-sm-4 control-label small']) !!}
                                    <div class="col-sm-8">
                                        <table class="table table-bordered small">
                                            <caption class="text-info bolder"><i class="fa fa-info-circle"></i> Previously selected qualifications</caption>
                                            @foreach ($plan->qualifications as $programmeQual)
                                                <tr>
                                                    <td>[{{ $programmeQual->qan }}] {{ $programmeQual->title }}</td>
                                                </tr>
                                            @endforeach
                                        </table>
                                    </div>
    ...
    <script>
        $(function() {
            ...
            $("select[name=programme_id]").on("change", function(e){
                e.preventDefault();
                refreshQuals(this.value);
            });

            $("form[name=frmIqaSamplePlan]").on('submit', function(){
                if($('input[name="qualifications[]"]').filter(':checked').length == 0)
                {
                    bootbox.alert({
                        title: "Error: Incomplete Form",
                        message: 'You have not selected any qualification for this plan.'
                    });
                    return false;
                }
            ...
            function refreshQuals(programmeId)
            {
                ...
                $.ajax({
                    ...
                    type: 'get',
                    data: {programme_id: programmeId, plan_id: {{ $plan->id ?? '' }} }
                }).done(function(data) {
                    var qualsCount = 0;
            ...
            $(function(){
                refreshQuals({{ $plan->programme_id }});
            });
        });
