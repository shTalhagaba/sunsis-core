 ...
 *                      defines                          *
 ********************************************************/
define('VERSION', 'Perspective Ltd.');
...
class baaChart
{
    ...
    public $subtitle;
    public $bgd;
    public $cbgd, $cbgd2;            // chart band colours
    public $txtcol;
    public $white;
    public $black;
    public $gridcol;
    ...
    public $height;
    public $cwidth;
    public $cheight;        // chart dimensions
    public $scolors;
    public $lm, $rm, $tm, $bm;        // margins
    public $ytitle;                // y axis settings
    public $ymin = 0;
    public $ymax = 0;
    public $ygrid = false;
    public $ygridint;
    public $xtitle;                // x axis settings
    public $xgrid = false;
    public $xgridint;
    public $xcount;                // count of values in each series
    public $seriescount;
    public $colwidth;
    public $transparent;
    public $ispiechart;
    ...
    public $xlabels;
    public $data;
    public $xmaxima;                // series max values
    public $xminima;
    public $stypes;                // series types
    public $stacked;                // series stacked?
    public $stackbase;
    public $pic;
    public $clip;
    public $picx;
    ...
    {
        $this->baaChart($awidth, $aheight, $pic, $clip, $picx, $picy);
    }
    ...
    function addDataSeries($type, $stacked, $vals, $legend)
    {
        ...
        //array_pad($d, $this->xcount, 0); // ISS
        $this->data[$n] = $d;
        $this->legends[$n] = $legend == '' ? '_' : $legend;
        $this->showvals[$n] = 0;

        // ISS: Pad all series with the new value of xcount
        foreach ($this->data as &$seriesData) {
            if (count($seriesData) < $this->xcount) {
                $seriesData = array_pad($seriesData, $this->xcount, 0); // pad to the right
            }
        }
    }
        ...
        switch ($this->stypes[$i]) {
            ...
            case 'C':
                ...
                for ($p = 0; $p < $this->xcount; $p++) {
                    ...
                    $x0 = round($this->lm + 5 + $p * $this->xgridint + $colpos * $this->colwidth);
                    $x1 = round($x0 + $this->colwidth);
                    $y0 = round($this->_val2y($this->data[$i][$p], $this->stackbase[$p]));
                    $y1 = round($this->_val2y($this->stackbase[$p], (0 > $this->ymin) ? (0) : ($this->ymin)));
                    if ($stacked) {
    ...
    function _drawVals($posArray)
    {
        ...
        foreach ($this->data as $i => $parray) {
            if (!isset($this->showvals[$i + 1]) || !$this->showvals[$i + 1]) {
                continue;
            }
    ...
    function _drawtitles()
    {
        ...
        $l = strlen($this->title ?: '');
        $tw = $cw * $l;
        $x = round($this->lm + ($this->cwidth - $tw) / 2);
    ...
    function _drawyaxis()
    {
        ...
        for ($y = $y1, $v = $this->ymax, $i = 0; $y < $y0 - 2; $y += $div, $v -= $this->ygridint, $i++) {
            ...
            imageline($this->image, $x0, (int)$y, $grx, (int)$y, $this->gridcol);
            $tw = strlen("$v") * imagefontwidth(2);
            imagestring($this->image, 2, $x0 - $tw - 5, (int)$y - 5, $v, $this->txtcol);
        }
        $tw = (int)strlen("$this->ymin") * imagefontwidth(2);
        $y = (int)$this->tm + $this->cheight;
    ...
    function _drawxaxis()
    {
        ...
        $x0 = (int) $this->lm;
        $y0 = (int) $this->tm + $this->cheight;
        $x1 = (int) $x0 + $this->cwidth;
        $y1 = $y0;
        $div = $this->xgridint;
        imageline($this->image, $x0, $y0, $x1, $y1, $this->gridcol);
        $gry = $this->xgrid ? $this->tm : $this->tm + $this->cheight + 3;
        for ($x = $x1, $i = $this->xcount - 1; $x > $x0 + 3; $x -= $div, $i--) {
            imageline($this->image, (int) $x, (int) $gry, (int) $x, (int) $y0, $this->gridcol);
            $v = $this->xlabels[$i];
            $tw = strlen("$v") * imagefontwidth(2);
            $th = imagefontheight(2);
            if ($maxlabwid < $div) {
                imagestring($this->image, 2, round($x - ($div + $tw) / 2), $y0 + 5, $v, $this->txtcol);
            } else {
                imagestringup($this->image, 2, round($x - ($div + $th) / 2), $y0 + 5 + $tw, $v, $this->txtcol);
            }
        }
        $y = round($this->height - 30);
        $tw = strlen($this->xtitle ?: '') * imagefontwidth(3);
        $x = round(($this->lm + $this->cwidth + $this->lm - $tw) / 2);
    ...
    function _calcymax()
    {
        ...
        if ($this->stackcount == 0) {
            ...
            foreach ($b as $v) {
                #$m += $this->xmaxima[$v];
                for ($i = 0; $i < $this->xcount; $i++) {
                    if (isset($stacktots[$i])) {
                        $stacktots[$i] += (isset($this->data[$v][$i]) ? $this->data[$v][$i] : 0);
                    } else {
                        $stacktots[$i] = (isset($this->data[$v][$i]) ? $this->data[$v][$i] : 0);
                    }
                }
    ...
    function _backgroundImage()
    {
        ...
        switch ($this->pict) {
            case 1:
                $this->bgimage = imagecreatefromgif($this->pic);
                break;
            case 2:
                $this->bgimage = imagecreatefromjpeg($this->pic);
                break;
            case 3:
                $this->bgimage = imagecreatefrompng($this->pic);
                break;
            default :
                $this->pic = '';
                return;
