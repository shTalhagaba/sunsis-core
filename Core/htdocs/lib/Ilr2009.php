...
class Ilr2009 extends Entity
{
	...
	public static function loadFromXML($xml)
	{
		...
		$vo->aims[0] = new Aim2009();
				...
				foreach($ilr->programmeaim as $programmeaim)
				{
					...
					$vo->programmeaim->A09=$programmeaim->A09;
					$vo->programmeaim->A10=$programmeaim->A10;
					$vo->programmeaim->A11=$programmeaim->A11;
		...
		if($st)
		{
			...
			{	
				...
				foreach($ilr->learner as $learner)
				{
					...
					fwrite($handle,$learner->L16);
					fwrite($handle,str_pad($learner->L17,8));
					fwrite($handle,str_pad(substr($learner->L18,0,30),30));
					fwrite($handle,str_pad(substr($learner->L19,0,30),30));
					fwrite($handle,str_pad(substr($learner->L20,0,30),30));
					fwrite($handle,str_pad(substr($learner->L21,0,30),30));
					fwrite($handle,str_pad($learner->L22,8));
					fwrite($handle,str_pad($learner->L23,15));
					fwrite($handle,$learner->L24);
					fwrite($handle,$learner->L25);
	...
	public static function generateStream3(PDO $link, $submission, $contracts, $con1, $L25, $transmission)
	{
		...
		$sql = "SELECT * FROM ilr WHERE submission = '$submission' and contract_id in ($contracts) and is_active=1 ORDER BY substr(ilr,locate('<L09>',ilr)+5,(locate('</L09>',ilr)-locate('<L09>',ilr)-5)), l03, tr_id";
...
*/
					...
					//	print(str_pad($learner->L06,2,'0',STR_PAD_LEFT));
						...
						print(str_pad($l28a,2,'0',STR_PAD_LEFT));
						print(str_pad($l28b,2,'0',STR_PAD_LEFT));
...
//						print(str_pad($learner->L38,2,'0',STR_PAD_LEFT));
				...
				if($learner->L08!="Y")
				{
					...
					{
						if( ($aim->A15!='99' && $aim->A15!='' && $aim->A15!='0') || $aim->A10=='70')
						{
							$no_of_aims++;
...
//							print(str_pad($aim->A55,10,'9',STR_PAD_LEFT));
							...
							foreach($ilr->main as $mmainaim)
							{
								...
								print(str_pad($mmainaim->A64,5,'0',STR_PAD_LEFT));
								print(str_pad($mmainaim->A65,5,'0',STR_PAD_LEFT));
