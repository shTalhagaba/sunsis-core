...
class ViewEvents extends View
{
	...
	public static function getInstance($link,$id)
	{
		...
		if(!isset($_SESSION[$key]))
		{
			...
			$sql = <<<HEREDOC
SELECT 
	events_template.*, student_events.*, DATE_FORMAT(event_date,"%d-%m-%Y") as event_date 
FROM 
	events_template
LEFT JOIN 
	student_events ON student_events.event_id = events_template.id and student_events.tr_id = '$id'
HEREDOC;

			$view = $_SESSION[$key] = new ViewEvents();
			$view->setSQL($sql);
	...
	public function render(PDO $link)
	{
		...
		if($st) 
		{
			...
			echo '<thead><tr><th>&nbsp;</th><th>Event Title</th><th>Status</th><th>Date</th><th>Comments</th></tr></thead>';
