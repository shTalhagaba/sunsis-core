...
class GeoLocation
{
	...
	public function setPostcode($postcode, PDO $link = null)
	{
		...
		if(DAO::schemaEntityExists($link, "postcodes"))
		{
			...
			{
				// Approximate match
				$os = DAO::getObject($link, "SELECT AVG(longitude) AS 'longitude', AVG(latitude) AS 'latitude' FROM postcodes.ukpostcodes WHERE postcode_outward='".addslashes((string)$outward)."' GROUP BY postcode_outward");
			}
			else
			{
				// Precise match
				$os = DAO::getObject($link, "SELECT longitude,latitude FROM postcodes.ukpostcodes WHERE postcode='".addslashes((string)$postcode)."'");
			}
			if($os){
		...
		if(DAO::schemaEntityExists($link, "central", "lookup_postcode_geolocation"))
		{
			...
			{
				// Approximate match
				$os = DAO::getObject($link, "SELECT ROUND(AVG(easting)) AS 'easting', ROUND(AVG(northing)) AS 'northing' FROM central.lookup_postcode_geolocation WHERE outward='".addslashes((string)$outward)."' GROUP BY outward");
			}
			else
			{
				// Precise match
				...
				{
					$os = DAO::getObject($link, "SELECT easting,northing FROM central.lookup_postcode_geolocation WHERE postcode='".addslashes((string)$postcode)."'");
				}
				else
				{
					$os = DAO::getObject($link, "SELECT easting,northing FROM central.lookup_postcode_geolocation WHERE outward='".addslashes((string)$outward)."' AND inward='".addslashes((string)$inward)."'");
				}
			}
			if($os){
				$this->setEastingNorthing($os->easting, $os->northing);
		...
		{
			$os = DAO::getObject($link, "SELECT ROUND(AVG(easting)) AS 'easting', ROUND(AVG(northing)) AS 'northing' FROM central.lookup_postcodes_geolocation WHERE outward='".addslashes((string)$outward)."' GROUP BY outward");
			if($os){
				$this->setEastingNorthing($os->easting, $os->northing);
				$this->postcode = $postcode;
				return true;
