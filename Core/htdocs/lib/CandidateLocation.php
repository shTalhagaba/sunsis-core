<?php
class CandidateLocation extends Entity
{
	public static function loadFromDatabase(PDO $link, $id)
	{
		...
		if($id!='')
		{
			$query = "SELECT * FROM employer_locations WHERE id=" . addslashes((string)$id) . ";";
			$st = $link->query($query);
	...
	public function save(PDO $link)
	{
		...
		$query = "SELECT COUNT(*) FROM employer_locations WHERE organisations_id={$this->organisations_id} AND is_legal_address=1;";
		...
		{
			$query = "UPDATE employer_locations SET is_legal_address=0 WHERE organisations_id=" . $this->organisations_id;
		...
		if($this->id != '')
		{
			...
			$sql = "SELECT short_name FROM employer_locations WHERE id='".$this->id."'";
		...
		return DAO::saveObjectToTable($link, 'employer_locations', $this);		
	}
