...
class WsdlClass extends stdClass implements ArrayAccess,Iterator,Countable
{
     ...
     * @var string
     */
    const VALUE_WSDL_URL = 'https://soapapi.findapprenticeship.service.gov.uk/Services/VacancyManagement/VacancyManagement51.svc?WSDL';
    /**
    ...
    {
        if(class_exists('WSDLClassMap',true))
        {
            $wsdlOptions = array();
            $wsdlOptions['classmap'] = WSDLClassMap::classMap();
    ...
    {
        if(class_exists('SoapClient') && is_subclass_of('SoapClient','SoapClient'))
            return 'SoapClient';
        else
            return 'SoapClient';
    }
