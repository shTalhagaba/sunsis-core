...
class Ilr2023 extends Entity
{
    ...
    public static function generateStream4(PDO $link, $submission, $contracts, $con1, $beta = 0)
    {
        ...
        {
            $table_name = "ilr2023W13";
            DAO::execute($link, "TRUNCATE TABLE ilr2023W13");
            DAO::execute($link, "INSERT INTO {$table_name} SELECT ilr.* FROM ilr INNER JOIN contracts ON ilr.`contract_id` = contracts.`id` WHERE contracts.`contract_year` = '2023' AND ilr.`submission` = 'W13'");
        }
        ...
        if($stouter)
        {
            ...
            $batch_file_xml .= '<Message xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns="ESFA/ILR/2023-24" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="ESFA/ILR/2023-24">';
            $batch_file_xml .= "<Header>";
            $batch_file_xml .= "<CollectionDetails>";
            $batch_file_xml .= "<Collection>ILR</Collection>";
            $batch_file_xml .= "<Year>2324</Year>";	
            ...
            while($rowouter = $stouter->fetch())
            {
                ...
                if($st)
                {
                    while($row = $st->fetch())
                    {
                        ...
                        $record++;
                        $ilr = Ilr2023::loadFromXML($ilr);
                        $funding_type = Ilr2023::FundingType($ilr);
                        if($record==1)
                        {
                            ...
                            $plan_eep_hours_total = array_map('floatval', (array) $plan_eep_hours);
                            $plan_eep_hours = array_sum($plan_eep_hours_total);
                            ...
                            if(DB_NAME=='am_city_skills' or DB_NAME=='am_ela')
                            {
                                $sqlprior = "SELECT * FROM ilr left join contracts on contracts.id = ilr.contract_id WHERE submission = '$submission' and contract_id in ($contracts) and l03 = '$l03' and is_active=1 ORDER BY tr_id limit 1";
                                ...
                                if($stprior)
                                {
                                    while($rowprior = $stprior->fetch())
                                    {
                                        ...
                                        $ilrprior = Ilr2023::loadFromXML($ilrprior);
                           ...
                           */
                            ...
                            if($stemp)
                            {
                                ...
                                while($rowemp = $stemp->fetch())
                                {
                                    ...
                                    $ilremp = Ilr2023::loadFromXML($ilremp);
            ...
            if($exists)
            {
                ...
                if($stdestinations)
                {
                    while($rowdestinations = $stdestinations->fetch())
                    {
                        ...
                        $uln = DAO::getSingleValue($link, "SELECT extractvalue(ilr,'/Learner/ULN') FROM {$table_name} WHERE tr_id = '$tr_id' ORDER BY contract_id DESC, submission DESC LIMIT 0,1");
