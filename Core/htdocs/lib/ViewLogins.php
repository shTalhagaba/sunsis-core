...
HEREDOC;
    ...
    public function exportToCSV(PDO $link, $columns = '', $extra = '', $key = '', $where = '')
    {
        ...
        if ($st) {
            ...
            header('Content-Disposition: attachment; filename=LoginsReport.csv');
            if (strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE') !== false) {
                header('Pragma: public');
                header('Cache-Control: max-age=0');
            }
            echo "Username,Firstnames,Surname,Organisation,User Type,Browser,Platform,IP,Login At,";
            echo "\n";
            while ($row = $st->fetch(DAO::FETCH_ASSOC)) {
                $browser = $this->getBrowser($row['user_agent']);
                echo HTML::csvSafe($row['username']) . ",";
                echo HTML::csvSafe($row['firstnames']) . ",";
                echo HTML::csvSafe($row['surname']) . ",";
                echo HTML::csvSafe($row['organisation_legal_name']) . ",";
                echo HTML::csvSafe($row['user_type']) . ",";
                if (isset($browser['name']))
                    echo isset($browser['version']) ? HTML::csvSafe($browser['name']) . ' ' . HTML::csvSafe($browser['version']) . "," : HTML::csvSafe($browser['name']) . ",";
                else
                    echo ',';
                echo isset($browser['platform']) ? HTML::csvSafe($browser['platform']) . "," : ",";
                echo isset($browser['ip']) ? HTML::csvSafe($browser['ip']) . "," : ",";
                echo Date::to($row['date'], Date::DATETIME) . ",";
                echo "\n";
            }
        } else {
