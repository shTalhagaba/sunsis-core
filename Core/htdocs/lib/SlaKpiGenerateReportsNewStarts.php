<?php
class SlaKpiGenerateReportsNewStarts extends View
{
    ...
    public static function getInstance(PDO $link)
	{
        ...
        {
            $sql ="SELECT year( tr.start_date ) AS year, count( tr.id ) AS learner_count, tr.employer_id, o.legal_name AS employer_name
                    FROM tr
        ...
        {
            $sql ="SELECT year( tr.start_date ) AS year, count( tr.id ) AS learner_count, tr.provider_id, o.legal_name AS training_provider_name
                    FROM tr
        ...
        {
            $sql ="SELECT year( tr.start_date ) AS year, count( tr.id ) AS learner_count, tr.contract_id, c.title AS contract_name
                    FROM tr
        ...
        {
            $sql ="SELECT year(tr.start_date) as year, count( tr.id ) AS learner_count,
                    tr.assessor,
                    concat( u.firstnames, u.surname ) AS assessor_name
                    FROM tr
                    LEFT JOIN users u ON tr.assessor = u.id
                    WHERE concat( u.firstnames, u.surname ) IS NOT NULL and tr.assessor != ''
        ...
        //echo "sql = ".$sql;
        $view = $_SESSION[$key] = new SlaKpiGenerateReportsNewStarts();
        $view->setSQL($sql);
