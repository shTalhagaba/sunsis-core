...
class Note extends Entity
{
	...
	public function save(PDO $link)
	{
		...
		// Clean note field
		$this->note = preg_replace('/[\n\r]+/', "\n", (string)$this->note); // Remove superfluous newlines
...
HEREDOC;
	...
	*/
	...
	{
		$key_pt = addslashes((string)$parent_table);
...
HEREDOC;
				...
				if($row['work_email'] != '')
				{
					echo "<div class=\"author\" title=\"{$row['firstnames']} {$row['surname']}, Tel: {$row['work_telephone']}\"><a href=\"mailto:{$row['work_email']}\">{$row['fqn']}</a>";
				}
				else
				{
					echo "<div class=\"author\">{$row['fqn']}";
				}
