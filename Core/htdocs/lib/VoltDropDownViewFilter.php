<?php
class VoltDropDownViewFilter implements VoltIViewFilter
{
	/**
	 * Constructor
	 *
	...
	public function toHTML($allow_null = false)
	{
		...
		// Open HTML element
		$html = '<select id = "' . $id . '" name="' . $id . '" onchange="if(window.' . $id .'_onchange){window.' . $id . '_onchange(this);}" >';
		...
		for($i = 0; $i < count($this->options); $i++)
		{
				...
				&& $option_group != $current_option_group )
			{
				...
				$html .= '<optgroup label="' . htmlspecialchars((string)$option_group) . '">';
			...
			{
				$html .= '<option value="' . htmlspecialchars((string)$value) . '" selected="selected">' . htmlspecialchars((string)$label) . '</option>';
			}
			else
			{
...
document.getElementById('$id').resetToDefault = function(){
		...
		if(strpos($sql, '{{') !== false)
		{
			if(preg_match_all('/(\{\{[a-zA-Z0-9_]+\}\})/', $sql, $matches))
			{
				...
				{			
					...
					{
						// String
						$sql = str_replace($m, "'" . addslashes((string)$filter_value) . "'", $sql);
					}
				}
			}			
		}
