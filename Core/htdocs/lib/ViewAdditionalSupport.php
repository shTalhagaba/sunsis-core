<?php
class ViewAdditionalSupport extends View
{
...
,tr.id as tr_id
,tr.contract_id as contract_id
,additional_support.due_date as due_date
,additional_support.actual_date as actual_date
#,IF(subject_area=0,'Assessment Plans', IF(subject_area=1,'Reflective Hours', IF(subject_area=2, 'Functional Skills',IF(subject_area=3,'Other','')))) as subject_area
,CASE additional_support.`subject_area`
	WHEN '0' THEN 'Assessment Plans'
	WHEN '1' THEN 'Reflective Hours'
	WHEN '2' THEN 'Functional Skills'
	WHEN '3' THEN 'Other'
	WHEN '4' THEN 'Competency Workshops'
END AS subject_area
,IF(contact_type=0,'OLL', IF(contact_type=1,'Workplace', IF(contact_type=2, 'Telephone',''))) as contact_type
,IF(manager_attendance='true','Yes','No') as manager_attendance
,concat(users.firstnames,' ',users.surname) as assessor
,induction_fields.assigned_assessor
,additional_support.adobe as recording_of_the_session
,additional_support.cancellation_comments as session_notes
...
FROM
	...
	LEFT JOIN (
  		...
  		(SELECT CONCAT(users.`firstnames`, ' ', users.`surname`) FROM users WHERE users.id = induction.`assigned_assessor`) AS `assigned_assessor`
...
HEREDOC;
            $view = $_SESSION[$key] = new ViewAdditionalSupport();
            $view->setSQL($sql);
            ...
            $options = array(
                ...
                2=>array(2, '2. Assessment Plans', null, 'WHERE additional_support.subject_area = 0'));
...
ORDER BY firstnames, surname;";
	...
	<tr>
		...
		<th>Recording Of The Session</th>
		<th>Session Notes</th>
...
HEREDOC;
            ...
            while($row = $st->fetch())
            {
                ...
                echo '<td>' . HTML::cell($row['subject_area']) . '</td>';
                echo '<td>' . HTML::cell($row['contact_type']) . '</td>';
                echo '<td>' . HTML::cell($row['manager_attendance']) . '</td>';
                echo '<td>' . HTML::cell($row['assessor']) . '</td>';
				echo '<td>' . HTML::cell($row['assigned_assessor']) . '</td>';
                echo '<td>' . HTML::cell($row['recording_of_the_session']) . '</td>';
                echo '<td>' . HTML::cell($row['session_notes']) . '</td>';
                echo '</tr>';
            }
