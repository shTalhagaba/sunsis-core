<?php
class SlaKpiGenerateReportsEarlyLeavers extends View
{
    ...
    public static function getInstance(PDO $link)
	{
        ...
        if($filter_drilldown_default_value == 'quarter' || $filter_drilldown_default_value == 'month' || $filter_drilldown_default_value == 'week')
        {
            ...
            $sql="SELECT
                    ...
                    FROM tr
                    LEFT JOIN contracts AS c ON (c.id = tr.contract_id)
                    WHERE (tr.status_code = 3 or tr.status_code=4) AND (TO_DAYS(tr.target_date) - TO_DAYS(tr.closure_date)) > 0 AND tr.closure_date >= c.start_date
                    ".$group_by;
        }
        ...
        elseif($filter_drilldown_default_value == 'assessor')
        {
            $sql ="SELECT year(tr.closure_date) as year, count( tr.id ) AS learner_count,
					...
					LEFT JOIN contracts AS c ON (c.id = tr.contract_id)
                    WHERE concat( u.firstnames, u.surname ) IS NOT NULL and tr.assessor != '' AND (tr.status_code = 3 or tr.status_code=4) AND (TO_DAYS(tr.target_date) - TO_DAYS(tr.closure_date)) > 0 AND tr.closure_date >= c.start_date
        ...
        //echo "sql = ".$sql;
        $view = $_SESSION[$key] = new SlaKpiGenerateReportsEarlyLeavers();
        $view->setSQL($sql);
