...
class Ilr2025 extends Entity
{
    ...
    public static function generateStream4(PDO $link, $submission, $contracts, $con1, $beta = 0)
    {
        ...
        if($stouter)
        {
            ...
            $batch_file_xml .= '<Message xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns="ESFA/ILR/2025-26">';
            $batch_file_xml .= "<Header>";
            $batch_file_xml .= "<CollectionDetails>";
            $batch_file_xml .= "<Collection>ILR</Collection>";
            $batch_file_xml .= "<Year>2526</Year>";	
            ...
            while($rowouter = $stouter->fetch())
            {
                ...
                if($st)
                {
                    while($row = $st->fetch())
                    {
                        ...
                        $record++;
                        $ilr = Ilr2025::loadFromXML($ilr);
                        $funding_type = Ilr2025::FundingType($ilr);
                        if($record==1)
                        {
                            ...
                            if(DB_NAME=='am_city_skills' or DB_NAME=='am_ela' or DB_NAME=='am_eet')
                            {
                                ...
                                if($stprior)
                                {
                                    while($rowprior = $stprior->fetch())
                                    {
                                        ...
                                        $ilrprior = Ilr2025::loadFromXML($ilrprior);
                           ...
                           */
                            ...
                            if(isset($ilr->ProviderSpecLeailrrnerMonitoring))
                            {
                            ...
                            if($stemp)
                            {
                                ...
                                while($rowemp = $stemp->fetch())
                                {
                                    ...
                                    $ilremp = Ilr2025::loadFromXML($ilremp);
