...
class FundingCalculator_2016 extends FundingCalculator
{
    ...
    public function getData($link, $hook_fields = '', $hook_joins = '', $hook_where = '')
    {
        ...
        {
            $ilr = Ilr2016::loadFromXML($rowxml['ilr']);
            ...
            foreach($ilr->LearningDelivery as $aim)
            {
                ...
                if($a10!='99' && $a10!='')
                {
                    ...
                    $Trailblazer = Array();
                    if( ($aim->FundModel=='81' or $aim->FundModel=='36') && $aim->ProgType=='25' && $aim->AimType=='1')
                    {
                        $StdCode=$aim->StdCode;
                        //if($StdCode)
                        {
                            ...
                            if($xmlarray['aim_type'] == "16-18 Trailblazer" or $xmlarray['aim_type'] == "16-18 Apprenticeships")
                            {
                                ...
                                //if($days_on_programme>=90)
                                {
                                    ...
                                    {
                                        $month+=5;
                                        $month += 12*($year-2016-1);
                                    }
                                    $Incentive1618 = DAO::getSingleValue($link, "select 1618Incentive from lars201617.Core_LARS_StandardFunding Where StandardCode = '$StdCode' order by EffectiveFrom DESC LIMIT 0,1");
                    ...
                    $tnp2=0;
                    $total = 0;
                    if($aim->FundModel=='36' && $aim->ProgType=='25' && $aim->AimType=='1')
                    {
                    ...
                    if($xmlarray['programme_type'] == 1)
                        ...
                        $xmlarray['total_funding'] = DAO::getSingleValue($link, "SELECT RateWeighted FROM lars201516.Core_LARS_Funding WHERE FundingCategory='Matrix' AND LearnAimRef='$a09' LIMIT 0,1");

                    if($xmlarray['FundModel']=='36')
                        if($xmlarray['StdCode']!='')
                            $xmlarray['LevyCap'] = DAO::getSingleValue($link, "SELECT MaxEmployerLevyCap FROM lars201617.Core_LARS_ApprenticeshipFunding WHERE ApprenticeshipType = 'STD' AND ApprenticeshipCode='{$xmlarray['StdCode']}' LIMIT 0,1");
                        else
                            $xmlarray['LevyCap'] = DAO::getSingleValue($link, "SELECT MaxEmployerLevyCap FROM lars201617.Core_LARS_ApprenticeshipFunding WHERE ApprenticeshipType = 'FWK' AND ApprenticeshipCode='{$xmlarray['FworkCode']}' and PwayCode='{$xmlarray['PwayCode']}' LIMIT 0,1");
...
//        $learners = $ilrs->getElementsByTagName("Learner");
        ...
        $the_first_day_of_the_current_funding_year = "2016-08-01";
        $the_last_day_of_the_current_funding_year = "2017-07-31";
