...
class FundingCalculator_2013 extends FundingCalculator
{
	...
	public function getData($link, $hook_fields = '', $hook_joins = '', $hook_where = '')
	{
		...
		{
			$ilr = Ilr2013::loadFromXML($rowxml['ilr']);
            $ukprn = $rowxml['ukprn'];
			$edrs = '';
			$ac_postcode = '';
			$main_aim = 0;
			...
			foreach($ilr->LearningDelivery as $aim)
			{
				...
				if($a10!='99' && $a10!='81' && $a10!='' && $aim->AimType!='1')
				{
                    ...
                    $xmlarray['funding_type'] = $funding_type;
					...
					// Aim Type
					$dob = $ilr->DateOfBirth;
                    $start_date = new Date('01/08/2014');
                    ...
                    $xmlarray['aim_type'] = '';
					if($ProgType=='2' || $ProgType=='3' || $ProgType=='10' || $ProgType=='20' || $ProgType=='21' || $ProgType=='22' || $ProgType=='23')
					{
						if($age<19)
							$xmlarray['aim_type'] = "16-18 Apprenticeships";
						elseif($age<24 || ($age<25 && $LearnStartDate->before('01-08-2013')))
					...
					if($xmlarray['programme_type'] == 1)
                        ...
                        $xmlarray['total_funding'] = DAO::getSingleValue($link, "SELECT LARS_FundingRate FROM lars201314.LARS_Funding1314 WHERE LARS_FundCategory='APP_ACC_COST' AND LARS_FundingRateType='20' AND LARS_LearnAimRef='$a09' LIMIT 0,1");
	...
	public function getOtherData($link, $funding)
	{
		...
		$large_query = "select ERN from lars201314.LargeEmployers where ERN in ($edrsstring)";
    ...
    public function calculateEFA($link, $learner, $ukprn)
    {
        $total_funding = 0;
        /*
         *  Historic Retention
...
//        $learners = $ilrs->getElementsByTagName("Learner");
        ...
        $the_first_day_of_the_current_funding_year = "2013-08-01";
        $the_last_day_of_the_current_funding_year = "2014-07-31";
        ...
        $the_national_rate_for_part_time_band_4_learners = 4000;
        ...
        if($the_learners_planned_number_of_days_this_funding_year >= 168)
        ...
        */
        ...
        pre($the_providers_historic_retention_factor);
