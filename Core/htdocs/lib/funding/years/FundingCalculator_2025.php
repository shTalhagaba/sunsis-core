...
class FundingCalculator_2025 extends FundingCalculator
{
    ...
    public function getData($link, $hook_fields = '', $hook_joins = '', $hook_where = '')
    {
        ...
        while ($rowxml = $stxml->fetch()) {
            $ilr = Ilr2025::loadFromXML($rowxml['ilr']);
            ...
            foreach ($ilr->LearningDelivery as $aim) {
                ...
                if ($a10 != '99' && $a10 != '') {
                    ...
                    $xpath = $ilr->xpath("/Learner/LearningDelivery[AimType='1' or AimType='4']/Outcome");
                    $achieved = (empty($xpath[0])) ? '' : $xpath[0];
                    $achieved = "" . $achieved;
                    ...
                    $xpath = $aim->xpath("./LearningDeliveryFAM[LearnDelFAMType='FFI']/LearnDelFAMCode");
                    $ffi = (empty($xpath[0])) ? '' : $xpath[0];
                    $fully_funded = "" . $ffi;
                    $xmlarray['fully_funded'] = $fully_funded;

                    $xpath = $aim->xpath("./LearningDeliveryFAM[LearnDelFAMType='LSF']/LearnDelFAMCode");
                    $lsf = (empty($xpath[0])) ? '0' : $xpath[0];
                    $lsf = "" . $lsf;
    ...
    public function getOtherData($link, $funding)
    {
        ...
        if (!empty($aimsstring)) {
            ...
            if ($aimsst) { // check query succeeded
                while ($aimsrow = $aimsst->fetch(PDO::FETCH_ASSOC)) {
        ...
        $return['rates'] = $rates; // previous section

        // Funding Program Weights
        $pws = array();
        ...
        if (!empty($aimsstring)) { // ADD CHECK
            $ksquery = "SELECT LEARNING_AIM_REF, LEARNING_AIM_TYPE_DESC 
                FROM lad201314.learning_aim
                LEFT JOIN lad201314.learning_aim_types  
                  ON learning_aim.`LEARNING_AIM_TYPE_CODE` = learning_aim_types.`LEARNING_AIM_TYPE_CODE`
        ...
        // Build Array for Distinct Postcodes// Postcodes
        $postcodes = array();
        if (!empty($poststring)) {
    ...
    public static function calculateEFA($link, $learner, $ukprn)
    {
        ...
        $the_first_day_of_the_current_funding_year = "2025-08-01";
        $the_last_day_of_the_current_funding_year = "2026-07-31";
