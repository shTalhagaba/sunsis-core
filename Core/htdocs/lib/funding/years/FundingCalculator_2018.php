...
class FundingCalculator_2018 extends FundingCalculator
{
    ...
    public function getData($link, $hook_fields = '', $hook_joins = '', $hook_where = '')
    {
        ...
        {
            $ilr = Ilr2018::loadFromXML($rowxml['ilr']);
                ...
                if($a10!='99' && $a10!='')
                {
                    ...
                    if($xmlarray['FundModel']=='36')
                        if($xmlarray['StdCode']!='')
                            ...
                            $xmlarray['LevyCap'] = DAO::getSingleValue($link, "SELECT MaxEmployerLevyCap FROM lars201718.Core_LARS_ApprenticeshipFunding WHERE ApprenticeshipType = 'FWK' AND ApprenticeshipCode='{$xmlarray['FworkCode']}' and PwayCode='{$xmlarray['PwayCode']}' AND ProgType='$ProgType' AND EffectiveFrom<='$a27' and (EffectiveTO is null or EffectiveTo>='$a27')");
...
//        $learners = $ilrs->getElementsByTagName("Learner");
        ...
        $the_first_day_of_the_current_funding_year = "2018-08-01";
        $the_last_day_of_the_current_funding_year = "2019-07-31";
