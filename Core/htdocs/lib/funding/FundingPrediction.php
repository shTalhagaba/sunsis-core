...
class FundingPrediction extends FundingCore
{
    ...
    function __construct($link, $contracts, $sqid = 0, $course = 0, $assessor = '', $employer = 0, $submissionp = '', $shadow = false, $tutor = '', $emp_b_code = '')
    {
        ...
        $wgts = $otherData['wgts'] ?? array();
        $pws  = $otherData['pws'] ?? array();
        ...
        foreach ($funding as $key => $data) {
            ...
            for ($i = 1; $i <= $predictor_duration; $i++) {
                ...
                if (!isset($this->totalFunding1923Apps["$i"])) {
                    //$this->totalFunding1923Apps["$i"] = array('adjusted' => '', 'on_program' => '', 'balance' => '', 'achievement' => '', 'ach_profiled' => '', 'total' => '');
                    $this->totalFunding1923Apps["$i"] = array('on_program' => '', 'balance' => '', 'achievement' => '', 'ach_profiled' => '', 'total' => '', 'at_risk' => '');
                }
                //				pre($learner->get($i, 'on_program'));
                $submission = "W" . str_pad($i, 2, "0", STR_PAD_LEFT);
                $profileamount = DAO::getSingleValue($link, "select profile from lookup_profile_values where contract_id in ('$contracts') and submission = '$submission'");
                ...
                if (!isset($this->totalFunding24Apps["$i"])) {
                    //$this->totalFunding24Apps["$i"] = array('adjusted' => '', 'on_program' => '', 'balance' => '', 'achievement' => '', 'ach_profiled' => '', 'total' => '');
                    $this->totalFunding24Apps["$i"] = array('on_program' => '', 'balance' => '', 'achievement' => '', 'ach_profiled' => '', 'total' => '', 'at_risk' => '');
                }
                //				pre($learner->get($i, 'on_program'));
                $submission = "W" . str_pad($i, 2, "0", STR_PAD_LEFT);
                $profileamount = DAO::getSingleValue($link, "select profile from lookup_profile_values where contract_id in ('$contracts') and submission = '$submission'");
                ...
                if (!isset($this->totalFundingApps1618LevyMay17["$i"])) {
                    $this->totalFundingApps1618LevyMay17["$i"] = array('on_program' => '', 'balance' => '', 'achievement' => '', 'ach_profiled' => '', 'total' => '', 'at_risk' => '');
                }
                $submission = "W" . str_pad($i, 2, "0", STR_PAD_LEFT);
                $profileamount = DAO::getSingleValue($link, "select profile from lookup_profile_values where contract_id in ('$contracts') and submission = '$submission'");
                $pframount = DAO::getSingleValue($link, "select profile from lookup_pfr_values where contract_id in ('$contracts') and submission = '$submission'");
                ...
                if (!isset($this->totalFundingApps1618NLNPMay17["$i"])) {
                    $this->totalFundingApps1618NLNPMay17["$i"] = array('on_program' => '', 'balance' => '', 'achievement' => '', 'ach_profiled' => '', 'total' => '', 'at_risk' => '');
                }
                $submission = "W" . str_pad($i, 2, "0", STR_PAD_LEFT);
                $profileamount = DAO::getSingleValue($link, "select profile from lookup_profile_values where contract_id in ('$contracts') and submission = '$submission'");
                $pframount = DAO::getSingleValue($link, "select profile from lookup_pfr_values where contract_id in ('$contracts') and submission = '$submission'");
                ...
                if (!isset($this->totalFundingApps19LevyMay17["$i"])) {
                    $this->totalFundingApps19LevyMay17["$i"] = array('on_program' => '', 'balance' => '', 'achievement' => '', 'ach_profiled' => '', 'total' => '', 'at_risk' => '');
                }
                $submission = "W" . str_pad($i, 2, "0", STR_PAD_LEFT);
                $profileamount = DAO::getSingleValue($link, "select profile from lookup_profile_values where contract_id in ('$contracts') and submission = '$submission'");
                $pframount = DAO::getSingleValue($link, "select profile from lookup_pfr_values where contract_id in ('$contracts') and submission = '$submission'");
                ...
                if (!isset($this->totalFundingApps19NLNPMay17["$i"])) {
                    $this->totalFundingApps19NLNPMay17["$i"] = array('on_program' => '', 'balance' => '', 'achievement' => '', 'ach_profiled' => '', 'total' => '', 'at_risk' => '');
                }
                $submission = "W" . str_pad($i, 2, "0", STR_PAD_LEFT);
                $profileamount = DAO::getSingleValue($link, "select profile from lookup_profile_values where contract_id in ('$contracts') and submission = '$submission'");
                $pframount = DAO::getSingleValue($link, "select profile from lookup_pfr_values where contract_id in ('$contracts') and submission = '$submission'");
                ...
                if (!isset($this->totalFundingApps19NLPMay17[$i])) {
                    $this->totalFundingApps19NLPMay17[$i] = ['on_program' => 0];
                }
                $submission = "W" . str_pad($i, 2, "0", STR_PAD_LEFT);
                $profileamount = DAO::getSingleValue($link, "select profile from lookup_profile_values where contract_id in ('$contracts') and submission = '$submission'");
                $pframount = DAO::getSingleValue($link, "select profile from lookup_pfr_values where contract_id in ('$contracts') and submission = '$submission'");
        ...
        foreach ($this->totalFundingTraineeship1924PMay17 as $period => $data) {
            $this->dataTraineeship1924PMay17[] = array('period' => 'W' . str_pad($period, 2, '0', STR_PAD_LEFT) . '</a>', 'on_program' => $data['on_program'], 'balance' => $data['balance'], 'achievement' => $data['achievement'], 'ach_profiled' => $data['ach_profiled'], 'total' => sprintf("%.2f", $data['total']), 'profile' => sprintf("%.2f", $data['profile']), 'PFR' => sprintf("%.2f", $data['PFR']), 'at_risk' => sprintf("%.2f", $data['at_risk']));
        }

        foreach ($this->totalFundingAEBOtherLearningNP as $period => $data) {
            $this->dataAEBOtherLearningNP[] = array('period' => 'W' . str_pad($period, 2, '0', STR_PAD_LEFT) . '</a>', 'on_program' => $data['on_program'], 'balance' => $data['balance'], 'achievement' => $data['achievement'], 'ach_profiled' => $data['ach_profiled'], 'total' => sprintf("%.2f", $data['total']), 'profile' => sprintf("%.2f", $data['profile']), 'PFR' => sprintf("%.2f", $data['PFR']), 'at_risk' => sprintf("%.2f", $data['at_risk']));
        }

        foreach ($this->totalFundingAEBOtherLearningPNov17 as $period => $data) {
            $this->dataAEBOtherLearningPNov17[] = array('period' => 'W' . str_pad($period, 2, '0', STR_PAD_LEFT) . '</a>', 'on_program' => $data['on_program'], 'balance' => $data['balance'], 'achievement' => $data['achievement'], 'ach_profiled' => $data['ach_profiled'], 'total' => sprintf("%.2f", $data['total']), 'profile' => sprintf("%.2f", $data['profile']), 'PFR' => sprintf("%.2f", $data['PFR']), 'at_risk' => sprintf("%.2f", $data['at_risk']));
        }
