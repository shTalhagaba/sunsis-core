...
class SystemConfig
{
	...
	private static function _buildIncludePath()
	{
		$link = DAO::getConnection();
		$ini_path = '';
		$config = DAO::getLookupTable($link, "SELECT DISTINCT entity,value FROM configuration", "system config");
		...
		foreach ( $config as $configuration_setting => $configuration_value )
		{
			if(strpos($configuration_setting, "module_") === 0 && $configuration_value)
			{
				...
				{
					//DAO::execute($link, "UPDATE configuration SET value='0' WHERE entity='".addslashes((string)$configuration_setting)."';");
