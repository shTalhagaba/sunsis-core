<?php
class ViewLearnerGroupLearners extends View
{
...
HEREDOC;
			...
			$view = $_SESSION[$key] = new ViewLearnerGroupLearners();
			$view->setSQL($sql);
	...
	public function render(PDO $link)
	{
		...
		if($st) 
		{
			...
			{
				echo '<th class="topRow ProgressStatistic" colspan="2">Progress Statistics</th>';
			}
			echo '</tr><tr>';
			echo <<<HEREDOC
...
HEREDOC;
			...
			while($row = $st->fetch())
			{
				...
				$que = "select (IF(unitsUnderAssessment>100,100,unitsUnderAssessment)/100*proportion) from student_qualifications where tr_id={$row['tr_id']} and framework_id='$framework_id'";
