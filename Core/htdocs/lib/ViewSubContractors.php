<?php
class ViewSubContractors extends View
{
	...
	where organisations.organisation_type like '%5%';
HEREDOC;
	
			$view = $_SESSION[$key] = new ViewSubContractors();
			$view->setSQL($sql);
			...
			$options = array(
				...
				2=>array(3, 'Location (asc), Provider name (asc)', null, 'ORDER BY address_line_3, adress_line_2, legal_name'),
	...
	public function render(PDO $link)
	{
		...
		if($st) 
		{
			...
			while($row = $st->fetch())
			{
				echo HTML::viewrow_opening_tag('/do.php?_action=read_subcontractor&id=' . $row['id']);
