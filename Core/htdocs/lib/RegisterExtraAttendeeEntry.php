<?php
class RegisterExtraAttendeeEntry extends ValueObject implements Iterator
{
	###############################################
	## Iterator interface
	###############################################
	...
	public function save(PDO $link)
	{
		...
		if(is_null($this->id) || $this->id == 0)
		{
			...
			// New record
			$sql = "INSERT INTO register_extra_attendees_entries SET " . $this->toNameValuePairs($exclude_fields) . ";";
		}
		else
		{
			...
			$sql = "SELECT re.*, codes.description FROM register_extra_attendees_entries AS re LEFT OUTER JOIN lookup_register_entry_codes AS codes ON re.entry = codes.code WHERE id=" . $this->id;
			$st2 = $link->query($sql);
			...
			$sql = "UPDATE register_extra_attendees_entries SET " . $this->toNameValuePairs($exclude_fields) . " WHERE id=" . $this->id . ";";
		}
