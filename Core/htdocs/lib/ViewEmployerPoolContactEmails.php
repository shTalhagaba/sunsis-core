<?php
class ViewEmployerPoolContactEmails extends View
{
...
WHERE employer_pool_contact_email_notes.org_id = $org_id
HEREDOC;

			$view = $_SESSION[$key] = new ViewEmployerPoolContactEmails();
			$view->setSQL($sql);
	...
	public function render(PDO $link)
	{
		...
		if($st)
		{
			...
			while($row = $st->fetch())
			{
				...
				echo '<td align="center"><a href="do.php?_action=view_html_preview_of_email&candidate=false&employer=false&pool=' . $row['id'] . '" target="blank">HTML Preview</a></td>';
				echo '</tr>';
			}
