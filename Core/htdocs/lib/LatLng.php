<?php
  ...
  class LatLng {
    ...
    function OSGB36ToWGS84() {
      ...
      $v = $a / (sqrt(1 - $eSquared * PHPCoordUtils::sinSquared($phi)));
      $H = 0; // height
      ...
      for ($i = 1; $i < 10; $i++) {
        $v = $a / (sqrt(1 - $eSquared * PHPCoordUtils::sinSquared($phiN)));
    ...
    function toOSRef() {
      ...
      $N = 0.0;
      $n = ($a - $b) / ($a + $b);
      $v = $a * $OSGB_F0 * pow(1.0 - $eSquared * PHPCoordUtils::sinSquared($phi), -0.5);
      $rho =
        $a * $OSGB_F0 * (1.0 - $eSquared) * pow(1.0 - $eSquared * PHPCoordUtils::sinSquared($phi), -1.5);
      ...
      $III =
        ($v / 24.0)
          * sin($phi)
          * pow(cos($phi), 3.0)
          * (5.0 - PHPCoordUtils::tanSquared($phi) + (9.0 * $etaSquared));
      $IIIA =
        ($v / 720.0)
          * sin($phi)
          * pow(cos($phi), 5.0)
          * (61.0 - (58.0 * PHPCoordUtils::tanSquared($phi)) + pow(tan($phi), 4.0));
      $IV = $v * cos($phi);
      $V = ($v / 6.0) * pow(cos($phi), 3.0) * (($v / $rho) - PHPCoordUtils::tanSquared($phi));
      $VI =
        ($v / 120.0)
          * pow(cos($phi), 5.0)
          * (5.0
            - (18.0 * PHPCoordUtils::tanSquared($phi))
            + (pow(tan($phi), 4.0))
            + (14 * $etaSquared)
            - (58 * PHPCoordUtils::tanSquared($phi) * $etaSquared));
    ...
    function toUTMRef() {
      ...
      $UTMZone = PHPCoordUtils::getUTMLatitudeZoneLetter($latitude);
