<?php
class ViewAssessmentPlans extends View
{
...
FROM
	...
	LEFT JOIN student_frameworks ON student_frameworks.id = lookup_assessment_plan_log_mode.`framework_id`
	...
	LEFT JOIN group_members ON group_members.tr_id = student_frameworks.tr_id
	...
	LEFT JOIN assessment_plan_log ON assessment_plan_log.`tr_id` = student_frameworks.tr_id AND assessment_plan_log.mode = lookup_assessment_plan_log_mode.id
...
HEREDOC;
            $view = $_SESSION[$key] = new ViewAssessmentPlans();
            $view->setSQL($sql);
            ...
            $options = array(
                0=>array(0, 'Show all', null, null),
                1=>array(1, '1. Planned', null, 'WHERE assessment_plan_log.id is not null'),
                2=>array(2, '2. Unplanned', null, 'WHERE assessment_plan_log.id is null'));
            $f = new DropDownViewFilter('filter_planned', $options, 2, false);
            $f->setDescriptionFormat("Show: %s");
            $view->addFilter($f);
