...
class ValidateILR2025
{
    ...
    private function excludeLearningDelivery(PDO $link, $delivery)
    {
        if( 
                // substr($delivery->LearnAimRef->__toString(), -2) == "EP" || 
                // substr($delivery->LearnAimRef->__toString(), -2) == "GW" ||
                ( isset($delivery->Exclude) && $delivery->Exclude->__toString() == "1" )
            )
        {
            return true;
        }
    ...
    private function rule_Learning_Delivery_Rules($link, $ilr)
    {
        ...
        {
            if($this->excludeLearningDelivery($link, $delivery))
            {
                continue;
            }
            ...
            $LatestActEndDate = new Date('1900-01-01');
            $DD10="N";
            ...
            foreach($ilr->LearningDelivery as $deliverydd10)
            {
                ...
                if(Date::isDate($deliverydd10->LearnActEndDate) && $LatestActEndDate->before($deliverydd10->LearnActEndDate))
                    $LatestActEndDate = new Date($deliverydd10->LearnActEndDate);    
            }
            ...
            $LatestACTToDate = new Date('1900-01-01');
            $LatestACTFromDate = new Date('2100-01-01');
            foreach($delivery->LearningDeliveryFAM as $LearnDelFAM)
            {
                ...
                if($LearnDelFAM->LearnDelFAMType=="ACT" && Date::isDate("".$LearnDelFAM->LearnDelFAMDateTo))
                {
                    if($LatestACTToDate->before("".$LearnDelFAM->LearnDelFAMDateTo))
                        $LatestACTToDate = new Date("".$LearnDelFAM->LearnDelFAMDateTo);
                }

                if($LearnDelFAM->LearnDelFAMType=="ACT" && Date::isDate("".$LearnDelFAM->LearnDelFAMDateFrom))
                {
                    if($LatestACTFromDate->after("".$LearnDelFAM->LearnDelFAMDateFrom))
                        $LatestACTFromDate = new Date("".$LearnDelFAM->LearnDelFAMDateFrom);
                }

            }

            //R102
            if($FundModel=="36" and $AimType==1 and !$LatestACTFromDate->equals(Date::toShort($delivery->LearnStartDate)))
            {
                $errors[] = "R102: The aim must have an Apprenticeship contract type in the first day " . $delivery->LearnAimRef;
            }
            ...
            foreach($delivery->TrailblazerApprenticeshipFinancialRecord as $TrailblazerApprenticeshipFinancialRecord)
            {
                ...
                if($TrailblazerApprenticeshipFinancialRecord->TBFinType=="TNP" and ($TrailblazerApprenticeshipFinancialRecord->TBFinCode=='1' or $TrailblazerApprenticeshipFinancialRecord->TBFinCode=='2' or $TrailblazerApprenticeshipFinancialRecord->TBFinCode=='3' or $TrailblazerApprenticeshipFinancialRecord->TBFinCode=='4'))
                    if(Date::isDate($TrailblazerApprenticeshipFinancialRecord->TBFinDate) and $LearnStartDate->after(Date::toShort($TrailblazerApprenticeshipFinancialRecord->TBFinDate)))
                        $errors[] = "R119: The apprenticeship financial record cannot be before the Learning start date when the total negotiated price has been returned (" . $delivery->LearnAimRef . ")";
            }

            //R121
            if($delivery->ProgType=="25" and Date::isDate($delivery->AchDate) && (!$LatestACTToDate->equals($delivery->AchDate) or $LatestACTToDate->equals("1900-01-01")))
            {
                $errors[] = "R121: There must be a Date applies to that matches the achievement date for this aim. " . $delivery->LearnAimRef;
            }

            //R122
            if($delivery->FundModel=="36" and $delivery->ProgType=="25" and $delivery->CompStatus!="1" and ($delivery->AchDate=="" or $delivery->AchDate=="dd/mm/yyyy") and !$LatestACTToDate->equals($delivery->LearnActEndDate) and !$LatestACTFromDate->equals("2100-01-01"))
            {
                $errors[] = "R122: The achievement date is not known, therefore the Date applies to record must be the same as the Learning actual end date " . $delivery->LearnAimRef;
            }
            ...
            if(isset($AchDate_Date))
                if($AchDate_Date->after("31/07/2026"))
            ...
            if(isset($AchDate) && isset($LearnStartDate) && $LearnStartDate_Date->after("2015-07-31") && $ProgType!='24' && !($ProgType='25' && $AimType=='1'))
                $errors[] = "AchDate_09: The Achievement date must not be completed for aims that are not traineeship or apprenticeship standard programme aims (" . $delivery->LearnAimRef . ")";

            //AchDate_10
            if($delivery->ProgType=='24' && $AimType=='1' && isset($AchDate) && isset($LearnActEndDate) && $AchDate_Date->after($LearnActEndDate6Months->formatMySQL()))
                $errors[] = "AchDate_10: The Achievement date must be within 6 months of a traineeship programme Learning actual end date";

            //AchDate_12
            if($delivery->ProgType=='25' && $AimType=='1' && !isset($AchDate) && isset($LearnActEndDate) && $delivery->CompStatus=='2')
                $errors[] = "AchDate_12: The Achievement date must be returned";
            ...
            // CompStatus_02
            if($CompStatus=='1' && Date::isDate($delivery->LearnActEndDate) && $delivery->AimType!='1')
            ...
            if($Outcome=='' && $CompStatus!='1')
                $errors[] = "CompStatus_04: If the Outcome is not returned, the Completion status must be 'the learner is continuing or intending to continue the learning activities leading to the learning aim' " . $delivery->LearnAimRef;
            ...
            // CompStatus_07
            if($CompStatus=='1' and Date::isDate($delivery->AchDate))
                $errors[] = "CompStatus_07: The completion status has not been returned " . $delivery->LearnAimRef;
            ...
            // FD_DelLocPostCode_AL
            if($delivery->DelLocPostCode=="")
                $errors[] = "FD_DelLocPostCode_AL: The length of the value returned for DelLocPostCode is not between 1 and 8 characters (" . $delivery->LearnAimRef . ")";

            // DelLocPostCode_11
            if($delivery->DelLocPostCode!="" and !$this->checkPostcode($DelLocPostCode))
            ...
            foreach($delivery->LearningDeliveryFAM as $LearnDelFAM)
            {
                ...
                if($LearnDelFAM->LearnDelFAMType=="ACT" && $LearnDelFAM->LearnDelFAMCode!='' && Date::isDate($LearnDelFAM->LearnDelFAMDateFrom))
                    $there_is_no_record_of_ACT = false;
	        ...
	        if(!in_array($FundModel, [10, 25, 35, 36, 37, 38, 70, 81, 82, 99]))
            ...
            foreach($delivery->LearningDeliveryFAM as $LearnDelFAM)
            {
                if($LearnDelFAM->LearnDelFAMType=="LDM" && $LearnDelFAM->LearnDelFAMCode=='378' && $delivery->FundModel=='36')
                    $errors[] = "FundModel_15: This funding model is not valid for this LDM code (" . $delivery->LearnAimRef . ")";
            }

            //FwordCode_01
            if($FworkCode=='' && $DD07=="Y" && $delivery->ProgType!="24" && $delivery->ProgType!="25")
                $errors[] = "FworkCode_01: The Framework code must be returned for all aims that are part of an apprenticeship (not including Trailblazer apprenticeships) (" . $delivery->LearnAimRef . ")";
            ...
            //LearnStartDate_03
            if($DD07=="N" && $ProgType!='24' && $LearnStartDate_Date->after("31/07/2026"))
            ...
            */
            ...
            //LearnDelFAMType_22
            if($FundModel=='36' && !$there_is_no_record_of_ffi)
                $errors[] = "LearnDelFAMType_22: The Full or co-funding indicator must not be returned for this Funding model (" . $delivery->LearnAimRef . ")";
            ...
            foreach($delivery->LearningDeliveryFAM as $LearnDelFAM)
            {
                if($FundModel=='36' and $LearnDelFAM->LearnDelFAMType=="DAM" and $LearnDelFAM->LearnDelFAMCode!='')
                    $errors[] = "LearnDelFAMType_78: This funding model is not valid for this Devolved area monitoring code (DAM) (" . $delivery->LearnAimRef . ")";
            }
            ...
            foreach($delivery->LearningDeliveryFAM as $LearnDelFAM)
            {
                if($LearnDelFAM->LearnDelFAMType=="ACT" && $LearnDelFAM->LearnDelFAMCode!='' && $delivery->FundModel=='99')
                    $errors[] = "LearnDelFAMType_63: The Apprenticeship contract type must not be returned for this learning aim (" . $delivery->LearnAimRef . ")";
            }

            //LearnDelFAMDateFrom_02
            foreach($delivery->LearningDeliveryFAM as $LearnDelFAM)
            {
                $LearnStartDate = new Date($delivery->LearnStartDate);
                if($delivery->FundModel=="36" and Date::isDate($LearnDelFAM->LearnDelFAMDateFrom) and $LearnStartDate->after(Date::toShort($LearnDelFAM->LearnDelFAMDateFrom)))
                    $errors[] = "LearnDelFAMDateFrom_02: The Date applies from must be after the Learning start date (" . $delivery->LearnAimRef . ")";
            }

            //LearnDelFAMDateTo_03
            foreach($delivery->LearningDeliveryFAM as $LearnDelFAM)
            {
                if(Date::isDate($delivery->LearnActEndDate))
                {
                    $LearnActEndDate = new Date($delivery->LearnActEndDate);
                    if(!Date::isDate($delivery->AchDate) and $delivery->FundModel=="36" and Date::isDate($LearnDelFAM->LearnDelFAMDateTo) and $LearnActEndDate->before(Date::toShort($LearnDelFAM->LearnDelFAMDateTo)))
                        $errors[] = "LearnDelFAMDateTo_03: The Date applies to must not be after the Learning actual end date (" . $delivery->LearnAimRef . ")";
                }
            }
            ...
            $DateOfBirth_25_Under19 = DAO::getSingleValue($link, "SELECT IF(DATE_ADD('$DateOfBirth',INTERVAL 19 YEAR)>DATE_ADD('2025-08-31',INTERVAL 1 DAY),1,0);");
            ...
            $LearnStartDate_46_date->addDays(365);
            //$errors[] = $LearnPlanEndDate;
            if(!$Restart and $LearnStartDate_Date->after("2016-07-31") and $LearnStartDate_Date->before("2025-08-01") and $DateOfBirth_46_Under16 != '1' and ($FundModel=='36' or $FundModel=='81') and $ProgType=='25' and $AimType=='1' and $LearnStartDate_46_date->after(Date::toMySQL($LearnPlanEndDate)))
            ...
            //StdCode_01
            if($delivery->ProgType == "25" and $StdCode=='')
                $errors[] = "StdCode_01: If the learning aim is part of an apprenticeship standard programme, then the standard code must be returned (" . $delivery->LearnAimRef . $ProgType . ")";
            ...
            foreach($delivery->TrailblazerApprenticeshipFinancialRecord as $TrailblazerApprenticeshipFinancialRecord)
            {
                if(Date::toMySQL($delivery->LearnStartDate) == Date::toMySQL($TrailblazerApprenticeshipFinancialRecord->TBFinDate))
                    if($TrailblazerApprenticeshipFinancialRecord->TBFinType=="TNP" and ($TrailblazerApprenticeshipFinancialRecord->TBFinCode=='1' or $TrailblazerApprenticeshipFinancialRecord->TBFinCode=='2' or $TrailblazerApprenticeshipFinancialRecord->TBFinCode=='3' or $TrailblazerApprenticeshipFinancialRecord->TBFinCode=='4'))
                        $price_record_is_present = true;
            }
            if($delivery->AimType==1 and $delivery->FundModel==36 and !$price_record_is_present)
                $errors[] = "AFinType_13: There must be a price record that applies from the start of the programme (" . $delivery->LearnAimRef . ")";
            ...
            */
            ...
            if(($FundModel=="36" or ($FundModel=="81" and $ProgType=="25")) and $AimType=="1" and $apprenticeship_financial_record_is_present==false)
                $errors[] = "AFinType_09: The Apprenticeship Financial Record must be returned (" . $delivery->LearnAimRef . ")";

            // TBFinType_12
            if($FundModel=="36" and $AimType=="1" and $price_record_is_present==false)
                $errors[] = "AFinType_12: If the programme is an apprenticeship funded programme, there must be a price record (" . $delivery->LearnAimRef . ")";

            // TBFinType_13
            //if($FundModel=="36" and $AimType=="1" and $price_record_on_learning_delivery_is_present==false)
             //   $errors[] = "AFinType_13: An apprenticeship funded programme must have a price record that applies from the start date of the programme (" . $delivery->LearnAimRef . ")";
            ...
            //OTJActHours_02
            if($FundModel=="36" and $delivery->AimType==1 and $delivery->CompStatus=="2" and $delivery->OTJActHours=="")
            {
                $errors[] = "OTJActHours_02: Actual Hours for Off the Job Training must be returned. (".$delivery->LearnAimRef.")";
            }
    ...
    private function rule_FD_DateLevelApp_MA($link, $ilr)
    {
        foreach($ilr->PriorAttain as $prior)
        {
            if($prior->PriorLevel != '')
            {
                if(!Date::isDate($prior->DateLevelApp))
                {
                    return "FD_DateLevelApp_MA: The prior attainment applies from date has not been returned";
                }
            }
        }
    }
...
///////////////////////// FD_LEARNER Ends//////////////////////////////
    ...
    {
        $NINumber = strtoupper(trim($ilr->NINumber));	
        if($NINumber!='')
        {
...
/*    private function rule_EmpId_13($link, $ilr)
    ...
    private function rule_ESMType_05($link, $ilr)
    {
        foreach($ilr->LearnerEmploymentStatus as $les)
        {
            if(("".$les->DateEmpStatApp)!='' && ("".$les->DateEmpStatApp)!='dd/mm/yyyy')
            {
                ...
                if($EmpStat=='10' && $loufound==true)
                {
                    return "ESMType_05: The learner's Employment status is 'in paid employment' and the Length of unemployment has been returned";
                }
            }
        }
    }
    ...
    private function rule_R29($link, $ilr)
    {
        ...
        {
            if($this->excludeLearningDelivery($link, $les))
            {
                continue;
            }
    ...
    /*private function rule_Outcome_04($link, $ilr)
    {
        foreach($ilr->LearningDelivery as $delivery)
        {
            ...
            if($delivery->AchDate!='' && $delivery->AchDate!='undefined' && $delivery->AchDate!='dd/mm/yyyy')
            {
                if($delivery->Outcome!='1' and $delivery->ProgType!='25')
                    return "Outcome_04: If the Achievement date is returned then the Outcome must be 'Achieved' (".$delivery->LearnAimRef.$delivery->FundModel.")";
            }
        }
    }*/
    ...
    private function rule_R52($link, $ilr)
    {
        foreach($ilr->LearningDelivery as $delivery)
        {
            ...
            foreach($delivery->LearningDeliveryFAM as $ldf)
            {
                if($ldf->LearnDelFAMCode!='' && $ldf->LearnDelFAMCode!='undefined')
                {
                    $value = $ldf->LearnDelFAMType . $ldf->LearnDelFAMCode . $ldf->LearnDelFAMDateFrom . $ldf->LearnDelFAMDateTo;
