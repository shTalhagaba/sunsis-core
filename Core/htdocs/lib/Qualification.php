...
//		$q->units = QualificationUnits::loadFromDatabase($link, $q->id);
    ...
    public static function loadFromXML($xml)
    {
        ...
        $q->total_credit_value = utf8_decode($root['total_credit_value']);
    ...
    public static function loadFromDatabaseByAutoId(PDO $link, $auto_id)
    {
        ...
        $key = addslashes((string)$auto_id);
        $st = $link->query("SELECT * FROM qualifications WHERE id = '{$key}'");
        $qualification = null;
        if($st)
        {
            $qualification = null;
...
HEREDOC;
    ...
    public function toXML($prefix = null, $namespace = null)
    {
        ...
        $xml = "<{$p}qualification $xmlns "
            ...
            .$p.'mainarea="'.htmlspecialchars((string)$this->mainarea).'" '
            .$p.'subarea="'.htmlspecialchars((string)$this->subarea).'" '
            .$p.'total_credit_value="'.htmlspecialchars((string)$this->total_credit_value).'" '
            .$p.'guided_learning_hours="'.htmlspecialchars((string)$this->guided_learning_hours).'">' . "\n";
...
//		}
    ...
    public $total_credit_value= NULL;
    public $qual_status = NULL;
    public $active = 1;
    public $auto_id;
    public $tqt = null;
    public $ebs_ui_code = null;
