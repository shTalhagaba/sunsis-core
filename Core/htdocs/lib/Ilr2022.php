...
class Ilr2022 extends Entity
{
    ...
    public static function generateStream4(PDO $link, $submission, $contracts, $con1, $beta = 0)
    {
        ...
        {
            $table_name = "ilr2022W13";
            DAO::execute($link, "TRUNCATE TABLE ilr2022W13");
            DAO::execute($link, "INSERT INTO {$table_name} SELECT ilr.* FROM ilr INNER JOIN contracts ON ilr.`contract_id` = contracts.`id` WHERE contracts.`contract_year` = '2022' AND ilr.`submission` = 'W13'");
        }
        ...
        if($stouter)
        {
            ...
            $batch_file_xml .= '<Message xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns="ESFA/ILR/2022-23" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="ESFA/ILR/2022-23">';
            $batch_file_xml .= "<Header>";
            $batch_file_xml .= "<CollectionDetails>";
            $batch_file_xml .= "<Collection>ILR</Collection>";
            $batch_file_xml .= "<Year>2223</Year>";
            ...
            while($rowouter = $stouter->fetch())
            {
                ...
                if($st)
                {
                    while($row = $st->fetch())
                    {
                        $ilr = $row['ilr'];
                        $tr_id = $row['tr_id'];
                        $cp = 1; //DAO::getSingleValue($link,"select start_date>'2018-05-25' from tr where id = '$tr_id'");
                        $ilr = str_replace("&", "a", $ilr);
                        $record++;
                        $ilr = Ilr2022::loadFromXML($ilr);
                        $funding_type = Ilr2022::FundingType($ilr);
                        if($record==1)
                        {
                           ...
                           */
                            ...
                            if($stemp)
                            {
                                ...
                                while($rowemp = $stemp->fetch())
                                {
                                    ...
                                    $ilremp = Ilr2022::loadFromXML($ilremp);
			...
			if($uln == '')
                        {
                            $uln = DAO::getSingleValue($link, "SELECT uln FROM tr WHERE tr.id = '$tr_id'");
                        }
