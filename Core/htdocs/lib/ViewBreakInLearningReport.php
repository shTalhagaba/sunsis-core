<?php
class ViewBreakInLearningReport extends View
{
...
FROM
	bil_report2
    LEFT JOIN tr ON bil_report2.`tr_id` = tr.id
$where
ORDER BY
	bil_report2.status, bil_report2.l03
;
HEREDOC;

			$view = $_SESSION[$key] = new ViewBreakInLearningReport();
			$view->setSQL($sql);

			$f = new TextboxViewFilter('filter_surname', "WHERE bil_report2.surname LIKE '%s%%'", null);
			$f->setDescriptionFormat("Surname: %s");
			$view->addFilter($f);

			$f = new TextboxViewFilter('filter_firstname', "WHERE bil_report2.firstname LIKE '%s%%'", null);
			$f->setDescriptionFormat("Firstname: %s");
			$view->addFilter($f);

			$f = new TextboxViewFilter('filter_uln', "WHERE bil_report2.uln LIKE '%s%%'", null);
			$f->setDescriptionFormat("ULN: %s");
			$view->addFilter($f);

			$f = new TextboxViewFilter('filter_l03', "WHERE bil_report2.l03 LIKE '%s%%'", null);
			$f->setDescriptionFormat("L03: %s");
			$view->addFilter($f);

			$options = "SELECT id, CONCAT(firstnames,' ',surname), null, CONCAT('WHERE bil_report2.l03 IN (SELECT l03 FROM tr WHERE tr.assessor=',char(39),id,char(39), ')') FROM users where type=3 order by users.firstnames";
...
CONCAT('WHERE bil_report2.l03 IN (SELECT tr.l03 FROM group_members INNER JOIN groups ON group_members.`groups_id` = groups.`id`
...
CONCAT('WHERE bil_report2.l03 IN (SELECT tr.l03 FROM tr INNER JOIN courses_tr ON tr.id = courses_tr.`tr_id` INNER JOIN courses ON courses.id = courses_tr.`course_id`
...
/*			$options = array(
	...
	public function render(PDO $link)
	{
		...
		if($st)
		{
			...
			echo '<thead><tr><th class="topRow" colspan="7">Learner Information</th><th class="topRow"></th><th class="topRow" colspan="6">Break In Learning</th><th class="topRow"></th><th class="topRow" bgcolor="black"></th><th class="topRow" colspan="8">Restart</th></tr>';
