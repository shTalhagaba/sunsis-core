...
HEREDOC;
			...
			0=>array(1, '', null, 'ORDER BY qualification_type, level'),
	...
	public function render(PDO $link,$framework_title)
	{
		...
		if($st) 
		{
			...
			echo '<thead><tr><th>&nbsp;</th><th>Title</th><th>Internal Title</th><th>Type</th><th>Level</th><th>QAN</th><th>Proportion</th><th>Duration</th><th>Total Units</th><th>Main Aim</th></tr></thead>';
			...
			while($row = $st->fetch())
			{
				...
				echo '<td align="center">' . htmlspecialchars((string)$row['duration_in_months'] . " months") . "</td>";
				...
				echo '<td align="center">' . htmlspecialchars((string)$row['main_aim']) . "</td>";
				//echo '<td align="center">' . htmlspecialchars((string)$row['mandatory_units']) . " / " . htmlspecialchars((string)$row['units_required']-$row['mandatory_units']) .  "</td>";
