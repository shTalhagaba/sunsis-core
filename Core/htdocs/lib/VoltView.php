...
class VoltView extends CoreView
{
	...
	public function refresh(array $form_submission, PDO $link)
	{
		...
		if (array_key_exists("_reset", $form_submission)) {
			$filters_have_changed = $this->resetFilters();
		}
		...
		if ($filters_have_changed) {
			...
			$this->pageNumber = (int) $form_submission[View::KEY_PAGE_NUMBER];
		}
	...
	public function getSQLStatement()
	{
		...
		if (array_key_exists(View::KEY_PAGE_SIZE, $this->filters)) {
			$records_per_page = (int)$this->filters[View::KEY_PAGE_SIZE]->getValue();
