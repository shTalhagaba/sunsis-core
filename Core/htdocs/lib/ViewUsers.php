...
SELECT DISTINCT
	...
	users.firstnames,
	users.job_role,
	lookup_people_type.people_type AS system_user_type,
	users.employer_id,
...
FROM
	...
	LEFT JOIN lookup_people_type ON lookup_people_type.id = users.type
			...
			if(
				...
				$_SESSION['user']->type == User::TYPE_MANAGER ||
				$_SESSION['user']->type == User::TYPE_ORGANISATION_VIEWER ||
				$_SESSION['user']->type == User::TYPE_SCHOOL_VIEWER) && (!in_array(DB_NAME, ["am_lead_demo", "am_lead"]))
				&& !$_SESSION['user']->isAdmin()
			)
...
ORDER BY legal_name';
			...
			$options = 'SELECT DISTINCT lookup_user_types.id, lookup_user_types.description, null, CONCAT("WHERE users.type=",lookup_user_types.id) FROM lookup_user_types WHERE id != 5 ORDER BY description ';
			...
			$view->addFilter($f);
			            ...
			            $format = "WHERE users.created >= '%s'";
            $f = new DateViewFilter('from_created', $format, '');
            $f->setDescriptionFormat("From created: %s");
            $view->addFilter($f);
            $format = "WHERE users.created <= '%s'";
            $f = new DateViewFilter('to_created', $format, '');
...
HEREDOC;
			...
			while($row = $st->fetch())
			{
				...
				echo '<td>' . HTML::cell($row['location']) . '</td>';
