<?php
class VoltCheckboxViewFilter implements VoltIViewFilter
{
	/**
	 * Constructor
	 *
	...
	public function toHTML($allow_null = false)
	{
		...
		if(is_array($this->default_selected))
		{
			...
			{
				$jsDefaultValue .= ("'".addslashes((string)$value)."',");
			}
		}
		else
		{
			$jsDefaultValue .= "'".addslashes((string)$this->default_selected)."'";
		}
		$jsDefaultValue = '}';
	...
	//			$checked = in_array($option[0], $this->selected) ? 'checked="checked"':'';
				...
				$html .= '<td title="'.htmlspecialchars((string)$option[2]).'" style="padding-right:20px">'
					...
					.htmlspecialchars((string)$option[1]).'</td>';
