...
class KPI_Report_newstarts_period5 extends KPI_Report
{
	...
	protected function getData($link)
	{
		...
		$sql = "
			...
			LEFT JOIN
				central.lookup_submission_dates ON tr.start_date >= central.lookup_submission_dates.start_submission_date AND tr.start_date <= central.lookup_submission_dates.last_submission_date AND central.lookup_submission_dates.contract_type = 2 and submission!='W13'
			LEFT JOIN
			...
			WHERE 
				tr.start_date >= '2010-08-01' and tr.start_date <= '2011-07-31' $where ORDER BY tr.start_date, tr.surname ASC;"	;
