...
class KPI_Report_all_learners extends KPI_Report
{
	...
	protected function getData($link)
	{
		...
		$sql = "
			SELECT distinct
				...
				, DATEDIFF(ADDDATE(tr.dob,INTERVAL 19 YEAR),NOW()) AS days_to_19
...
#			LEFT JOIN
#				student_frameworks ON student_frameworks.tr_id = tr.id
#			LEFT JOIN
#				framework_qualifications ON framework_qualifications.framework_id = student_frameworks.id AND main_aim = 1
#			LEFT JOIN
#				student_qualifications AS sq ON sq.tr_id = tr.id AND sq.id = framework_qualifications.id
			LEFT JOIN 
				brands on brands.id = org.manufacturer					
			WHERE c.contract_year = '" . $this->year . "' $where			
		...
		while($row = $query->fetch())
		{
            ...
            if($contract_year<2012)
                $additional = DAO::getResultset($link, "select extractvalue(ilr,'/ilr/learner/L13'), extractvalue(ilr,'/ilr/learner/L15'), extractvalue(ilr,'/ilr/learner/L12'), extractvalue(ilr,'/ilr/learner/L29'), extractvalue(ilr,'/ilr/learner/L17') from ilr left join contracts on contracts.id = ilr.contract_id where tr_id = $tr_id order by contract_year desc, submission desc limit 0,1");
            else
                $additional = DAO::getResultset($link, "select extractvalue(ilr,'/Learner/Sex'), extractvalue(ilr,'/Learner/LLDDandHealthProblem[LLDDType=\'DS\']/LLDDCode'), extractvalue(ilr,'/Learner/Ethnicity'), extractvalue(ilr,'/Learner/LearningDelivery/LearningDeliveryFAM[LearnDelFAMType=\'ALN\']/LearnDelFAMCode') from ilr left join contracts on contracts.id = ilr.contract_id where tr_id = $tr_id order by contract_year desc, submission desc limit 0,1");
