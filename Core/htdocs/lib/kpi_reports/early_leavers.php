...
class KPI_Report_early_leavers extends KPI_Report
{
	...
	protected function getData($link)
	{
		...
		$sql = "
			SELECT DISTINCT
				...
				#, framework_qualifications.id as NVQ_aim_reference
				, org.retailer_code as region
				, org.district as area_code
				#, framework_qualifications.title as NVQ_Title
				...
				, TIMESTAMPDIFF(DAY, tr.start_date, tr.closure_date) AS learning_days
...
#			LEFT JOIN
#				framework_qualifications on framework_qualifications.framework_id = frameworks.id and main_aim = 1
			LEFT JOIN
			...
			LEFT JOIN
				organisations as providers on providers.id = tr.provider_id
			LEFT JOIN
	...
	#			student_qualifications as sq on sq.tr_id = tr.id and sq.qualification_type = 'NVQ'
			...
			WHERE 
				(status_code = 3 or status_code=4) AND (TO_DAYS(tr.target_date) - TO_DAYS(tr.closure_date)) > 0
		...
		//	elseif ( preg_match('/Level 1/i',$row['NVQ_Title']) ) {  $row['NVQ_Title'] = 'Level 1';   }
			...
			if($ilr!='')
			{
				...
				if ( isset($ilr) && $ilr != "" ) {
					...
					$row['national_insurance'] = $ilr->learnerinformation->L26;
				}
			}
			$this->data[] = $row;
		}
