<?php
class GetFrameworks extends View
{
...
//		{
			...
			$sql = <<<HEREDOC
SELECT
	id, title, duration_in_months, comments 
FROM
	frameworks where id!=$fid;
HEREDOC;

			$view = $_SESSION[$key] = new GetFrameworks();
			$view->setSQL($sql);
...
//		}
	...
	public function render(PDO $link, $fid)
	{
		...
		if($st) 
		{
			...
			echo '<thead><tr><th>&nbsp;</th><th>Title</th><th> Duration </th><th>Comments</th></tr></thead>';

			echo '<tbody>';
			while($row = $st->fetch())
			{
	
				//$que = "select description from lookup_sector_types where id={$row['sector']}";
				//$sector = trim(DAO::getSingleValue($link, $que));
	
				echo HTML::viewrow_opening_tag('do.php?_action=copy_framework2&source_id=' . rawurlencode($fid) . '&target_id=' . rawurlencode($row['id']));
				echo '<td><img src="/images/rosette.gif" /></td>';
				echo '<td align="left">' . HTML::cell($row['title']) . "</td>";
				echo '<td align="center">' . HTML::cell($row['duration_in_months']) . "</td>";
				//echo '<td align="left">' . HTML::cell($row['start_date']) . "</td>";
				//echo '<td align="left">' . HTML::cell($row['end_date']) . "</td>";
				//echo '<td align="left">' . htmlspecialchars((string)$sector) . "</td>";
