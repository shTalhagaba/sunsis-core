<?php
class ViewOrganisationTrainingRecords extends View
{
...
HEREDOC;
			...
			while($row = $st->fetch())
			{
				...
				$tr_id = $row['tr_id'];
				$que = "select DATE_FORMAT(start_date,'%m') from student_frameworks where tr_id='$tr_id'";
				$framework_start_month = (int)trim(DAO::getSingleValue($link, $que));
				$que = "select DATE_FORMAT(start_date,'%Y') from student_frameworks where tr_id='$tr_id'";
				$framework_start_year = (int)trim(DAO::getSingleValue($link, $que));
				$current_year = (int)date("Y");
				$current_month = (int)date("m");
				$current_month_since_framework_start_date = ($current_year - $framework_start_year) * 12;
		
				if($current_month > $framework_start_month)
					$current_month_since_framework_start_date += ($current_month - $framework_start_month + 1);
				else
					$current_month_since_framework_start_date += ($current_month - $framework_start_month + 1);
				
				/*if($framework_title==NULL || $framework_title=='')
					$current_month_since_framework_start_date = NULL;
				*/
				$que = "select targets from student_frameworks where tr_id='$tr_id'";
				$targets = trim(DAO::getSingleValue($link, $que));
				...
				if($targets!='')
				{	
					//$targetsxml = new SimpleXMLElement($targets);
					$targetsxml = XML::loadSimpleXML($targets);
					foreach($targetsxml->month as $month)
					{	
						$m++;
						if($m>=$current_month_since_framework_start_date && $month>0)
						{
							$target_month = $m;
							$target = $month;
							break;		
				...
				*/
				...
				echo "<td align=\"center\" style=\"$textStyle;font-size:80%;border-right-style:solid\">" . HTML::cell($target) . '</td>';
