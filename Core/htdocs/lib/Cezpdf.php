...
class Cezpdf extends Cpdf
{
    ...
    function ezTable(&$data, $cols = '', $title = '', $options = '')
    {
        ...
        while ($abortTable) {
            ...
            if (!isset($this->ez['pageWidth'])) {
                $this->ez['pageWidth'] = 595; // fallback: A4 width in points
            }
            foreach ($data as $row) {
                $cnt++;
                ...
                while (!$abortTable && $ok == 0) {
                    ...
                    while (!$abortTable && ($newPage || $newRow)) {
                        ...
                        if ($newPage || $y < $this->ez['bottomMargin'] || (isset($options['minRowSpace']) && $y < ($this->ez['bottomMargin'] + $options['minRowSpace']))) {
                            ...
                            if (!isset($this->ez['pageHeight'])) {
                                $this->ez['pageHeight'] = 842; // fallback: A4 width in points
                            }
    ...
    function alink($info, $internal = 0)
    {
        ...
        switch ($info['status']) {
            ...
            case 'sol':
                ...
                $i = $info['nCallback'];
                $this->ez['links'][$i] = array('x' => $info['x'], 'y' => $info['y'], 'angle' => $info['angle'], 'decender' =>
            ...
            case 'eol':
                ...
                // add underlining
                if ($internal) {
                    $this->addInternalLink($start['url'], $start['x'], $start['y'] + $start['decender'], $info['x'], $start['y'] +
                        $start['decender'] + $start['height']);
                } else {
                    ...
                    $this->addLink($start['url'], $start['x'], $start['y'] + $start['decender'], $info['x'], $start['y'] +
                        $start['decender'] + $start['height']);
                    $this->restoreState();
                }
                break;
