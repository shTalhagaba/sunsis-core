    ...
    private function rule_Learning_Delivery_Rules($link, $ilr)
    {
        ...
        foreach($ilr->LearningDelivery as $delivery)
        {
            ...
            if(isset($AchDate_Date))
                if($AchDate_Date->after("2017-07-31"))
            ...
            //AddHours_3
            $BasicSkillsType = DAO::getSingleValue($link, "SELECT BasicSkillsType FROM lars201617.Core_LARS_AnnualValue WHERE LearnAimRef = '$LearnAimRef' and EffectiveFrom >= '2016-08-01' LIMIT 0,1");
            ...
            //LearnActEndDate_03
            if(isset($LearnActEndDate) && $LearnActEndDate_Date->after("2018-07-31"))
            ...
            // LearnAimRef_01
            $ValidLookupOnLARS = DAO::getSingleValue($link, "SELECT LearnAimRef FROM lars201617.Core_LARS_LearningDelivery WHERE LearnAimRef = '$LearnAimRef';");
            if($ValidLookupOnLARS=='' && $LearnAimRef!='ZPROG001')
                $errors[] = "LearnAimRef_01: The Learning aim reference must be a valid lookup on LARS (".$LearnAimRef.")";

            //LearnAimRef_03
            $MatchingCategory_Apprenticeship = DAO::getSingleValue($link, "SELECT COUNT(*) FROM lars201617.Core_LARS_Validity WHERE LearnAimRef = '$LearnAimRef' and ValidityCategory='APPRENTICESHIPS';");
            ...
            //AimType_06
            if($FundModel=='25' && $AimType=='5' && $LearnStartDate_Date->after("2017-07-31"))
            ...
            //LearnStartDate_03
            if($DD07=="N" && $ProgType!='24' && $LearnStartDate_Date->after("2017-07-31"))
            ...
            //LearnStartDate_06
            $EffectiveTo = DAO::getSingleValue($link, "SELECT EffectiveTo FROM lars201617.Core_LARS_Framework WHERE FworkCode='$FworkCode' and ProgType='$ProgType' and PwayCode='$PwayCode'");
            ...
            */
            ...
            /*if($FundModel=='35' or $FundModel=='36' or $FundModel=='81' or $FundModel=='99')
                if(Date::isDate($LearnActEndDate))
                    foreach($delivery->LearningDeliveryFAM as $LearnDelFAM)
                        if(Date::isDate($LearnDelFAM->LearnDelFAMDateTo))
                            if($LearnActEndDate_Date->before(Date::toMySQL($LearnDelFAM->LearnDelFAMDateTo)))
                                //$errors[] = "LearnDelFAMDateTo_03: If returned, the date applies to must not be after the Learning actual end date of the aim (" . $delivery->LearnAimRef . ")";
                */
            //LearnDelFAMType_02
            ...
            //DateOfBirth_25
            $DateOfBirth_25_Under19 = DAO::getSingleValue($link, "SELECT IF(DATE_ADD('$DateOfBirth',INTERVAL 19 YEAR)>DATE_ADD('2016-08-31',INTERVAL 1 DAY),1,0);");
...
///////////////////////// FD_LEARNER Ends//////////////////////////////
    ...
    private function rule_LearnAimRef_09($link, $ilr)
    {
        foreach($ilr->LearningDelivery as $delivery)
        {
            ...
            $found = DAO::getSingleValue($link, "SELECT COUNT(*) FROM lars201617.Core_LARS_Validity WHERE LearnAimRef = '$delivery->LearnAimRef' AND UPPER(ValidityCategory) = 'APPRENTICESHIPS' AND IF(EndDate IS NOT NULL, '$LearnStartDate' BETWEEN StartDate AND EndDate, '$LearnStartDate' >= StartDate) ;");
    ...
    private function rule_LearnAimRef_10($link, $ilr)
    {
        foreach($ilr->LearningDelivery as $delivery)
        {
            ...
            $found = DAO::getSingleValue($link, "SELECT COUNT(*) FROM lars201617.Core_LARS_Validity WHERE LearnAimRef = '$delivery->LearnAimRef' AND UPPER(ValidityCategory) = '1619_EFA' AND (('$LearnStartDate' BETWEEN StartDate AND EndDate ) OR ('$LearnStartDate' >= StartDate AND EndDate IS NULL));");
    ...
    private function rule_FworkCode_05($link, $ilr)
    {
        foreach($ilr->LearningDelivery as $delivery)
        {
            if($delivery->FworkCode!='' && $delivery->FworkCode!='undefined' && $delivery->LearnAimRef!='ZPROG001' && $delivery->FundModel!='99')
            {
                ...
                $first = DAO::getSingleValue($link, "SELECT COUNT(*) FROM lars201617.Core_LARS_FrameworkAims WHERE LearnAimRef = '$LearnAimRef' AND FworkCode = '$FworkCode' AND PwayCode = '$PwayCode' AND ProgType = '$ProgType'");
                $second = DAO::getSingleValue($link, "SELECT COUNT(*) FROM lars201617.Core_LARS_FrameworkCmnComp WHERE ProgType = '$ProgType' AND FworkCode = '$FworkCode' AND PwayCode = '$PwayCode' AND CommonComponent IN (SELECT FrameworkCommonComponent FROM lars201617.Core_LARS_LearningDelivery WHERE LearnAimRef = '$LearnAimRef');");
