...
class clsMsDocGenerator{
	 ...
	 */
	function addTableRow($cells, $aligns = NULL, $vAligns = NULL, $inlineStyle = NULL, $classesName = NULL, $width, $height){
		if(! $this->tableIsOpen)
...
//		if($width==NULL) // Khushnood
//			$this->documentBuffer .= "<td width=\"$cellWidth\" align=\"$align\" valign=\"$vAlign\" style=\"$style\"{$classAttr}>$cells[$i]</td>\n";
//		else
		$this->documentBuffer .= "<td height=\"$height[$i]\" width=\"$width[$i]\" align=\"$align\" valign=\"$vAlign\" style=\"$style\"{$classAttr}>$cells[$i]</td>\n";
