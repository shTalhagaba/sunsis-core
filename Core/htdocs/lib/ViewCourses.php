...
;
	...
	public function render(PDO $link)
	{
		...
		if ($st) {
			...
			{
				echo in_array(DB_NAME, ["am_demo"]) ? '<th class="topRow" colspan="7">Training Records</th>' : '<th class="topRow" colspan="5">Training Records</th>';
			}
...
HEREDOC;
			if($this->getPreference('showStudentNumbers') == '1')
			{
				...
				{
					echo <<<HEREDOC
				<th class="bottomRow small">Capacity</th>
				<th class="bottomRow small">Total</th>
				<th class="bottomRow small">Remaining</th>
...
HEREDOC;
			...
			while ($row = $st->fetch())
			{
				...
				if($this->getPreference('showStudentNumbers'))
				{
					...
					if( in_array(DB_NAME, ["am_demo"]) )
					{
						echo '<td align="center">'.$row['course_capacity'].'</td>';
						echo '<td align="center"'.($row['total'] == 0?' style="color:silver" ':'').'><a href="'.$link_tr.'SHOW_ALL">'.$row['total'].'</a></td>';
						if( $row['course_capacity'] != '' )
						{
							$cc = (int)$row['course_capacity'];
							$ct = (int)$row['total'];
							$cr = $cc - $ct;
