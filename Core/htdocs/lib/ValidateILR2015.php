	...
	private function rule_Learning_Delivery_Rules($link, $ilr)
	{
		...
		foreach($ilr->LearningDelivery as $delivery)
		{
			...
			if(Date::isDate("".$delivery->LearnActEndDate))
			{
				...
				$LearnActEndDate6Months = $LearnActEndDate_Date;
			...
			// AchDate_08
			if( ($ProgType=='24' || $ProgType=='25') && $AimType=='1' && $Outcome == '1' && (!isset($AchDate)))
			...
			// AddHours_1
			if(($FundModel=='35' || $FundModel=='81') && $AddHours!='' && $LearnStartDate_Date->before("2015-08-01"))
			...
			//AddHours_3
			$BasicSkillsType = DAO::getSingleValue($link, "SELECT BasicSkillsType FROM lars201516.Core_LARS_AnnualValue WHERE LearnAimRef = '$LearnAimRef' and EffectiveFrom >= '2015-08-01' LIMIT 0,1");
			...
			//AimType_05
			if( ($AimType=='35' || $AimType=='81' || $AimType=='70' || $AimType=='10' || $AimType=='99') && $AimType=='5')
			...
			if($CompStatus=='1' && isset($LearnActEndDate))
				$errors[] = "ComptStatus_02: If the Learning actual end date is returned, the Completion status must not be 'the learner is continuing or intending to continue the learning activities leading to the learning aim'";
			...
			// ConRefNumber_03
			if( ($FundModel=='10' || $FundModel=='25' || $FundModel=='35' || $FundModel=='81' || $FundModel=='82' || $FundModel=='99') && $ConRefNumber!='')
			...
			// DelLocPostCode_12
			if($DelLocPostCode=='')
				$errors[] = "DelLocPostCode_12: The Delivery location postcode must be returned";
			...
			// EmpOutcome_02
			if( ($FundModel=='35' || $FundModel=='81' || $FundModel=='10') && $EmpOutcome!='' && $EmpOutcome!='1' && $EmpOutcome!='2')
			...
			// EmpOutcome_03
			if(($there_is_no_record_of_wpl==false || $ProgType=='25') && $EmpOutcome!='')
				$errors[] = "EmpOutcome_03: If the learning aim is workplace learning or is part of a Trailblazer apprenticeship, the Employment outcome must not be returned";


			// FundModel_01
			if($FundModel!='10' && $FundModel!='25' && $FundModel!='35' && $FundModel!='70' && $FundModel!='81' && $FundModel!='82' && $FundModel!='99')
				$errors[] = "FundModel_01: The Funding model must be a valid lookup";

			// FundModel_03
			if($FundModel!='99' && $DD10=='Y')
			...
			// FundModel_05
			if($FundModel=='82')
				$errors[] = "FundModel_05: Funding Model 82 is not valid for use";

			//FwordCode_01
			if($FworkCode=='' && $DD07=="Y" && $ProgType!='24')
			...
			//LearnActEndDate_03
			//if(isset($LearnActEndDate) && $LearnActEndDate_Date->after("2017-07-31"))
			...
			// LearnAimRef_01
			$ValidLookupOnLARS = DAO::getSingleValue($link, "SELECT LearnAimRef FROM lars201516.Core_LARS_LearningDelivery WHERE LearnAimRef = '$LearnAimRef';");
			if($ValidLookupOnLARS=='')
				$errors[] = "LearnAimRef_01: The Learning aim reference must be a valid lookup on LARS";




			// LearnAimRef_01
			$found = DAO::getSingleValue($link, "SELECT COUNT(*) FROM lars201516.Core_LARS_LearningDelivery WHERE LearnAimRef = '$LearnAimRef';");
			if(!$found)
				$errors[] = "LearnAimRef_01: The Learning aim reference must be a valid lookup on LARS";

			//LearnAimRef_03
			$MatchingCategory_Apprenticeship = DAO::getSingleValue($link, "SELECT COUNT(*) FROM lars201516.Core_LARS_Validity WHERE LearnAimRef = '$LearnAimRef' and ValidityCategory='APPRENTICESHIPS';");
			if($LearnStartDate_Date->before("2011-08-01") && $FundModel=="35" && $DD07=="Y" && $AimType==3 && (!$MatchingCategory_Apprenticeship))
			...
			//AimType_06
			if($FundModel=='25' && $AimType=='5' && $LearnStartDate_Date->after("2016-07-31"))
			...
			//LearnStartDate_03
			if($DD07=="N" && $ProgType!='24' && $LearnStartDate_Date->after("2016-07-31"))
				$errors[] = "LearnStartDate_03: If the learning aim is not part of an apprenticeship or traineeship, the learning start date must not be after the current teaching year";
			...
			//LearnStartDate_06
			$EffectiveTo = DAO::getSingleValue($link, "SELECT EffectiveTo FROM lars201516.Core_LARS_Framework WHERE FworkCode='$FworkCode' and ProgType='$ProgType' and PwayCode='$PwayCode'");
			...
			//LearnStartDate_07
			$EffectiveTo = DAO::getSingleValue($link, "SELECT EffectiveTo FROM lars201516.Core_LARS_FrameworkAims WHERE LearnAimRef = '$LearnAimRef' and FworkCode='$FworkCode' and ProgType='$ProgType' and PwayCode='$PwayCode'");
			...
			//LearnDelFAMType_44
			if($LearnStartDate_Date->after("2015-07-31") && ($FundModel == '35' || $FundModel == '70' || $FundModel == '81') && $there_is_no_record_of_hhs && $there_is_no_record_of_ldm_034 && !$isComponentAim)
				$errors[] = "LearnDelFAMType_44: The household situation must be returned (" . $delivery->LearnAimRef . ")";

			//DateOfBirth_25
			$DateOfBirth_25_Under19 = DAO::getSingleValue($link, "SELECT IF(DATE_ADD('$DateOfBirth',INTERVAL 19 YEAR)>DATE_ADD('2015-08-31',INTERVAL 1 DAY),1,0);");
		...
		// PlanLearnHours_01
		if(($FundModel==25 || $FundModel==82 || $FundModel==35 || $FundModel==81 || $FundModel==10 || $FundModel==99) && $PlanLearnHours=="" && $DD07=="N" && $there_is_no_record_of_wpl)
...
///////////////////////// FD_LEARNER Ends//////////////////////////////
	...
	private function rule_LearnAimRef_01($link, $ilr)
	{
		foreach($ilr->LearningDelivery as $delivery)
		{
			$found = DAO::getSingleValue($link, "SELECT COUNT(*) FROM lars201516.Core_LARS_LearningDelivery WHERE LearnAimRef = '$delivery->LearnAimRef';");
			if($found=='0' && $delivery->LearnAimRef!='ZPROG001' && $delivery->LearnAimRef!='Z0007835' && $delivery->LearnAimRef!='CMISC001' && $delivery->LearnAimRef!='ZVOC0004')
			{
	...
	private function rule_LearnAimRef_03($link, $ilr)
	{
		foreach($ilr->LearningDelivery as $delivery)
		{
			...
			{
				$found = DAO::getSingleValue($link, "SELECT COUNT(*) FROM lars201617.Core_LARS_Validity WHERE LearnAimRef = '$delivery->LearnAimRef'");
				if($found=='0')
				{
	...
	private function rule_LearnAimRef_36($link, $ilr)
	{
		...
		foreach($ilr->LearningDelivery as $delivery)
		{
			...
			if($LearnStartDate->before('01/08/2011') AND in_array($delivery->FundModel, array('81', '70')))
				return "LearnAimRef_36: This Learning aim reference (".$delivery->LearnAimRef.") is not valid in LARS for this Funding model and this Learning start date.";
			$found = DAO::getSingleValue($link, "SELECT COUNT(*) FROM lars201415.Core_LARS_Validity WHERE LearnAimRef = '$delivery->LearnAimRef' AND LOWER(ValidityCategory) = 'any';");
			if($delivery->FundModel == '99' AND $DD10 == 'N' AND $found == 0)
				return "LearnAimRef_36: This Learning aim reference (".$delivery->LearnAimRef.") is not valid in LARS for this Funding model and this Learning start date.";
		}
	}
	...
	private function rule_LearnAimRef_09($link, $ilr)
	{
		foreach($ilr->LearningDelivery as $delivery)
		{
			...
			$found = DAO::getSingleValue($link, "SELECT COUNT(*) FROM lars201516.Core_LARS_Validity WHERE LearnAimRef = '$delivery->LearnAimRef' AND UPPER(ValidityCategory) = 'APPRENTICESHIPS' AND IF(EndDate IS NOT NULL, '$LearnStartDate' BETWEEN StartDate AND EndDate, '$LearnStartDate' >= StartDate) ;");
	...
	private function rule_LearnAimRef_10($link, $ilr)
	{
		foreach($ilr->LearningDelivery as $delivery)
		{
			...
			$found = DAO::getSingleValue($link, "SELECT COUNT(*) FROM lars201516.Core_LARS_Validity WHERE LearnAimRef = '$delivery->LearnAimRef' AND UPPER(ValidityCategory) = '1619_EFA' AND (('$LearnStartDate' BETWEEN StartDate AND EndDate ) OR ('$LearnStartDate' >= StartDate AND EndDate IS NULL));");
			if(in_array($delivery->FundModel, array('25')) AND $found == 0)
	...
	private function rule_LearnStartDate_03($link, $ilr)
	{
		foreach($ilr->LearningDelivery as $delivery)
		{
			...
			{
				return "LearnStartDate_03: The Learning start date must not be after the current teaching year. (".$delivery->LearnAimRef.")";
			}
		}
	}
	...
	private function rule_LearnDelFAMType_09($link, $ilr)
	{
		foreach($ilr->LearningDelivery as $delivery)
		{
			if(($delivery->AimType=='1' || $delivery->AimType=='4') && $delivery->FundModel=='35')
			{
				...
				if($sof==0)
					return "LearnDelFAMType_09: One of the Source of funding records must be 105 (Skills Funding Agency)  (".$delivery->LearnAimRef.")";
			}
		}
	}
			...
			if(isset($delivery->LearnActEndDate))
				if(("".$delivery->LearnActEndDate)!='' && ("".$delivery->LearnActEndDate)!='dd/mm/yyyy' && ("".$delivery->LearnActEndDate)!='00000000')
					if($delivery->CompStatus=='1')
						return "CompStatus_02: If the Learning actual end date is returned, the Completion status must not be 'the learner is continuing or intending to continue the learning activities leading to the learning aim' (".$delivery->LearnAimRef.")";
		}
	}
