...
{
	const PORT = 80;
	const HOST_NAME = 'applicants.remit.co.uk';
	...
	public function getQualification()
	{
		...
		$uri = '/vacancytest/xmltrvacancyexport.asp';
		...
		if(stripos($html, '<title>Runtime Error</title>') !== false)
		{
			return null;
		}
	...
	public function getUnit($uri)
	{
		...
		//$xml .= '<raw>'.htmlspecialchars(print_r($fields, true)).'</raw>';
		$xml .= '<url>'.htmlspecialchars('http://'.NDAQ2::HOST_NAME.$uri)."</url>\n";
		$xml .= '<timestamp>'.htmlspecialchars(date('Y-m-d\TH:i:sP'))."</timestamp>\n";
		...
		foreach($childNodes as $child)
		{
			if( ($child->nodeType == XML_ELEMENT_NODE) && ($child->tagName == "li") )
			{
				...
				else
				{
					...
					$query = './/a';
					$nodelist = $xpath->query($query, $child);
					$content = $nodelist->item(0)->getAttribute('href');
	...
	private function getPage($url)
	{
		...
		if($this->proxy_addr != '')
		{
			$in =  "GET https://".GetApplicants::HOST_NAME."$url HTTP/1.0\r\n";
		}
		else
		{
			$in =  "GET $url HTTP/1.0\r\n";
		...
		$in .= 'Host: ' . GetApplicants::HOST_NAME . "\r\n";
		...
		if(($start = strpos($doc, "\r\n\r\n")) !== false)
		{
			$doc = substr($doc, $start + 4);
		}
		elseif(($start = strpos($doc, "\r\r")) !== false)
		{
			$doc = substr($doc, $start + 2);
		}
		elseif(($start = strpos($doc, "\n\n")) !== false)
		{
			$doc = substr($doc, $start + 2);
		}
	...
	{
		if(DEBUG) echo "<p style=\"font-weight:bold\">readResponse()</p>";
		...
		if($out === false)
		{
			throw new Exception("Problem reading response from NDAQ. Reason: (" . socket_last_error($socket) . "): " . socket_strerror(socket_last_error($socket)));
		}

		if(DEBUG) echo '<pre style="margin-left:20px;font-size:8pt;color:blue">' . htmlspecialchars(substr($doc, 0, 1024)) . '</pre>';
		return $doc;
	}
