<?php
class ViewSchools extends View
{
...
SELECT STRAIGHT_JOIN
	...
	(select count(*) from users where users.employer_id = organisations.id and users.type=5) as no_of_students,
...
WHERE 
	organisations.organisation_type like '%6%' $where
GROUP BY			
	organisations.id
HEREDOC;

			$view = $_SESSION[$key] = new ViewSchools();
			$view->setSQL($sql);
			...
			$options = array(
				0=>array(1, 'With and without students ', null, 'where (select count(users.surname) from users where users.employer_id = organisations.id and users.type=5)>=0'),
				1=>array(2, 'With students', null, 'where (select count(users.surname) from users where users.employer_id = organisations.id and users.type=5)>0'),
				2=>array(3, 'Without students ', null, 'where (select count(users.surname) from users where users.employer_id = organisations.id and users.type=5)=0'));
			...
			$f = new TextboxViewFilter('filter_minimum_learners', "WHERE (select count(users.surname) from users where users.employer_id = organisations.id and users.type=5) >= '%s'", null);
			$f->setDescriptionFormat("Minimum Learners: %s");
			$view->addFilter($f);
			
			// Maximum Learners Filter
			$f = new TextboxViewFilter('filter_maximum_learners', "WHERE (select count(users.surname) from users where users.employer_id = organisations.id and users.type=5) <= '%s'", null);
	...
	public function render(PDO $link)
	{
		...
		if($st) 
		{
			...
			echo '<thead><tr><th>&nbsp;</th><th>Name</th><th>Town</th><th>Contact Person</th><th>Post Code</th><th>Telephone</th><th>County</th><th>No of <br> Students</th></tr></thead>';
			...
			while($row = $st->fetch())
			{
				...
				echo '<tr>';
				echo HTML::viewrow_opening_tag('/do.php?_action=read_school&id=' . $row['org']);
				echo '<td><img src="/images/blue-building.png" width="25" height="30" border="0" /></td>';
				echo '<td align="left">' . HTML::cell($row['name']) . '</td>';
				...
				//echo  '<td align="center"><a href="http://maps.google.co.uk/maps?f=q&hl=en&q='.urlencode($row['postcode']).'">' . $row['postcode'] . '</a></td>';
				echo '<td align="left">' . HTML::cell($row['telephone']) . '</td>';
				echo '<td align="left">' . HTML::cell($row['address_line_4']) . '</td>';
				echo '<td align="center">' . HTML::cell($row['no_of_students']) . '</td>';
		//		echo '<td align="left">' . HTML::cell($row['sector']). '</td>';
		//		echo '<td align="center">' . HTML::cell($row['company_number']) . '</td>';

//				echo '<td align="center">' . HTML::cell($row['health']) . '</td>';
	...
	/*			if($row['health']!='')
				...
				//echo '<td align="center">' . HTML::cell($row['complient']) . '</td>';
*/				
				echo '</tr>';
			}
