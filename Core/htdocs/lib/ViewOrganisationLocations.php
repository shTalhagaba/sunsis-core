...
HEREDOC;
			...
			$options = array(
				0=>array(1, 'Type (asc), Level (asc)', null, 'ORDER BY is_legal_address DESC, full_name ASC'),
	...
	public function render(PDO $link, $back)
	{
		...
		if($st) 
		{
			...
			echo '<tr><th>&nbsp;</th><th>Name</th><th>Building & Street</th><th>Locality</th><th>Town</th><th>County</th><th>Postcode</th><th>Telephone</th><th>Assessors</th><th>Health & Safety</th><th>Compliant</th><th>Store Number</th><th>SFA Area Cost Factor</th><th>EFA Area Cost Factor</th></tr>';
			echo '<tbody>';
			while($loc = $st->fetch())
			{
				echo HTML::viewrow_opening_tag('do.php?_action=read_location&id=' . $loc['id'] . '&back=' . $back . '&organisation_id=' . $loc['organisations_id']);
				echo $loc['is_legal_address'] == 1 ? '<td style="background-color: green">' : '<td>';
				echo '<a href="do.php?_action=read_location&id=' . $loc['id'] . '&organisation_id=' . $loc['organisations_id'] .  '"><img src="/images/building-icon.png" border="0" title="#' . $loc['id'] . '"/></a>';
				echo '</td>';
