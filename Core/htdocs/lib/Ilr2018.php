...
class Ilr2018 extends Entity
{
    ...
    public static function generateStream4(PDO $link, $submission, $contracts, $con1, $beta = 0)
    {
		...
		$table_name = "ilr";
		if(DB_NAME == "am_baltic" && $submission == "W13")
		{
			$table_name = "ilr2018W13";
			DAO::execute($link, "TRUNCATE TABLE ilr2018W13");
			DAO::execute($link, "INSERT INTO {$table_name} SELECT ilr.* FROM ilr INNER JOIN contracts ON ilr.`contract_id` = contracts.`id` WHERE contracts.`contract_year` = '2018' AND ilr.`submission` = 'W13'");
		}
        ...
        if($stouter)
        {
            ...
            $batch_file_xml .= '<Message xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns="ESFA/ILR/2018-19" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="ESFA/ILR/2018-19">';
            $batch_file_xml .= "<Header>";
            $batch_file_xml .= "<CollectionDetails>";
            $batch_file_xml .= "<Collection>ILR</Collection>";
            $batch_file_xml .= "<Year>1819</Year>";
            ...
            $batch_file_xml .= "<Release>V 8</Release>";
            ...
            while($rowouter = $stouter->fetch())
            {
                ...
                if($st)
                {
                    while($row = $st->fetch())
                    {
                        ...
                        $record++;
                        $ilr = Ilr2018::loadFromXML($ilr);
                        $funding_type = Ilr2018::FundingType($ilr);
                        if($record==1)
                        {
                            ...
                            if(isset($ilr->OTJHours) and $ilr->OTJHours!='')
                                $batch_file_xml .= "<OTJHours>" . $ilr->OTJHours . "</OTJHours>";
			   ...
			   */
                            ...
                            if($stemp)
                            {
                                ...
                                while($rowemp = $stemp->fetch())
                                {
                                    ...
                                    $ilremp = Ilr2018::loadFromXML($ilremp);
                                    foreach($ilremp->LearnerEmploymentStatus as $empstatusemp)
                                    {
                                        ...
                                        if(("".$empstatusemp->EmpStat)!='' && ("".$empstatusemp->DateEmpStatApp)!='' && ("".$empstatusemp->DateEmpStatApp)!='dd/mm/yyyy' && (!in_array(("".$empstatusemp->DateEmpStatApp),$Duplicate)) && (sizeof($EmploymentRecord)==0 || $EmpRec!=$EmploymentRecord[sizeof($EmploymentRecord)-1]))
                                        {
                                            ...
                                            if($AgreeId!='')
                                                $batch_file_xml .= "<AgreeId>" . $AgreeId . "</AgreeId>";
