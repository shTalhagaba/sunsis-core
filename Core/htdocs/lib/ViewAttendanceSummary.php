<?php
class ViewAttendanceSummary extends View
{
...
FROM
	...
	AND acl.resource_id IN (select id from groups where tutor='$id' or assessor='$id')
...
HEREDOC;
			...
			$view = $_SESSION[$key] = new ViewAttendanceSummary();
			$view->setSQL($sql);
		...
		$f = new DropDownViewFilter('school', $options, null, true);
...
SELECT DISTINCT
	...
	NULL,
	CONCAT('WHERE attendance_reports.group_id IN (', GROUP_CONCAT(DISTINCT groups.id), ')')
FROM
	users
LEFT JOIN
	groups
ON
	users.id = groups.assessor

WHERE
	users.type=3
GROUP BY users.id
ORDER BY users.firstnames, users.surname
	;
HEREDOC;
...
SELECT DISTINCT
	...
	CONCAT(users.firstnames,' ',users.surname),
	NULL,
	CONCAT('WHERE attendance_reports.pot_id IN (', GROUP_CONCAT(DISTINCT tr.id), ')')
FROM
	users
LEFT JOIN
	tr
ON
	users.id = tr.assessor
