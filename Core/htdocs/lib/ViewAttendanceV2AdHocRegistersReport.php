<?php
class ViewAttendanceV2AdHocRegistersReport extends View
{
	...
	'' AS actual_hours
FROM
	lesson_extra_learners INNER JOIN lessons INNER JOIN tr INNER JOIN attendance_module_groups INNER JOIN attendance_modules
	ON lesson_extra_learners.`lesson_id` = lessons.id
	AND tr.id = lesson_extra_learners.tr_id
	AND lessons.groups_id = attendance_module_groups.id
...
SQL;
			...
			$view = $_SESSION[$key] = new ViewAttendanceV2AdHocRegistersReport();
			$view->setSQL($sql);
