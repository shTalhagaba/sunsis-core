...
class DAO
{
    ...
    public static function saveObjectToTable(PDO $link, $table, &$data, $allow_UPDATE = true)
    {
        ...
        if (($num_corresponding_keys == count($pri_keys)) && count($pri_keys) > 0) {
            ...
            foreach ($pri_keys as $key_name => $extra) {
                ...
                $sql .= " `{$key_name}`='" . addslashes((string)$array[$key_name]) . "' ";
            }
        ...
        if (!$recordExists) {
            ...
            if ($fieldCount > 0) {
                ...
                foreach ($pri_keys as $key_name => $extra) {
                    if ((strpos($extra, 'auto_increment') !== false) && array_key_exists($key_name, $array)) {
                        if (is_array($data)) {
            ...
            foreach ($array as $field => $value) {
                if (array_key_exists($field, $table_metadata)) {
                    ...
                    if ($table == "assessor_review_forms") {
                        if (is_null($value) || $value == "" || ($column_key == 'PRI')) {
                            continue;
                        }
                    } elseif (is_null($value) || ($column_key == 'PRI')) {
                        continue;
                    }
            ...
            foreach ($table_metadata as $fieldname => $meta) {
                if (($meta['COLUMN_KEY'] == 'PRI') && array_key_exists($fieldname, $array)) {
                    ...
                    $sql .= " `{$fieldname}`='" . addslashes((string)$array[$fieldname]) . "' ";
                }
            }
            ...
            if (strpos($extra, 'auto_increment') === false) {
                foreach ($rows as $row) {
                    if (is_array($row)) {
                        ...
                        $value = property_exists($row, $key_name) ? $row->$key_name : '';
                    } else {
                        $value = '';
                    }
    ...
    public static function schemaEntityExists(PDO $link, $db = "", $table = "", $column = "", $cache_key = null, $cache_key_ttl = 3600)
    {
        ...
        if ($column) {
            $sql = "SELECT 1 FROM information_schema.columns WHERE table_schema='" . addslashes((string)$db) . "' AND table_name='" . addslashes((string)$table) . "' AND column_name='" . addslashes((string)$column) . "'";
        } elseif ($table) {
            $sql = "SELECT 1 FROM information_schema.tables WHERE table_schema='" . addslashes((string)$db) . "' AND table_name='" . addslashes((string)$table) . "'";
        } elseif ($db) {
            $sql = "SELECT 1 FROM information_schema.schemata WHERE schema_name='" . addslashes((string)$db) . "'";
        } else {
            throw new Exception("At least one of the following arguments must be specified: {db}, {table} or {table and column}");
        }
