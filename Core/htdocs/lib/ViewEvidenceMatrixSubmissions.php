<?php
class ViewEvidenceMatrixSubmissions extends View
{
...
SELECT
    ...
    ,(SELECT CONCAT(firstnames, ' ', surname) FROM users WHERE project_submissions.assessor = users.id) AS assessor_2
    ,'' AS project_status
    ,'' AS project_progress
    ,'' AS months_on_project
    ,'' AS project_2
    ,tr.start_date
	...
	,project_submissions.due_date
	,project_submissions.submission_date
	,project_submissions.marked_date AS marked_date_1
	,project_submissions.completion_date
	,(SELECT COUNT(*) FROM project_submissions WHERE project_submissions.project_id = tr_projects.`id`) AS total_submissions
	,(SELECT COUNT(*) FROM project_submissions AS s2  WHERE s2.project_id = tr_projects.`id` AND  s2.id <= project_submissions.id) AS submission
	,tr.contract_id
	,tr_projects.tr_id
    ,project_submissions.due_date < CURDATE() AS expired
    ,project_submissions.iqa_status
    ,project_submissions.sent_iqa_date
    ,project_submissions.assessor_signed_off
    ,project_submissions.acc_rej_date
    ,IF(project_submissions.iqa_status=1,"Accepted",IF(project_submissions.iqa_status=2,"Rejected","")) AS iq_status
    ,project_submissions.set_date
    ,project_submissions.iqa_recheck_date
    ...
    ,CASE WHEN project_submissions.system = 1 THEN "Skilsure" WHEN project_submissions.system = 2 THEN "Smart Assessor" END AS `system`
    ,assessorsng.reduced_sample
FROM
    project_submissions
...
where tr_projects.tr_id IS NOT NULL
HEREDOC;
            $view = $_SESSION[$key] = new ViewEvidenceMatrixSubmissions();
            $view->setSQL($sql);
            ...
            $options = array(
                0=>array(0, 'Show all', null, null),
                1=>array(1, '1. Red', null, 'WHERE assessorsng.reduced_sample=1'),
                2=>array(2, '2. Amber', null, 'WHERE assessorsng.reduced_sample=2'),
                3=>array(3, '3. Green', null, 'WHERE assessorsng.reduced_sample=3'));
            $f = new DropDownViewFilter('filter_reduced_sample', $options, 0, false);
            $f->setDescriptionFormat("Show: %s");
            $view->addFilter($f);
            ...
            $format = "WHERE project_submissions.marked_date >= '%s'";
            ...
            $format = "WHERE project_submissions.assessor_signed_off >= '%s'";
...
ORDER BY firstnames, surname;";
            ...
            $options = "SELECT username, CONCAT(firstnames,' ',surname), LEFT(firstnames, 1), CONCAT('WHERE project_submissions.assessor=',username) FROM users WHERE type=3 ORDER BY firstnames";
            $f = new DropDownViewFilter('filter_manager', $options, null, true);
            $f->setDescriptionFormat("Person Reviewed: %s");
            $view->addFilter($f);

            $options = "SELECT id, title, LEFT(title, 1), CONCAT('WHERE courses.id=',id) FROM courses WHERE active=1 ORDER BY title";
            $f = new DropDownViewFilter('filter_programme', $options, null, true);
            $f->setDescriptionFormat("Programme: %s");
            $view->addFilter($f);

            $options = "SELECT id, project, null, CONCAT('WHERE project_submissions.project_id=',id) FROM evidence_project ORDER BY project";
            $f = new DropDownViewFilter('filter_project', $options, null, true);
            $f->setDescriptionFormat("Project: %s");
            $view->addFilter($f);
            ...
            $options = array(
                0=>array(0, 'Show all', null, null),
                1=>array(1, '1. First submission', null, 'WHERE ((SELECT COUNT(*) FROM project_submissions AS s2  WHERE s2.`project_id` = tr_projects.`id`) = 1) '),
                2=>array(2, '2. Second submission', null, 'WHERE ((SELECT COUNT(*) FROM project_submissions AS s2  WHERE s2.`project_id` = tr_projects.`id`) = 2 )'));
            $f = new DropDownViewFilter('filter_submission_count', $options, 0, false);
            $f->setDescriptionFormat("Show: %s");
            $view->addFilter($f);
...
HEREDOC;
            ...
            while($row = $st->fetch())
            {
                ...
                echo '<td>' . HTML::cell($row['project']) . '</td>';
                ...
                elseif($row['iqa_recheck_date']!='')
                    $status = "IQA Recheck";
                elseif($row['sent_iqa_date']!='' and ($row['iqa_status']!='2' or $row['iqa_status']!='2'))
                    $status = "IQA";
                ...
                if($row['reduced_sample']=="1")
                    echo '<td class="bg-red">' . HTML::cell($row['assessor']) . '</td>';
                elseif($row['reduced_sample']=="2")
                    echo '<td class="bg-amber">' . HTML::cell($row['assessor']) . '</td>';
                elseif($row['reduced_sample']=="3")
                    echo '<td class="bg-green">' . HTML::cell($row['assessor']) . '</td>';
                else
                ...
                if($class=='bg-green')
                    ...
                    echo '<td>Behind</td>';
                echo is_array($total_units) && count($total_units) != 0 ? '<td style="cursor:pointer;" class="text-center '.$class.'" onclick="showApProgressLookup(\'' . $row['tr_id'] . '\');">' . $passed_units . '/' . $total_units . ' = ' . round(($passed_units/$total_units) * 100)  . '%</td>': '<td class="text-center '.$class.'">0%</td>';
                echo '<td>' . HTML::cell($current_training_month) . '</td>';
                echo is_array($total_units) && count($total_units) != 0 ? '<td style="cursor:pointer;" class="text-center '.$class.'" onclick="showApProgressLookup(\'' . $row['tr_id'] . '\');">' . $passed_units2 . '/' . $total_units . ' = ' . round(($passed_units2/$total_units) * 100)  . '%</td>': '<td class="text-center '.$class.'">0%</td>';
                ...
                echo '<td>' . HTML::cell($row['system']) . '</td>';
                echo '</tr>';
            }
