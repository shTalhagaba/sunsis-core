...
class Ilr2021 extends Entity
{
    ...
    public static function generateStream4(PDO $link, $submission, $contracts, $con1, $beta = 0)
    {
        ...
        $table_name = "ilr";
        if(in_array(DB_NAME, ["am_baltic", "am_lead"]) && $submission == "W13")
        {
            $table_name = "ilr2021W13";
            DAO::execute($link, "TRUNCATE TABLE ilr2021W13");
            DAO::execute($link, "INSERT INTO {$table_name} SELECT ilr.* FROM ilr INNER JOIN contracts ON ilr.`contract_id` = contracts.`id` WHERE contracts.`contract_year` = '2021' AND ilr.`submission` = 'W13'");
        }
        ...
        if($stouter)
        {
            ...
            $batch_file_xml .= '<Message xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns="ESFA/ILR/2021-22" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="ESFA/ILR/2021-22">';
            $batch_file_xml .= "<Header>";
            $batch_file_xml .= "<CollectionDetails>";
            $batch_file_xml .= "<Collection>ILR</Collection>";
            $batch_file_xml .= "<Year>2122</Year>";
            ...
            while($rowouter = $stouter->fetch())
            {
                ...
                if($st)
                {
                    while($row = $st->fetch())
                    {
                        ...
                        $record++;
                        $ilr = Ilr2021::loadFromXML($ilr);
                        $funding_type = Ilr2021::FundingType($ilr);
                        if($record==1)
                        {
                           ...
                           */
                            ...
                            if($stemp)
                            {
                                ...
                                while($rowemp = $stemp->fetch())
                                {
                                    ...
                                    $ilremp = Ilr2021::loadFromXML($ilremp);
