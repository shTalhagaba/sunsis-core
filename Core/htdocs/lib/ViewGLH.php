...
class ViewGLH extends View
{
    ...
    public static function getInstance(PDO $link, $id)
    {
        ...
        $sql = <<<HEREDOC
SELECT
	glh.*, lookup_otj_types.`description`,TIME_FORMAT(TIMEDIFF(time_to,time_from),"%H") as duration
FROM
	glh
left JOIN lookup_otj_types on lookup_otj_types.id = glh.type
WHERE tr_id='$id'
ORDER BY date
;
HEREDOC;
    ...
    public function render(PDO $link)
    {
		...
		$tr_id = $row['tr_id'];
                if($_SESSION['user']->type == User::TYPE_LEARNER || $_SESSION['user']->type == User::TYPE_REVIEWER)
                    ...
                    echo HTML::viewrow_opening_tag('/do.php?_action=edit_glh_hours&glh_id=' . $row['id'] . '&tr_id=' . $row['tr_id']);
...
SQL;
                ...
                foreach($records AS $row)
                {
                    ...
                    echo '<td align="center" style="font-size: 11px;">' . ViewGLH::convertToHoursMins($minutes, '%02d hours %02d minutes') . '</td>';
                    //echo '<td align="center" style="font-size: 11px;">' . $row['duration_hours'] . ' hour(s) ' . $row['duration_minutes'] . ' minutes</td>';
