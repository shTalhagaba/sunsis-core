...
class Ilr2017Temp extends Entity
{
	...
	public static function loadFromDatabase(PDO $link, $submission, $contract_id, $tr_id, $L03)
	{
		...
		$vo = XML::loadSimpleXML(DAO::getSingleValue($link, "select ilr from ilr2017W13 WHERE submission='$submission' and contract_id=$contract_id and tr_id = $tr_id and L03='$L03'"));
		return $vo;
	}
	...
	public static function generateStream4(PDO $link, $submission, $contracts, $con1, $beta = 0)
	{
		...
		DAO::execute($link, "TRUNCATE TABLE ilr2017W13");
		$sql = <<<SQL
INSERT INTO `ilr2017W13`
SELECT
	ilr.*
FROM
	ilr INNER JOIN contracts ON ilr.contract_id = contracts.id
WHERE
 	ilr.submission = '$submission' AND ilr.contract_id IN ($contracts)
SQL;
		...
		$sqlouter = "SELECT distinct l03 FROM ilr2017W13 left join contracts on contracts.id = ilr2017W13.contract_id WHERE submission = '$submission' and contract_id in ($contracts) and is_active=1 ORDER BY l03, tr_id";
		...
		if($stouter)
		{
			...
			while($rowouter = $stouter->fetch())
			{
				...
				$AimSeqNumber =0;
				$no_of_aims=0;
				$sql = "SELECT * FROM ilr2017W13 left join contracts on contracts.id = ilr2017W13.contract_id WHERE submission = '$submission' and contract_id in ($contracts) and l03 = '$l03' and is_active=1 ORDER BY tr_id desc";
				$st = $link->query($sql);
				if($st)
				{
					while($row = $st->fetch())
					{
						...
						$record++;
						$trs[] = $row['tr_id'];
						$ilr = Ilr2017Temp::loadFromXML($ilr);
						$funding_type = Ilr2017Temp::FundingType($ilr);
						if($record==1)
						{
							...
							$plan_learn_hours = DAO::getSingleColumn($link,"SELECT extractvalue(ilr,'/Learner/PlanLearnHours') FROM ilr2017W13 left join contracts on contracts.id = ilr2017W13.contract_id WHERE submission = '$submission' and contract_id in ($contracts) and l03 = '$l03' and is_active=1 ORDER BY tr_id desc");
							$plan_learn_hours = array_sum($plan_learn_hours);

							$plan_eep_hours = DAO::getSingleColumn($link,"SELECT extractvalue(ilr,'/Learner/PlanEEPHours') FROM ilr2017W13 left join contracts on contracts.id = ilr2017W13.contract_id WHERE submission = '$submission' and contract_id in ($contracts) and l03 = '$l03' and is_active=1 ORDER BY tr_id desc");
							...
							foreach($ilr->LearnerFAM as $lf)
							{
								if($lf->LearnFAMType=='EDF' and ($lf->LearnFAMCode=='1' or $lf->LearnFAMCode=='2'))
								{
									...
									$EDF1 = DAO::getSingleColumn($link,"SELECT extractvalue(ilr,'/Learner/LearnerFAM[LearnFAMType=\"EDF\" and LearnFAMCode=\"1\"]/LearnFAMCode') FROM ilr2017W13 left join contracts on contracts.id = ilr2017W13.contract_id WHERE submission = '$submission' and contract_id in ($contracts) and l03 = '$l03' and is_active=1 ORDER BY tr_id desc");
									$EDF1 = array_sum($EDF1);
									if($EDF1>=1)
										$batch_file_xml .= "<LearnerFAM><LearnFAMType>EDF</LearnFAMType><LearnFAMCode>1</LearnFAMCode></LearnerFAM>";

									$EDF2 = DAO::getSingleColumn($link,"SELECT extractvalue(ilr,'/Learner/LearnerFAM[LearnFAMType=\"EDF\" and LearnFAMCode=\"2\"]/LearnFAMCode') FROM ilr2017W13 left join contracts on contracts.id = ilr2017W13.contract_id WHERE submission = '$submission' and contract_id in ($contracts) and l03 = '$l03' and is_active=1 ORDER BY tr_id desc");
									$EDF2 = array_sum($EDF2);
									if($EDF2>=2)
							...
							$sqlemp = "SELECT * FROM ilr2017W13 left join contracts on contracts.id = ilr2017W13.contract_id WHERE submission = '$submission' and contract_id in ($contracts) and l03 = '$l03' and is_active=1 ORDER BY tr_id";
							...
							if($stemp)
							{
								...
								while($rowemp = $stemp->fetch())
								{
									...
									$ilremp = Ilr2017Temp::loadFromXML($ilremp);
			...
			if($exists)
			{
				...
				if($stdestinations)
				{
					while($rowdestinations = $stdestinations->fetch())
					{
						...
						$uln = DAO::getSingleValue($link, "SELECT extractvalue(ilr,'/Learner/ULN') FROM ilr2017W13 WHERE tr_id = '$tr_id' ORDER BY contract_id DESC, submission DESC LIMIT 0,1");
	...
	public static function getFilename(PDO $link, $contract_id, $submission, $L01)
	{
		...
		$sql = "SELECT * FROM ilr2017W13 WHERE submission = '$submission' and contract_id ='$contract_id' and is_active=1;";

		// R06 record level validation starts
		$que = "select count(DISTINCT concat(L01,L03)) from ilr2017W13 where submission = '$submission' and contract_id='$contract_id' and is_active=1;";
		$no_of_distinct_ilrs = trim(DAO::getSingleValue($link, $que));
		$que = "select count(concat(L01,L03)) from ilr2017W13 where submission = '$submission' and contract_id = '$contract_id' and is_active=1;";
	...
	{
		$shouldMigrate = false;
		$vo = Ilr2017Temp::loadFromDatabase($link, "W13", $contract_id, $tr_id, $l03);
