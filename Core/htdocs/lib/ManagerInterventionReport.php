<?php
class ManagerInterventionReport extends View
{
...
WHERE
(description in ("Review Form 72HR Emailed to Learner","Review Form 168HR Emailed to Employer") AND DATE_ADD(`date`, INTERVAL 1 DAY) < NOW()
AND form_id NOT IN (SELECT review_id FROM arf_introduction WHERE `signature_employer_font` IS NOT NULL))
UNION
SELECT
tr.`firstnames`,
tr.`surname`,
...
LEFT JOIN users ON users.id = tr.`assessor`
WHERE assessment_plan_log_submissions.due_date < CURDATE()
    and assessment_plan_log_submissions.`completion_date` IS NULL
    AND DATE_ADD(`date`, INTERVAL 1 DAY) < NOW()
    AND form_type = "Plan"
    AND sent_iqa_date is null
AND forms_audit.description = "Assessment Plan Chaser 2 sent"
GROUP BY form_id;
HEREDOC;
            $view = $_SESSION[$key] = new ManagerInterventionReport();
            $view->setSQL($sql);
        }
