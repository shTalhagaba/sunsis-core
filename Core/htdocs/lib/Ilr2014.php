...
class Ilr2014 extends Entity
{
	...
	public static function generateStream4(PDO $link, $submission, $contracts, $con1, $beta = 0)
	{
		...
		if($stouter)
		{
			...
			print("\r\n");
			print ('<Message xmlns="http://www.theia.org.uk/ILR/2014-15/1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">');
			print("\r\n\t");
			print("<Header>");
			print("\r\n\t\t");
			...
			while($rowouter = $stouter->fetch())
			{
				...
				if($st)
				{
					while($row = $st->fetch())
					{
						...
						$record++;
						$ilr = Ilr2014::loadFromXML($ilr);
						$funding_type = Ilr2014::FundingType($ilr);
						if($record==1)
						{
							print("\r\n\t<Learner>\r\n\t\t");
							...
							if(!empty($xpath[0]))
								print("<LearnerFAM><LearnFAMType>HNS</LearnFAMType><LearnFAMCode>" . $xpath[0]  . "</LearnFAMCode></LearnerFAM>");
							...
							if(!empty($xpath[0]))
								print("<LearnerFAM><LearnFAMType>MGA</LearnFAMType><LearnFAMCode>" . $xpath[0]  . "</LearnFAMCode></LearnerFAM>");

							$xpath = $ilr->xpath("/Learner/LearnerFAM[LearnFAMType='EGA']/LearnFAMCode");
							if(!empty($xpath[0]))
								print("<LearnerFAM><LearnFAMType>EGA</LearnFAMType><LearnFAMCode>" . $xpath[0]  . "</LearnFAMCode></LearnerFAM>");

							$xpath = $ilr->xpath("/Learner/LearnerFAM[LearnFAMType='FME']/LearnFAMCode");
							if(!empty($xpath[0]))
								print("<LearnerFAM><LearnFAMType>FME</LearnFAMType><LearnFAMCode>" . $xpath[0]  . "</LearnFAMCode></LearnerFAM>");

							$xpath = $ilr->xpath("/Learner/LearnerFAM[LearnFAMType='PPE']/LearnFAMCode");
							if(!empty($xpath[0]))
								print("<LearnerFAM><LearnFAMType>PPE</LearnFAMType><LearnFAMCode>" . $xpath[0]  . "</LearnFAMCode></LearnerFAM>");
							if(!empty($xpath[1]))
								print("<LearnerFAM><LearnFAMType>PPE</LearnFAMType><LearnFAMCode>" . $xpath[1]  . "</LearnFAMCode></LearnerFAM>");
							...
							if($stemp)
							{
								...
								while($rowemp = $stemp->fetch())
								{
									...
									$ilremp = Ilr2014::loadFromXML($ilremp);
									foreach($ilremp->LearnerEmploymentStatus as $empstatusemp)
									{
											...
											//if(("".$empstatusemp->EmpStat)!='' && ("".$empstatusemp->DateEmpStatApp)!='' && ("".$empstatusemp->DateEmpStatApp)!='dd/mm/yyyy')
										{
											...
											if(!empty($xpath[0]))
												print("<EmploymentStatusMonitoring><ESMType>SEM</ESMType><ESMCode>" . $xpath[0] . "</ESMCode></EmploymentStatusMonitoring>");
											print("</LearnerEmploymentStatus>");
						...
						foreach($ilr->LearningDelivery as $delivery)
						{
							...
							if(DB_NAME=="am_lead")
							{
								if($delivery->AimType == '4' && strpos($row['ilr'], 'ZPROG001') !== false)
									continue;
							}
							$AimSeqNumber++;
							print("\r\n<LearningDelivery>");
							...
							if($delivery->AimType=="1" || $delivery->AimType=="4")
							{
								...
								$xpath = $delivery->xpath("./LearningDeliveryFAM[LearnDelFAMType='WPP']/LearnDelFAMCode"); $wpp = (empty($xpath[0]))?'':$xpath[0];
								if($wpp!='' && $wpp!='undefined')
									print("<LearningDeliveryFAM><LearnDelFAMType>WPP</LearnDelFAMType><LearnDelFAMCode>" . $wpp . "</LearnDelFAMCode></LearningDeliveryFAM>");
							}
							...
							if($delivery->AimType=="3" || $delivery->AimType=="4")
							{
								...
								if($pod!='' && $pod!='undefined')
									print("<LearningDeliveryFAM><LearnDelFAMType>POD</LearnDelFAMType><LearnDelFAMCode>" . $pod . "</LearnDelFAMCode></LearningDeliveryFAM>");
							}
							...
							if($delivery->AimType=="1")
							{
								...
								if($tbs!='' && $tbs!='undefined')
									print("<LearningDeliveryFAM><LearnDelFAMType>TBS</LearnDelFAMType><LearnDelFAMCode>" . $tbs . "</LearnDelFAMCode></LearningDeliveryFAM>");
							}
							...
							if($delivery->LearnAimRef=='Z0007834' || $delivery->LearnAimRef=='Z0007835' || $delivery->LearnAimRef=='Z0007836' || $delivery->LearnAimRef=='Z0007837' || $delivery->LearnAimRef=='Z0007838')
							{
								foreach( $delivery->LearningDeliveryWorkPlacement as $ldwp)
								{
									...
									{
										print("<LearningDeliveryWorkPlacement>");
										print("<WorkPlaceStartDate>" . Date::toMySQL($ldwp->WorkPlaceStartDate) . "</WorkPlaceStartDate>");
										if($ldwp->WorkPlaceEndDate!='')
											print("<WorkPlaceEndDate>" . Date::toMySQL($ldwp->WorkPlaceEndDate) . "</WorkPlaceEndDate>");
										if($ldwp->WorkPlaceMode!='')
											print("<WorkPlaceMode>". $ldwp->WorkPlaceMode."</WorkPlaceMode>");
										if($ldwp->WorkPlaceEmpId!='')
											print("<WorkPlaceEmpId>".$ldwp->WorkPlaceEmpId."</WorkPlaceEmpId>");
										print("</LearningDeliveryWorkPlacement>");
									}
								}
							}
							...
							if($delivery->AimType=="1")
							{
								foreach($delivery->ApprenticeshipTrailblazerFinancialDetails as $ldwp)
								{
									if($ldwp->TBFinType!='')
									{
										print("<ApprenticeshipTrailblazerFinancialDetails>");
										print("<TBFinType>".$ldwp->TBFinType."</TBFinType>");
										print("<TBFinCode>".$ldwp->TBFinCode."</TBFinCode>");
										print("<TBFinDate>". $ldwp->TBFinDate."</TBFinDate>");
										print("<TBFinAmount>".$ldwp->TBFinAmount."</TBFinAmount>");
										print("</ApprenticeshipTrailblazerFinancialDetails>");
									}
								}
							}
							...
							{
								$xpath = $delivery->xpath("./LearningDeliveryFAM[LearnDelFAMType='HEM']/LearnDelFAMCode");
								if(!empty($xpath[0]) && in_array('1', $xpath))
									print("<LearningDeliveryFAM><LearnDelFAMType>HEM</LearnDelFAMType><LearnDelFAMCode>1</LearnDelFAMCode></LearningDeliveryFAM>");
								if(!empty($xpath[0]) && in_array('3', $xpath))
									print("<LearningDeliveryFAM><LearnDelFAMType>HEM</LearnDelFAMType><LearnDelFAMCode>3</LearnDelFAMCode></LearningDeliveryFAM>");
								if(!empty($xpath[0]) && in_array('5', $xpath))
									print("<LearningDeliveryFAM><LearnDelFAMType>HEM</LearnDelFAMType><LearnDelFAMCode>5</LearnDelFAMCode></LearningDeliveryFAM>");


								$LearningDeliveryHETagWritten = false;
								//Learning Delivery HE fields
								if($delivery->LearningDeliveryHE->NUMHUS !='' && $delivery->LearningDeliveryHE->NUMHUS !='undefined')
								{
									if(!$LearningDeliveryHETagWritten)
									{
										$LearningDeliveryHETagWritten = true;
										print("\r\n<LearningDeliveryHE>");
									}
									print("\r\n<NUMHUS>" . $delivery->LearningDeliveryHE->NUMHUS . "</NUMHUS>");
								}
								if($delivery->LearningDeliveryHE->SSN !='' && $delivery->LearningDeliveryHE->SSN !='undefined')
								{
									...
									print("\r\n<SSN>" . $delivery->LearningDeliveryHE->SSN . "</SSN>");
								}
								if($delivery->LearningDeliveryHE->QUALENT3 !='' && $delivery->LearningDeliveryHE->QUALENT3 !='undefined')
								{
									...
									print("\r\n<QUALENT3>" . $delivery->LearningDeliveryHE->QUALENT3 . "</QUALENT3>");
								}
								if($delivery->LearningDeliveryHE->SOC2000 !='' && $delivery->LearningDeliveryHE->SOC2000 !='undefined')
								{
									...
									print("\r\n<SOC2000>" . $delivery->LearningDeliveryHE->SOC2000 . "</SOC2000>");
								}
								if($delivery->LearningDeliveryHE->SEC !='' && $delivery->LearningDeliveryHE->SEC !='undefined')
								{
									...
									print("\r\n<SEC>" . $delivery->LearningDeliveryHE->SEC . "</SEC>");
								}
								if($delivery->LearningDeliveryHE->UCASAPPID !='' && $delivery->LearningDeliveryHE->UCASAPPID !='undefined')
								{
									...
									print("\r\n<UCASAPPID>" . $delivery->LearningDeliveryHE->UCASAPPID . "</UCASAPPID>");
								}
								if($delivery->LearningDeliveryHE->TYPEYR !='' && $delivery->LearningDeliveryHE->TYPEYR !='undefined')
								{
									...
									print("\r\n<TYPEYR>" . $delivery->LearningDeliveryHE->TYPEYR . "</TYPEYR>");
								}
								if($delivery->LearningDeliveryHE->MODESTUD !='' && $delivery->LearningDeliveryHE->MODESTUD !='undefined')
								{
									...
									print("\r\n<MODESTUD>" . $delivery->LearningDeliveryHE->MODESTUD . "</MODESTUD>");
								}
								if($delivery->LearningDeliveryHE->FUNDLEV !='' && $delivery->LearningDeliveryHE->FUNDLEV !='undefined')
								{
									...
									print("\r\n<FUNDLEV>" . $delivery->LearningDeliveryHE->FUNDLEV . "</FUNDLEV>");
								}
								if($delivery->LearningDeliveryHE->FUNDCOMP !='' && $delivery->LearningDeliveryHE->FUNDCOMP !='undefined')
								{
									...
									print("\r\n<FUNDCOMP>" . $delivery->LearningDeliveryHE->FUNDCOMP . "</FUNDCOMP>");
								}
								if($delivery->LearningDeliveryHE->STULOAD !='' && $delivery->LearningDeliveryHE->STULOAD !='undefined')
								{
									...
									print("\r\n<STULOAD>" . $delivery->LearningDeliveryHE->STULOAD . "</STULOAD>");
								}
								if($delivery->YEARSTU !='' && $delivery->YEARSTU !='undefined')
								{
									...
									print("\r\n<YEARSTU>" . $delivery->YEARSTU . "</YEARSTU>");
								}
								if($delivery->MSTUFEE !='' && $delivery->MSTUFEE !='undefined')
								{
									...
									print("\r\n<MSTUFEE>" . $delivery->MSTUFEE . "</MSTUFEE>");
								}
								...
								if($delivery->PCFLDCS !='' && $delivery->PCFLDCS !='undefined')
								{
									...
									print("\r\n<PCFLDCS>" . $delivery->PCFLDCS . "</PCFLDCS>");
								}
								if($delivery->PCSLDCS !='' && $delivery->PCSLDCS !='undefined')
								{
									...
									print("\r\n<PCSLDCS>" . $delivery->PCSLDCS . "</PCSLDCS>");
								}
								if($delivery->PCTLDCS !='' && $delivery->PCTLDCS !='undefined')
								{
									...
									print("\r\n<PCTLDCS>" . $delivery->PCTLDCS . "</PCTLDCS>");
								}
								...
								if($delivery->DOMICILE !='' && $delivery->DOMICILE !='undefined')
								{
									...
									print("\r\n<DOMICILE>" . $delivery->DOMICILE . "</DOMICILE>");
								}
			...
			//$exists = DAO::getSingleValue($link, "select count(*) from destinations");
			...
			if($exists)
			{
				$destinations = "SELECT destinations.tr_id FROM destinations INNER JOIN tr ON destinations.`tr_id` = tr.`id` INNER JOIN contracts ON tr.`contract_id` = contracts.id WHERE contracts.id IN ($contracts) GROUP BY destinations.tr_id ORDER BY tr_id";
				$stdestinations =$link->query($destinations);
				if($stdestinations)
				{
					while($rowdestinations = $stdestinations->fetch())
					{
						$tr_destinations = $link->query("SELECT destinations.* FROM destinations INNER JOIN tr ON destinations.`tr_id` = tr.`id` INNER JOIN contracts ON tr.`contract_id` = contracts.id WHERE contracts.id IN (" . $contracts . ") AND destinations.tr_id = " . $rowdestinations['tr_id']);
						$tr_id = $rowdestinations['tr_id'];
						$l03 = DAO::getSingleValue($link, "select l03 from tr where id = '$tr_id'");
						$uln = DAO::getSingleValue($link, "select uln from tr where id = '$tr_id'");
						print("\r\n<LearnerDestinationandProgression>");
						print("\r\n<LearnRefNumber>");
						print($l03);
						print("</LearnRefNumber>");
						print("\r\n<ULN>" . str_pad($uln,10,'9',STR_PAD_LEFT) . "</ULN>");
						while($tr_dest = $tr_destinations->fetch())
						{
							print("\r\n<DPOutcome>");
							print("\r\n<OutType>" . $tr_dest['outcome_type'] . "</OutType>");
							print("\r\n<OutCode>" . $tr_dest['outcome_code'] . "</OutCode>");
							print("\r\n<OutStartDate>" . $tr_dest['outcome_start_date'] . "</OutStartDate>");
							if($tr_dest['outcome_end_date']!='' && $tr_dest['outcome_end_date']!='0000-00-00')
								print("\r\n<OutEndDate>" . $tr_dest['outcome_end_date'] . "</OutEndDate>");
							if($tr_dest['outcome_collection_date']!='' && $tr_dest['outcome_collection_date']!='0000-00-00')
								print("\r\n<OutCollDate>" . $tr_dest['outcome_collection_date'] . "</OutCollDate>");
							print("\r\n</DPOutcome>");
						}
						print("\r\n</LearnerDestinationandProgression>");
					}
