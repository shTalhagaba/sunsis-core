...
class ProgressBar
{
	...
	function render($processing) {
		...
		//$GLOBALS['CONTENT'] = '';
		print($this->getContent($processing));
		$this->flush();
		//$this->setProgressBarProgress(0);
	}

	function getContent($processing) {
		$content = "";
	...
	function setProgressBarProgress($percentDone, $text = '') {
		...
		if ($text) {
			print('document.getElementById("'.$this->textid.'").innerHTML = "'.htmlspecialchars((string)$text).'";');
		}
		print('}</script>'."\n");
		$this->flush();
	}
...
echo '</div>';
for ($i = 0; $i < ($size = 50); $i++) {
	$p->setProgressBarProgress($i*100/$size,'','Processing');
	usleep(1000000*0.1);
}
