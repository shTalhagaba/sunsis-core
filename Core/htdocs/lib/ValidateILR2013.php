 ...
 */
	...
	private function rule_LearnAimRef_01($link, $ilr)
	{
		foreach($ilr->LearningDelivery as $delivery)
		{
			...
			if($found=='0' && $delivery->LearnAimRef!='ZPROG001' && $delivery->LearnAimRef!='Z0007835')
			{
	...
	private function rule_LearnStartDate_03($link, $ilr)
	{
		...
		{
			$LearnStartDate = new Date($delivery->LearnStartDate);
			if(($delivery->FundModel=='21' || $delivery->FundModel=='22' || $delivery->FundModel=='70' || $delivery->FundModel=='10')&& $LearnStartDate->after('31/07/2014'))
			{
	...
	private function rule_FworkCode_01($link, $ilr)
	{
		foreach($ilr->LearningDelivery as $delivery)
		{
			if($delivery->ProgType!='99' && $delivery->ProgType!='' && $delivery->FworkCode=='')
	...
	private function rule_FworkCode_05($link, $ilr)
	{
		foreach($ilr->LearningDelivery as $delivery)
		{
			if($delivery->FworkCode!='' && $delivery->FworkCode!='undefined' && $delivery->LearnAimRef!='ZPROG001' && $delivery->FundModel!='99')
			{
				...
				$first = DAO::getSingleValue($link, "SELECT COUNT(*) FROM lars201314.LARS_FrameworkAims1314 WHERE LARS_LearnAimRef = '$LearnAimRef' AND LARS_FwkSectorCode = '$FworkCode' AND LARS_FwkPway = '$PwayCode' AND LARS_FwkProgType = '$ProgType'");
