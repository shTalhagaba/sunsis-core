...
HEREDOC;
		...
		{
			throw new Exception("Could not execute database query to find lead record. " . '----' . $query . '----' . $link->errorCode());
		}
	...
	public function save(PDO $link)
	{
		...
		$result = DAO::saveObjectToTable($link, 'crm_leads', $this);
...
USING
	crm_leads
	LEFT OUTER JOIN	crm_activities ON (crm_leads.id = crm_activities.entity_id AND crm_activities.entity_type = 'lead')
	LEFT OUTER JOIN crm_entities_comments ON (crm_leads.id = crm_entities_comments.entity_id AND crm_entities_comments.entity_type = 'lead')
	LEFT OUTER JOIN crm_entities_files ON (crm_leads.id = crm_entities_files.entity_id AND crm_entities_files.entity_type = 'lead')
	LEFT OUTER JOIN notes n1 ON (crm_leads.id = n1.parent_id AND n1.parent_table = 'crm_leads')
...
WHERE
	crm_leads.id={$this->id};
HEREDOC;
			$r = DAO::execute($link, $query);
			if($r > 0)
			{
				$repository = Repository::getRoot().'/crm/Lead/'.$lead_id;
	...
	{
		$activities_ids = DAO::getSingleValue($link, "SELECT GROUP_CONCAT(id) FROM crm_activities WHERE `entity_id` = '{$this->id}' AND entity_type = 'lead';");
...
(
	...
	FROM
		...
		ON notes.username = users.username
	WHERE
		notes.parent_table='crm_leads' AND notes.parent_id='$this->id'
...
HEREDOC;
	...
	public static function getListLeadTaskStatus($individual)
	{
		$options = [
			'1' => 'Not Started',
			'2' => 'Started',
	...
	public static function getDDLLeadTaskStatus()
	{
		$options = array(
			array('1', 'Not Started'),
			array('2', 'Started'),
	...
	public static function getListLeadMeetingType($individual)
	{
		$options = [
			'1' => 'Meeting',
			'2' => 'Training',
	...
	public static function getDDLLeadMeetingType()
	{
		$options = array(
			array('1', 'Meeting'),
			array('2', 'Training'),
	...
	public static function getListLeadMeetingStatus($individual)
	{
		$options = [
			'1' => 'Planned',
			'2' => 'Held',
	...
	public static function getDDLLeadMeetingStatus()
	{
		$options = array(
			array('1', 'Planned'),
			array('2', 'Held'),
	...
	public static function getListLeadCallStatus1($individual)
	{
		$options = [
			'1' => 'Inbound',
			'2' => 'Outbound'
	...
	public static function getDDLLeadCallStatus1()
	{
		$options = array(
			array('1', 'Inbound'),
			array('2', 'Outbound')
	...
	public static function getListLeadCallStatus2($individual)
	{
		$options = [
			'1' => 'Planned',
			'2' => 'Held',
	...
	public static function getDDLLeadCallStatus2()
	{
		$options = array(
			array('1', 'Planned'),
			array('2', 'Held'),
	...
	public static function getDDLLeadTaskPriority()
	{
		$options = array(
			array('1', 'Low'),
			array('2', 'Medium'),
	...
	public static function getListLeadTaskPriority($individual = '')
	{
		$a = array(
			'1' => 'Low',
			'2' => 'Medium',
	...
	public static function getListLeadStatus($individual = '')
	{
		$a = array(
			'1' => 'Open',
			'2' => 'In Progress',
	...
	public static function getDDLLeadStatus()
	{
		$a = array(
			array('1', 'Open'),
			array('2', 'In Progress'),
	...
	public static function getDDLAYears()
	{
		$a = [];
		for($i = 2020; $i <= 2030; $i++)
		{
	...
	{
		return $this->industry != '' ? DAO::getSingleValue($link, "SELECT GROUP_CONCAT(description SEPARATOR ' | ') FROM lookup_sector_types WHERE id IN ({$this->industry})") : '';
	}
	...
	{
		return DAO::getSingleValue($link, "SELECT COUNT(*) FROM crm_activities WHERE entity_id = '{$this->id}' AND entity_type = 'lead' AND activity_type = '{$activity_type}'");
	}
	...
	public static function getListHWC()
	{
		return array('H' => 'Hot', 'W' => 'Warm', 'C' => 'Cold');
	}

	public function isLocked()
	{
		return $this->converted ? true : (($this->isWon() || $this->isLost()) ? true : false);
	}

	public function getLinkedOpportunityID(PDO $link)
	{
		return DAO::getSingleValue($link, "SELECT crm_opportunities.id FROM crm_opportunities WHERE crm_opportunities.lead_id = '{$this->id}'");
	}

	public function isConverted()
	{
		return $this->converted ? true : false;
	}

	public function filesCount(PDO $link)
	{
		return DAO::getSingleValue($link, "SELECT COUNT(*) FROM crm_entities_files WHERE entity_id = '{$this->id}' AND entity_type = 'lead'");
	}

	public $id= NULL;
	public $status= NULL;
	public $lead_title= NULL;
