...
<?php
$ss_result = DAO::getLookupTable($link, "SELECT lookup_skills_scan.`description`, skills_scan.`grade`
FROM skills_scan
INNER JOIN student_frameworks ON student_frameworks.tr_id = skills_scan.tr_id
INNER JOIN lookup_skills_scan ON lookup_skills_scan.id = skills_scan.plan_id AND lookup_skills_scan.framework_id = student_frameworks.id
WHERE skills_scan.tr_id = $tr_id;");
?>
<table class="table1" style="width: 900px">
    <thead>
    <th colspan=5>&nbsp;&nbsp;&nbsp;Skills Scan and Progress Summary</th>
    </thead>
    <tr><td colspan=2>KSB Criteria</td><td>Starting Point</td><td>Previous Skills Scan Status</td><td>Current Skills Scan Status</td></tr>
    <tr>
    ...
    <tr>
        ...
        <?php echo "<td style='text-align:center; vertical-align:middle'>" . $ss_result['Locates and migrates data from already identified sources'] . "</td>"; ?>
        <?php echo "<td style='text-align:center; vertical-align:middle'>" . $ss_result['Locates and migrates data from already identified sources'] . "</td>"; ?>
