...
HEREDOC;
  	...
  	<ul>
		...
		<li><a href="#tab-4">Successful Candidates</a></li>
	</ul>
  	<?php  if ( isset($pc) && $pc != '' ) { ?>
  <div id="tab-1">
  	<form method="post" action="do.php?_action=view_vacancy">
  		<h3>
			...
			Candidate Surname<input name="surname" value="<?php if(isset($surname)) echo $surname; ?>" type="text" size="20" maxlength="20" />
...
<?php
		    ...
		    if($surname != '')
		    {
			    $surname = " AND candidate.surname LIKE '{$surname}%'";
		    }
...
if($sta) {
	while( $row = $sta->fetch() ) {
		if ( $row['learner_name'] === NULL ) {
			...
			echo '<td><a href="/do.php?_action=fill_vacancy&amp;id='.$vacancy->id.'&amp;pc='.htmlentities($vacancy->postcode).'&amp;cd_id='.$row['id'].'&amp;radius='.$vacancy->radius.'&amp;rmv=1">&nbsp; delete candidate &raquo;</a></td>';	
			echo '</tr>';
