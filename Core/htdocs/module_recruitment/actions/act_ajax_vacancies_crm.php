...
class ajax_vacancies_crm implements IAction
{
	...
	public function display_candidate_actions(PDO $link ) {
		...
		if( $result = $link->query($sql_request_retrieval) ) {
			while( $row = $result->fetch() ) {
				...
				$daterange[$current_date] .= ' border-top: 1px solid #e9e9e9; width: 98%"><a href="/do.php?_action=view_vacancy&pc='.rawurlencode($row['postcode']).'&id='.$row['vac_id'].'&display='.$row['id'].'">'.$row['firstnames'].' '.$row['surname'].'</a><div style="padding: 0; margin: 0; float:right; width:auto;"> ('.$row['application_comments'].')</div><p style="padding: 0; margin: 0;">'.$row['job_title'].'</p></div>';
	...
	public function display_employer_actions(PDO $link ) {
		...
		if ( isset($_SESSION['user']->department) ) {
			// 	$region_code = array('North West' => 1, 'North East' => 2, 'Midlands' => 3, 'East Midlands' => 4, 'West Midlands' => 5, 'London North' => 6, 'London South' => 7, 'Peterborough' => 8, 'Yorkshire' => 9);
                ...
                $sql_request_retrieval .= 'AND employerpool_notes.status = "'.$regions[$_SESSION['user']->department].'" ';
        }
		$sql_request_retrieval .= 'GROUP BY dpn ';
        $sql_request_retrieval .= 'order by employerpool_notes.next_action asc';
		...
		if( $result = $link->query($sql_request_retrieval) ) {
			...
			while( $row = $result->fetch() ) {
				...
				if ( $current_date != $row['next_action']) {
					...
					$employerdata['employer_data'] .= '<div style="border-top: 1px solid #e9e9e9; background-color: #E0EAD0; line-height: 1.2em; font-size:1.2em; font-weight: bold; text-spacing: 0.9em; text-align: right; padding-top: 2px; width: 98%">'.date("D d M Y", strtotime($row['next_action'])).'</div>';
				}
