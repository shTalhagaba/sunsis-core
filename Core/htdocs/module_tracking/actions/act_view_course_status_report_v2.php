<?php
class view_course_status_report_v2 implements IAction
{
    public function execute(PDO $link)
    {
        $_SESSION['bc']->add($link, "do.php?_action=view_course_status_report_v2", "Course Status Report V2");
        $view = VoltView::getViewFromSession('view_course_status_report_v2', 'view_course_status_report_v2'); /* @var $view VoltView */
        if(is_null($view))
        {
            $view = $_SESSION['view_course_status_report_v2'] = $this->buildView($link);
        }
        ...
        include 'tpl_view_course_status_report_v2.php';
    }
...
SELECT DISTINCT
  ...
  sch_table.comments,
  tr_operations.coordinator_comments,
  CASE tr_operations.reason_outside_matrix
      WHEN 'SC' THEN 'Scheduling Contraints'
      WHEN 'ER' THEN 'Employer Request'
      WHEN 'R' THEN 'Reschedule'
  END AS reason_outside_matrix,
  '' AS first_course_date,
  '' AS weeks_before_first_course,
  CASE 
...
;
        ...
        $view = new VoltView('view_course_status_report_v2', $sql->__toString());
...
FROM sessions 
    INNER JOIN session_entries ON sessions.id = session_entries.entry_session_id
		    ...
		    elseif($column == 'course_trainer')
                    {
                        echo '<td>' . $session_details->course_trainer . '</td>';
                    }
...
SQL;
		    ...
		    elseif($column == 'first_course_date')
                    {
                        $first_course_date = TrainingRecord::getFirstCourseDate($link, $row['tracker_id'], $row['training_id']);
                        echo !is_null($first_course_date) ? '<td>' . $first_course_date->formatShort() . '</td>' : '<td></td>';
                    }
                    elseif($column == 'weeks_before_first_course')
                    {
                        // make sure this column in sql query is always after the 'first_course_date'
                        if(!is_null($first_course_date) && $row['induction_date'] != '')
                        {
                            $ind_d = Date::toMySQL($row['induction_date']);
                            echo '<td>' . DAO::getSingleValue($link, "SELECT ROUND(DATEDIFF('{$first_course_date->__toString()}', '{$ind_d}')/7, 0)") . '</td>';
                        }
                        else
                        {
                            echo '<td></td>';
...
FROM sessions 
    INNER JOIN session_entries ON sessions.id = session_entries.entry_session_id
		    ...
		    elseif($column == 'course_trainer')
                    {
                        echo HTML::csvSafe($session_details->course_trainer) . ',';
                    }	
                    elseif(in_array($column, ["session_start_time", "session_end_time", "duration_hours", "duration_minutes", "mock_1", "mock_2", "mock_3"]))
                    {
...
SQL;
		    ...
		    elseif($column == 'first_course_date')
                    ...
                    elseif($column == 'weeks_before_first_course')
                    {
                        ...
                        {
                            $ind_d = Date::toMySQL($row['induction_date']);
                            echo DAO::getSingleValue($link, "SELECT ROUND(DATEDIFF('{$first_course_date->__toString()}', '{$ind_d}')/7, 0)") . ',';
                        }
                        else
                        {
                            echo ',';
