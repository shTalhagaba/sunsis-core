<?php /* @var $complaint ComplaintEmployer */ ?>
...
<div class="row">
	...
	<div class="col-sm-8">
		<form autocomplete="off" class="form-horizontal" method="post" name="frmComplaintEmployer" id="frmComplaintEmployer" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
			<input type="hidden" name="_action" value="save_complaint_employer" />
	...
	<div class="col-sm-4">
		<div class="well well-sm">
			<?php $org = $complaint->getEmployerDetails($link); ?>
			<h2><?php echo htmlspecialchars((string)$org->legal_name) . ' ' . htmlspecialchars(strtoupper($org->trading_name)); ?></h2>
			<ul class="list-unstyled">
				<?php
				$location_id = DAO::getSingleValue($link, "SELECT id FROM locations WHERE locations.organisations_id = '{$org->id}'");
				$location = Location::loadFromDatabase($link, $location_id);
				?>
				<?php echo trim($location->address_line_1) != ''?'<li> '.htmlspecialchars((string)$location->address_line_1).'</li>':''; ?>
				<?php echo trim($location->address_line_2) != ''?'<li> '.htmlspecialchars((string)$location->address_line_2).'</li>':''; ?>
				<?php echo trim($location->address_line_3) != ''?'<li> '.htmlspecialchars((string)$location->address_line_3).'</li>':''; ?>
				<?php echo trim($location->address_line_4) != ''?'<li> '.htmlspecialchars((string)$location->address_line_4).'</li>':''; ?>
				<?php echo trim($location->postcode) != ''?'<li> '.htmlspecialchars((string)$location->postcode).'</li>':''; ?>
				<?php echo trim($location->telephone) != ''?'<li> '.htmlspecialchars((string)$location->telephone).'</li>':''; ?>
				<?php echo trim($location->fax) != ''?'<li> '.htmlspecialchars((string)$location->fax).'</li>':''; ?>
			</ul>
		</div>
	</div>
...
<script language="JavaScript">
	...
	function save()
	{
		var myForm = document.forms["frmComplaintEmployer"];
