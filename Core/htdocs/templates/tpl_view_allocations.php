...
<div class="row">
    <div class="col-lg-12">
        <div class="banner">
            <div class="Title" style="margin-left: 6px;">Allocations</div>
            <div class="ButtonBar">
                <?php if($_SESSION['user']->type != User::TYPE_ORGANISATION_VIEWER && $_SESSION['user']->type != User::TYPE_SYSTEM_VIEWER) {?>
                <button class="btn btn-default btn-sm" onclick="window.location.href='do.php?_action=edit_allocation';"><i class="fa fa-plus"></i> Create New Allocation</button>
                <?php } ?>
            </div>
                ...
                <form method="get" action="<?php echo $_SERVER['PHP_SELF'] ?>" name="applyFilter" id="applyFilter">
                    ...
                    <div id="filterBox" class="clearfix small">
                        <fieldset>
                            ...
                            <div class="field float"><label>Holder:</label><?php echo $view->getFilterHTML('filter_contract_holder'); ?></div>
                            <div class="field float"><label>Year:</label><?php echo $view->getFilterHTML('filter_contract_year'); ?></div>
                            <div class="field float"><label>Active:</label><?php echo $view->getFilterHTML('filter_active'); ?></div>
                            <div class="field float"><label>Funded:</label><?php echo $view->getFilterHTML('filter_funded'); ?></div>
                        </fieldset>
                        <fieldset>
