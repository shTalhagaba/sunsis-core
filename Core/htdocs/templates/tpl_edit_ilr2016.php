...
<head>
    ...
    <script src="/js/ilr2016.js?n=<?php echo time(); ?>"></script>
</head>
...
-->
                        ...
                        <div class="button_header" id="b5" onclick="if(prompt('Password','')=='pscd2016')changeL03();"><img src='../images/edit.png' height='15' width='15' /> Change LRN</div>

                        <?php } ?>

                    <?php if ($_SESSION['user']->isAdmin() || (DB_NAME == "am_lead" && $_SESSION['user']->type == User::TYPE_MANAGER)) { ?>
                    <div class="button_header" onclick="if(prompt('Password','')=='pscd2016')changeDates();"><img src='../images/calendar-icon.gif' height='15' width='15' /> Change Dates</div>
        ...
        <tr><td colspan="2">
            <fieldset class="innerFieldset tooltip" title="To identify any additional monitoring characteristics required for the learner.">
                ...
                $NLM_dropdown = array(
                    ...
                    array('21', '21 Unassigned'),
                    array('22', '22 Unassigned'),
...
<div>
    ...
    foreach($vo->LearningDelivery as $delivery)
    {
		...
		$condition5 = DAO::getSingleValue($link, "SELECT EnglandFEHEStatus FROM lars201617.Core_LARS_LearningDelivery WHERE LearnAimRef = '$delivery->LearnAimRef';");
...
foreach ($vo->LearningDelivery as $delivery) {
    ...
    if (!isset($delivery) || $delivery->FworkCode == '')
        $PwayCode_dropdown = DAO::getResultset($link, "SELECT DISTINCT PwayCode, LEFT(CONCAT(PwayCode, ' ', COALESCE(PathwayName,'')),50) ,NULL FROM lars201617.Core_LARS_Framework ORDER BY FworkCode, PwayCode;", DAO::FETCH_NUM);
    else
        $PwayCode_dropdown = DAO::getResultset($link, "SELECT DISTINCT PwayCode, LEFT(CONCAT(PwayCode, ' ', COALESCE(PathwayName,'')),50) ,NULL FROM lars201617.Core_LARS_Framework WHERE FworkCode = '$delivery->FworkCode' AND ProgType='$delivery->ProgType' ORDER BY FworkCode, PwayCode;", DAO::FETCH_NUM);
...
SQL;
    ...
    $ShouldLDHE = DAO::getSingleValue($link, "SELECT EnglandFEHEStatus FROM lars201617.Core_LARS_LearningDelivery WHERE LearnAimRef = '$LearnAimRef';");
    if ($ShouldLDHE=="H") {
...
<div id = "delivery_template"  class = "Unit" style="display: none;">
    ...
    $PwayCode_dropdown = DAO::getResultset($link, "SELECT DISTINCT PwayCode, CONCAT(PwayCode, ' ', COALESCE(PathwayName,'')) ,NULL FROM lars201617.Core_LARS_Framework ORDER BY FworkCode;", DAO::FETCH_NUM);
