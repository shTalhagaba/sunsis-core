...
function addLessons(event)
{
	...
	f.elements['_action'].value = "add_attendance_module_lesson";
...
{
	var f = document.forms['addLesson'];
	f.elements['_action'].value = "edit_attendance_module_lessons";
...
{
	var f = document.forms['addLesson'];
	f.elements['_action'].value = "delete_attendance_module_lessons";
...
<body>
<div class="banner">
	<div class="Title">Attendance Module: Lessons</div>
	<div class="ButtonBar">
...
<form method="GET" action="<?php echo $_SERVER['PHP_SELF'] ?>" onsubmit="return validateFilters(this);">
	...
	<div id="div_filters" style="display:none;" >
		...
		<input type="hidden" name="_action" value="view_attendance_module_lessons" />
...
<form name="addLesson" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post">
	<input type="hidden" name="module_id" value="<?php echo $module_id; ?>" />
	<input type="hidden" name="_action" value="add_attendance_module_lesson" />
	...
	<div id="div_addLesson" align="center" style="margin-top:10px;margin-bottom:20px;<?php echo $showPanel == '1'?'display:block':'display:none'; ?>">
		<table border="0" style="margin:10px;background-color:silver; border:1px solid black;padding:3px; border-radius: 15px;">
			<tr>
				...
				<td><?php echo HTML::select('groups_id', $groupnames, $lesson_group, true); ?></td>
	...
	<div align="center" style="margin-top:30px;">
		...
		<table class="resultset" border="0" cellpadding="6" cellspacing="0">
			<tr>
				...
				<th>Qualification</th>
				<th>FS Tutor</th>
				<th title="Off the job duration">OTJ Duration</th>
			</tr>
			...
			if($st)
			{
				while($row = $st->fetch())
				{
					echo HTML::viewrow_opening_tag("do.php?_action=edit_attendance_module_lesson&id={$row['id']}");
					//echo '<tr>';
...
<td><a title="View register #{$row['id']}" href="do.php?_action=read_register&lesson_id={$row['id']}&attendance_module=true&attendance_module_id={$module_id}"><img src="/images/clipboard16x16.gif" border="0"/></a></td>
HEREDOC;
					}
					else
					{
...
HEREDOC;
					...
					$reg = new Register($row['id'], $link, true);
                    $reg->load($link);
                    $re = DAO::getSingleValue($link, "SELECT COUNT(*) FROM register_entries WHERE lessons_id = '{$row['id']}'");
                    $star = '';
                    if(count($reg->entries) == $re)
                        $star = '*';
					echo '<td align="center">' . HTML::cell($star . ' ' . $row['lesson_title']) . '</td>';
					...
					if($row['otj_hours'] == '' && $row['otj_minutes'] == '')
					{
						echo '<td></td>';
					}
					else
					{
						echo '<td align="left">' . HTML::cell($row['otj_hours'] . ' hour(s) and ' . $row['otj_minutes'] . ' minute(s)') . '</td>';
						echo '</tr>';
					}
					echo '</tr>';
				}
