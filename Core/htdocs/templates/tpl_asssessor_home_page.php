	...
	<div class="col-lg-6 col-md-6 col-sm-12">
		...
		<div class="box box-primary table-responsive collapsed-box">
			...
			<div class="box-body box-profile">
				<img class="profile-user-img img-responsive img-circle" src="<?php echo $photopath; ?>" alt="Assessor profile picture">
				...
				<div class="row">
					<div class="col-lg-6 col-xs-6">
						<div class="small-box bg-aqua">
							<div class="inner">
								<h3><?php echo DAO::getSingleValue($link, "SELECT COUNT(*) FROM tr INNER JOIN contracts ON tr.contract_id = contracts.id WHERE contract_year = '{$current_year}' AND tr.status_code = '1' AND tr.assessor = '{$_SESSION['user']->id}'");?></h3>
								<p>Learners in training</p>
							</div>
							<div class="icon"><i class="fa fa-hourglass-half"></i></div>
							<a href="do.php?_action=view_training_records&_reset=1&ViewTrainingRecords_filter_assessor=<?php echo $_SESSION['user']->id; ?>&ViewTrainingRecords_filter_record_status[]=1&ViewTrainingRecords_filter_contract_year={}" class="small-box-footer">Click to see <i class="fa fa-arrow-circle-right"></i></a>
						</div>
					</div>
					...
					<div class="col-lg-6 col-xs-6">
						<div class="small-box bg-red">
							<div class="inner">
								<h3><?php echo DAO::getSingleValue($link, "SELECT COUNT(*) FROM tr INNER JOIN contracts ON tr.`contract_id` = contracts.id WHERE contract_year = '{$current_year}' AND tr.`status_code` = 1 AND target_date < CURDATE()  AND tr.assessor = '{$_SESSION['user']->id}';");?></h3>
					...
					<div class="col-lg-6 col-xs-6">
						<div class="small-box bg-yellow">
							<div class="inner">
								<h3><?php echo DAO::getSingleValue($link, "SELECT COUNT(DISTINCT l03) FROM tr INNER JOIN contracts ON tr.contract_id = contracts.id WHERE contract_year = '{$current_year}' AND tr.assessor = '{$_SESSION['user']->id}' AND tr.status_code = '6' AND tr.l03 NOT IN (SELECT tr2.l03 FROM tr AS tr2 WHERE tr2.`start_date` > tr.`start_date` AND tr2.status_code != '6')");?></h3>
					...
					<div class="col-lg-6 col-xs-6">
						<div class="small-box bg-red">
							<div class="inner">
								<h3><?php echo DAO::getSingleValue($link, "SELECT COUNT(*) FROM tr INNER JOIN contracts ON tr.contract_id = contracts.id WHERE contract_year = '{$current_year}' AND tr.assessor = '{$_SESSION['user']->id}' AND tr.status_code = '3'");?></h3>
								<p>Learners withdrawn</p>
							</div>
							<div class="icon">
								<i class="fa fa-chain-broken"></i>
							</div>
							<a href="do.php?_action=view_training_records&_reset=1&ViewTrainingRecords_filter_assessor=<?php echo $_SESSION['user']->id; ?>&ViewTrainingRecords_filter_record_status[]=3&ViewTrainingRecords_filter_contract_year=<?php echo $current_year; ?>" class="small-box-footer">Click to see <i class="fa fa-arrow-circle-right"></i></a>
						</div>
					</div>
		...
		<div class="box box-success box-solid">
			...
			<div class="box-body no-padding table-responsive">
				<table id="tblLearners" class="table row-border small">
					<thead><tr><th></th><th>Learner</th><th>L03</th><th>Last Login</th><th>Progress</th><th></th><th></th><th></th><?php echo (SOURCE_LOCAL || DB_NAME == "am_sd_demo") ? '<th></th>' : ''; ?></tr></thead>
					<tbody>
					<?php
					$assessor_id = $_SESSION['user']->id;
					$result = DAO::getResultset($link, "SELECT DISTINCT
  tr.id,
  tr.username,
  tr.`firstnames`,
  tr.`surname`,
...
WHERE tr.`assessor` = '$assessor_id'
  AND tr.`status_code` = 1 AND REPLACE(student_qualifications.id, '/', '') IN ('Z0001875', '60313432', 'Z0001898', 'Z0001849');
					...
					else
					{
						foreach($result AS $row)
						{
							if($row['qan'] == Workbook::RETAIL_QAN)
								echo '<tr style="background-color: #fcf8e3;"><td><a href="do.php?_action=read_training_record&id=' . $row['id'] . '"><i class="fa fa-folder-open"></i></a></td>';
							else
							...
							else
							{
								foreach($workbook_result AS $workbook_row)
								{
									if($workbook_row['wb_title'] == 'WBCommunication')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_communication&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>Communication</a></li>';
									if($workbook_row['wb_title'] == 'WBCustomerExperience')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_customer_experience&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>Customer Experience</a></li>';
									if($workbook_row['wb_title'] == 'WBDevelopingSelf')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_developing_self&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>Developing Self</a></li>';
									if($workbook_row['wb_title'] == 'WBKnowingYourCustomers')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_knowing_your_customers&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>Knowing Your Customers</a></li>';
									if($workbook_row['wb_title'] == 'WBMeetingRegulationsAndLegislation')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_meeting_regulations_and_legislation&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>Regulations & Legislation</a></li>';
									if($workbook_row['wb_title'] == 'WBProductAndService')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_product_and_service&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>Product and Service</a></li>';
									if($workbook_row['wb_title'] == 'WBRoleResponsibility')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_role_responsibility&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>Role, Responsibility & Personal Organisation</a></li>';
									if($workbook_row['wb_title'] == 'WBSystemsAndResources')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_systems_and_resources&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>Systems and Resources</a></li>';
									if($workbook_row['wb_title'] == 'WBTeamWorking')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_team_working&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>Team Working</a></li>';
									if($workbook_row['wb_title'] == 'WBUnderstandingTheOrganisation')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_understanding_the_organisation&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>Understanding the Organisation</a></li>';
									if($workbook_row['wb_title'] == 'WBEnvironment')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_environment&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>Environment</a></li>';
									if($workbook_row['wb_title'] == 'WBFinancial')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_financial&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>Financial</a></li>';
									if($workbook_row['wb_title'] == 'WBHSAndSecurity')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_hs_and_security&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>H&S and Security</a></li>';
									if($workbook_row['wb_title'] == 'WBPersonalTeamPerformance')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_personal_team_performance&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>Personal and Team Performance</a></li>';
									if($workbook_row['wb_title'] == 'WBTechnical')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_technical&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>Technical</a></li>';
									if($workbook_row['wb_title'] == 'WBStock')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_stock&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>Stock</a></li>';
									if($workbook_row['wb_title'] == 'WBBusinessAndBrandReputation')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_business_and_brand_reputation&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>Business & Brand Reputation</a></li>';
									if($workbook_row['wb_title'] == 'WBMarketing')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_marketing&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>Marketing</a></li>';
									...
									if($workbook_row['wb_title'] == 'WBCustomer')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_customer&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>Customer</a></li>';
									if($workbook_row['wb_title'] == 'WBRetailProductAndService')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_retail_product_and_service&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>Product & Service</a></li>';
									if($workbook_row['wb_title'] == 'WBLegalAndGovernance')
										echo '<li><a href="#" onclick="return window.phpAssessorSignature == \'\' ? alert(\'Please first create your signature\') : window.location.href=\'do.php?_action=wb_legal_and_governance&id='.$workbook_row['id'].'&tr_id='.$workbook_row['tr_id'].'\'"><i class="' . Workbook::getWBStatusIcon($workbook_row['wb_status']) . '"></i>Legal and Governance</a></li>';
								}
...
WORKBOOKS;
							...
							elseif($row['qan'] == Workbook::RETAIL_QAN)
							{
...
<div id="eventContent" title="Event Details" style="display:none;">
	<table class="table row-border">
		<tr><th>DateTime:</th><td><span id="e_start"></span></td></tr>
		<tr><th>For:</th><td><span id="e_for"></span></td></tr>
		<tr><th>Type:</th><td><span id="e_type"></span></td></tr>
		<tr><th>Subject:</th><td><span id="e_subject"></span></td></tr>
		<tr><th>Agreed Action:</th><td><span id="e_agreed_action"></span></td></tr>
	</table>
</div>
...
<div id="reviewContent" title="Event Details" style="display:none;">
	<table class="table row-border">
		...
		<tr><th>For:</th><td><span id="r_for"></span></td></tr>
		<tr><th>Type:</th><td>Customer Service Practitioner Self-assessment / Review</td></tr>
		<!--<tr><th colspan="2" class="text-right"><a id="r_link" href="#">Open &raquo;</a> </th></tr>-->
	</table>
</div>
...
<script>
var phpAssessorSignature = '<?php echo $user_signature; ?>';
$(function(){
	...
	$('#calendar').fullCalendar({
		...
		weekends: false,
		events: 'do.php?_action=assessor_calendar_manager&id=<?php echo $_SESSION['user']->id; ?>',
		eventRender: function (event, element) {
			if(event.type != 'review')
			{
				element.attr('href', 'javascript:void(0);');
				element.click(function() {
					$("#e_start").html(moment(event.start).format('DD/MM/YYYY H:M'));
					$("#e_for").html(event.for);
					$("#e_type").html(event.contact_type);
					$("#e_subject").html(event.subject);
					$("#e_agreed_action").html(event.agreed_action);
