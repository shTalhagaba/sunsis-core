...
<script src="/js/ilr2014.js?n=10046"></script>
...
<?php if(DB_NAME == "am_pera") { ?>
	...
	var client = ajaxRequest('do.php?_action=save_ilr_2014', postData);
	if( client != null )
	{
	...
	if(migrate==1)
	{
		if(ilrmigrated=="")
		{
			var postData2 = 'tr_id=' + <?php echo $tr_id; ?>
...
function validation() {
	...
	var postData = 'id=' + document.getElementById('LearnRefNumber').value
		...
		+'&tr_id=' + <?php echo $tr_id;?>;
	$.ajax({
		url: "do.php?_action=validate_ilr2014",
		type: "post",
		async: true,
		data: postData,
		//dataType: "xml",
...
<div class="banner">
	<table border="0" cellspacing="0" cellpadding="0" height="100%" width="100%">
		<tr class="head">
			<td valign="bottom">
				<?php if($template!=1) { ?>
				<?php echo $vo->GivenNames.' '.$vo->FamilyName; ?> 2014/15 ILR
				<?php } else { ?>
				ILR Template
				<?php }  ?>
			</td>
	...
	<table border="0" cellspacing="0" cellpadding="0" height="100%" width="100%">
		<tr>
			<td valign="top" align="left" class="left"><div class="button_wrap">
                ...
                if(DB_NAME=='am_lema'){?>
                <div class="button_header" id="b1" onclick="alert('This change will affect the previous year\’s information.');return save();">Save</div>
                <?php }


				if(SOURCE_BLYTHE_VALLEY)
				{
					echo '<div class="button_header" id="b1" onclick="return save();">Save Perspective</div>';
				}
...
<table border="0" cellspacing="4" cellpadding="4">
	...
	<tr>
		<?php
		$this->dynamic_field_display('PrevLearnRefNumber',"<input class='optional validate[required]'  type='text' value='" . $vo->PrevLearnRefNumber . "' style='' id='PrevLearnRefNumber' name='PrevLearnRefNumber' maxlength=12 size=12 onKeyPress='return validLearnerReference(this, event)'>");
		$this->dynamic_field_display('PrevUKPRN',"<input class='optional validate[required]' type='text' value='" . $vo->PrevUKPRN . "' style='' id='PrevUKPRN' name='PrevUKPRN' maxlength=12 size=12 onKeyPress='return numbersonly(this, event)'>");
		?>
	</tr>
	...
	<tr>
		<?php $this->dynamic_field_display('ALSCost',"<input class='optional validate[required]' type='text' value='" . $vo->ALSCost . "' id='ALSCost' name='ALSCost' maxlength=40 size=40 onKeyPress='return numbersonly(this, event)'>"); ?>
	</tr>
	<?php
	echo '</tr>';
	echo '<tr>';
...
<div id="employment_set" style="border: solid 0px #A3A3A3; width: 830px;">
	...
	foreach( $vo->LearnerEmploymentStatus as $empstatus ) {
		...
		$id = "SEM" . $index;
		$this->dynamic_field_display('SEM',HTML::select($id, $SEM_dropdown, $sem, true, true));
		echo '</tr>';
		echo '</table>';
		echo '</div>';
	}
	...
	if( $index==0 ) {
		...
		echo '<tr>';
		$id = "SEM" . $index;
		$this->dynamic_field_display('SEM', HTML::select($id, $SEM_dropdown, '', true, true));
		echo '</tr>';
	}
	else
	{
...
<table border="0" cellspacing="4" cellpadding="4" >
	...
	<tr>
		...
		<?php
		$xpath = $vo->xpath("/Learner/ProviderSpecLearnerMonitoring[ProvSpecLearnMonOccur='B']/ProcLearnMon");
...
foreach($vo->LearningDelivery as $delivery)
{
	...
	if($delivery->AimType=='1')
	{
		...
		$TBFinCode_dropdown = array(array('1','1'), array('2','2'));
		foreach($delivery->ApprenticeshipTrailblazerFinancialDetails as $trailblazer)
		{
...
<div id = "tab99"  class = "Unit" style="display: none;">
	...
	<table class="ilr">
		...
		<form  name="pdf" id="pdf" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
			<input type="hidden" name="_action" value="pdf_from_ilr2014" />
