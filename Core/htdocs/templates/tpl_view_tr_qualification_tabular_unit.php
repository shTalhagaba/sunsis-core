...
t = '<div><table class="resultset" cellspacing="0" cellpadding="6"><thead><tr><th class="topRow">Units</th></tr></thead>';
...
function traverserecurse(xmlUnits, disp, grey) 
{
        ...
        for(var i=0; i<xmlUnits.childNodes.length; i++)
	 	{	
 	      	...
 	      	if(xmlUnits.childNodes[i].tagName=='unit')
 	      	{
				...
				if(grade==null)
					grade='';
				st = xmlUnits.childNodes[i].getAttribute('ustatus');	
				...
				t += "<tr name='unit' " + disp + "><td colspan=6 style='background-color: #C2D69B'><b>Title:</b> " + xmlUnits.childNodes[i].getAttribute('title');
				t += '<br><b>Reference: </b>' + xmlUnits.childNodes[i].getAttribute('reference');
			...
			//		t += "<br><b>Status: </b>" + "Optional";
				...
				t += "<br><b>Percentage &nbsp; </b><input " + grey + " id='" + ownerReference + "' type='text' size='3' onChange='markEvidence(this)' onKeyPress='return numbersonly(this, event)' value='" + unitPercentage + "'</input>";
				t += "<br><b>Grade : &nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</b><input id='grade" + ownerReference + "' type='text' size='3' onChange='' value='" + grade + "'</input>&nbsp;[P=Pass, M=Merit, D=Distinction, T=Transfer Credit, U=Unclassified]";

				t += '<br><b>Status:</b> ';

				if(st=='1')
					t += '<input type="radio" checked title="" name="evidencestatus' + ownerReference + '" value="1" >Assessment due';
				else
					t += '<input type="radio" title="" name="evidencestatus' + ownerReference + '" value="1" >Assessment due';
						
				if(st=='2')
					t += '<input type="radio" checked title="" name="evidencestatus' + ownerReference + '" value="2" >Submitted';
				else
					t += '<input type="radio" title="" name="evidencestatus' + ownerReference + '" value="2" >Submitted';

				if(st=='3')
					t += '<input type="radio" checked title="" name="evidencestatus' + ownerReference + '"  value="3" >Allocated for marking';
				else
					t += '<input type="radio" title="" name="evidencestatus' + ownerReference + '"  value="3" >Allocated for marking';

				if(st=='4')
					t += '<input type="radio" checked title="" name="evidencestatus' + ownerReference + '"  value="4" >Assessed';
				else
					t += '<input type="radio" title="" name="evidencestatus' + ownerReference + '"  value="4" >Assessed';

				if(st=='5')
					t += '<input type="radio" checked title="" name="evidencestatus' + ownerReference + '"  value="5" >Verified';
				else
					t += '<input type="radio" title="" name="evidencestatus' + ownerReference + '"  value="5" >Verified';

				t += "<br><b>Comments</b><input size = 50 id = 'comments" + ownerReference + "' value='" + comments + "' title='" + comments + "'></input>"	
				t += "<br><b>Chosen&nbsp;</b><input id ='chosen" + ownerReference + "'" + disabled + checked + "type=checkbox ></input></td></tr>";
 	      	}


	 	    //tags[++tagcount] = groupx;
 	      	traverserecurse(xmlUnits.childNodes[i], disp, grey);
 	    }
	}
}
...
function save(filenames)
{
	...
	{
		status = document.getElementsByName("evidencestatus"+owner_reference);
		ustatus = 0;

		if((typeof status[0] !== 'undefined' && status[0] !== null) && status[0].checked)
			ustatus = 1;
		else if((typeof status[0] !== 'undefined' && status[0] !== null) && status[1].checked)
			ustatus = 2;
		else if((typeof status[0] !== 'undefined' && status[0] !== null) && status[2].checked)
			ustatus = 3;
		else if((typeof status[0] !== 'undefined' && status[0] !== null) && status[3].checked)
			ustatus = 4;
		else if((typeof status[0] !== 'undefined' && status[0] !== null) && status[4].checked)
			ustatus = 5;

        //alert(owner_reference);
		pro = "pro"+owner_reference;
		grade = "grade"+owner_reference;	
		xml += '<unit owner_reference = "' + htmlspecialchars(owner_reference) + '" percentage="' 
			...
			+ htmlspecialchars(document.getElementById(grade).value) + '" ustatus="' 
			+ htmlspecialchars(ustatus) + '" chosen="' 
			+ htmlspecialchars(document.getElementById("chosen"+owner_reference).checked) + '" comments="' 
			+ htmlspecialchars(document.getElementById("comments"+owner_reference).value) + '"></unit>';
	...
	if(percentage == 100)
	{
		...
		for(var i = 0; i < rows.length; i++)
		{
			if(rows[i].name=='evidence')
			{
				...
				t++;
				if(rows[i].childNodes[2].childNodes[0].title == unit_id)
				{
					// Display of auto date
					var currentTime = new Date();
					var month = currentTime.getMonth() + 1;
...
<div class="banner">
	...
	<div class="ButtonBar">
		...
		<?php if($_SESSION['user']->type==2 || $_SESSION['user']->type==3 || $_SESSION['user']->type==4 || $_SESSION['user']->type==1 || $_SESSION['user']->isAdmin()){?>
...
<div id="container">
<div style="background-image:url('/images/paper-background-grey.jpg');" id='simpledialog1'>
<form>
