...
<head>
    ...
    <script src="/js/ilr2025.js?n=<?php echo time(); ?>"></script>
    <style>	
...
<div class="banner">
    <table border="0" cellspacing="0" cellpadding="0" height="100%" width="100%">
        <tr class="head">
            <td valign="bottom">
                <?php if ($template != 1) { ?>
                <?php echo $vo->GivenNames . ' ' . $vo->FamilyName; ?> 2025/26 ILR
                <?php } else { ?>
                ILR Template
                <?php }  ?>
            </td>
...
<div>
    <fieldset>
        ...
        <table border="0" cellspacing="4" cellpadding="4" style="float: left;">
            ...
            <tr>
                <?php
                if ((string)$vo->DateOfBirth != '00000000' && (string)$vo->DateOfBirth != '' && (string)$vo->DateOfBirth != '00/00/0000') {
                    ...
                    <table border="0" cellspacing="4" cellpadding="4">
                        <tr>
                            <td colspan="2" class="tooltip" title="To take into account learners' wishes about the use of their data. The data held in this field is used by the FE Choices.">
                                <?php
                                /*$RUI_dropdown = array(
                                    array('4', 'Learner is not to be contacted, for example where a learner has suffered severe illness during the programme or other circumstance'),
...
<fieldset>
    ...
    <table border="0" cellspacing="4" cellpadding="4">
        ...
        <tr><td colspan="2">
            <fieldset class="innerFieldset tooltip" title="English and Maths Minimum Hours">
                <legend>English and Maths Minimum Hours</legend>
                <table border="0" cellspacing="4" cellpadding="4">
                    <tr>
                        <td>
                            <?php
                            $EMH_dropdown = array(array('1', '1 Student has at least the minimum classroom teaching hours planned for an eligible English qualification'),array('2', '2 Student does not have at least the minimum classroom teaching hours planned for an eligible English qualification'));
                            $MMH_dropdown = array(array('1', '1 Student has at least the minimum classroom teaching hours planned for an eligible maths qualification'),array('2', '2 Student does not have at least the minimum classroom teaching hours planned for an eligible maths qualification'));
                            $this->dynamic_field_display('EMH', HTML::select('EMH', $EMH_dropdown, (string)$vo->EMH, true, true));
                            $this->dynamic_field_display('MMH', HTML::select('MMH', $MMH_dropdown, (string)$vo->MMH, true, true));
                            ?>
                        </td></tr></table>
            </fieldset>
        </tr>
...
<ul>
    ...
    foreach ($vo->LearningDelivery as $aim) {
        ...
        echo ( isset($aim->Exclude) && $aim->Exclude->__toString() == 1 ) ?
                                '<li class="tooltip" title="' . $title . '"><a style="color: silver;">' . $aim->LearnAimRef . '</a></li>' : 
                                '<li class="tooltip" title="' . $title . '"><a>' . $aim->LearnAimRef . '</a></li>';
    }
    ?>
</ul>
...
foreach ($vo->LearningDelivery as $delivery) {
    ...
    $this->dynamic_field_display('ConRefNumber', "<input class='optional tooltip' type='text' value='" . (string)$delivery->ConRefNumber . "' style='' id='ConRefNumber' name='ConRefNumber' maxlength=30 size=30>");
    echo '</tr>';
    ...
    if ((string)$delivery->AimType == "1" || (string)$delivery->AimType == "4" || (string)$delivery->AimType == "5" || (string)$delivery->AimType == "3") {
        ...
        $EVI_dropdown = array(
            array('1', '1 Offer of an interview for a role which matches skills acquired'),
            array('2', '2 Offer of a new role or added responsibilities with existing employer'),
            array('3', '3 Receipt of plan from learner to acquire new self-employment opportunities or contracts'),
            array('4', '4 Checked HGV Provisional licence'),
            array('5', '5 Passed HGV ADR (dangerous goods) exam'),
            array('6', '6 Passed HGV PDP (petroleum driver passport) exam'),
            array('7', '7 Passed HGV Part 4 of the driver certificate of professional competence'),
            array('8', '8 Received HGV Evidence that CPC has been brought up to date'),
            array('9', '9 Taken HGV Part 3b of the driver certificate of professional competence: On-Road Driving'),
            array('10', '10 Passed HGV Part 3b of the driver certificate of professional competence: On-Road Driving')
        );
        ...
        foreach($delivery->LearningDeliveryFAM as $ldf)
        {
            if($ldf->LearnDelFAMType == 'EVI')
            {
                $index++;
                $evi[$index] = (string)$ldf->LearnDelFAMCode;
            }
        }
        $xpath = $delivery->xpath("./LearningDeliveryFAM[LearnDelFAMType='EVI']/LearnDelFAMCode");
        echo '<tr>';
        $this->dynamic_field_display('EVI', HTML::select(('EVI1'), $EVI_dropdown, $evi[0], true, false));
        $this->dynamic_field_display('EVI', HTML::select(('EVI2'), $EVI_dropdown, $evi[1], true, false));
        echo '</tr>';
        echo '<tr>';
        $this->dynamic_field_display('EVI', HTML::select(('EVI3'), $EVI_dropdown, $evi[2], true, false));
        $this->dynamic_field_display('EVI', HTML::select(('EVI4'), $EVI_dropdown, $evi[3], true, false));
        echo '</tr>';
    }
...
SQL;
    ...
    if ((string)$delivery->AimType == '1' && ((string)$delivery->ProgType == '25' || (string)$delivery->FundModel=='36')) {
        echo '<fieldset>';
        echo '<legend>Apprenticeship Financial Details - (Aim Reference: ' . (string)$delivery->LearnAimRef . ', Aim Type: ' . (string)$delivery->AimType . ')</legend>';
        echo '<table border="0" cellspacing="4" cellpadding="4">';
        echo '<col width="150"/><col />';
        ...
        if(DB_NAME=='am_baltic')
        {
            ...
            echo '<td><input class="optional" type="text" name="OverallTNP" onKeyPress="return numbersonly(this, event)" value="' . (string)$delivery->OverallTNP . '" maxlength="50"/></td>';
            echo '</tr>';
        }
    ...
    if ((string)$delivery->LearnAimRef == 'Z0007834' || (string)$delivery->LearnAimRef == 'Z0007835' || (string)$delivery->LearnAimRef == 'Z0007836' || (string)$delivery->LearnAimRef == 'Z0007837' || $delivery->LearnAimRef == 'Z0007838' || $delivery->LearnAimRef == 'ZWRKX001') {
        echo '<fieldset>';
        ...
        foreach ($delivery->LearningDeliveryWorkPlacement as $ldwp) {
            if ($ldwp->WorkPlaceStartDate != '' && (string)$ldwp->WorkPlaceStartDate != 'undefined') {
                $ldwpindex++;
                echo '<tr>';
    ...
    echo '<tr>';
    $this->dynamic_field_display('LearnActEndDate', HTML::datebox('LearnActEndDate_' . (string)$delivery->LearnAimRef . $a, $delivery->LearnActEndDate));
...
foreach ($vo->LearnerEmploymentStatus as $empstatus) {
    if(!$tr_start_date->after(Date::toShort($empstatus->DateEmpStatApp)))
    {
        ...
        echo '<col width="394"/><col width="435" />';
        $empstat_desc = DAO::getSingleValue($link, "SELECT EmpStaCode_Desc from lis201415.ilr_empstatcode where EmpStatCode = '" . (string)$empstatus->EmpStat . "'");
        echo '<tr><td style="background-color: #F3FAE5; border:1px solid #648827; padding:1px; ">' . Date::toShort((string)$empstatus->DateEmpStatApp) . ' - ' . $empstat_desc . '</td><td><span class="button"><a onclick="$(\'tr.emp_stat_' . $index . '\').toggle();" >update</a></span></td></tr>';
        $id = "EmpStat" . $index;
...
if ($index == -1) {
    ...
    echo '<td class="fieldLabel_compulsory">Employer:';
    $EmployerId = "EmployerId" . $index;
    echo HTML::select($EmployerId, $employers, '', true, false); // $empstatus->EmployerId	
    echo '</td>';
    echo '</tr>';
...
<div id = "delivery_template"  class = "Unit" style="display: none;">
    ...
    $this->dynamic_field_display('ConRefNumber', "<input class='optional tooltip' type='text' value='' style='' id='ConRefNumber' name='ConRefNumber' maxlength=30 size=30>");
    echo '</tr>';
