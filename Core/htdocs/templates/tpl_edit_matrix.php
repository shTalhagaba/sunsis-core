...
trainingRecordsIds = new Array();
trs = 0;
nmbrOfCheckboxes = 0;
unitBoxes = new Array(2);
...
function updateEvidences(data, i)
{
	...
	// Updating evidences
	while(x = data.indexOf("<evidence", 0)) // converting all the previous evidenes to a constant i.e. <Perspective>"
	{
		if(x>0)
		{
	...
	gcount = 0;
	while(x = data.indexOf("<Perspective>", 0)) // putting new evidences
	{
		if(x>0)
		{
			gcount++;
	...
	while(x = data.indexOf("<Perspective>", 0)) // putting new exvidences
	{
		...
		{
			gcount++;
			newstring = '<element title="' + htmlspecialchars(elementsDetails[i+'-'+gcount].title) + '" percentage="' + elementsDetails[i+'-'+gcount].percentage + '">';
			data = data.replace("<Perspective>",newstring);
		}
		else
			break;
	...
	// Updating units
	while(x = data.indexOf("<unit ", 0)) // converting all the previous units to a constant i.e. <KhushnoodAhmedKhan>"
	{
		if(x>0)
		{
	...
	while(x = data.indexOf("<Perspective>", 0)) // putting new units
	{
		if(x>0)
		{
			gcount++;
			newstring = '<unit title="' + htmlspecialchars(unitsDetails[i+'-'+gcount].title) + '" owner_reference="' + unitsDetails[i+'-'+gcount].owner_reference + '" reference="' + unitsDetails[i+'-'+gcount].reference + '" proportion="' + unitsDetails[i+'-'+gcount].proportion + '" mandatory="' + unitsDetails[i+'-'+gcount].mandatory + '" chosen="' + unitsDetails[i+'-'+gcount].chosen + '" percentage="' + unitsDetails[i+'-'+gcount].percentage + '">';
			data = data.replace("<Perspective>",newstring);
	...
	var postData = 'qualification_id=' + <?php echo '"' . $qualification_id . '"';?>
		...
		+ '&evidences=' + encodeURIComponent(data)
		+ '&units=' + units
	...
	var request = ajaxBuildRequestObject();
	request.open("POST", expandURI('do.php?_action=save_matrix'), false); // (method, uri, synchronous)
...
?>
	...
	var handleCopyAllEvidence = function()
	{

		for(a=1; a<trainingRecordsIds.length; a++)
		{
			if(this.form.evidenceStatus[0].checked==true || this.form.evidenceStatus[0].checked=="true")
			{
				st = "a";
			...
			this.form.evidence_id.value = trainingRecordsIds[a] + this.form.evidence_id.value.substr(this.form.evidence_id.value.indexOf("-"),this.form.evidence_id.value.length);
	...
	var handleCopyAllEvidence2 = function()
	{
		firstTrainingRecord = trainingRecordsIds[1];
		lastTrainingRecord = trainingRecordsIds[trainingRecordsIds.length - 1];

//		this.form.evidence_id.value = trainingRecordsIds[1] + this.form.evidence_id.value.substr(this.form.evidence_id.value.indexOf("-"),this.form.evidence_id.value.length);
//		alert(this.form.evidence_id.value);
//		bb = 32;
//		this.form.evidence_id.value = trainingRecordsIds[1] + "-" + bb;
//		alert(this.form.evidence_id.value);

		str = document.getElementById(this.form.evidence_id.value).id.split('-');
		trainingrecord_id = evidenceDetails[this.form.evidence_id.value].tr_id;
		box_number = str[1];
		unit_number = elementsDetails[tr_id+'-'+evidenceDetails[this.form.evidence_id.value].element].unit;

		StartValue = unitBoxes[unit_number][0];
		EndValue = unitBoxes[unit_number][1];

//		alert("first tr = " + firstTrainingRecord + ', last tr = ' + lastTrainingRecord);
//		alert('start box = ' + StartValue + ', last box = ' + EndValue);return;
		//****************************** Starting to copy vertically ***************************************
		for(a=1; a<trainingRecordsIds.length; a++)
		{
			for(b=StartValue; b<EndValue; b++)
			{
				if(this.form.evidenceStatus[0].checked==true || this.form.evidenceStatus[0].checked=="true")
				{
					st = "a";
				...
				//this.form.evidence_id.value = trainingRecordsIds[a] + this.form.evidence_id.value.substr(this.form.evidence_id.value.indexOf("-"),this.form.evidence_id.value.length);
				this.form.evidence_id.value = trainingRecordsIds[a] + "-" + b;
//				alert(document.getElementById(this.form.evidence_id.value));
		...
		this.cancel();
		return;
		//****************************** Starting to copy horizontally ***************************************

		//alert('unit = ' + unit_number + ', ' + unitBoxes[unit_number][0] + ', ' + unitBoxes[unit_number][1]);return;
		// //alert(document.getElementById(this.form.evidence_id.value).id);return;
		//alert('training_record = ' + trainingrecord_id + ', unit = ' + unit_number + ', ' + StartValue+','+EndValue);
		for(a=StartValue; a<EndValue; a++)
		{
			if(this.form.evidenceStatus[0].checked==true || this.form.evidenceStatus[0].checked=="true")
			{
				st = "a";
			...
			//this.form.evidence_id.value = trainingRecordsIds[a] + this.form.evidence_id.value.substr(this.form.evidence_id.value.indexOf("-"),this.form.evidence_id.value.length);
			this.form.evidence_id.value = trainingrecord_id+"-"+a;
			//alert(this.form.evidence_id.value + " and boxes are " + boxes);
			evidenceDetails[this.form.evidence_id.value].status = st;
			evidenceDetails[this.form.evidence_id.value].reference = this.form.evidenceReference.value;
			evidenceDetails[this.form.evidence_id.value].portfolio = this.form.evidencePortfolio.value;
			evidenceDetails[this.form.evidence_id.value].method = this.form.evidenceAssessmentMethod.selectedIndex;
	...
	var handleCopyAllEvidence1 = function()
	{
		str = document.getElementById(this.form.evidence_id.value).id.split('-');
		//alert(document.getElementById(this.form.evidence_id.value).id);return;
		trainingrecord_id = evidenceDetails[this.form.evidence_id.value].tr_id;
	...
	var handleEvidenceDatabase= function()
	{
		...
		window.open('do.php?_action=view_evidence&qualification_id=<?php echo rawurlencode($qualification_id); ?>&internaltitle=<?php echo rawurlencode($internaltitle);?>&framework_id=<?php echo rawurlencode($framework_id);?>&tr_id=' + tr_id + '&group_id=<?php echo $group_id;?>');
	}
	...
	YAHOO.am.scope.evidenceDialog = new YAHOO.widget.Dialog("evidenceDialog",
		{
			...
			buttons : [ { text:"Save", handler:handleSaveEvidence, isDefault:true },
				...
				{ text:"Copy to all Vertically", handler:handleCopyAllEvidence, isDefault: true},
				{ text:"Copy to all Horizontally", handler:handleCopyAllEvidence1, isDefault: true},
				{ text:"Copy to complete unit", handler:handleCopyAllEvidence2, isDefault: true}]
		} );
...
foreach($progress as $record)
{
	?>
	trs = trs + 1;
	trainingRecordsIds[trs] = <?php echo $record['tr_id']; ?>;
	var request = ajaxBuildRequestObject();
...
function getHeader(xmlUnits)
{
	...
	units += 	"<td class='separator0' align='center' title='" + "Total" + "'rowspan='" + 1 + "'>"  + "<img src=do.php?_action=generate_image&title=" + encodeURIComponent("Total") + "></img>" +  "</td>";
	...
	if(xmlUnits.hasChildNodes())
	{
		for(var i=0; i<xmlUnits.childNodes.length; i++)
		{
			...
			if(xmlUnits.childNodes[i].tagName=='unit')
			{
				...
				t= data;
				ch = countChildren(xmlUnits.childNodes[i]);
				units += '<td class="separator0" align="center" title="' + xmlUnits.childNodes[i].getAttribute('title') + '"colspan="' + ch + '">'  + '<img src=do.php?_action=generate_image&title=' + encodeURIComponent(t) + '&n=' + ch + '></img>' +  '</td>';
			}
...
<div class="banner">
	...
	<div class="ButtonBar">
		<button class="toolbarbutton" onclick="if(window.confirm('Are you sure you dont want to save?'))window.location.href='<?php echo $_SESSION['bc']->getPrevious();?>';">Close</button>
		<?php if($_SESSION['user']->type!=12){?>
		<button id="saveButton" onclick="if(window.confirm('Are you sure?'))save();">Save</button>
		<?php }?>
...
<div id="evidenceDialog" style="overflow: auto;">
	...
	<form>
		...
		<table border="0" cellspacing="4" cellpadding="4" style="margin-left:10px; width: 400px;">
			<tr>
			...
			<tr>
				<td colspan="2" class="fieldLabel_optional" valign="top">
					Assessor Comments<br/><textarea  class="optional" style="font-family:sans-serif; font-size:10pt" name="evidenceComments" rows="5" cols="70" ></textarea></td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="2" class="fieldLabel_optional" valign="top">Verifier Comments<br/>
					<textarea  class="optional" style="font-family:sans-serif; font-size:10pt" name="evidenceVComments" rows="5" cols="70" ></textarea></td>
			</tr>
		</table>
	</form>
</div>
