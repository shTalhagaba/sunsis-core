...
<head>
	...
	<script src="/js/ilr2015.js?n=<?php echo time(); ?>"></script>
</head>
...
<div class="banner">
	<table border="0" cellspacing="0" cellpadding="0" height="100%" width="100%">
		<tr class="head">
			<td valign="bottom">
				<?php if ($template != 1) { ?>
				<?php echo $vo->GivenNames . ' ' . $vo->FamilyName; ?> 2015/16 ILR
				<?php } else { ?>
				ILR Template
				<?php }  ?>
			</td>
...
-->
						...
						<div class="button_header" id="b5" onclick="if(prompt('Password','')=='pscd2015')changeL03();"><img src='../images/edit.png' height='15' width='15' /> Change LRN</div>

						<?php } ?>

					<?php if ($_SESSION['user']->isAdmin()) { ?>
					<div class="button_header" onclick="if(prompt('Password','')=='pscd2015')changeDates();"><img src='../images/calendar-icon.gif' height='15' width='15' /> Change Dates</div>
					...
					if (DB_NAME == "am_reed") {
						...
						if ($is_active == 1)
							echo "<div class='button_header'><img src='../images/active.png' height='15' width='15' /> <input type=checkbox id='active' checked> Active</div>";
						else
							echo "<div class='button_header'><img src='../images/active.png' height='15' width='15' /> <input type=checkbox id='active'> Active</div>";
					}
					?>
				</div>
			</td>
	...
	<table border="0" cellspacing="4" cellpadding="4">
		...
		<tr><td colspan="2">
			<fieldset class="innerFieldset"><legend>Does the learner have any of the following (tick those that apply)</legend>
				<table border="0" cellspacing="4" cellpadding="4">
					<tr>
						<td colspan="2" class="tooltip" title="Learning difficulty assessment: To identify all learners in post-16 provision with significant additional needs and to establish funding eligibility.<br>
																							Education Health Care plan: To indicate if the learner has an Education Health Care Plan.<br>
							...
							$LLDDFAM_dropdown = array(
								array('LDA', 'Learning difficulty assessment'),
							...
							$sql = <<<SQL
SELECT
extractvalue(ilr, '/Learner/LearnerFAM[LearnFAMType=\'LDA\']/LearnFAMCode') AS LDA,
...
SQL;
							...
							if(isset($result_set[0]['LDA']) && $result_set[0]['LDA'] == '1')
								$selected_lldm_fam[] = 'LDA';
				...
				<table border="0" cellspacing="4" cellpadding="4">
					<tr>
						<td>
							...
							$LSR_dropdown = array(array('36', '36 Care to Learn (EFA only)'), array('55', '55 16-19 Bursary Fund - learner is a member of a vulnerable group (EFA Only)'), array('56', '56 16-19 Bursary Fund - learner has been awarded a discretionary bursary'), array('57', '57 Residential support (EFA only)'), array('58', '58 19+ Hardship (SFA only)'), array('59', '59 20+ Childcare (SFA only)'), array('60', '60 Residential Access Fund (SFA only)'), array('61', '61 Unassigned'));
...
<div>
	...
	{
		$xpath = $delivery->xpath("./LearningDeliveryFAM[LearnDelFAMType='SOF']/LearnDelFAMCode"); $hefce_sof = (empty($xpath[0]))?'':$xpath[0];
		$xpath = $delivery->xpath("./LearningDeliveryFAM[LearnDelFAMType='WPL']/LearnDelFAMCode"); $hefce_wpl = (empty($xpath[0]))?'':$xpath[0];
	...
	if($condition1 || $condition2 || $condition3 || $condition4)
	{
		...
		$xpath = '';
		if(isset($LearnerHE))
			$xpath = $LearnerHE->xpath("./LearnerHEFinancialSupport[FINTYPE='1']/FINAMOUNT");
		$financial_support_type_1 = (empty($xpath[0]))?'':$xpath[0];
		...
		if(isset($LearnerHE))
			$xpath = $LearnerHE->xpath("./LearnerHEFinancialSupport[FINTYPE='2']/FINAMOUNT");
		$financial_support_type_2 = (empty($xpath[0]))?'':$xpath[0];
		...
		echo '</tr>';
		if(isset($LearnerHE))
			$xpath = $LearnerHE->xpath("./LearnerHEFinancialSupport[FINTYPE='3']/FINAMOUNT");
		$financial_support_type_3 = (empty($xpath[0]))?'':$xpath[0];
		...
		if(isset($LearnerHE))
			$xpath = $LearnerHE->xpath("./LearnerHEFinancialSupport[FINTYPE='4']/FINAMOUNT");
		$financial_support_type_4 = (empty($xpath[0]))?'':$xpath[0];
...
foreach ($vo->LearningDelivery as $delivery) {
	...
	if (!isset($delivery) || $delivery->FworkCode == '')
		$PwayCode_dropdown = DAO::getResultset($link, "SELECT DISTINCT PwayCode, LEFT(CONCAT(PwayCode, ' ', COALESCE(PathwayName,'')),50) ,NULL FROM lars201516.Core_LARS_Framework ORDER BY FworkCode, PwayCode;", DAO::FETCH_NUM);
	else
		$PwayCode_dropdown = DAO::getResultset($link, "SELECT DISTINCT PwayCode, LEFT(CONCAT(PwayCode, ' ', COALESCE(PathwayName,'')),50) ,NULL FROM lars201516.Core_LARS_Framework WHERE FworkCode = '$delivery->FworkCode' AND ProgType='$delivery->ProgType' ORDER BY FworkCode, PwayCode;", DAO::FETCH_NUM);
	...
	echo '</tr>';
	echo '<tr>';
	//$TBS_dropdown = array(array('001', '001'), array('002', '002'));
	$xpath = $delivery->xpath("./LearningDeliveryFAM[LearnDelFAMType='TBS']/LearnDelFAMCode");
	$tbs = (empty($xpath[0])) ? '' : $xpath[0];
	$this->dynamic_field_display('TBS', HTML::select('TBS', $TBS_dropdown, $tbs, true, false));
	echo '</tr>';
...
SQL;
		...
		echo '	      <legend>Household Situation (select up to 2 that apply) - (Aim Reference: ' . $delivery->LearnAimRef . ', Aim Type: ' . $delivery->AimType . ')</legend>';
		echo '	        <table border="0" cellspacing="4" cellpadding="4">';
		echo '	          <tr>';
		echo '	            <td>';
		$HHS_dropdown = array(
	...
	$xpath = $delivery->xpath("./LearningDeliveryFAM[LearnDelFAMType='SOF']/LearnDelFAMCode");
	$hefce_sof = (empty($xpath[0])) ? '' : $xpath[0];
	$xpath = $delivery->xpath("./LearningDeliveryFAM[LearnDelFAMType='WPL']/LearnDelFAMCode");
	$hefce_wpl = (empty($xpath[0])) ? '' : $xpath[0];
	$condition1 = $hefce_sof != 'undefined' && $hefce_sof == '1';
	$condition2 = $hefce_sof != 'undefined' && $hefce_sof == '107' && $delivery->ProgType >= 20;
	$condition3 = $hefce_wpl != 'undefined' && $hefce_wpl != '1' && $delivery->FundModel == 35 && $delivery->ProgType >= 20;
	$condition4 = $delivery->FundModel == 99 && $delivery->ProgType >= 20;
	if ($condition1 || $condition2 || $condition3 || $condition4) {
		...
		echo '</tr>';
		echo '<tr>';
		$this->dynamic_field_display('HEPostCode', "<input class='optional validate[required]' type='text' value='" . $delivery->LearningDeliveryHE->HEPostCode . "' style='' id='HEPostCode' name='HEPostCode' maxlength='8' size='8' >");
		echo '</tr>';
		echo '</table>';
		echo '</td></tr>';
		echo '<tr><td colspan="2">';
...
SQL;
	...
	if ($delivery->AimType == '1' && $delivery->ProgType == '25') {
		echo '<fieldset>';
		echo '<legend>Apprenticeship Trailblazer Financial Details - (Aim Reference: ' . $delivery->LearnAimRef . ', Aim Type: ' . $delivery->AimType . ')</legend>';
		echo '<table border="0" cellspacing="4" cellpadding="4">';
		echo '<col width="150"/><col />';
		$trailblazerindex = 0;
		$TBFinType_dropdown = array(array('TNP', 'TNP Trailblazer negotiated price'), array('PMR', 'PMR Payment record'));
		$TBFinCode_dropdown = array(array('1', '1'), array('2', '2'), array('3', '3'));
		foreach ($delivery->TrailblazerApprenticeshipFinancialRecord as $trailblazer) {
			if ($trailblazer->TBFinType != '') {
				...
				$this->dynamic_field_display('TBFinAmount', "<input class='compulsory validate[required]' type='text' value='" . $tbfinamount . "' style='' id='TBFinAmount$trailblazerindex' name='TBFinAmount$trailblazerindex' maxlength=30 size=30>");
				echo '</tr>';
			}
		}
		$trailblazerindex++;
		...
		$this->dynamic_field_display('TBFinAmount', "<input class='compulsory validate[required]' type='text' value='" . '' . "' style='' id='TBFinAmount$trailblazerindex' name='TBFinAmount$trailblazerindex' maxlength=30 size=30>");
		echo '</tr>';
		echo '</table>';
		echo '</fieldset>';
	}
	...
	<fieldset>
		...
		if ($count > 0) {
			...
			if ($st) {
				...
				echo '<thead><tr><th>&nbsp;</th><th>System Username</th><th>Name</th><th>Date and Time (most recent first)</th></tr></thead>';
				echo '<tbody>';
				$ids = array();
				while ($row = $st->fetch()) {
...
<div id = "delivery_template"  class = "Unit" style="display: none;">
	...
	$PwayCode_dropdown = DAO::getResultset($link, "SELECT DISTINCT PwayCode, CONCAT(PwayCode, ' ', COALESCE(PathwayName,'')) ,NULL FROM lars201516.Core_LARS_Framework ORDER BY FworkCode;", DAO::FETCH_NUM);
...
<form  name="pdf" id="pdf" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
	<input type="hidden" name="_action" value="pdf_from_ilr2015" />
