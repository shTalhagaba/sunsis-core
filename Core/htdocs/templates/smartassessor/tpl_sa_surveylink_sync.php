...
<head>
	...
	<title>Smart Assessor: Link Learner Survey URL</title>
	...
	<script type="text/javascript">
		...
		function onFilterClick(e) {
			...
			$('div#content').load('do.php?_action=sa_surveylink_sync&subaction=rendercontent' + filters, null, onContentLoad);
		}
		...
		function onContentLoad(responseText, textStatus, xmlHttpRequest)
		{
            ...
            $('input#BtnUpdateInSA').click(updateRecords);
        ...
        function updateRecords(e)
		{
			...
			if ($selectedBoxes.length == 0) {
				alert("Please select one or more records to update in Smart Assessor");
				return;
			}
			...
			showProgressAnimation('loading51.gif');
			var client = ajaxRequest('do.php?_action=sa_surveylink_sync&subaction=updaterecords' + data);
			if (client) {
/*				alert(client.responseText);
			...
			//$('div#content').load('do.php?_action=sa_surveylink_sync&subaction=rendercontent', null, onContentLoad);
		...
		function createRecords(e)
		{
			...
			showProgressAnimation('loading51.gif');
			var client = ajaxRequest('do.php?_action=sa_surveylink_sync&subaction=createrecords' + data);
			if (client) {
/*				alert(client.responseText);
				if (window.console) {
					window.console.log(client.responseText);
		...
		function linkRecords(e)
		{
			...
			showProgressAnimation('loading51.gif');
			var client = ajaxRequest('do.php?_action=sa_surveylink_sync&subaction=linkrecords' + data);
			if (!client) {
				//
			}
			//$('div#content').load('do.php?_action=sa_surveylink_sync&subaction=rendercontent', null, onContentLoad);
		...
		function unlinkRecords(e)
		{
			...
			showProgressAnimation('loading51.gif');
			var client = ajaxRequest('do.php?_action=sa_surveylink_sync&subaction=unlinkrecords' + data);
			if (!client) {
				//
			}
			//$('div#content').load('do.php?_action=sa_surveylink_sync&subaction=rendercontent', null, onContentLoad);
        ...
        function createRecordsinSunesis(e)
		{
			...
			showProgressAnimation('loading51.gif');
			var client = ajaxRequest('do.php?_action=sa_surveylink_sync&subaction=createrecordsinsunesis' + data);
			if (client) {
/*				alert(client.responseText);
				if (window.console) {
					window.console.log(client.responseText);
...
<body>
<div class="banner">
	<div class="Title">Smart Assessor: Learner Survey URL Synchronisation</div>
	<div class="ButtonBar">
...
<div style="width:1150px;margin-left:auto;margin-right:auto;">
	...
	<div class="Newspaper">
	<p class="introduction">Sunesis learner survey link data may be synchronised with learner data in Smart Assessor. In
		Sunesis and Smart Assessor require the pre-existance of an learner record, so learner records must first be synchronised between
		the two systems before learner survey link can be synchronised.</p>
	<p class="introduction">This page provides tools to create survey link in existing learner records in Sunesis and Smart Assessor.</p>
	...
	<ul class="introduction">
		<li>Linked learner records. It will be display Learner records which has Survey link exist in Smart Assessor.</li>
		<li>Smart Assessor Learner which are migrated in Sunesis Learners but does not contains Learner Survey Link.</li>
	</ul>
	</div>
