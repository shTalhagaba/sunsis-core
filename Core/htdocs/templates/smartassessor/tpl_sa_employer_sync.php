...
<head>
	...
	<title>Smart Assessor: Employer Sync</title>
	...
	function onFilterClick(e) {
		...
		$('div#content').load('do.php?_action=sa_employer_sync&subaction=rendercontent' + filters, null, onContentLoad);
	}
	...
	function createRecords(e)
	{
		...
		showProgressAnimation('loading51.gif');
		var client = ajaxRequest('do.php?_action=sa_employer_sync&subaction=createrecords' + data);
		if (!client) {
			//alert("Operation aborted");
		}
		//$('div#content').load('do.php?_action=sa_employer_sync&subaction=rendercontent', null, onContentLoad);
	...
	function linkRecords(e)
	{
		...
		var data = '';
		for (var i = 0, max = $selectedBoxes.length; i < max; i++) {
			$select = $selectedBoxes.eq(i).closest('tr').find('select');
			if($select.val()) {
				data += '&ids[]=' + encodeURIComponent($selectedBoxes.eq(i).val()) + '-' + encodeURIComponent($select.val());
			}
		}

		//alert('do.php?_action=sa_employer_sync&subaction=linkrecords' + data);
		//return;

		showProgressAnimation('loading51.gif');
		var client = ajaxRequest('do.php?_action=sa_employer_sync&subaction=linkrecords' + data);
		if (!client) {
			//alert("Operation aborted");
		}
		//$('div#content').load('do.php?_action=sa_employer_sync&subaction=rendercontent', null, onContentLoad);
	...
	function unlinkRecords(e)
	{
		...
		showProgressAnimation('loading51.gif');
		var client = ajaxRequest('do.php?_action=sa_employer_sync&subaction=unlinkrecords' + data);
		if (!client) {
			//alert("Operation aborted");
		}
		//$('div#content').load('do.php?_action=sa_employer_sync&subaction=rendercontent', null, onContentLoad);
    ...
    function createRecordsinSunesis(e)
	{
		...
		showProgressAnimation('loading51.gif');
		var client = ajaxRequest('do.php?_action=sa_employer_sync&subaction=createrecordsinsunesis' + data);
		if (!client) {
			//alert("Operation aborted");
		}
		//$('div#content').load('do.php?_action=sa_employer_sync&subaction=rendercontent', null, onContentLoad);
...
<body>
<div class="banner">
	<div class="Title">Smart Assessor: Employer Synchronisation</div>
	<div class="ButtonBar">
...
<div style="width:1150px;margin-left:auto;margin-right:auto;">
	...
	<div class="Newspaper">
	<p class="introduction">Sunesis employer data synchronised with employer data in Smart Assessor. </p>
	<p class="introduction">This page provides tools to create or remove links between existing employer records in Sunesis and Smart Assessor, and to
	create new employer records in Smart Assessor where an employer record in Sunesis has no equivalent record in Smart Assessor.</p>
	<p class="introduction">Employer records are requested from Smart Assessor and compared to employer records in Sunesis. The results of the
	comparison are collated into four groups:</p>
	<ul class="introduction">
		<li>Linked employer records. These employer records are identified as being equivalent, either because they were created in Smart Assessor
			by Sunesis or because they have been manually linked.</li>
		<li>Unlinked employer records in Sunesis that have potential matches in Smart Assessor. Use this tool to link pre-existing records.</li>
		<li>Sunesis records which are not linked to Smart Assessor records and for which no potential equivalent record in Smart Assessor
		could be identified. Use this tool to create new employer records in Smart Assessor.</li>
		<li>Smart Assessor employer records which are not linked to Sunesis employer records and for which no potential equivalent record in Sunesis
		could be identified. Use this tool to create employer records in Sunesis.</li>
	</ul>
	</div>
