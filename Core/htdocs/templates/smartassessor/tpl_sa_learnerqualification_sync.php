...
<head>
	...
	<title>Smart Assessor: Qualification Link with Learner</title>
	...
	<script type="text/javascript">
		...
		function onFilterClick(e) {
			...
			$('div#content').load('do.php?_action=sa_learnerqualification_sync&subaction=rendercontent' + filters, null, onContentLoad);
		}
		...
		function createRecords(e)
		{
			...
			showProgressAnimation('loading51.gif');
			var client = ajaxRequest('do.php?_action=sa_learnerqualification_sync&subaction=createrecords' + data);
			if (client) {
/*				alert(client.responseText);
			...
			//$('div#content').load('do.php?_action=sa_learnerqualification_sync&subaction=rendercontent', null, onContentLoad);
		...
		function linkRecords(e)
		{
			...
			showProgressAnimation('loading51.gif');
			var client = ajaxRequest('do.php?_action=sa_learnerqualification_sync&subaction=linkrecords' + data);
			if (!client) {
				//
			}
			//$('div#content').load('do.php?_action=sa_learnerqualification_sync&subaction=rendercontent', null, onContentLoad);
		...
		function unlinkRecords(e)
		{
			...
			showProgressAnimation('loading51.gif');
			var client = ajaxRequest('do.php?_action=sa_learnerqualification_sync&subaction=unlinkrecords' + data);
			if (!client) {
				//
			}
			//$('div#content').load('do.php?_action=sa_learnerqualification_sync&subaction=rendercontent', null, onContentLoad);
        ...
        function createRecordsInSunesis(e)
		{
			...
			showProgressAnimation('loading51.gif');
			var client = ajaxRequest('do.php?_action=sa_learnerqualification_sync&subaction=createrecordsinsunesis' + data);
			if (client) {
/*				alert(client.responseText);
				if (window.console) {
					window.console.log(client.responseText);
...
<body>
<div class="banner">
	<div class="Title">Smart Assessor: Qualification Link with Learner</div>
	<div class="ButtonBar">
...
<div style="width:1150px;margin-left:auto;margin-right:auto;">
	...
	<!--<p class="introduction">
         Sunesis learner qualification data may be synchronised with learner course in Smart Assessor.
         There must be a Learner and associated Qualifications in Smart Assessor with same Learner and Qualification data in Sunesis.
         If learner course is not available in Smart Assessor but exists in Sunesis then System will migrate Sunesis learner qualification with Smart Assessor learner course.
    </p>-->
	<p class="introduction">This page provides tools to view the learners qualifications records in Sunesis and Smart Assessor.</p>
	<p class="introduction">Learner courses are requested from Smart Assessor and compared to the Learner Qualifications in Sunesis. The results of the
		comparison are collated into two groups:</p>
	<ul class="introduction">
		<li>Linked learner qualification records. These learner qualification records are identified as being equivalent.</li>
		<li>Sunesis learner qualification records which are different to Smart Assessor learner course records or for which no potential equivalent record in Smart Assessor
			could be identified. This tool helps to view the differences.</li>
	</ul>
	</div>
