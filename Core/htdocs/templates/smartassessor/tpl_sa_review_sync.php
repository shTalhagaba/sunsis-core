...
<head>
	...
	<title>Smart Assessor: Review Sync</title>
	...
	<script type="text/javascript">
		...
		function onFilterClick(e) {
			...
			$('div#content').load('do.php?_action=sa_review_sync&subaction=rendercontent' + filters, null, onContentLoad);
		}
		...
		function createRecords(e)
		{
			...
			showProgressAnimation('loading51.gif');
			var client = ajaxRequest('do.php?_action=sa_review_sync&subaction=createrecords' + data);
			if (client) {
/*				alert(client.responseText);
			...
			//$('div#content').load('do.php?_action=sa_review_sync&subaction=rendercontent', null, onContentLoad);
		...
		function linkRecords(e)
		{
			...
			showProgressAnimation('loading51.gif');
			var client = ajaxRequest('do.php?_action=sa_review_sync&subaction=linkrecords' + data);
			if (!client) {
				//
			}
			//$('div#content').load('do.php?_action=sa_review_sync&subaction=rendercontent', null, onContentLoad);
		...
		function unlinkRecords(e)
		{
			...
			showProgressAnimation('loading51.gif');
			var client = ajaxRequest('do.php?_action=sa_review_sync&subaction=unlinkrecords' + data);
			if (!client) {
				//
			}
			//$('div#content').load('do.php?_action=sa_review_sync&subaction=rendercontent', null, onContentLoad);
        ...
        function createRecordsInSunesis(e)
		{
			...
			showProgressAnimation('loading51.gif');
			var client = ajaxRequest('do.php?_action=sa_review_sync&subaction=createrecordsinsunesis' + data);
			if (client) {
/*				alert(client.responseText);
				if (window.console) {
					window.console.log(client.responseText);
...
<body>
<div class="banner">
	<div class="Title">Smart Assessor: Review Synchronisation</div>
	<div class="ButtonBar">
...
<div style="width:1150px;margin-left:auto;margin-right:auto;">
	...
	<div class="Newspaper">
	<p class="introduction">Sunesis review data may be synchronised with review data in Smart Assessor. Review records in 
	Sunesis require the pre-existence of a Training Record, so Training Records must be decided before review records can be synchronised in Sunesis.</p>
	<p class="introduction">This page provides tools to create records between Sunesis and Smart Assessor.
	</p>
	<p class="introduction">Review records are requested from Smart Assessor and compared to review records in Sunesis. The results of the
		comparison are collated into two groups:</p>
	<ul class="introduction">
		<li>Linked review records. These review records are identified as being equivalent, either because they were created in Smart Assessor
			by Sunesis or because they have been manually linked.</li>
		<li>Smart Assessor review records which are not linked to Sunesis review records and for which no potential equivalent record in Sunesis
			could be identified. Use this tool to create new review records in Sunesis.</li>
	</ul>
	</div>
