...
<head>
	...
	<title>Smart Assessor: Learner Sync</title>
	...
	<script type="text/javascript">
		...
		function onFilterClick(e) {
			...
			$('div#content').load('do.php?_action=sa_learner_sync&subaction=rendercontent' + filters, null, onContentLoad);
		}
		...
		function createRecords(e)
		{
			...
			showProgressAnimation('loading51.gif');
			var client = ajaxRequest('do.php?_action=sa_learner_sync&subaction=createrecords' + data);
			if (client) {
/*				alert(client.responseText);
			...
			//$('div#content').load('do.php?_action=sa_learner_sync&subaction=rendercontent', null, onContentLoad);
		...
		function linkRecords(e)
		{
			...
			showProgressAnimation('loading51.gif');
			var client = ajaxRequest('do.php?_action=sa_learner_sync&subaction=linkrecords' + data);
			if (!client) {
				//
			}
			//$('div#content').load('do.php?_action=sa_learner_sync&subaction=rendercontent', null, onContentLoad);
		...
		function unlinkRecords(e)
		{
			...
			showProgressAnimation('loading51.gif');
			var client = ajaxRequest('do.php?_action=sa_learner_sync&subaction=unlinkrecords' + data);
			if (!client) {
				//
			}
			//$('div#content').load('do.php?_action=sa_learner_sync&subaction=rendercontent', null, onContentLoad);
        ...
        function createRecordsinSunesis(e)
		{
			...
			showProgressAnimation('loading51.gif');
			var client = ajaxRequest('do.php?_action=sa_learner_sync&subaction=createrecordsinsunesis' + data);
			if (client) {
/*				alert(client.responseText);
				if (window.console) {
					window.console.log(client.responseText);
...
<body>
<div class="banner">
	<div class="Title">Smart Assessor: Learner Synchronisation</div>
	<div class="ButtonBar">
...
<div style="width:1150px;margin-left:auto;margin-right:auto;">
	...
	<div class="Newspaper">
	<p class="introduction">Sunesis learner data may be synchronised with learner data in Smart Assessor. Learner records in both
		Sunesis and Smart Assessor require the pre-existance of an employer record, so employer records must first be synchronised between
		the two systems before learner records can be synchronised.</p>
	<p class="introduction">This page provides tools to create or remove links between existing learner records in Sunesis and Smart Assessor, and to
		create new learner records in Smart Assessor where a learner record in Sunesis has no equivalent record in Smart Assessor.
		Only learners of linked employer records are displayed.</p>
	<p class="introduction">Learner records are requested from Smart Assessor and compared to learner records in Sunesis. The results of the
		comparison are collated into four groups:</p>
	<ul class="introduction">
		<li>Linked learner records. These learner records are identified as being equivalent, either because they were created in Smart Assessor
			by Sunesis or because they have been manually linked.</li>
		<li>Unlinked learner records in Sunesis that have potential matches in Smart Assessor. Use this tool to link pre-existing records.</li>
		<li>Sunesis learner records which are not linked to Smart Assessor learner records and for which no potential equivalent record in Smart Assessor
			could be identified. Use this tool to create new learner records in Smart Assessor.</li>
		<li>Smart Assessor learner records which are not linked to Sunesis learner records and for which no potential equivalent record in Sunesis
			could be identified. Use this tool to create new learner records in Sunesis.</li>
	</ul>
	</div>
