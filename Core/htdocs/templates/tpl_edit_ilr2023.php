...
<head>
    ...
    <script src="/js/ilr2023.js?n=<?php echo time(); ?>"></script>
    <style>	
        #NINumber, #CurrentPostcode, #PostcodePriorEnrolment, #DelLocPostCode, #LSDPostcode{text-transform:uppercase}
    </style>
</head>
...
<div class="banner">
    <table border="0" cellspacing="0" cellpadding="0" height="100%" width="100%">
        <tr class="head">
            <td valign="bottom">
                <?php if ($template != 1) { ?>
                <?php echo $vo->GivenNames . ' ' . $vo->FamilyName; ?> 2023/24 ILR
                <?php } else { ?>
                ILR Template
                <?php }  ?>
            </td>
         ...
         <tr>
            <td valign="top" align="left" class="left">
                <div class="button_wrap">
                    ...
                    <?php if (false) { ?>
                    <div class="button_header" id="b1" onclick="return save();"><img src="../images/save.png" height="15" width="15" /> Save & Close</div>
                    <div class="button_header" id="b11" onclick="return save_and_stay();"><img src="../images/save.png" height="15" width="15" /> Save & Stay</div>
                    <?php
                    }
                    ...
                    if( true || in_array(DB_NAME, ["am_ela", "am_demo", "am_baltic_demo"]) )
                        echo '<div class="button_header" id="btnOpenEndInfoDialog"><img src="../images/stop_aim.png" height="12" width="12" /> End Info.</div>';
            ...
            <td valign="top" align="right" class="right"><span class="button_start"></span>
                <img src="/images/pdf_icon.png" onclick="PDF()" height="25" width="27" title="Generate PDF view"/>
...
<div>
    <fieldset>
        ...
        <table border="0" cellspacing="4" cellpadding="4" style="float: left;">
            <tr>
                <?php
                $this->dynamic_field_display('LearnRefNumber', "<input class='compulsory validate[required]' disabled type='text' value='" . (string)$vo->LearnRefNumber . "' style='' id='LearnRefNumber' name='LearnRefNumber' maxlength=12 size=12 onKeyPress='return validLearnerReference(this, event)'>");
                $this->dynamic_field_display('PrevLearnRefNumber', "<input class='optional'  type='text' value='" . (string)$vo->PrevLearnRefNumber . "' style='' id='PrevLearnRefNumber' name='PrevLearnRefNumber' maxlength=12 size=12 onKeyPress='return validLearnerReference(this, event)'>");
                ?>
            </tr>

            <tr>
                <?php
                $this->dynamic_field_display('PrevUKPRN', "<input class='optional' type='text' value='" . (string)$vo->PrevUKPRN . "' style='' id='PrevUKPRN' name='PrevUKPRN' maxlength=12 size=12 onKeyPress='return numbersonly(this, event)'>");
            ...
            <tr>
                <?php
                $this->dynamic_field_display('PMUKPRN', "<input class='optional' type='text' value='" . (string)$vo->PMUKPRN . "' style='' id='PMUKPRN' name='PMUKPRN' maxlength=8 size=8 onKeyPress='return numbersonly(this, event)'>");
                $this->dynamic_field_display('CampId', "<input class='optional' type='text' value='" . (string)$vo->CampId . "' style='' id='CampId' name='CampId' maxlength=8 size=8>");
                ?>
            </tr>
            ...
            <tr>
                ...
                $this->dynamic_field_display('Ethnicity', HTML::select('Ethnicity', $Ethnicity_dropdown, (string)$vo->Ethnicity, true, true));
                ?>
            </tr>
            ...
            <tr>
                ...
                if ($funding_type != "ASL")
                    $this->dynamic_field_display('NINumber', "<input class='compulsory validate[required]' type='text' value='" . (string)$vo->NINumber . "' style='' id='NINumber' name='NINumber' maxlength=9 size=20>");
                ?>
            </tr>
...
<fieldset>
    ...
    <table border="0" cellspacing="4" cellpadding="4">
        ...
        <tr>
            <?php
            $this->dynamic_field_display('LLDDHealthProb', HTML::select('LLDDHealthProb', $LLDDHealthProb_dropdown, (string)$vo->LLDDHealthProb, true, true), 'colspan="2"');
            ?>
        </tr>
        <tr><td colspan="2">
            <fieldset class="innerFieldset">
                ...
                <table>
                    ...
                    foreach($vo->LLDDandHealthProblem as $LLDD)
                    {
                        ...
                        {
                            echo "<tr>";
                            $this->dynamic_field_display('LLDDCat', HTML::select($id, $LLDDCat_dropdown, (string)$LLDD->LLDDCat, true, true));
                            if($index==1)
                            {
                                if ((string)$LLDD->PrimaryLLDD == '1')
...
SQL;
                ...
                <table><tr><?php $this->dynamic_field_display('ALSCost', "<input class='optional' type='text' value='" . (string)$vo->ALSCost . "' id='ALSCost' name='ALSCost' maxlength=40 size=40 onKeyPress='return numbersonly(this, event)'>"); ?></tr></table>
            </fieldset>
        </td></tr>
            ...
            <table border="0" cellspacing="4" cellpadding="4">
                <tr>
                    <td>
                        ...
                        {
                            if((string)$_lFAM->LearnFAMType == 'ECF')
                                $LearnerFAMECF = $_lFAM->LearnFAMCode->__toString();
                            if((string)$_lFAM->LearnFAMType == 'MCF')
    ...
    <table border="0" cellspacing="4" cellpadding="4">
        ...
        echo '<table>';
            ...
            foreach ($vo->PriorAttain as $ldf)
            {
                ...
                echo '<tr>';
                $this->dynamic_field_display('PriorAttain', HTML::select($prior, $PriorAttain_dropdown2, (string)$ldf->PriorLevel, true, false));
                $this->dynamic_field_display('DateLevelApp', HTML::datebox($from, (string)$ldf->DateLevelApp, false, false));
                echo '</tr>';
            }
            $index++;
            $prior = "Prior_" . $index;
        ...
        <table><tr>
            ...
            $this->dynamic_field_display('PlanLearnHours', "<input class='optional validate[required]' type='text' value='" . (string)$vo->PlanLearnHours . "' id='PlanLearnHours' name='PlanLearnHours' maxlength=40 size=40 onKeyPress='return numbersonly(this, event)'>");
            $this->dynamic_field_display('PlanEEPHours', "<input class='optional validate[required]' type='text' value='" . (string)$vo->PlanEEPHours . "' id='PlanEEPHours' name='PlanEEPHours' maxlength=40 size=40 onKeyPress='return numbersonly(this, event)'>");
            ?>
        </tr>
        <tr>
            <?php
            $xpath = $vo->xpath("/Learner/Accom");
            $accom = (empty($xpath[0])) ? '' : (string)$xpath[0];
            if ($accom == '5')
                ...
                <table border="0" cellspacing="4" cellpadding="4">
                    <tr>
                        <td>
                            ...
                            $this->dynamic_field_display('EngGrade', HTML::select('EngGrade', $EngMathGrade_dropdown, (string)$vo->EngGrade, true, true));
                            $this->dynamic_field_display('MathGrade', HTML::select('MathGrade', $EngMathGrade_dropdown, (string)$vo->MathGrade, true, true));
                            ?>
                        </td></tr></table>
            </fieldset>
        </tr>
    ...
    <fieldset>
        ...
        <table border="0" cellspacing="4" cellpadding="4">
            ...
            <tr>
                ...
                {
                    if((string)$pslm->ProvSpecLearnMonOccur=='A')
                        $ProvSpecLearnMon1 = (string)$pslm->ProvSpecLearnMon;
                    if((string)$pslm->ProvSpecLearnMonOccur=='B')
                        $ProvSpecLearnMon2 = (string)$pslm->ProvSpecLearnMon;
                }
...
<div>
    ...
    foreach($vo->LearningDelivery as $delivery)
    {
        ...
        {
            if((string)$ldf->LearnDelFAMType == 'SOF')
                $hefce_sof = (string)$ldf->LearnDelFAMCode;
            if((string)$ldf->LearnDelFAMType == 'WPL')
                $hefce_wpl = (string)$ldf->LearnDelFAMCode;
        }
        $condition1 = $hefce_sof != 'undefined' && $hefce_sof == '1';
        $condition2 = $hefce_sof != 'undefined' && $hefce_sof == '107' && (string)$delivery->ProgType >= 20;
        $condition3 = $hefce_wpl != 'undefined' && $hefce_wpl != '1' && (string)$delivery->FundModel == 35 && (string)$delivery->ProgType >= 20;
        $condition4 = (string)$delivery->FundModel == 99 && (string)$delivery->ProgType >= 20;
    ...
    if($condition1 || $condition2 || $condition3 || $condition4 || $condition5)
    {
        ...
        if(isset($LearnerHE))
        {
            ...
            {
                if((string)$lhfs->FINTYPE=='1')
                    $financial_support_type_1 = (string)$lhfs->FINAMOUNT;
                if((string)$lhfs->FINTYPE=='2')
                    $financial_support_type_2 = (string)$lhfs->FINAMOUNT;
                if((string)$lhfs->FINTYPE=='3')
                    $financial_support_type_3 = (string)$lhfs->FINAMOUNT;
                if((string)$lhfs->FINTYPE=='4')
                    $financial_support_type_4 = (string)$lhfs->FINAMOUNT;
            }
        }
...
<ul>
    ...
    foreach ($vo->LearningDelivery as $aim) {
        ...
        echo ( isset($aim->Exclude) && $aim->Exclude->__toString() == 1 ) ?
                                '<li class="tooltip" title="' . $title . '"><a style="color: silver;">' . (string)$aim->LearnAimRef . '</a></li>' : 
                                '<li class="tooltip" title="' . $title . '"><a>' . (string)$aim->LearnAimRef . '</a></li>';
    }
    ?>
</ul>
...
foreach ($vo->LearningDelivery as $delivery) {
    ...
    echo '<fieldset>';
    echo '<legend>Learning Information - (Aim Reference: ' . (string)$delivery->LearnAimRef . ', Aim Type: ' . (string)$delivery->AimType . ')</legend>';	
    echo '<table class="ilr" border="0" cellspacing="4" cellpadding="4">';
    echo '<col width="150"/><col />';

    if((DB_NAME=='am_baltic' or DB_NAME=='am_ela') && (string)$delivery->AimType!='1')
    {
        if ((string)$delivery->Exclude == '1')
        {
            echo '<tr><td><table><tr><td><input type="checkbox" title="To exclude learning aim from ILR Batch file" checked name="Exclude" /></td><td>Exclude learning aim from ILR return</td></tr></table></td></tr>';
        }
        else
            echo '<tr><td><table><tr><td><input type="checkbox" title="To exclude learning aim from ILR Batch file" name="Exclude" /></td><td>Exclude learning aim from ILR return</td></tr></table></td></tr>';
    }

    echo '<tr>';
    $this->dynamic_field_display('AimType', HTML::select('AimType', $aimtype_dropdown, (string)$delivery->AimType, true, true));
    $this->dynamic_field_display('LearnAimRef', "<input class='compulsory validate[required]' type='text' value='" . (string)$delivery->LearnAimRef . "' style='' id='LearnAimRef' name='LearnAimRef' maxlength=8 size=8>");
    echo '</tr>';

    echo '<tr>';
    $this->dynamic_field_display('LearnStartDate', HTML::datebox('LearnStartDate_' . (string)$delivery->LearnAimRef . $a, (string)$delivery->LearnStartDate, true, true));
    $this->dynamic_field_display('LearnPlanEndDate', HTML::datebox('LearnPlanEndDate_' . (string)$delivery->LearnAimRef . $a, (string)$delivery->LearnPlanEndDate, true, true));
    echo '</tr>';

    echo '<tr>';
    $this->dynamic_field_display('OrigLearnStartDate', HTML::datebox('OrigLearnStartDate_' . (string)$delivery->LearnAimRef . $a, (string)$delivery->OrigLearnStartDate, true, false));
    echo '</tr>';
    ...
    $this->dynamic_field_display('FundModel', HTML::select('FundModel', $FundModel_dropdown, (string)$delivery->FundModel, true, true));
    ...
    $ProgType_dropdown = array(
        ...
        array('32', '32 Skills Bootcamp'),
        array('33', '33 Combined Authorities')
    );
    $this->dynamic_field_display('ProgType', HTML::select('ProgType', $ProgType_dropdown, (string)$delivery->ProgType, true, false));
    echo '</tr>';

    echo '<tr>';
    if ((string)$delivery->ProgType == '2')
        $this->dynamic_field_display('FworkCode', HTML::select('FworkCode', $FworkCode2_dropdown, (string)$delivery->FworkCode, true, false));
    elseif ($delivery->ProgType == '3')
        $this->dynamic_field_display('FworkCode', HTML::select('FworkCode', $FworkCode3_dropdown, (string)$delivery->FworkCode, true, false));
    elseif ($delivery->ProgType == '21')
        $this->dynamic_field_display('FworkCode', HTML::select('FworkCode', $FworkCode4_dropdown, (string)$delivery->FworkCode, true, false));
    elseif ($delivery->ProgType == '20')
        $this->dynamic_field_display('FworkCode', HTML::select('FworkCode', $FworkCode5_dropdown, (string)$delivery->FworkCode, true, false));
    else
        $this->dynamic_field_display('FworkCode', HTML::select('FworkCode', $FworkCode_dropdown, (string)$delivery->FworkCode, true, false));
    if (!isset($delivery) || (string)$delivery->FworkCode == '')
    ...
    $this->dynamic_field_display('PwayCode', HTML::select('PwayCode', $PwayCode_dropdown, (string)$delivery->PwayCode, true, true));
    echo '</tr>';
    echo '<tr>';
    $this->dynamic_field_display('StdCode', HTML::select('StdCode', $StdCode_dropdown, (string)$delivery->StdCode, true, false));
    echo '<td colspan="2">';
    echo '<table>';
    $res = '';
    $wpl = '';
    ...
    foreach($delivery->LearningDeliveryFAM as $ldf)
    {
        if((string)$ldf->LearnDelFAMType == 'RES' and (string)$ldf->LearnDelFAMCode!='undefined')
            $res = (string)$ldf->LearnDelFAMCode;
        if((string)$ldf->LearnDelFAMType == 'WPL' and (string)$ldf->LearnDelFAMCode!='undefined')
            $wpl = (string)$ldf->LearnDelFAMCode;
        if((string)$ldf->LearnDelFAMType == 'FLN' and (string)$ldf->LearnDelFAMCode!='undefined')
            $fln = (string)$ldf->LearnDelFAMCode;
        if((string)$ldf->LearnDelFAMType == 'ADL' and (string)$ldf->LearnDelFAMCode!='undefined')
            $adl = (string)$ldf->LearnDelFAMCode;
        if((string)$ldf->LearnDelFAMType == 'SOF' and (string)$ldf->LearnDelFAMCode!='undefined')
            $sof = (string)$ldf->LearnDelFAMCode;
        if((string)$ldf->LearnDelFAMType == 'FFI' and (string)$ldf->LearnDelFAMCode!='undefined')
            $ffi = (string)$ldf->LearnDelFAMCode;
        if((string)$ldf->LearnDelFAMType == 'EEF' and (string)$ldf->LearnDelFAMCode!='undefined')
            $eef = (string)$ldf->LearnDelFAMCode;
        if((string)$ldf->LearnDelFAMType == 'ASL' and (string)$ldf->LearnDelFAMCode!='undefined')
            $asl = (string)$ldf->LearnDelFAMCode;
        if((string)$ldf->LearnDelFAMType == 'NSA' and (string)$ldf->LearnDelFAMCode!='undefined')
            $nsa = (string)$ldf->LearnDelFAMCode;
        if((string)$ldf->LearnDelFAMType == 'POD' and (string)$ldf->LearnDelFAMCode!='undefined')
            $pod = (string)$ldf->LearnDelFAMCode;
    }
    if ($res == '1')
    ...
    echo '</table></td></tr>';
    echo '<tr>';
    $this->dynamic_field_display('PartnerUKPRN', "<input class='compulsory validate[required]' type='text' value='" . (string)$delivery->PartnerUKPRN . "' style='' id='PartnerUKPRN' name='PartnerUKPRN' maxlength=8 size=8>");
    $this->dynamic_field_display('DelLocPostCode', "<input class='compulsory validate[required]' type='text' value='" . (string)$delivery->DelLocPostCode . "' style='' id='DelLocPostCode' name='DelLocPostCode' maxlength=8 size=8>");
    echo '</tr>';

    echo '<tr>';
    $this->dynamic_field_display('PriorLearnFundAdj', "<input class='optional tooltip' type='text' value='" . (string)$delivery->PriorLearnFundAdj . "' style='' id='PriorLearnFundAdj' name='PriorLearnFundAdj' maxlength=8 size=8 onKeyPress='return numbersonly99(this, event)'>");
    $this->dynamic_field_display('OtherFundAdj', "<input class='optional tooltip' type='text' value='" . (string)$delivery->OtherFundAdj . "' style='' id='OtherFundAdj' name='OtherFundAdj' maxlength=8 size=8>");
    echo '</tr>';

    echo '<tr>';
    $this->dynamic_field_display('AddHours', "<input class='optional tooltip' type='text' value='" . (string)$delivery->AddHours . "' style='' id='AddHours' name='AddHours' maxlength=8 size=8 onKeyPress='return numbersonly(this, event)'>");
    $this->dynamic_field_display('ConRefNumber', "<input class='optional tooltip' type='text' value='" . (string)$delivery->ConRefNumber . "' style='' id='ConRefNumber' name='ConRefNumber' maxlength=8 size=8>");
    echo '</tr>';

    echo '<tr>';
    $this->dynamic_field_display('EPAOrgID', "<input class='optional tooltip' type='text' value='" . (string)$delivery->EPAOrgID . "' style='' id='EPAOrgID' name='EPAOrgID' maxlength='7' >");
    $this->dynamic_field_display('LSDPostcode', "<input class='optional tooltip' type='text' value='" . (string)$delivery->LSDPostcode . "' style='' id='LSDPostcode' name='LSDPostcode' maxlength='8' >");
    echo '</tr>';

    echo '<tr>';
    if ((string)$delivery->AimType == '1' or (string)$delivery->AimType == '99')
    {
        $this->dynamic_field_display('PHours', "<input class='optional tooltip' type='text' value='" . (string)$delivery->PHours . "' style='' id='PHours' name='PHours' maxlength=7 size=7 >");
        $this->dynamic_field_display('OTJActHours', "<input class='optional tooltip' type='text' value='" . (string)$delivery->OTJActHours . "' style='' id='OTJActHours' name='OTJActHours' maxlength=4 size=4 >");
    }
    echo '</tr>';
    ...
    echo '</fieldset>';
    echo '<fieldset>';
    echo '<legend>Funding and Monitoring Information - (Aim Reference: ' . (string)$delivery->LearnAimRef . ', Aim Type: ' . (string)$delivery->AimType . ')</legend>';
    echo '<table border="0" cellspacing="4" cellpadding="4">';
    echo '<col width="150"/><col />';
    if ((string)$delivery->AimType == '1' || (string)$delivery->AimType == '4') // As told by Khush
    {
        ...
        $index = 0;
        foreach ($delivery->LearningDeliveryFAM as $ldf) {
            if ((string)$ldf->LearnDelFAMType == 'LSF' && (string)$ldf->LearnDelFAMCode != '' && (string)$ldf->LearnDelFAMCode != 'undefined') {
                $index++;
                $lsf = "LSF" . $index;
                $from = "LSFFrom_" . (string)$delivery->LearnAimRef . $index;
                $to = "LSFTo_" . (string)$delivery->LearnAimRef . $index;
                echo '<tr>';
                $this->dynamic_field_display('LSFType', HTML::select($lsf, $LSF_dropdown, (string)$ldf->LearnDelFAMCode, true, false));
                echo '</tr>';
                echo '<tr>';
                $this->dynamic_field_display('LearnDelFAMDateFrom', HTML::datebox($from, (string)$ldf->LearnDelFAMDateFrom, false, false));
                $this->dynamic_field_display('LearnDelFAMDateTo', HTML::datebox($to, (string)$ldf->LearnDelFAMDateTo, false, false));
                echo '</tr>';
            }
        }
        $index++;
        ...
        foreach ($delivery->LearningDeliveryFAM as $ldf) {
            if ((string)$ldf->LearnDelFAMType == 'ALB' && (string)$ldf->LearnDelFAMCode != '' && (string)$ldf->LearnDelFAMCode != 'undefined') {
                $index++;
                $alb = "ALB" . $index;
                $from = "ALBFrom_" . (string)$delivery->LearnAimRef . $index;
                $to = "ALBTo_" . (string)$delivery->LearnAimRef . $index;
                echo '<tr>';
                $this->dynamic_field_display('ALBType', HTML::select($alb, $ALB_dropdown, (string)$ldf->LearnDelFAMCode, true, false));
                echo '</tr>';
                echo '<tr>';
                $this->dynamic_field_display('LearnDelFAMDateFrom', HTML::datebox($from, (string)$ldf->LearnDelFAMDateFrom, false, false));
                $this->dynamic_field_display('LearnDelFAMDateTo', HTML::datebox($to, (string)$ldf->LearnDelFAMDateTo, false, false));
    ...
    if ((string)$delivery->AimType == '1' || (string)$delivery->AimType == '3') // ACT
    {
        echo '<tr><td colspan="2">';
        echo '<fieldset class="innerFieldset">';
        ...
        $index = 0;
        foreach ($delivery->LearningDeliveryFAM as $ldf) {
            if ((string)$ldf->LearnDelFAMType == 'ACT' && (string)$ldf->LearnDelFAMCode != '' && (string)$ldf->LearnDelFAMCode != 'undefined') {
                $index++;
                $lsf = "ACT" . $index;
                $from = "ACTFrom_" . (string)$delivery->LearnAimRef . $index;
                $to = "ACTTo_" . (string)$delivery->LearnAimRef . $index;
                echo '<tr>';
                $this->dynamic_field_display('ACTType', HTML::select($lsf, $ACT_dropdown, (string)$ldf->LearnDelFAMCode, true, false));
                echo '</tr>';
                echo '<tr>';
                $this->dynamic_field_display('LearnDelFAMDateFrom', HTML::datebox($from, (string)$ldf->LearnDelFAMDateFrom, false, false));
                $this->dynamic_field_display('LearnDelFAMDateTo', HTML::datebox($to, (string)$ldf->LearnDelFAMDateTo, false, false));
    ...
    if ((string)$delivery->AimType == '1' || (string)$delivery->AimType == '4') {
        if ($adl == '1')
    ...
    echo '</table></td></tr>';
    echo '<tr>';
    $this->dynamic_field_display('SOF', HTML::select('SOF', $SOF_dropdown, $sof, true, true, true, 1, 'title = "' . (string)$delivery->AimType . '" '));
    $this->dynamic_field_display('FFI', HTML::select('FFI', $FFI_dropdown, $ffi, true, false, true, 1, 'title = "' . (string)$delivery->AimType . '" '));
    echo '</tr>';
    ...
    if ((string)$delivery->AimType == "1" || (string)$delivery->AimType == "4" || (string)$delivery->AimType == "5" || (string)$delivery->AimType == "3") {

        $ldm = Array('','','','');
        $index = -1;
        foreach($delivery->LearningDeliveryFAM as $ldf)
        {
            if((string)$ldf->LearnDelFAMType == 'LDM')
            {
                $index++;
                $ldm[$index] = (string)$ldf->LearnDelFAMCode;
            }
        }
    ...
    if (true) {
        ...
        foreach($delivery->LearningDeliveryFAM as $ldf)
        {
            if((string)$ldf->LearnDelFAMType == 'DAM')
            {
                $index++;
                $dam[$index] = (string)$ldf->LearnDelFAMCode;
            }
        }
        $DAM_dropdown = array(
            array('001', '001'),
    ...
    if ((string)$delivery->AimType == "3" || (string)$delivery->AimType == "4") {
        echo '<tr>';
    ...
    if($template != '1')
    {
        if ((string)$delivery->AimType == "1" || (string)$delivery->AimType == "4")
        {
            $delivery_aim_reference = (string)$delivery->LearnAimRef;
            $sql = <<<SQL
SELECT
...
SQL;
    ...
    echo '<fieldset>';
    echo '<legend>Provider Specified Delivery Monitoring Information - (Aim Reference: ' . (string)$delivery->LearnAimRef . ', Aim Type: ' . (string)$delivery->AimType . ')</legend>';
    echo '<table border="0" cellspacing="4" cellpadding="4">';
    echo '<col width="150"/><col />';
    echo '<tr>';
    $ProvSpecDelMonA = '';
    ...
    {
        if((string)$psdm->ProvSpecDelMonOccur == 'A')
            $ProvSpecDelMonA = (string)$psdm->ProvSpecDelMon;
        if((string)$psdm->ProvSpecDelMonOccur == 'B')
            $ProvSpecDelMonB = (string)$psdm->ProvSpecDelMon;
        if((string)$psdm->ProvSpecDelMonOccur == 'C')
            $ProvSpecDelMonC = (string)$psdm->ProvSpecDelMon;
        if((string)$psdm->ProvSpecDelMonOccur == 'D')
            $ProvSpecDelMonD = (string)$psdm->ProvSpecDelMon;
    }
    ...
    if ($ShouldLDHE=="H") {
        ...
        echo '<fieldset>';
        echo '<legend>Learning Delivery HE - (Aim Reference: ' . (string)$delivery->LearnAimRef . ', Aim Type: ' . (string)$delivery->AimType . ')</legend>';
        echo '<table border="0" cellspacing="4" cellpadding="4">';
        echo '<col width="150"/><col />';
        echo '<tr>';
        $this->dynamic_field_display('NUMHUS', "<input class='optional validate[required]' type='text' value='" . (string)$delivery->LearningDeliveryHE->NUMHUS . "' style='' id='NUMHUS' name='NUMHUS' maxlength=20 size=20 >");
        $this->dynamic_field_display('SSN', "<input class='optional validate[required]' type='text' value='" . (string)$delivery->LearningDeliveryHE->SSN . "' style='' id='SSN' name='SSN' maxlength=13 size=13 >");
        echo '</tr>';
        echo '<tr>';
        $this->dynamic_field_display('UCASAPPID', "<input class='optional validate[required]' type='text' value='" . (string)$delivery->LearningDeliveryHE->UCASAPPID . "' style='' id='UCASAPPID' name='UCASAPPID' maxlength=9 size=9 >");
        $this->dynamic_field_display('SOC2000', HTML::select('SOC2000', $SOC2000_dropdown, (string)$delivery->LearningDeliveryHE->SOC2000, true, false));
        echo '</tr>';
        echo '<tr>';
        $this->dynamic_field_display('SEC', HTML::select('SEC', $SEC_dropdown, (string)$delivery->LearningDeliveryHE->SEC, true, false));
        $this->dynamic_field_display('QUALENT3', HTML::select('QUALENT3', $QUALENT3_dropdown, (string)$delivery->LearningDeliveryHE->QUALENT3, true, false));
        echo '</tr>';
        echo '<tr>';
        $this->dynamic_field_display('ELQ', HTML::select('ELQ', $ELQ_dropdown, (string)$delivery->LearningDeliveryHE->ELQ, true, false));
        $this->dynamic_field_display('DOMICILE', HTML::select('DOMICILE', $Domicile_dropdown, (string)$delivery->LearningDeliveryHE->DOMICILE, true, false));
        echo '</tr>';
        echo '<tr><td colspan="2">';
        echo '<fieldset class="innerFieldset">';
        ...
        echo '<col width="150"/><col />';
        echo '<tr>';
        $this->dynamic_field_display('TYPEYR', HTML::select('TYPEYR', $TypeYr_dropdown, (string)$delivery->LearningDeliveryHE->TYPEYR, true, true));
        $this->dynamic_field_display('PCOLAB', "<input class='optional validate[required]' type='text' value='" . (string)$delivery->LearningDeliveryHE->PCOLAB . "' style='' id='PCOLAB' name='PCOLAB' maxlength=6 size=6 >");
        echo '</tr>';
        echo '<tr>';
        $this->dynamic_field_display('MODESTUD', HTML::select('MODESTUD', $ModeStud_dropdown, (string)$delivery->LearningDeliveryHE->MODESTUD, true, true));
        $this->dynamic_field_display('PCFLDCS', "<input class='optional validate[required]' type='text' value='" . (string)$delivery->LearningDeliveryHE->PCFLDCS . "' style='' id='PCFLDCS' name='PCFLDCS' maxlength=6 size=6 >");
        echo '</tr>';
        echo '<tr>';
        $this->dynamic_field_display('FUNDLEV', HTML::select('FUNDLEV', $FundLev_dropdown, (string)$delivery->LearningDeliveryHE->FUNDLEV, true, true));
        $this->dynamic_field_display('PCSLDCS', "<input class='optional validate[required]' type='text' value='" . (string)$delivery->LearningDeliveryHE->PCSLDCS . "' style='' id='PCSLDCS' name='PCSLDCS' maxlength=6 size=6 >");
        echo '</tr>';
        echo '<tr>';
        $this->dynamic_field_display('YEARSTU', "<input class='compulsory validate[required]' type='text' value='" . (string)$delivery->LearningDeliveryHE->YEARSTU . "' style='' id='YEARSTU' name='YEARSTU' maxlength=2 size=2 >");
        $this->dynamic_field_display('PCTLDCS', "<input class='optional validate[required]' type='text' value='" . (string)$delivery->LearningDeliveryHE->PCTLDCS . "' style='' id='PCTLDCS' name='PCTLDCS' maxlength=6 size=6 >");
        echo '</tr>';
        echo '<tr>';
        $this->dynamic_field_display('FUNDCOMP', HTML::select('FUNDCOMP', $FundComp_dropdown, (string)$delivery->LearningDeliveryHE->FUNDCOMP, true, true));
        $this->dynamic_field_display('STULOAD', "<input class='optional validate[required]' type='text' value='" . (string)$delivery->LearningDeliveryHE->STULOAD . "' style='' id='STULOAD' name='STULOAD' maxlength=6 size=6 >");
        echo '</tr>';
        echo '</table>';
        echo '</td></tr>';
        echo '<tr><td colspan="2">';
        ...
        echo '<col width="150"/><col />';
        echo '<tr>';
        $this->dynamic_field_display('MSTUFEE', HTML::select('MSTUFEE', $MSTuFee_dropdown, (string)$delivery->LearningDeliveryHE->MSTUFEE, true, true));
        $this->dynamic_field_display('SPECFEE', HTML::select('SPECFEE', $SpecFee_dropdown, (string)$delivery->LearningDeliveryHE->SPECFEE, true, true));
        echo '</tr>';
        echo '<tr>';
        $this->dynamic_field_display('NETFEE', "<input class='optional validate[required]' type='text' value='" . (string)$delivery->LearningDeliveryHE->NETFEE . "' style='' id='NETFEE' name='NETFEE' maxlength=6 size=6 >");
        $this->dynamic_field_display('GROSSFEE', "<input class='optional validate[required]' type='text' value='" . (string)$delivery->LearningDeliveryHE->GROSSFEE . "' style='' id='GROSSFEE' name='GROSSFEE' maxlength=6 size=6 >");
        echo '</tr>';
        echo '<tr>';
        $this->dynamic_field_display('HEPostCode', "<input class='optional validate[required]' type='text' value='" . (string)$delivery->LearningDeliveryHE->HEPostCode . "' style='' id='HEPostCode' name='HEPostCode' maxlength=6 size=6 >");
        echo '</tr>';
        echo '</table>';
        echo '</td></tr>';
        echo '<tr><td colspan="2">';
...
SQL;
    ...
    if ($delivery->AimType == '1' && ($delivery->ProgType == '25' || $delivery->FundModel=='36')) {
        ...
        if(DB_NAME=='am_baltic')
        {
            ...
            echo '<td><input class="optional" type="text" name="OverallTNP" onKeyPress="return numbersonly(this, event)" value="' . $delivery->OverallTNP . '" maxlength="50"/></td>';
            echo '</tr>';
        }
        ...
        foreach ($delivery->TrailblazerApprenticeshipFinancialRecord as $trailblazer) {
            if ((string)$trailblazer->TBFinType != '') {
                $trailblazerindex++;
                echo '<tr>';
                $tbfintype = (string)$trailblazer->TBFinType;
                $this->dynamic_field_display('TBFinType', HTML::select('TBFinType' . $trailblazerindex, $TBFinType_dropdown, $tbfintype, true, true));
                $tbfincode = (string)$trailblazer->TBFinCode;
                $this->dynamic_field_display('TBFinCode', HTML::select('TBFinCode' . $trailblazerindex, $TBFinCode_dropdown, $tbfincode, true, true));
                echo '</tr>';
                echo '<tr>';
                $tbfindate = (string)$trailblazer->TBFinDate;
                $this->dynamic_field_display('TBFinDate', HTML::datebox('TBFinDate' . $trailblazerindex, $tbfindate, true));
                $tbfinamount = (string)$trailblazer->TBFinAmount;
                $this->dynamic_field_display('TBFinAmount', "<input class='compulsory validate[required]' type='text' onKeyPress='return numbersonly(this, event)' value='" . $tbfinamount . "' style='' id='TBFinAmount$trailblazerindex' name='TBFinAmount$trailblazerindex' maxlength=30 size=30 onKeyPress='return numbersonly(this, event)'>");
                echo '</tr>';
            }
        }
        $trailblazerindex++;
        ...
        $this->dynamic_field_display('TBFinAmount', "<input class='compulsory validate[required]' onKeyPress='return numbersonly(this, event)' type='text' value='" . '' . "' style='' id='TBFinAmount$trailblazerindex' name='TBFinAmount$trailblazerindex' maxlength=30 size=30 onKeyPress='return numbersonly(this, event)'> ");
        echo '</tr>';
        $trailblazerindex++;
        echo '<tr>';
        $this->dynamic_field_display('TBFinType', HTML::select('TBFinType' . $trailblazerindex, $TBFinType_dropdown, '', true, true));
    ...
    if ((string)$delivery->LearnAimRef == 'Z0007834' || (string)$delivery->LearnAimRef == 'Z0007835' || (string)$delivery->LearnAimRef == 'Z0007836' || (string)$delivery->LearnAimRef == 'Z0007837' || (string)$delivery->LearnAimRef == 'Z0007838' || (string)$delivery->LearnAimRef == 'ZWRKX001') {
        echo '<fieldset>';
        echo '<legend>Work Placement - (Aim Reference: ' . (string)$delivery->LearnAimRef . ', Aim Type: ' . (string)$delivery->AimType . ')</legend>';
        $ldwpindex = 0;
        echo '<table border="0" cellspacing="4" cellpadding="4">';
        echo '<col width="150"/><col />';
        foreach ($delivery->LearningDeliveryWorkPlacement as $ldwp) {
            if ((string)$ldwp->WorkPlaceStartDate != '' && (string)$ldwp->WorkPlaceStartDate != 'undefined') {
                $ldwpindex++;
                echo '<tr>';
                $workplace_mode = "WorkPlaceMode_" . (string)$delivery->LearnAimRef . $ldwpindex;
                $workplace_emp_id = "WorkPlaceEmpId_" . (string)$delivery->LearnAimRef . $ldwpindex;
                $workplace_hours = "WorkPlaceHours_" . (string)$delivery->LearnAimRef . $ldwpindex;

                $from = "WorkPlaceStartDate_" . (string)$delivery->LearnAimRef . $ldwpindex;
                $to = "WorkPlaceEndDate_" . (string)$delivery->LearnAimRef . $ldwpindex;
                $this->dynamic_field_display('WorkPlaceStartDate', HTML::datebox($from, (string)$ldwp->WorkPlaceStartDate));
                $this->dynamic_field_display('WorkPlaceEndDate', HTML::datebox($to, (string)$ldwp->WorkPlaceEndDate));
                echo '</tr>';
                echo '<tr>';
                $WPMode_dropdown = array(array('1', '1 Internal (simulated) work placement'), array('2', '2 External work placement'));
                $this->dynamic_field_display('WorkPlaceMode', HTML::select($workplace_mode, $WPMode_dropdown, (string)$ldwp->WorkPlaceMode, true, true));
                $this->dynamic_field_display('WorkPlaceEmpId', "<input class='compulsory validate[required]' type='text' value='" . (string)$ldwp->WorkPlaceEmpId . "' style='' id='$workplace_emp_id' name='$workplace_emp_id' maxlength=30 size=30>");
                echo '</tr>';
                echo '<tr>';
                $this->dynamic_field_display('WorkPlaceHours', "<input class='compulsory validate[required]' type='text' value='" . (string)$ldwp->WorkPlaceHours . "' style='' id='$workplace_hours' name='$workplace_hours' maxlength=30 size=30 onKeyPress='return numbersonly(this, event)'>");
                echo '</tr>';
            }
        }
        $ldwpindex++;
    ...
    echo '<fieldset>';
    echo '<legend>Learning End Information - (Aim Reference: ' . (string)$delivery->LearnAimRef . ', Aim Type: ' . (string)$delivery->AimType . ')</legend>';
    echo '<table border="0" cellspacing="4" cellpadding="4">';
    echo '<col width="150"/><col />';

    echo '<tr>';
    $this->dynamic_field_display('LearnActEndDate', HTML::datebox('LearnActEndDate_' . (string)$delivery->LearnAimRef . $a, (string)$delivery->LearnActEndDate));
    $this->dynamic_field_display('AchDate', HTML::datebox('AchDate_' . (string)$delivery->LearnAimRef . $a, (string)$delivery->AchDate));
    echo '</tr>';

    echo '<tr>';
    $this->dynamic_field_display('CompStatus', HTML::select('CompStatus', $CompStatus_dropdown, (string)$delivery->CompStatus, false, true));
    $this->dynamic_field_display('WithdrawReason', HTML::select('WithdrawReason', $WithdrawReason_dropdown, (string)$delivery->WithdrawReason, true, true));
    echo '</tr>';
    ...
    $this->dynamic_field_display('Outcome', HTML::select('Outcome', $Outcome_dropdown, (string)$delivery->Outcome, true, true));
    $this->dynamic_field_display('EmpOutcome', HTML::select('EmpOutcome', $EmpOutcome_dropdown, (string)$delivery->EmpOutcome, true, true));
    echo '</tr>';

    echo '<tr>';
    $this->dynamic_field_display('OutGrade', HTML::select('OutGrade', $OutGrade_dropdown, (string)$delivery->OutGrade, true, true));
    echo '</tr>';
...
$OET_dropdown = array(array('1', '1 Learner has been made redundant'),array('2', '2 Small or Medium Employer'),array('3', '3 Employer has changed'));
// Prior enrolment
foreach ($vo->LearnerEmploymentStatus as $empstatus) {
    ...
    if($tr_start_date->after(Date::toShort($empstatus->DateEmpStatApp)))
    {
        ...
        $empstat_desc = DAO::getSingleValue($link, "SELECT EmpStaCode_Desc from lis201415.ilr_empstatcode where EmpStatCode = '" . (string)$empstatus->EmpStat . "'");
        ...
        echo '<td class="fieldLabel_compulsory">Employer:';
        $EmployerId = "EmployerId" . $index;
        if((string)$empstatus->EmployerId=="")
            echo HTML::select($EmployerId, $employers, (string)$training_record->employer_id, true, false);
        else
            echo HTML::select($EmployerId, $employers, (string)$empstatus->EmployerId, true, false);
        echo '</td>';
        echo '</tr>';

        echo '<tr style="display:none" class="emp_stat_' . $index . '">';
        $this->dynamic_field_display('EmpStat', HTML::select($id, $EmpStat_dropdown, (string)$empstatus->EmpStat, true, true));
        $id = "DateEmpStatApp" . $index;
        $this->dynamic_field_display('DateEmpStatApp', HTML::datebox($id, Date::toShort((string)$empstatus->DateEmpStatApp), true, true));
        echo '</tr>';

        echo '<tr style="display:none;" class="emp_stat_' . $index . '">';
        $this->dynamic_field_display('EmpId', "<input class='compulsory validate[required]' type='text' value='" . (string)$empstatus->EmpId . "' style='' id='EmpId$index' name='EmpId$index' maxlength=30 size=30>");
...
if ($index == -1) {
    ...
    echo '<td class="fieldLabel_compulsory">Employer:';
    $EmployerId = "EmployerId" . $index;
    echo HTML::select($EmployerId, $employers, (isset($empstatus->EmployerId) ? $empstatus->EmployerId : ''), true, false);
    echo '</td>';
    echo '</tr>';
...
<div id = "delivery_template"  class = "Unit" style="display: none;">
    ...
    $this->dynamic_field_display('FFI', HTML::select('FFI', $FFI_dropdown, '', true, false, true));
    echo '</tr>';
...
<div id="endInfoDialog" title="Learning End Information" >
    <p class="introduction">Use this functionality to update Learning End Information for all learning aims.</p>
    <form name="frmEndInfo">
        <table id="tblEndInfo" border="0" class="resultset" cellspacing="4" cellpadding="4" style="margin-left:10px">
            <tr><th>Learning Actual End Date: </th><td><?php echo HTML::datebox('endInfoActEndDate', ''); ?></td></tr>
            <tr><th>Completion Status: </th><td><?php echo HTML::select('endInfoCompStatus', $CompStatus_dropdown, '', false); ?></td></tr>
            <tr><th>Outcome: </th><td><?php echo HTML::select('endInfoOutcome', $Outcome_dropdown, '', true); ?></td></tr>
            <tr><th>Achievement Date: </th><td><?php echo HTML::datebox('endInfoAchDate', ''); ?></td></tr>
            <tr><th>Withdrawl Reason: </th><td><?php echo HTML::select('endInfoWithdrawReason', $WithdrawReason_dropdown, '', true); ?></td></tr>
            <tr><th>Employment Outcome: </th><td><?php echo HTML::select('endInfoEmpOutcome', $EmpOutcome_dropdown, '', true); ?></td></tr>
            <tr><th>Outcome Grade: </th><td><?php echo HTML::select('endInfoOutGrade', $OutGrade_dropdown, '', true); ?></td></tr>
            <tr><th>Actual OTJ: </th><td><input class='optional' type='text' value='' style='' id='endInfoActOTJ' name='endInfoActOTJ' maxlength=3 size=3></td></tr>
        </table>
    </form>
</div>
...
function batchUpdateEndInfo()
{
    var number_of_aims = ($('.Unit').length);
    for(subaims = 1; subaims <= number_of_aims; subaims++)
    {
        var comp_status = getCompStatus(subaims);
        if(comp_status==1)
        {
            setSubsidiaryAimFields(subaims);
        }
    }
}

function setSubsidiaryAimFields(aimNumber)
{
    ...
    if(div != null)
    {
        ...
        if(document.forms['frmEndInfo'].endInfoActEndDate.value!="")
        {
            document.ilr.OTJActHours.value = document.forms['frmEndInfo'].endInfoActOTJ.value;
        }
        ...
        for(var i = 0; i < elements.length; i++)
        {
                ...
                || (elements[i].type == "radio" && elements[i].checked) )
            {
                ...
                {
                    if(elements[i].name == "LearnAimRef")
                        qan = elements[i].value;

                    if(elements[i].name == "LearnActEndDate_" + qan + aimNumber)
                    {
                        elements[i].value = document.forms['frmEndInfo'].endInfoActEndDate.value;
                    }
                    if(elements[i].name == "AchDate_" + qan + aimNumber)
                    {
                        elements[i].value = document.forms['frmEndInfo'].endInfoAchDate.value;
                    }
        ...
        {
            {
                if(elements[i].name=="CompStatus")
                    elements[i].value = document.forms['frmEndInfo'].endInfoCompStatus.value;
                if(elements[i].name=="Outcome")
                    elements[i].value = document.forms['frmEndInfo'].endInfoOutcome.value;
                if(elements[i].name=="WithdrawReason")
                    elements[i].value = document.forms['frmEndInfo'].endInfoWithdrawReason.value;
                if(elements[i].name=="EmpOutcome")
                    elements[i].value = document.forms['frmEndInfo'].endInfoEmpOutcome.value;
                if(elements[i].name=="OutGrade")
                    elements[i].value = document.forms['frmEndInfo'].endInfoOutGrade.value;
            }
        }
    }
}
...
function getCompStatus(aimNumber)
{
    var comp_status = 0;
    var div = document.getElementById('tab' + aimNumber);
    if(div != null)
    {
        ...
        for(var i = 0; i < elements.length; i++)
        {
            ...
            if(elements[i].name=="CompStatus" && elements[i].value == "1")
            {
                comp_status = 1;
            }
        }
        ...
        for(var i = 0; i < elements.length; i++)
        {
            ...
            {
                if(elements[i].name == "LearnAimRef" && (elements[i].value == "119EP" || elements[i].value == "119GW"))
                    comp_status = 0;
            }
        }
    }
...
$(function() {
    ...
    $( "#btnOpenEndInfoDialog" ).click(function() {
        document.forms["frmEndInfo"].reset();
        $( "#endInfoDialog" ).dialog( "open" );
    });
});
