...
$_qs = [
    'filter_username_preferences' => '',
    'options' => [],
];

parse_str($export_url, $_output);

if(isset($_output['filter_username_preferences']))
    $_qs['filter_username_preferences'] = $_output['filter_username_preferences'];
if(isset($_output['options']))
    $_qs['options'] = $_output['options'];

$export_extra = '&amp;' . http_build_query($_qs);
...
<div class="banner">
    ...
    <div class="ActionIconBar">
        ...
        <button onclick="window.location.href='do.php?_action=support_requests&amp;export=csv<?php echo $export_extra; ?>'" title="Export to .CSV file" id="btn-excel"></button>
    ...
    <div class="ButtonBar">
        <form method="get" name="preferences" action="<?php echo $_SERVER['PHP_SELF']; ?>">
            ...
            <label><input class="chckbox" type="checkbox" name="options[]" <?php echo $checked2; ?> value="Assigned" onclick="submitPreferencesForm();"/>Being Looked Into By Perspective</label>
            <?php $checked4 = in_array('Awaiting Client',$options)? 'checked' : '';  ?>
            <label><input class="chckbox" type="checkbox" name="options[]" <?php echo $checked4; ?> value="Awaiting Client" onclick="submitPreferencesForm();"/>Requiring Your Feedback</label>
