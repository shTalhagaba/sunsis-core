...
<head>
	...
	<title><?php echo $vo->id == ''?'Add OTJ Details':'Edit OTJ Details'; ?></title>
...
<div class="row">
	<div class="col-lg-12">
		<div class="banner">
			<div class="Title" style="margin-left: 6px;"><?php echo $vo->id == ''?'Add OTJ Details':'Edit OTJ Details'; ?></div>
			<div class="ButtonBar">
...
<div class="row">
	<form class="form-horizontal" name="frmOTJ" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
		<input type="hidden" name="_action" value="save_otj" />
...
<script language="JavaScript">
	...
	function save()
	{
		var myForm = document.forms["frmOTJ"];
	...
	function delete_record(record_id)
	{
		if(!confirm('This action cannot be undone, are you sure you want to delete this record?'))
			return;
		var client = ajaxRequest('do.php?_action=edit_otj_hours&ajax_request=true&otj_id='+ encodeURIComponent(record_id));
    ...
    function deleteFile(path)
    {
        confirmation("Deletion is permanent and irrecoverable.  Continue?").then(function (answer) {
            ...
            if(ansbool){
                ...
                if(client)
                    window.location.replace("do.php?_action=edit_otj_hours&tr_id="+encodeURIComponent(<?php echo $vo->tr_id; ?>)+"&otj_id=" + encodeURIComponent(<?php echo $vo->id; ?>));
            }
        });
    }
