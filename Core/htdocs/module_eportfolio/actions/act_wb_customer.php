<?php
class wb_customer implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($id == '')
		{
			...
			$exists = DAO::getSingleValue($link, "SELECT id FROM workbooks WHERE tr_id = '{$tr_id}' AND wb_title = 'WBCustomer'");
			if($exists != "")
				$wb = WBCustomer::loadFromDatabase($link, $exists);
			else
			{
		...
		include_once('tpl_wb_customer.php');
	}
}