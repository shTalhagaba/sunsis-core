<?php
class save_wb_stock implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($wb_id == '')
		{
			$wb = new WBStock($tr_id);
			// first time saving the workbook so create the XML of workbook content
			$wb->wb_content = WBStock::getBlankXML();
		}
		else
		{
			$wb = WBStock::loadFromDatabase($link, $wb_id);
		}
		...
		if($_SESSION['user']->type == User::TYPE_LEARNER)
		{
			...
			for($i = 1; $i <= count(WBStock::getLearningJourneyItems($wb->savers_or_sp)); $i++)
			{
				$key = 'DC'.$i;
			...
			$ProtectingStock = $Answers->addChild('ProtectingStock');
			for($i = 1; $i <= 2; $i++)
			{
				$k = 'Q'.$i;
				$ProtectingStock->addChild($k, htmlspecialchars((string)$_REQUEST['ProtectingStock'.$k]));
			}

			$PossibleRisks = $Answers->addChild('PossibleRisks');
			for($i = 1; $i <= 4; $i++)
			{
				$k = 'Scenario'.$i;
				$PossibleRisks->addChild($k, htmlspecialchars((string)$_REQUEST[$k]));
			}

			$InStoreStorageFacilities = $Answers->addChild('InStoreStorageFacilities');
			for($i = 1; $i <= 3; $i++)
			{
				$k = 'Q'.$i;
				$InStoreStorageFacilities->addChild($k, htmlspecialchars((string)$_REQUEST['InStoreStorageFacilities'.$k]));
			}

			$Deliveries = $Answers->addChild('Deliveries');
			for($i = 1; $i <= 4; $i++)
			{
				$k = 'Q'.$i;
				$Deliveries->addChild($k, htmlspecialchars((string)$_REQUEST['Deliveries'.$k]));
			}
...
//			pre($Answers);
		...
		elseif($_SESSION['user']->type == User::TYPE_ASSESSOR)
		{
			...
			$ProtectingStock = $Feedback->addChild('ProtectingStock');
			$ProtectingStock->addChild('Status', htmlspecialchars((string)$_REQUEST['status_ProtectingStock']));
			$ProtectingStock->addChild('Comments', htmlspecialchars((string)$_REQUEST['comments_ProtectingStock']));
			$PossibleRisks = $Feedback->addChild('PossibleRisks');
			$PossibleRisks->addChild('Status', htmlspecialchars((string)$_REQUEST['status_PossibleRisks']));
			$PossibleRisks->addChild('Comments', htmlspecialchars((string)$_REQUEST['comments_PossibleRisks']));
			$InStoreStorageFacilities = $Feedback->addChild('InStoreStorageFacilities');
			$InStoreStorageFacilities->addChild('Status', htmlspecialchars((string)$_REQUEST['status_InStoreStorageFacilities']));
			$InStoreStorageFacilities->addChild('Comments', htmlspecialchars((string)$_REQUEST['comments_InStoreStorageFacilities']));
			$Deliveries = $Feedback->addChild('Deliveries');
			$Deliveries->addChild('Status', htmlspecialchars((string)$_REQUEST['status_Deliveries']));
			$Deliveries->addChild('Comments', htmlspecialchars((string)$_REQUEST['comments_Deliveries']));
			...
			if($wb->full_save == 'Y' && $_SESSION['user']->type == User::TYPE_LEARNER)
			{
				...
				if($wb->wb_status == Workbook::STATUS_LEARNER_COMPLETED)
				{
					...
					if(isset($tr_details->assessor))
					{
						...
						$notification->link = 'do.php?_action=wb_stock&id='.$wb->id.'&tr_id='.$wb->tr_id;
