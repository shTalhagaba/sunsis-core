<?php
class wb_financial implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($id == '')
		{
			...
			$exists = DAO::getSingleValue($link, "SELECT id FROM workbooks WHERE tr_id = '{$tr_id}' AND wb_title = 'WBFinancial'");
			if($exists != "")
				$wb = WBFinancial::loadFromDatabase($link, $exists);
			else
			{
		...
		include_once('tpl_wb_financial.php');
	}
}