<?php
class wb_meeting_regulations_and_legislation implements IAction
{
	public function execute(PDO $link)
	{
		$id = isset($_REQUEST['id'])?$_REQUEST['id']:'';
		$tr_id = isset($_REQUEST['tr_id'])?$_REQUEST['tr_id']:'';
		$_index = isset($_REQUEST['wizard_index'])?$_REQUEST['wizard_index']:'0';
		...
		if($id == '')
		{
			...
			$exists = DAO::getSingleValue($link, "SELECT id FROM workbooks WHERE tr_id = '{$tr_id}' AND wb_title = 'WBMeetingRegulationsAndLegislation'");
			if($exists != "")
				$wb = WBMeetingRegulationsAndLegislation::loadFromDatabase($link, $exists);
			else
			{
		...
		include_once('tpl_wb_meeting_regulations_and_legislation.php');
	}
}