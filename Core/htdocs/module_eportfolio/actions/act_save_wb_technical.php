<?php
class save_wb_technical implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($wb_id == '')
		{
			$wb = new WBTechnical($tr_id);
			// first time saving the workbook so create the XML of workbook content
			$wb->wb_content = WBTechnical::getBlankXML();
		}
		else
		{
			$wb = WBTechnical::loadFromDatabase($link, $wb_id);
		}
		...
		if($_SESSION['user']->type == User::TYPE_LEARNER)
		{
			...
			for($i = 1; $i <= count(WBTechnical::getLearningJourneyItems($wb->savers_or_sp)); $i++)
			{
				$key = 'DC'.$i;
				$Journey->addChild($key, htmlspecialchars((string)$_REQUEST['Journey'.$key]));
			}

			if($wb->savers_or_sp != 'savers')
				$Answers->addChild('HowToRegisterBeautyCard', htmlspecialchars((string)$_REQUEST['HowToRegisterBeautyCard']));

			$TechQuestions = $Answers->addChild('TechQuestions');
			for($i = 1; $i <= 5; $i++)
			{
				$key = 'Q'.$i;
				$TechQuestions->addChild($key, htmlspecialchars((string)$_REQUEST[$key]));
			}

			$InstoreActivity = $Answers->addChild('InstoreActivity');
			for($i = 1; $i <= count(WBTechnical::getLearningJourneyItems2($wb->savers_or_sp)); $i++)
			{
				$key = 'DC'.$i;
				$InstoreActivity->addChild($key, htmlspecialchars((string)$_REQUEST['InstoreActivity'.$key]));
			}
		...
		elseif($_SESSION['user']->type == User::TYPE_ASSESSOR)
		{
			...
			if($wb->savers_or_sp != 'savers')
			{
				$HowToRegisterBeautyCard = $Feedback->addChild('HowToRegisterBeautyCard');
				$HowToRegisterBeautyCard->addChild('Status', htmlspecialchars((string)$_REQUEST['status_HowToRegisterBeautyCard']));
				$HowToRegisterBeautyCard->addChild('Comments', htmlspecialchars((string)$_REQUEST['comments_HowToRegisterBeautyCard']));
			}
			$TechQuestions = $Feedback->addChild('TechQuestions');
			$TechQuestions->addChild('Status', htmlspecialchars((string)$_REQUEST['status_TechQuestions']));
			$TechQuestions->addChild('Comments', htmlspecialchars((string)$_REQUEST['comments_TechQuestions']));
			...
			if($wb->full_save == 'Y' && $_SESSION['user']->type == User::TYPE_LEARNER)
			{
				...
				if($wb->wb_status == Workbook::STATUS_LEARNER_COMPLETED)
				{
					...
					if(isset($tr_details->assessor))
					{
						...
						$notification->link = 'do.php?_action=wb_technical&id='.$wb->id.'&tr_id='.$wb->tr_id;
