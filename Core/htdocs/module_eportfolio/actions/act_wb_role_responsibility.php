<?php
class wb_role_responsibility implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($id == '')
		{
			...
			$exists = DAO::getSingleValue($link, "SELECT id FROM workbooks WHERE tr_id = '{$tr_id}' AND wb_title = 'WBRoleResponsibility'");
			if($exists != "")
				$wb = WBRoleResponsibility::loadFromDatabase($link, $exists);
			else
			{
		...
		include_once('tpl_wb_role_responsibility.php');
	}
}