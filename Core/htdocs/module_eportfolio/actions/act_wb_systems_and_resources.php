<?php
class wb_systems_and_resources implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($id == '')
		{
			...
			$exists = DAO::getSingleValue($link, "SELECT id FROM workbooks WHERE tr_id = '{$tr_id}' AND wb_title = 'WBSystemsAndResources'");
			if($exists != "")
				$wb = WBSystemsAndResources::loadFromDatabase($link, $exists);
			else
			{
				$wb = new WBSystemsAndResources($tr_id);
				$wb->tr_id = $tr->id;
				$wb->save($link);
		...
		include_once('tpl_wb_systems_and_resources.php');
	}
}