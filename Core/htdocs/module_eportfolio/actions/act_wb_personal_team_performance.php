<?php
class wb_personal_team_performance implements IAction
{
    public function execute(PDO $link)
    {
        ...
        if($id == '')
        {
            ...
            $exists = DAO::getSingleValue($link, "SELECT id FROM workbooks WHERE tr_id = '{$tr_id}' AND wb_title = 'WBPersonalTeamPerformance'");
            if($exists != "")
                $wb = WBPersonalTeamPerformance::loadFromDatabase($link, $exists);
            else
            {
        ...
        include_once('tpl_wb_personal_team_performance.php');
    }
}