<?php
class wb_communication implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($id == '')
		{
			...
			$exists = DAO::getSingleValue($link, "SELECT id FROM workbooks WHERE tr_id = '{$tr_id}' AND wb_title = 'WBCommunication'");
			if($exists != "")
				$wb = WBCommunication::loadFromDatabase($link, $exists);
			else
			{
		...
		include_once('tpl_wb_communication.php');
	}
}