<?php
class wb_technical implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($id == '')
		{
			...
			$exists = DAO::getSingleValue($link, "SELECT id FROM workbooks WHERE tr_id = '{$tr_id}' AND wb_title = 'WBTechnical'");
			if($exists != "")
				$wb = WBTechnical::loadFromDatabase($link, $exists);
			else
			{
		...
		include_once('tpl_wb_technical.php');
	}
}
