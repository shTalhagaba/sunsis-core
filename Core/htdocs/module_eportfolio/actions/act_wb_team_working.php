<?php
class wb_team_working implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($id == '')
		{
			...
			$exists = DAO::getSingleValue($link, "SELECT id FROM workbooks WHERE tr_id = '{$tr_id}' AND wb_title = 'WBTeamWorking'");
			if($exists != "")
				$wb = WBTeamWorking::loadFromDatabase($link, $exists);
			else
			{
		...
		include_once('tpl_wb_team_working.php');
	}
}