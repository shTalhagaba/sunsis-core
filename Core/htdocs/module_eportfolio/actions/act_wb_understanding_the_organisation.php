<?php
class wb_understanding_the_organisation implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($id == '')
		{
			...
			$exists = DAO::getSingleValue($link, "SELECT id FROM workbooks WHERE tr_id = '{$tr_id}' AND wb_title = 'WBUnderstandingTheOrganisation'");
			if($exists != "")
				$wb = WBUnderstandingTheOrganisation::loadFromDatabase($link, $exists);
			else
			{
		...
		include_once('tpl_wb_understanding_the_organisation.php');
	}
}