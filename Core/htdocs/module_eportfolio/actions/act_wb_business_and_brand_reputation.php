<?php
class wb_business_and_brand_reputation implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($id == '')
		{
			...
			$exists = DAO::getSingleValue($link, "SELECT id FROM workbooks WHERE tr_id = '{$tr_id}' AND wb_title = 'WBBusinessAndBrandReputation'");
			if($exists != "")
				$wb = WBBusinessAndBrandReputation::loadFromDatabase($link, $exists);
			else
			{
		...
		include_once('tpl_wb_business_and_brand_reputation.php');
	}
}