<?php
class save_wb_meeting_regulations_and_legislation implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($wb_id == '')
		{
			$wb = new WBMeetingRegulationsAndLegislation($tr_id);
			// first time saving the workbook so create the XML of workbook content
			$wb->wb_content = WBMeetingRegulationsAndLegislation::getBlankXML();
		}
		else
		{
			$wb = WBMeetingRegulationsAndLegislation::loadFromDatabase($link, $wb_id);
		}
		...
		if(isset($_FILES['FloorPlan']) && $_FILES['FloorPlan']['error'] != UPLOAD_ERR_NO_FILE && isset($_FILES['FloorPlan']['size']) && $_FILES['FloorPlan']['size'] <= 1024000)
		{
			...
			http_redirect('do.php?_action=wb_meeting_regulations_and_legislation&id='.$wb->id.'&tr_id='.$wb->tr_id.'&wizard_index=21');
		}
		...
		if($_SESSION['user']->type == User::TYPE_LEARNER)
		{
			...
			$DataProtectionAct->addChild('HowToKeepInfo', htmlspecialchars((string)$_REQUEST['DPAHowToKeepInfo']));
			...
			$ConsumerContractsRegulations = $Answers->addChild('ConsumerContractsRegulations');
			$ConsumerContractsRegulations->addChild('Information', htmlspecialchars((string)$_REQUEST['CCRInformation']));
			$ConsumerContractsRegulations->addChild('HowProtect', htmlspecialchars((string)$_REQUEST['CCRHowProtect']));
			$ConsumerContractsRegulations->addChild('HowImpact', htmlspecialchars((string)$_REQUEST['CCRHowImpact']));
			...
			if($wb->full_save == 'Y' && $_SESSION['user']->type == User::TYPE_LEARNER)
			{
				...
				if($wb->wb_status == Workbook::STATUS_LEARNER_COMPLETED)
				{
					...
					if(isset($tr_details->assessor))
					{
						...
						$notification->link = 'do.php?_action=wb_meeting_regulations_and_legislation&id='.$wb->id.'&tr_id='.$wb->tr_id;
