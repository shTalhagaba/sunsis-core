<?php
class wb_developing_self implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($id == '')
		{
			...
			$exists = DAO::getSingleValue($link, "SELECT id FROM workbooks WHERE tr_id = '{$tr_id}' AND wb_title = 'WBDevelopingSelf'");
			if($exists != "")
				$wb = WBDevelopingSelf::loadFromDatabase($link, $exists);
			else
			{
		...
		include_once('tpl_wb_developing_self.php');
	}
}