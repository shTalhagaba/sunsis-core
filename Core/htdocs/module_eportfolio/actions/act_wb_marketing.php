<?php
class wb_marketing implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($id == '')
		{
			...
			$exists = DAO::getSingleValue($link, "SELECT id FROM workbooks WHERE tr_id = '{$tr_id}' AND wb_title = 'WBMarketing'");
			if($exists != "")
				$wb = WBMarketing::loadFromDatabase($link, $exists);
			else
			{
		...
		include_once('tpl_wb_marketing.php');
	}
}