<?php
class wb_retail_product_and_service implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($id == '')
		{
			...
			$exists = DAO::getSingleValue($link, "SELECT id FROM workbooks WHERE tr_id = '{$tr_id}' AND wb_title = 'WRetailBProductAndService'");
			if($exists != "")
				$wb = WBRetailProductAndService::loadFromDatabase($link, $exists);
			else
			{
		...
		include_once('tpl_wb_retail_product_and_service.php');
	}
}