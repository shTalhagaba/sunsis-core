<?php
class wb_legal_and_governance implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($id == '')
		{
			...
			$exists = DAO::getSingleValue($link, "SELECT id FROM workbooks WHERE tr_id = '{$tr_id}' AND wb_title = 'WBLegalAndGovernance'");
			if($exists != "")
				$wb = WBLegalAndGovernance::loadFromDatabase($link, $exists);
			else
			{
		...
		include_once('tpl_wb_legal_and_governance.php');
	}
}