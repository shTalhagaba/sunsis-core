<?php
class wb_product_and_service implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($id == '')
		{
			...
			$exists = DAO::getSingleValue($link, "SELECT id FROM workbooks WHERE tr_id = '{$tr_id}' AND wb_title = 'WBProductAndService'");
			if($exists != "")
				$wb = WBProductAndService::loadFromDatabase($link, $exists);
			else
			{
		...
		include_once('tpl_wb_product_and_service.php');
	}
}