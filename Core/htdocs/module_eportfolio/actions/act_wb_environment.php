<?php
class wb_environment implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($id == '')
		{
			...
			$exists = DAO::getSingleValue($link, "SELECT id FROM workbooks WHERE tr_id = '{$tr_id}' AND wb_title = 'WBEnvironment'");
			if($exists != "")
				$wb = WBEnvironment::loadFromDatabase($link, $exists);
			else
			{
		...
		include_once('tpl_wb_environment.php');
	}
}