<?php
class wb_hs_and_security implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($id == '')
		{
			...
			$exists = DAO::getSingleValue($link, "SELECT id FROM workbooks WHERE tr_id = '{$tr_id}' AND wb_title = 'WBHSAndSecurity'");
			if($exists != "")
				$wb = WBHSAndSecurity::loadFromDatabase($link, $exists);
			else
			{
		...
		include_once('tpl_wb_hs_and_security.php');
	}
}
