<?php
class save_wb_retail_product_and_service implements IAction
{
...
//		pre($_REQUEST);
		...
		if($wb_id == '')
		{
			$wb = new WBRetailProductAndService($tr_id);
			// first time saving the workbook so create the XML of workbook content
			$wb->wb_content = WBRetailProductAndService::getBlankXML();
		}
		else
		{
			$wb = WBRetailProductAndService::loadFromDatabase($link, $wb_id);
		}
		...
		if($_SESSION['user']->type == User::TYPE_LEARNER)
		{
			...
			for($i = 1; $i <= count(WBRetailProductAndService::getLearningJourneyItems()); $i++)
			{
				$key = 'DC'.$i;
			...
			foreach(WBRetailProductAndService::getDepartmentsList() AS $department)
			{
			...
			if(isset($_REQUEST['PromotionExample']))
			{
			...
			{
				$OwnBrand = $Answers->addChild('OwnBrand');
				$OwnBrand->addChild('PromotionExample', '');
				$Skin = $OwnBrand->addChild('Skin');
				$Skin->addChild('Branded', '');
				$Skin->addChild('OwnBrand', '');
				$Mens = $OwnBrand->addChild('Mens');
				$Mens->addChild('Branded', '');
				$Mens->addChild('OwnBrand', '');
				$Cosmetics = $OwnBrand->addChild('Cosmetics');
				$Cosmetics->addChild('Branded', '');
				$Cosmetics->addChild('OwnBrand', '');
			}

			$IdentifyingCustomerNeeds = $Answers->addChild('IdentifyingCustomerNeeds');
			for($i = 1; $i <= count(WBRetailProductAndService::getClosedOpenProbingQuestionsList()); $i++)
			{
				$key = 'Question'.$i;
			...
			$DealConflict->addChild('Question1', htmlspecialchars((string)$_REQUEST['DealConflictQ1']));
			$DealConflict->addChild('Question2', htmlspecialchars((string)$_REQUEST['DealConflictQ2']));
			$DealConflict->addChild('Question3', htmlspecialchars((string)$_REQUEST['DealConflictQ3']));
			$DealConflict->addChild('Question4', htmlspecialchars((string)$_REQUEST['DealConflictQ4']));
			$DealConflict->addChild('Question5', htmlspecialchars((string)$_REQUEST['DealConflictQ5']));
			$DealConflict->addChild('Question6', htmlspecialchars((string)$_REQUEST['DealConflictQ6']));

			$QualificationQuestions = $Answers->addChild('QualificationQuestions');
			$QualificationQuestions->addChild('Question1', htmlspecialchars((string)$_REQUEST['QualificationQuestions1']));
			$QualificationQuestions->addChild('Question2', htmlspecialchars((string)$_REQUEST['QualificationQuestions2']));
			$QualificationQuestions->addChild('Question3', htmlspecialchars((string)$_REQUEST['QualificationQuestions3']));
			$QualificationQuestions->addChild('Question4', htmlspecialchars((string)$_REQUEST['QualificationQuestions4']));
			$QualificationQuestions->addChild('Question5', htmlspecialchars((string)$_REQUEST['QualificationQuestions5']));
			$QualificationQuestions->addChild('Question6', htmlspecialchars((string)$_REQUEST['QualificationQuestions6']));
			$QualificationQuestions->addChild('Question7', htmlspecialchars((string)$_REQUEST['QualificationQuestions7']));

			$QQuestions = $Answers->addChild('QQuestions');
			$QQuestions->addChild('QQuestion1', htmlspecialchars((string)$_REQUEST['QQuestion1']));
			$QQuestions->addChild('QQuestion2', htmlspecialchars((string)$_REQUEST['QQuestion2']));
		...
		elseif($_SESSION['user']->type == User::TYPE_ASSESSOR)
		{
			...
			if(isset($_REQUEST['status_OwnBrand']))
			{
			...
			{
				$OwnBrand = $Feedback->addChild('OwnBrand');
				$OwnBrand->addChild('Status', 'A');
				$OwnBrand->addChild('Comments', '');
			}
			...
			$QQuestions = $Feedback->addChild('QQuestions');
			$QQuestions->addChild('Status', htmlspecialchars((string)$_REQUEST['status_QQuestions']));
			$QQuestions->addChild('Comments', htmlspecialchars((string)$_REQUEST['comments_QQuestions']));
			...
			if($wb->full_save == 'Y' && $_SESSION['user']->type == User::TYPE_LEARNER)
			{
				...
				if($wb->wb_status == Workbook::STATUS_LEARNER_COMPLETED)
				{
					...
					if(isset($tr_details->assessor))
					{
						...
						$notification->link = 'do.php?_action=wb_retail_product_and_service&id='.$wb->id.'&tr_id='.$wb->tr_id;
