<?php
class wb_sales_promotion_marchandising implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($id == '')
		{
			...
			$exists = DAO::getSingleValue($link, "SELECT id FROM workbooks WHERE tr_id = '{$tr_id}' AND wb_title = 'WBSalesPromotionMarchandising'");
			if($exists != "")
				$wb = WBSalesPromotionMarchandising::loadFromDatabase($link, $exists);
			else
			{
		...
		include_once('tpl_wb_sales_promotion_marchandising.php');
	}
}