...
class baltic_view_candidate_vacancies implements IUnauthenticatedAction
{
	public function execute(PDO $link)
	{
		...
		if ( $pc && $distance ) {
			$distance_radius = $distance * METRES_IN_A_MILE;
		...
		$type_dropdown = "SELECT description, description, null FROM lookup_vacancy_type WHERE id != 14 ORDER BY description asc;";
		$type_dropdown = DAO::getResultset($link, $type_dropdown);
		
		require_once('tpl_baltic_view_candidate_vacancies.php');
	}	
}
?>