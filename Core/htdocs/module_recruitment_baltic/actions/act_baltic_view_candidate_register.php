<?php
class baltic_view_candidate_register implements IUnauthenticatedAction

{
	public function execute( PDO $link ) {
	    ...
	    if ( SystemConfig::getEntityValue($link, 'recruitment_bespokes') )
	    {
		...
		else
		{
			...
			else
			{
				...
				//$candidatePhotoLink = $candidate->getPhotoLink($candidate_id, $candidate->firstnames . ' ' . $candidate->surname);
				$mode = isset($_REQUEST['mode'])?$_REQUEST['mode']:'';
				$cand_selected_interests = DAO::getSingleColumn($link, "SELECT sector FROM candidate_sector_choice WHERE candidate_id = " . $candidate_id);
			}
			...
			$cand_interests = DAO::getResultset($link, 'SELECT id, description FROM lookup_vacancy_type ORDER BY description');

			require_once('tpl_baltic_view_candidate_register.php');
		}
	}
	...
	private function present_bespoke_registration( PDO $link ) {
		...
		require_once('tpl_baltic_view_candidate_bespoke_register.php');
	}
