...
class baltic_fill_vacancy implements IAction {
		...
		if( ( $id != '')&&($cid != '' ) ) {
			...
			if ( ( $vacancy->no_of_vacancies > 0 )||( isset($_REQUEST['rmv']) ) ) {	
				...
				if ( isset($_REQUEST['rmv']) ) {
					...
					// approved candidate
					$candidate->status = 1;
					if(isset($_REQUEST['send_to_emp']) && $_REQUEST['send_to_emp'] == 1)
						$candidate->status_code = 18;
					else
						$candidate->status_code = 19;
					$candidate->has_been_screened = 1;
					$candidate->save($link);
					// set the incremental value of spaces available
					// $vacancy->update($link);
					if(isset($_REQUEST['send_to_emp']) && $_REQUEST['send_to_emp'] == 1)
						$vacancy->feedback['message'] = '<strong>'.$candidate->firstnames.' '.$candidate->surname.'</strong>\'s CV send to Employer';
					else
					...
					$candidate_note = new CandidateNotes();
					$candidate_note->candidate_id = $candidate->id;
					if($candidate->status_code == 18)
						$candidate_note->note = 'Candidate cv sent to employer '.$vacancy->code;
					else
