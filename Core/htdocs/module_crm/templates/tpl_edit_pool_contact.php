...
<head>
	...
	<title><?php echo $vo->contact_id == ''?'Create Contact':'Edit Contact'; ?></title>
...
<div class="row">
	<div class="col-lg-12">
		<div class="banner">
			<div class="Title" style="margin-left: 6px;"><?php echo $vo->contact_id == ''?'Create Contact':'Edit Contact'; ?></div>
			<div class="ButtonBar">
				...
				<?php if($vo->contact_id != "" && ($_SESSION['user']->isAdmin() || in_array($_SESSION['user']->type, [User::TYPE_ADMIN, User::TYPE_CRM_FRON_DESK_USER]) ) ) { ?>
...
<div class="col-sm-8">
	<div class="box box-primary">
		...
		<form autocomplete="off" class="form-horizontal" name="frmOrgCRMContact" id="frmOrgCRMContact" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
			...
			<input type="hidden" name="org_id" value="<?php echo $vo->pool_id; ?>" />
			...
			<div class="box-body with-border">
				...
				<div class="form-group">
					...
					<div class="col-sm-8">
						<input type="checkbox" name="decision_maker" id="decision_maker" value="<?php echo $vo->decision_maker; ?>" data-toggle="toggle" data-on="Yes" data-off="No" data-onstyle="success" data-offstyle="danger" <?php echo $vo->decision_maker == '1'?'checked="checked"':''; ?> />
					</div>
				</div>
				<div class="callout">
					<div class="form-group">
...
<div class="col-sm-4">
	<div class="callout callout-default">
		<?php
		echo '<span class="lead text-bold">' . $pool->legal_name . '</span>';
		$result = DAO::getResultset($link, "SELECT * FROM pool_contact WHERE pool_id = '{$pool->id}' AND contact_id != '{$vo->contact_id}'", DAO::FETCH_ASSOC);
		if(count($result) > 0)
		{
			...
			foreach($result AS $row)
			{
				...
				echo '<a href="do.php?_action=edit_pool_contact&contact_id='.$row['contact_id'].'&pool_id='.$row['pool_id'].'&push=0" class="product-title '.$text_color.'">' . $row['contact_title'] . ' ' . $row['contact_name'] . '</a>';
...
<script language="JavaScript">
	...
	function deleteNote()
	{
		...
		{
			window.location.replace('do.php?_action=edit_pool_contact&subaction=delete_learner_crm_contact&contact_id=<?php echo $vo->contact_id; ?>&pool_id=<?php echo $vo->pool_id; ?>');
		}
	}
