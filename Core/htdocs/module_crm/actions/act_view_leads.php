<?php
class view_leads implements IAction
{
    public function execute(PDO $link)
    {
        $subaction = isset($_REQUEST['subaction']) ? $_REQUEST['subaction'] : '';

        $view = VoltView::getViewFromSession('ViewLeads', 'ViewLeads'); /* @var $view VoltView */
        if (is_null($view)) {
            $view = $_SESSION['ViewLeads'] = $this->buildView($link);
        }
        $view->refresh($_REQUEST, $link);

        $_SESSION['bc']->index = 0;
        $_SESSION['bc']->add($link, "do.php?_action=view_leads", "View Leads");
        ...
        include_once('tpl_view_leads.php');
    }
...
SELECT
	crm_leads.*,
	CONCAT(users.firstnames, ' ', users.surname) AS lead_owner,
	...
	END AS company,
	CASE crm_leads.`company_type`
		WHEN 'pool' THEN (SELECT CONCAT(COALESCE(contact_title, ''), ' ',COALESCE(contact_name, ''), ', ',COALESCE(job_title, ''), ' ') FROM pool_contact WHERE contact_id = crm_leads.`main_contact_id`)
		WHEN 'pool' THEN (SELECT CONCAT(COALESCE(contact_title, ''), ' ',COALESCE(contact_name, ''), ', ',COALESCE(job_title, ''), ' ') FROM organisation_contact WHERE contact_id = crm_leads.`main_contact_id`)
		ELSE ''
	END AS company_contact,
	CASE crm_leads.`company_type`
		WHEN 'pool' THEN (SELECT company_rating FROM pool WHERE id = crm_leads.`company_id`)
		WHEN 'employer' THEN (SELECT company_rating FROM organisations WHERE id = crm_leads.`company_id`)
		ELSE ''
...
;
        ...
        $view = new VoltView('ViewLeads', $sql->__toString());

        $f = new VoltTextboxViewFilter('filter_id', "WHERE crm_leads.id = '%s%%'", null);
        $f->setDescriptionFormat("ID: %s");
        $view->addFilter($f);

        $f = new VoltTextboxViewFilter('filter_title', "WHERE crm_leads.lead_title LIKE '%%%s%%'", null);
        ...
        $options = "SELECT DISTINCT users.id, CONCAT(users.firstnames,' ',users.surname), null, CONCAT('WHERE crm_leads.created_by=',users.id) FROM users INNER JOIN crm_leads ON users.id = crm_leads.created_by ORDER BY users.firstnames";
        ...
        $options = array(
            0 => array('1', 'Open', null, 'WHERE crm_leads.status = "1"'),
            1 => array('2', 'In Progress', null, 'WHERE crm_leads.status = "2"'),
            2 => array('3', 'Won', null, 'WHERE crm_leads.status = "3"'),
            3 => array('4', 'Lost', null, 'WHERE crm_leads.status = "4"')
        );
        ...
        $options = [
            0 => [1, 'Lead ID (descending)', null, 'ORDER BY crm_leads.id DESC'],
            1 => [2, 'Lead ID (ascending)', null, 'ORDER BY crm_leads.id ASC'],
            2 => [3, 'Lead Title', null, 'ORDER BY crm_leads.lead_title ASC'],
        ];
    ...
    private function renderView(PDO $link, VoltView $view)
    {
        ...
        if ($st) {
            ...
            echo '<th>Lead ID/Ref.</th><th>Lead Owner</th><th>Title</th><th>Lead Status</th><th>Company</th>';
            echo '<th>Contact Person</th><th>Address</th><th>Estimated Closed Date</th><th style="width: 20%;">Description</th>';
            echo '</tr></thead>';
            echo '<tbody>';
            $trophies = [
            ...
            while ($row = $st->fetch(DAO::FETCH_ASSOC)) {
                ...
                echo '<td>' . $row['id'] . '</td>';
                echo '<td>' . $row['lead_owner'] . '</td>';
                echo '<td>' . $row['lead_title'] . '</td>';
                if ($row['status'] == 2)
                    echo '<td><span class="label label-primary">' . Lead::getListLeadStatus($row['status']) . '</span></td>';
                elseif ($row['status'] == 3)
                    echo '<td><span class="label label-success">' . Lead::getListLeadStatus($row['status']) . '</span></td>';
                else
                    echo '<td><span class="label label-info">' . Lead::getListLeadStatus($row['status']) . '</span></td>';
                echo '<td>';
                echo '<span class="text-bold text-blue">' . $row['company'] . '</span><br>';
                // $rating_changes = DAO::getSingleColumn($link, "SELECT notes.note FROM notes WHERE parent_id = '{$row['id']}' AND parent_table = 'crm_leads' AND notes.subject = 'Lead Updated' AND note LIKE '%[Rating]%' ORDER BY id;");
                // $_i = 1;
    ...
    private function export_csv(PDO $link, VoltView $view)
    {
        ...
        if ($st) {
            ...
            echo "Lead ID/Ref.,Lead Owner,Title,Lead Status,Company,Contact Person,Contact Person Tel.,Contact Person Mob.,Contact Person Email,Address,Postcode,Estimated Closed Date,Converted,Description";
            echo "\n";
            while ($row = $st->fetch(DAO::FETCH_ASSOC)) {
                echo $row['id'] . ",";
                echo HTML::csvSafe($row['lead_owner'] ?? '') . ",";
                echo HTML::csvSafe($row['lead_title'] ?? '') . ",";
                echo HTML::csvSafe(Lead::getListLeadStatus($row['status'] ?? '')) . ",";
