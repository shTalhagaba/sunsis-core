...
if (isset($_SESSION['user']) && !belongs_to_whitelist_actions_for_password_redirect($action)) {
	...
	if ($days_remaining_for_password_change <= 0) {
		@include 'act_change_password.php';
		$action = new change_password();
	}
}
...
{
	if (($_SERVER['REQUEST_METHOD'] == 'POST') && (stripos((string) @$_SERVER['CONTENT_TYPE'], 'charset=UTF-8') !== FALSE)) {
...
function belongs_to_whitelist_actions_for_password_redirect($action)
{
	$whitelisted = false;
	$whitlisted_actions = [
		'change_password',
		'left_hand_menu',
		'logout',
		'ajax_check_password_strength',
		'ajax_diceware',
	];
	foreach ($whitlisted_actions as $wclass) {
		if ($action instanceof $wclass) {
			$whitelisted = true;
		}
	}
