...
function checkDatesValidity()
{
	...
	for(subaims = 1; subaims < number_of_aims; subaims++)
	{
		...
		if(sf['LearnAimRef'] == 'ZPROG001' && sf['LearnStartDate_ZPROG001'] != 'undefined' && sf['LearnStartDate_ZPROG001'] != undefined)
		{
			...
			var fundinglastdate = new Date("07/31/2016");
			var fundingstartdate = new Date("08/01/2015");
			...
			if((sf['AimType'] == '1' || sf['AimType'] == '4' || sf['AimType'] == '5') && (ed < fundingstartdate))
			{
				...
				//return false;
                alert("Learn Aim Ref: " + sf['LearnAimRef'] + " end date " + sf['LearnActEndDate_'+LearningDeliveryAimReference] + " is before this contract year, should be entered into 2014-15 ILR.", "Error");
			}
			break;
		}
	}
	...
	for(subaims = 1; subaims <= number_of_aims; subaims++)
	{
		...
		if(sf != null)
		{
			if(sf['LearnAimRef']!='ToBeDeleted' && sf['LearnAimRef']!='')
			{
				...
				for(a = 1; a<=5; a++)
				{
					...
					if(typeof(eval(v1))!='undefined' && eval(v1) != '')
					{
						...
						if(typeof(sf['LSFFrom_'+LearningDeliveryAimReference+a]) !='undefined' && sf['LSFFrom_'+LearningDeliveryAimReference+a] != '' && sf['LSFFrom_'+LearningDeliveryAimReference+a]!='dd/mm/yyyy')
							xml +="<LearnDelFAMDateFrom>" + (sf['LSFFrom_'+LearningDeliveryAimReference+a]) + "</LearnDelFAMDateFrom>";
				...
				if(sf['AimType'] == '1' && sf['ProgType'] == '25')
				{
					for(a = 1; a<=10; a++)
					{
						v1 = sf['TBFinType'+a];
...
function validationOnSave()
{
	...
	var client = ajaxRequest('do.php?_action=validate_ilr2015', postData);
	if( client != null )
	{
