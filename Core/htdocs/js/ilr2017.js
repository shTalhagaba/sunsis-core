...
function checkDatesValidity()
{
    ...
    for(subaims = 1; subaims < number_of_aims; subaims++)
    {
        ...
        if(sf['LearnAimRef'] == 'ZPROG001' && sf['LearnStartDate_ZPROG001'] != 'undefined' && sf['LearnStartDate_ZPROG001'] != undefined)
        {
            ...
            var fundingstartdate = new Date("08/01/2017");
            var fundinglastdate = new Date("07/31/2018");
        ...
        for(subaims = 1; subaims < number_of_aims; subaims++)
        {
            ...
            if(sf != null)
            {
                if(sf['LearnAimRef']!='ToBeDeleted' && sf['LearnAimRef']!='')
                {
                    ...
                    {
                        //alert("Learn Aim Ref: " + sf['LearnAimRef'] + " end date " + sf['LearnActEndDate'] + " is after this contract year, should be entered into 2016-17 ILR.");
                        custom_alert_OK_only("Learn Aim Ref: " + sf['LearnAimRef'] + " end date " + sf['LearnActEndDate_'+LearningDeliveryAimReference] + " is after this contract year, should be entered into 2017-18 ILR.", "Error");
                        return false;
                    }
...
function validationOnSave()
{
    ...
    var client = ajaxRequest('do.php?_action=validate_ilr2017', postData);
    if( client != null )
    {
...
function validation() {
    ...
    var postData = 'id=' + document.getElementById('LearnRefNumber').value
        ...
        +'&tr_id=' + phpTrId;
    $.ajax({
        url:"do.php?_action=validate_ilr2017",
        type:"post",
        async:true,
        data:postData,
...
function changeL03() {
    ...
    if (newL03.length > 0) {
        ...
        var client = ajaxRequest('do.php?_action=save_ilr_2017', postData);
        if (client != null) {
