...
function EmployerId1_onchange(emp)
{
    EmployerId = emp.options[emp.selectedIndex].value;
    query = "SELECT edrs FROM organisations WHERE id=";
    var request = ajaxRequest("do.php?_action=ajax_get_value&id=" + EmployerId + "&query=" + htmlspecialchars(query));
    edrs = request.responseText;
    document.ilr.EmpId1.value = edrs;
    document.ilr.EmpStat1.value = 10;
    setDelLocPostCode(EmployerId)
}
function EmployerId2_onchange(emp)
{
    EmployerId = emp.options[emp.selectedIndex].value;
    query = "SELECT edrs FROM organisations WHERE id=";
    var request = ajaxRequest("do.php?_action=ajax_get_value&id=" + EmployerId + "&query=" + htmlspecialchars(query));
    edrs = request.responseText;
    document.ilr.EmpId2.value = edrs;
    document.ilr.EmpStat2.value = 10;
    setDelLocPostCode(EmployerId)
}
function EmployerId3_onchange(emp)
{
    EmployerId = emp.options[emp.selectedIndex].value;
    query = "SELECT edrs FROM organisations WHERE id=";
    var request = ajaxRequest("do.php?_action=ajax_get_value&id=" + EmployerId + "&query=" + htmlspecialchars(query));
    edrs = request.responseText;
    document.ilr.EmpId3.value = edrs;
    setDelLocPostCode(EmployerId)
}
function EmployerId4_onchange(emp)
{
    EmployerId = emp.options[emp.selectedIndex].value;
    query = "SELECT edrs FROM organisations WHERE id=";
    var request = ajaxRequest("do.php?_action=ajax_get_value&id=" + EmployerId + "&query=" + htmlspecialchars(query));
    edrs = request.responseText;
    document.ilr.EmpId4.value = edrs;
    setDelLocPostCode(EmployerId)
}
function EmployerId5_onchange(emp)
{
    EmployerId = emp.options[emp.selectedIndex].value;
    query = "SELECT edrs FROM organisations WHERE id=";
    var request = ajaxRequest("do.php?_action=ajax_get_value&id=" + EmployerId + "&query=" + htmlspecialchars(query));
    edrs = request.responseText;
    document.ilr.EmpId5.value = edrs;
    setDelLocPostCode(EmployerId)
}

function setDelLocPostCode(EmployerId)
{
    query = "SELECT postcode FROM locations WHERE organisations_id=";
    var request = ajaxRequest("do.php?_action=ajax_get_value&id=" + EmployerId + "&additional=limit 1" + "&query=" + htmlspecialchars(query));
    postcode = request.responseText;

    var textBoxes = document.getElementsByTagName("input");
    for (var i = 0; i < textBoxes.length; i++) 
    {
        if (textBoxes[i].name === "DelLocPostCode") 
        {
            textBoxes[i].value = postcode;
        }
    }
}
...
function save_and_stay()
{
    ...
    function saveAndStayCallback()
    {
        $("#progress").hide();
    }

    var client = ajaxRequest('do.php?_action=save_ilr_2023', postData, null, saveAndStayCallback);
}
...
function checkDatesValidity()
{
    ...
    for(subaims = 1; subaims < number_of_aims; subaims++)
    {
        ...
        if(sf['LearnAimRef'] == 'ZPROG001' && sf['LearnStartDate_ZPROG001'] != 'undefined' && sf['LearnStartDate_ZPROG001'] != undefined)
        {
            ...
            var fundingstartdate = new Date("08/01/2023");
            var fundinglastdate = new Date("07/31/2024");
            console.log(sd + ', ' + ed);
            var cd = new Date();
            ...
            if((sf['AimType'] == '1' || sf['AimType'] == '4' || sf['AimType'] == '5') && (ed > fundinglastdate))
            {
               ...
               // custom_alert_OK_only("Learn Aim Ref: " + sf['LearnAimRef'] + " end date " + sf['LearnActEndDate_'+LearningDeliveryAimReference] + " is after this contract year, should be entered into 2023-24 ILR.", "Error");
               // return false;
            }
            ...
            if((sf['AimType'] == '1' || sf['AimType'] == '4' || sf['AimType'] == '5') && (ed < fundingstartdate))
            {
                ...
                //return false;
              //  alert("Learn Aim Ref: " + sf['LearnAimRef'] + " end date " + sf['LearnActEndDate_'+LearningDeliveryAimReference] + " is before this contract year, should be entered into 2020-21 ILR.", "Error");
            }
            break;
        }
    }
        ...
        for(subaims = 1; subaims < number_of_aims; subaims++)
        {
            ...
            if(sf != null)
            {
                if(sf['LearnAimRef']!='ToBeDeleted' && sf['LearnAimRef']!='')
                {
                    ...
                    var cd = new Date();
                    console.log(sd + ', ' + ed + ', ' + fundinglastdate);
                    if((sf['AimType'] == '1' || sf['AimType'] == '4' || sf['AimType'] == '5') && (ed > fundinglastdate))
                    {
                        //alert("Learn Aim Ref: " + sf['LearnAimRef'] + " end date " + sf['LearnActEndDate'] + " is after this contract year, should be entered into 2016-17 ILR.");
                        //custom_alert_OK_only("Learn Aim Ref: " + sf['LearnAimRef'] + " end date " + sf['LearnActEndDate_'+LearningDeliveryAimReference] + " is after this contract year, should be entered into 2021-22 ILR.", "Error");
                        return false;
                    }
                    ...
                    {
                        //alert("Learn Aim Ref: " + sf['LearnAimRef'] + " end date " + sf['LearnActEndDate'] + " is before this contract year, should be entered into 2014-15 ILR.");
                        //custom_alert_OK_only("Learn Aim Ref: " + sf['LearnAimRef'] + " end date " + sf['LearnActEndDate_'+LearningDeliveryAimReference] + " is before this contract year, should be entered into 2020-21 ILR.", "Error");
                        return false;
                    }
                }
            }
...
function toXML()
{
    ...
    if(document.ilr.PMUKPRN.value.trim() !='')
        xml += "<PMUKPRN>" + document.ilr.PMUKPRN.value.trim() + "</PMUKPRN>";
    if(document.ilr.CampId.value.trim() !='')
        xml += "<CampId>" + document.ilr.CampId.value.trim() + "</CampId>";
    ...
    for(a = 1; a<=10; a++)
    {
        p1 = "document.ilr.Prior_" + a;
        p2 = "document.ilr.Prior_" + a + ".options[document.ilr.Prior_" + a + ".selectedIndex].value.trim()";
        if(typeof(eval(p1))!='undefined' && eval(p2)!='')
        {
            xml+="<PriorAttain><PriorLevel>" + eval(p2) + "</PriorLevel>";

            v3 = "document.ilr.PriorDate_" + a + ".value.trim()";
            if(eval(v3)!='')
                xml+="<DateLevelApp>"+ formatDate(eval(v3)) +"</DateLevelApp>";
            xml+="</PriorAttain>";
        }
    }
    ...
    for(a = 1; a<=5; a++)
    {
        ...
        if(typeof(eval(v1))!='undefined' && eval(v2)!='')
        {
            ...
            EmployerId = "document.ilr.EmployerId" + a + ".options[document.ilr.EmployerId" + a + ".selectedIndex].value.trim()";
            if(eval(EmployerId)!='')
                xml+="<EmployerId>"+ eval(EmployerId) +"</EmployerId>";
            ...
            oet = "document.ilr.OET" + a + ".options[document.ilr.OET" + a + ".selectedIndex].value.trim()";
            if(eval(oet)!='')
                xml+="<EmploymentStatusMonitoring><ESMType>OET</ESMType><ESMCode>" + eval(oet) + "</ESMCode></EmploymentStatusMonitoring>";
    ...
    for(subaims = 1; subaims <= number_of_aims; subaims++)
    {
        ...
        if(sf != null)
        {
            if(sf['LearnAimRef']!='ToBeDeleted' && sf['LearnAimRef']!='')
            {
                ...
                if(sf['Exclude'])
                    xml +="<Exclude>1</Exclude>";
                if(typeof(sf['OverallTNP']) !='undefined' && sf['OverallTNP'] != '')
                    xml += "<OverallTNP>" + sf['OverallTNP'] + "</OverallTNP>";
                ...
                if(typeof(sf['LearnStartDate_'+LearningDeliveryAimReference+subaims]) !='undefined' && sf['LearnStartDate_'+LearningDeliveryAimReference+subaims] != '' && sf['LearnStartDate_'+LearningDeliveryAimReference+subaims]!='dd/mm/yyyy')
                    xml += "<LearnStartDate>" + formatDate(sf['LearnStartDate_'+LearningDeliveryAimReference+subaims]) + "</LearnStartDate>";
                ...
                if(typeof(sf['OrigLearnStartDate_'+LearningDeliveryAimReference+subaims]) !='undefined' && sf['OrigLearnStartDate_'+LearningDeliveryAimReference+subaims] != '' && sf['OrigLearnStartDate_'+LearningDeliveryAimReference+subaims]!='dd/mm/yyyy')
                    xml += "<OrigLearnStartDate>" + formatDate(sf['OrigLearnStartDate_'+LearningDeliveryAimReference+subaims]) + "</OrigLearnStartDate>";
                ...
                if(typeof(sf['LearnPlanEndDate_'+LearningDeliveryAimReference+subaims]) !='undefined' && sf['LearnPlanEndDate_'+LearningDeliveryAimReference+subaims] != '' && sf['LearnPlanEndDate_'+LearningDeliveryAimReference+subaims]!='dd/mm/yyyy')
                    xml += "<LearnPlanEndDate>" + formatDate(sf['LearnPlanEndDate_'+LearningDeliveryAimReference+subaims]) + "</LearnPlanEndDate>";
                ...
                if(typeof(sf['EPAOrgID']) !='undefined' && sf['EPAOrgID'] != '')
                    xml += "<EPAOrgID>" + sf['EPAOrgID'] + "</EPAOrgID>";
                if(typeof(sf['PHours']) !='undefined' && sf['PHours'] != '')
                    xml += "<PHours>" + sf['PHours'] + "</PHours>";
                if(typeof(sf['OTJActHours']) !='undefined' && sf['OTJActHours'] != '')
                    xml += "<OTJActHours>" + sf['OTJActHours'] + "</OTJActHours>";
                if(typeof(sf['LSDPostcode']) !='undefined' && sf['LSDPostcode'] != '')
                    xml += "<LSDPostcode>" + sf['LSDPostcode'] + "</LSDPostcode>";
                ...
                if(typeof(sf['LearnActEndDate_'+LearningDeliveryAimReference+subaims]) !='undefined' && sf['LearnActEndDate_'+LearningDeliveryAimReference+subaims]!='' && sf['LearnActEndDate_'+LearningDeliveryAimReference+subaims]!='dd/mm/yyyy')
                    xml +="<LearnActEndDate>"+ formatDate(sf['LearnActEndDate_'+LearningDeliveryAimReference+subaims])  +"</LearnActEndDate>";
                ...
                if(typeof(sf['AchDate_'+LearningDeliveryAimReference+subaims]) !='undefined' && sf['AchDate_'+LearningDeliveryAimReference+subaims]!='' && sf['AchDate_'+LearningDeliveryAimReference+subaims]!='dd/mm/yyyy')
                    xml +="<AchDate>"+ formatDate(sf['AchDate_'+LearningDeliveryAimReference+subaims])  +"</AchDate>";
                ...
                if(typeof(sf['DAM1']) !='undefined' && sf['DAM1'] != '')
                    xml +="<LearningDeliveryFAM><LearnDelFAMType>DAM</LearnDelFAMType><LearnDelFAMCode>" + sf['DAM1'] + "</LearnDelFAMCode></LearningDeliveryFAM>";
                if(typeof(sf['DAM2']) !='undefined' && sf['DAM2'] != '')
                    xml +="<LearningDeliveryFAM><LearnDelFAMType>DAM</LearnDelFAMType><LearnDelFAMCode>" + sf['DAM2'] + "</LearnDelFAMCode></LearningDeliveryFAM>";
                if(typeof(sf['DAM3']) !='undefined' && sf['DAM3'] != '')
                    xml +="<LearningDeliveryFAM><LearnDelFAMType>DAM</LearnDelFAMType><LearnDelFAMCode>" + sf['DAM3'] + "</LearnDelFAMCode></LearningDeliveryFAM>";
                if(typeof(sf['DAM4']) !='undefined' && sf['DAM4'] != '')
                    xml +="<LearningDeliveryFAM><LearnDelFAMType>DAM</LearnDelFAMType><LearnDelFAMCode>" + sf['DAM4'] + "</LearnDelFAMCode></LearningDeliveryFAM>";
                if(sf['RES'])
                ...
                for(a = 1; a<=10; a++)
                {
                    ...
                    if(typeof(v1)!='undefined' && v1 != '')
                    {
                        ...
                        if(typeof(sf['WorkPlaceHours_'+LearningDeliveryAimReference+a]) !='undefined' && sf['WorkPlaceHours_'+LearningDeliveryAimReference+a] != '')
                            xml +="<WorkPlaceHours>" + sf['WorkPlaceHours_'+LearningDeliveryAimReference+a] + "</WorkPlaceHours>";
                        xml +="</LearningDeliveryWorkPlacement>";
                    }
                }
...
function validationOnSave()
{
    ...
    var client = ajaxRequest('do.php?_action=validate_ilr2023', postData);
    if( client != null )
    {
...
function validation() {
    ...
    var postData = 'id=' + document.getElementById('LearnRefNumber').value
        ...
        +'&tr_id=' + phpTrId;
    $.ajax({
        url:"do.php?_action=validate_ilr2023",
        type:"post",
        async:true,
        data:postData,
...
function changeL03() {
    ...
    if (newL03.length > 0) {
        ...
        var client = ajaxRequest('do.php?_action=save_ilr_2023', postData);
        if (client != null) {
