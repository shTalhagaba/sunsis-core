...
function checkDatesValidity()
{
    ...
    for(subaims = 1; subaims < number_of_aims; subaims++)
    {
        ...
        if(sf['LearnAimRef'] == 'ZPROG001' && sf['LearnStartDate_ZPROG001'] != 'undefined' && sf['LearnStartDate_ZPROG001'] != undefined)
        {
            ...
            var fundingstartdate = new Date("08/01/2019");
            var fundinglastdate = new Date("07/31/2020");
            console.log(sd + ', ' + ed);
            var cd = new Date();
            ...
            if((sf['AimType'] == '1' || sf['AimType'] == '4' || sf['AimType'] == '5') && (ed > fundinglastdate))
            {
                ...
                custom_alert_OK_only("Learn Aim Ref: " + sf['LearnAimRef'] + " end date " + sf['LearnActEndDate_'+LearningDeliveryAimReference] + " is after this contract year, should be entered into 2019-20 ILR.", "Error");
                return false;
            }
            ...
            if((sf['AimType'] == '1' || sf['AimType'] == '4' || sf['AimType'] == '5') && (ed < fundingstartdate))
            {
                ...
                //return false;
                alert("Learn Aim Ref: " + sf['LearnAimRef'] + " end date " + sf['LearnActEndDate_'+LearningDeliveryAimReference] + " is before this contract year, should be entered into 2017-18 ILR.", "Error");
            }
            break;
        }
    }
        ...
        for(subaims = 1; subaims < number_of_aims; subaims++)
        {
            ...
            if(sf != null)
            {
                if(sf['LearnAimRef']!='ToBeDeleted' && sf['LearnAimRef']!='')
                {
                    ...
                    {
                        //alert("Learn Aim Ref: " + sf['LearnAimRef'] + " end date " + sf['LearnActEndDate'] + " is before this contract year, should be entered into 2014-15 ILR.");
                        custom_alert_OK_only("Learn Aim Ref: " + sf['LearnAimRef'] + " end date " + sf['LearnActEndDate_'+LearningDeliveryAimReference] + " is before this contract year, should be entered into 2017-18 ILR.", "Error");
                        return false;
                    }
                }
            }
...
function toXML()
{
    ...
    for(a = 1; a<=5; a++)
    {
        ...
        if(typeof(eval(v1))!='undefined' && eval(v2)!='')
        {
            ...
            v5 = "document.ilr.AgreeId" + a + ".value.trim()";
            if(eval(v5)!='')
                xml+="<AgreeId>"+ eval(v5) +"</AgreeId>";
...
function validationOnSave()
{
    ...
    var client = ajaxRequest('do.php?_action=validate_ilr2019', postData);
    if( client != null )
    {
...
function validation() {
    ...
    var postData = 'id=' + document.getElementById('LearnRefNumber').value
        ...
        +'&tr_id=' + phpTrId;
    $.ajax({
        url:"do.php?_action=validate_ilr2019",
        type:"post",
        async:true,
        data:postData,
...
function changeL03() {
    ...
    if (newL03.length > 0) {
        ...
        var client = ajaxRequest('do.php?_action=save_ilr_2019', postData);
        if (client != null) {
