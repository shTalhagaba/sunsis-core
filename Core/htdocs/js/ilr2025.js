...
function save_and_stay()
{
    ...
    var client = ajaxRequest('do.php?_action=save_ilr_2025', postData, null, saveAndStayCallback);
}
...
function checkDatesValidity()
{
    ...
    for(subaims = 1; subaims < number_of_aims; subaims++)
    {
        ...
        if(sf['LearnAimRef'] == 'ZPROG001' && sf['LearnStartDate_ZPROG001'] != 'undefined' && sf['LearnStartDate_ZPROG001'] != undefined)
        {
            ...
            var fundingstartdate = new Date("08/01/2025");
            var fundinglastdate = new Date("07/31/2026");
            console.log(sd + ', ' + ed);
            var cd = new Date();
                    ...
                    for(a = 1; a<=20; a++)
                    {
                        ...
                        if(typeof(v1)!='undefined' && v1 != '')
                        {
                            ...
                            if(typeof(sf['TBFinAmount'+a]) !='undefined' && sf['TBFinAmount'+a] != '')
                                xml +="<TBFinAmount>" + sf['TBFinAmount'+a].replace(/\D/g, '') + "</TBFinAmount>";
                            xml +="</TrailblazerApprenticeshipFinancialRecord>";
                        }
                    }
                }
...
function validationOnSave()
{
    ...
    var client = ajaxRequest('do.php?_action=validate_ilr2025', postData);
    if( client != null )
    {
...
function validation() {
    ...
    var postData = 'id=' + document.getElementById('LearnRefNumber').value
        ...
        +'&tr_id=' + phpTrId;
    $.ajax({
        url:"do.php?_action=validate_ilr2025",
        type:"post",
        async:true,
        data:postData,
...
function changeL03() {
    ...
    if (newL03.length > 0) {
        ...
        var client = ajaxRequest('do.php?_action=save_ilr_2025', postData);
        if (client != null) {
