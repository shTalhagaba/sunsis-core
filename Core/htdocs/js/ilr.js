		...
		if(sf != null)
		{
			if(sf['LearnAimRef']!='ToBeDeleted' && sf['LearnAimRef']!='')
			{
				...
				/*
				 ...
				 dBits =  sf['AchDate'].split("/");
				 ad = new Date(dBits[2],(dBits[1]-1),dBits[0]);

				 cd = new Date();
				 if(sd>cd || ed>cd || ad>cd)
				 {
				 return false;
				 }
				 */
				...
				if(sd>cd)
				{
					alert("Learn Aim Ref: "+sf['LearnAimRef']+" future start date "+sf['LearnStartDate']+" entered.");
					return false;
				}
				else if(ed>cd)
				{
					alert("Learn Aim Ref: "+sf['LearnAimRef']+" future end date "+sf['LearnActEndDate']+" entered.");
					return false;
				}
				else if(ad>cd)
				{
					alert("Learn Aim Ref: "+sf['LearnAimRef']+" future achievement date "+sf['AchDate']+" entered.");
					return false;
				}
		...
		if(sf != null)
		{
			if(sf['LearnAimRef']!='ToBeDeleted' && sf['LearnAimRef']!='')
			{
				if(sf['LearnActEndDate']=='' || sf['LearnActEndDate']=='dd/mm/yyyy')
					migrate = 1;
				else if(sf['Outcome']=='4' || sf['Outcome']=='5')
					migrate = 1;
			}
		}
	}
	...
	for(subaims = 2; subaims <= number_of_aims; subaims++)
	{
		...
		if(sf != null)
		{
			if(sf['LearnAimRef']!='ToBeDeleted' && sf['LearnAimRef']!='')
			{
				...
				fundinglastdate = new Date("07/31/2014");

				fundingstartdate = new Date("08/01/2013");
				...
				if(sf['LearnAimRef']=='ZPROG001' && ed>fundinglastdate){
					alert("Learn Aim Ref: "+sf['LearnAimRef']+" end date "+sf['LearnActEndDate']+" is after this contract year, should be entered into 2014-15 ILR.");
					return false;
				}

				if(sf['LearnAimRef']=='ZPROG001' && ed<fundingstartdate)
				{
					alert("Learn Aim Ref: "+sf['LearnAimRef']+" end date "+sf['LearnActEndDate']+" is before this contract year, should be entered into 2012-13 ILR.");
					return false;
				}
				//else if(ad>cd)
				//{
