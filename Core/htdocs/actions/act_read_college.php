<?php
class read_college implements IAction
{
	public function execute(PDO $link)
	{
		...
		$_SESSION['bc']->add($link, "do.php?_action=read_college&id=" . $id, "View College");
                ...
                if(!is_dir($full))
                {
                    if ($file != "." && $file != ".." && $file!='admin')
                    {
                        ...
                        else
                        {
                            ...
                            $href2 = "do.php?_action=delete_file&path=employers/" . $emp_id . '/' . "&f=" . rawurlencode($file) . "&redirect=" . rawurlencode('do.php?_action=read_college&id='.$emp_id);;
...
*/
        // Presentation
		include('tpl_read_college.php');
	}


	private function renderLocations(PDO $link, Colleg $vo)
	{
	...
	private function renderPersonnel(PDO $link, College $vo)
	{
	...
	private function renderLearners(PDO $link, College $vo)
	{
		...
		if(count($personnel) > 0)
		{
			...
			foreach($personnel as $per)
			{
				echo HTML::viewrow_opening_tag('do.php?_action=read_training_record&id=' . $per->id);
				echo '<td><img src="/images/blue-person.png" border="0" /></td>';
				echo '<td>' . HTML::cell($per->surname) . '</td>';
				echo '<td>' . HTML::cell($per->firstnames) . '</td>';
				echo '<td>' . HTML::cell($per->work_telephone) . '</td>';
				echo '</tr>';
