<?php
class view_evidence_matrix_submissions implements IAction
{
    public function execute(PDO $link)
    {
        $view = ViewEvidenceMatrixSubmissions::getInstance($link);
        $subaction = isset($_REQUEST['subaction']) ? $_REQUEST['subaction'] : '';

        $view->refresh($link, $_REQUEST);

        $_SESSION['bc']->index=0;
        $_SESSION['bc']->add($link, "do.php?_action=view_evidence_matrix_submissions", "View Evidence Matrix Submissions Report");
        ...
        require_once('tpl_view_evidence_matrix_submissions.php');
    }
                ...
                foreach($columns AS $column)
                {
                    ...
                    elseif($column=='status')
                    {
                        ...
                        elseif($row['expired']=='1' and $row['submission_date']=='')
                            $status = "Overdue";
                        elseif($row['set_date']!='' and $row['expired']=='0')
                            $status = "In progress";
                        else
                            $status = "";
                        echo $status . ",";
                    ...
                    elseif($column=='months_on_project')
                    {
