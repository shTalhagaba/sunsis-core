<?php
class save_ilr_2019 implements IAction
{
        ...
        if($template!=1)
        {
            ...
            if(!empty($xresult))
            {
                foreach($xresult AS $key => $node)
                {
                    ...
                    {
                        $s = "update student_qualifications set start_date = '$a27', end_date = '$a28', actual_end_date = $a31, achievement_date = $a40, compstatus = '$compstatus',outcome='$outcome' where REPLACE(id,'/','') = '$a09' and tr_id = $tr_id;";
                        DAO::execute($link, $s);
                    }
                    else
                    {
            ...
            $previous_ilr = Ilr2019::loadFromXML($previous_ilr_xml);

//if(SOURCE_BLYTHE_VALLEY && DB_NAME == "am_demo") throw new Exception($xml_escaped);
            ...
            // Update tr status
            $vo = Ilr2019::loadFromXML($xml);
            $funding_type = Ilr2019::FundingType($vo);
        ...
        // Migrate forward if required
        DAO::execute($link, "INSERT IGNORE INTO ilr SELECT L01,L03,A09,ilr,\"W01\",contract_type,tr_id,is_complete,is_valid,is_approved,is_active,(SELECT id FROM contracts WHERE parent_id = contract_id) FROM ilr WHERE LOCATE(\"<CompStatus>1</CompStatus>\",ilr)>0 AND submission = \"W13\" AND contract_id IN (SELECT id FROM contracts WHERE contract_year = 2019) AND ilr.tr_id = '$tr_id'");
        DAO::execute($link, "UPDATE tr SET contract_id = (SELECT MAX(contract_id) FROM ilr WHERE ilr.tr_id = tr.id) WHERE contract_id IN (SELECT id FROM contracts WHERE contract_year = 2019)");

        //Demigrate if required
        $shouldMigrate=DAO::getSingleValue($link, "SELECT (SELECT COUNT(*) FROM ilr WHERE LOCATE(\"<CompStatus>1</CompStatus>\",ilr)>0 AND submission = \"W13\" AND contract_id IN (SELECT id FROM contracts WHERE contract_year = 2019) AND ilr.tr_id = '$tr_id')+
(SELECT COUNT(*) FROM ilr
INNER JOIN tr ON ilr.`tr_id` = tr.id
LEFT JOIN tr AS t2 ON t2.`l03` = ilr.`L03` AND t2.id != tr.id AND tr.`start_date` < t2.`start_date`
WHERE LOCATE(\"<CompStatus>6</CompStatus>\",ilr)>0 AND submission = \"W13\" AND ilr.contract_id IN (SELECT id FROM contracts WHERE contract_year = 2019)
AND t2.`start_date` IS NULL AND ilr.tr_id = '$tr_id')+
(SELECT COUNT(*) FROM ilr WHERE LOCATE(\"<Outcome>8</Outcome>\",ilr)>0 AND submission = \"W13\" AND contract_id IN (SELECT id FROM contracts WHERE contract_year = 2019) AND ilr.tr_id = '$tr_id') AS m;");
        if($shouldMigrate==0)
        {
            DAO::execute($link, "delete from ilr where tr_id = '$tr_id' and (select start_date from tr where tr.id = ilr.tr_id) < '2020-08-01' and contract_id in (select id from contracts where contract_year = 2020)");
            DAO::execute($link, "UPDATE tr SET contract_id = (SELECT MAX(contract_id) FROM ilr WHERE ilr.tr_id = tr.id) WHERE contract_id IN (SELECT id FROM contracts WHERE contract_year = 2020)");
        }
