<?php
class save_ilr_2022 implements IAction
{
    ...
    private function process(PDO $link)
    {
        ...
        if($template!=1)
        {
            ...
            if(DB_NAME!='am_lead' and DB_NAME!='am_ela')
            {
            ...
            if($shoulddemigrate)
            {
                DAO::execute($link, "delete from ilr where tr_id = '$tr_id' and contract_id in (select id from contracts where contract_year = 2023)");
            ...
            {
                $c = DAO::getSingleValue($link, "select count(*) from ilr where tr_id='$tr_id' and contract_id in (select id from contracts where contract_year = 2023)");
                if($c==0)
                {
                    DAO::execute($link, "INSERT INTO ilr SELECT L01,L03,A09,ilr,'W01',contract_type,tr_id,is_complete,is_valid,is_approved,is_active,(SELECT id FROM contracts WHERE parent_id = contract_id) FROM ilr WHERE contract_id in (select id from contracts where contract_year = 2022) and submission = 'W13' AND tr_id = '$tr_id'");
                    DAO::execute($link, "UPDATE tr SET contract_id = (SELECT contract_id FROM ilr WHERE ilr.tr_id = tr.id ORDER BY contract_id DESC LIMIT 0,1) where tr.id = '$tr_id';");
                }
            }
            ...
            // Update tr status
            $vo = Ilr2022::loadFromXML($xml);
            $funding_type = Ilr2022::FundingType($vo);
            ...
            if(DB_NAME=="am_lead")
            {
                ...
                DAO::execute($link, "update tr set ni='$ni', target_date = '$target_date', start_date='$start_date',home_email = '$home_email', l42a = '$l42a', l42b = '$l42b', uln = '$uln', ilr_status = '$ilr_status', status_code = '$status_code', outcome='$outcome', closure_date = $closure_date, ethnicity = '$ethnicity' where id = '$tr_id'");
            }
            else
            {
