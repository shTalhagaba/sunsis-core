<?php
class read_and_display_xml implements IAction
{
	public $i;
	public $size;
	public $p;
	...
	public function execute(PDO $link)
	{
        ...
        $file_path = DATA_ROOT . '/uploads/' . DB_NAME . '/CollegeTools/' . $filename;
        ...
        else
        {
            ...
            {
                $zip->extractTo(DATA_ROOT . '/uploads/' . DB_NAME . '/CollegeTools/');
                $zip->close();
                unlink($file_path);
                $filename = str_replace(".zip",".xml",$filename);
                // Get the filename of latest uploaded file
                $dir = DATA_ROOT . '/uploads/' . DB_NAME . '/CollegeTools/';
                $lastMod = 0;
                $lastModFile = '';
                ...
                $content = file_get_contents(DATA_ROOT . '/uploads/' . DB_NAME . '/CollegeTools/' . $lastModFile);
            }
        }
        ...
        {
            //$p1->setProgressBarProgress($i * 100 / $size, 'Reading XML File (Learner ' . ($i+1) . '/' . $size . ')');
            $p1->setProgressBarProgress($i * 100 / $size, 'Checking for qualifications to be downloaded');
            $index++;
            ...
            if($index>=1)
            {
                ...
                foreach($ilr->LearningDelivery as $delivery)
                {
                    ...
                    if(DB_NAME=="am_lewisham")
                        if(("".$delivery->AimType)=='4' && ("".$delivery->FundModel=='35' || "".$delivery->FundModel=='99'))
...
//        DAO::execute($link, "Update tr set ilr_status = 1");
        ...
        if(sizeof($qualificationsDownload)>0)
            pre("The following qualifications need to be downloaded \n" . implode(",",$qualificationsDownload));
        ...
        {
            $i = 0;
            $size = count($Apps_Frameworks);
            foreach($Apps_Frameworks as $framework)
            {
                $p1->setProgressBarProgress($i * 100 / $size, 'Creating/ Updating Frameworks');
            ...
            foreach($Apps_Frameworks as $framework)
            {
                $p1->setProgressBarProgress($i * 100 / $size, 'Creating/ Updating Courses');
                $app = explode(",", $framework);
                $framework_code = $app[0];
                $prog_type = $app[1];
                $aim_ref = $app[2];
        ...
        foreach($learners as $learner)
        {
            $p1->setProgressBarProgress($i * 100 / $size, 'Creating/ Updating Employers, Learners, Training Records');
            $index++;
            $tr_id++;
            $learner = str_replace("'","\'",$learner);
            $learner = str_replace(array("\r\n", "\r"), "\n", $learner);
            ...
            if($index>=1)
            {
                ...
                foreach($ilr->LearningDelivery as $delivery)
                {
                    ...
                    if($ptype=="Apps")
                        if(("".$delivery->AimType)=='1' && ("".$delivery->FundModel)=='35')
                ...
                elseif(sizeof($Apprenticeships)>1)
                {
                    foreach($Apprenticeships as $frameworks)
                    {
                        ...
                        foreach($LearningDelivery as $ld)
                        {
                            ...
                            $LearnStartDate = $ld->getElementsByTagName('LearnStartDate')->item(0)->nodeValue;
                            $LearnPlanEndDate = $ld->getElementsByTagName('LearnPlanEndDate')->item(0)->nodeValue;
                            $FStartDate = new Date(Date::toShort($framework[2]));
                            $FPlanEndDate = new Date(Date::toShort($framework[3]));
                            if($FworkCode!=$framework[0] || $ProgType!=$framework[1] || $FStartDate->after($LearnStartDate) || $FPlanEndDate->before($LearnPlanEndDate))
                            {
                                $nodesToRemove[] = $ld;
                            }
                            else
                ...
                else
                {
                    ...
                    {
                        if($ld->AimType=='4' && $ld->LearnAimRef!='XESF0001' && $ld->LearnAimRef!='ZESF0001' && ($ld->FundModel=='35' || $ld->FundModel=='99'))
                        {
                            if(DB_NAME=="am_lewisham")
                            {
...
/*
        ...
        $size = 9;
        $i++;
        $p1->setProgressBarProgress($i * 100 / $size, 'Final Checks');
        ...
        $i++;
        $p1->setProgressBarProgress($i * 100 / $size, 'Final Checks');
        DAO::execute($link, "UPDATE tr LEFT JOIN ilr ON ilr.tr_id = tr.id AND submission = '$submission' LEFT JOIN organisations ON organisations.`ukprn` = LEFT(extractvalue(ilr,'/Learner/LearningDelivery/PartnerUKPRN'),8) SET provider_id = organisations.id WHERE organisations.id IS NOT NULL;");
        $i++;
        $p1->setProgressBarProgress($i * 100 / $size, 'Final Checks');
        DAO::execute($link, "UPDATE tr LEFT JOIN ilr ON ilr.tr_id = tr.id AND submission = '$submission' LEFT JOIN organisations ON organisations.`ukprn` = LEFT(extractvalue(ilr,'/ilr/main/A22|ilr/subaim/A22'),8) SET provider_id = organisations.id WHERE organisations.id IS NOT NULL;");
        $i++;
        $p1->setProgressBarProgress($i * 100 / $size, 'Final Checks');
...
*/
	...
	function lockTables(PDO $link, $tables, $lock)
	{
		$sql = "LOCK TABLES ";
		foreach ($tables as $t) {
			$sql .= $t . " " . $lock . ", ";
    ...
    public function createTrainingRecordAPP($link, $learner, $tr_id, $course_id, $type, $contract_id, $submission, $provider_id, $client = '')
    {
        ...
        $xpath = $ilr->xpath("/Learner/LearningDelivery[AimType=1]/ProviderSpecDeliveryMonitoring[ProvSpecDelMonOccur='C']/ProvSpecDelMon");
        ...
        $tr_id = DAO::getSingleValue($link, "SELECT tr.id FROM tr INNER JOIN courses_tr ON courses_tr.`tr_id` = tr.id INNER JOIN courses ON courses.id = courses_tr.`course_id` INNER JOIN frameworks ON frameworks.id = courses.`framework_id` AND frameworks.`framework_code` = '$FworkCode' AND frameworks.`framework_type` = '$ProgType' WHERE l03 = '$LearnRefNumber' AND tr.`start_date` = $start_date AND tr.`target_date` = $target_date;");
        if($tr_id)
        {
            $learner = str_replace("'","",$learner);
            DAO::execute($link, "update ilr set ilr = '$learner' where tr_id = '$tr_id' and submission = '$submission' and contract_id = '$contract_id'");
            DAO::execute($link, "update tr set upi = '$ProvSpecDelMonC', start_date = $start_date, target_date = $target_date, closure_date = $closure_date, status_code = '$status_code' where id = '$tr_id'");
            foreach($ilr->LearningDelivery as $ld)
            {
                if($ld->AimType!='1')
                {
            ...
            else
            {
                ...
                if(!$tr_id)
                {
                    ...
                    try
                    {
                        ...
                        $learner = str_replace("'","",$learner);
                        DAO::execute($link, "insert into ilr(l03,ilr,submission,tr_id,contract_id,contract_type) values(trim('$LearnRefNumber'),'$learner','$submission','$tr_id',$contract_id,'ER')");
                        $link->commit();
                    }
                    catch(Exception $e)
                    {
                ...
                else
                {
                    ...
                    if(DB_NAME=='am_doncaster')
                        DAO::execute($link, "update tr set upi = '$ProvSpecDelMonC' where id = '$tr_id'");
                }
    ...
    public function createTrainingRecordOTHER($link, $learner, $tr_id, $course_id, $type, $contract_id, $submission, $provider_id)
    {
        ...
        if(DB_NAME=='am_lewisham')
            $tr_id = DAO::getSingleValue($link, "SELECT tr_id FROM ilr WHERE trim(extractvalue(ilr,'/Learner/LearnRefNumber'))='$LearnRefNumber' AND extractvalue(ilr,'/Learner/LearningDelivery[AimType=4]/LearnAimRef') like '%$LearnAimRef%' and contract_id in (select id from contracts where contract_year = 2013) limit 0,1");
        else
            $tr_id = DAO::getSingleValue($link, "SELECT tr_id FROM ilr WHERE trim(extractvalue(ilr,'/Learner/LearnRefNumber'))='$LearnRefNumber' AND extractvalue(ilr,'/Learner/LearningDelivery[AimType=4]/LearnAimRef')='$LearnAimRef' and submission = '$submission' and contract_id in (select id from contracts where contract_year = 2013);");
        if($tr_id)
        {
            DAO::execute($link, "update ilr set ilr = '$learner' where tr_id = '$tr_id' and submission = '$submission' and contract_id = '$contract_id'");
        ...
        {
            $tr_id2 = DAO::getSingleValue($link, "SELECT tr_id FROM ilr WHERE trim(extractvalue(ilr,'/ilr/learner/L03'))='$LearnRefNumber' AND extractvalue(ilr,'/ilr/main/A09')='$LearnAimRef' and contract_id in (select id from contracts where contract_year < 2013);");
            if($tr_id2)
            {
            ...
            else
            {
                ...
                if(!$tr_id)
                {
                    ...
                    try
                    {
                        ...
                        DAO::execute($link, "insert into ilr(ilr,submission,tr_id,contract_id,contract_type) values('$learner','$submission','$tr_id',$contract_id,'ER')");
                        $link->commit();
                    }
                    catch(Exception $e)
                    {
