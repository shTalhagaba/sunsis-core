<?php
class save_framework_commercial_funding implements IAction
{
	public function execute(PDO $link)
	{
		...
		foreach($_REQUEST AS $key=>$value)
		{
			if($key != 'fwrk_id' && $key != '_action')
				$sql .= "REPLACE INTO fwrk_commercial_funding (fwrk_id, description, amount) VALUES ('{$framework_id}', '{$key}', '{$value}'); " . PHP_EOL;
		}
