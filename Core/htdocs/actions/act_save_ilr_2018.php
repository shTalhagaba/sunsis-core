<?php
class save_ilr_2018 implements IAction
{
    ...
    private function process(PDO $link)
    {
        ...
        if($template!=1)
        {
            ...
            $previous_ilr = Ilr2018::loadFromXML($previous_ilr_xml);
...
//if(SOURCE_BLYTHE_VALLEY && DB_NAME == "am_demo") throw new Exception($xml_escaped);
        ...
        DAO::execute($link, "INSERT IGNORE INTO ilr SELECT L01,L03,A09,ilr,\"W01\",contract_type,tr_id,is_complete,is_valid,is_approved,is_active,(SELECT id FROM contracts WHERE parent_id = contract_id) FROM ilr WHERE LOCATE(\"<CompStatus>1</CompStatus>\",ilr)>0 AND submission = \"W13\" AND contract_id IN (SELECT id FROM contracts WHERE contract_year = 2018) AND ilr.tr_id = '$tr_id'");
        DAO::execute($link, "UPDATE tr SET contract_id = (SELECT MAX(contract_id) FROM ilr WHERE ilr.tr_id = tr.id) WHERE contract_id IN (SELECT id FROM contracts WHERE contract_year = 2018)");

        //Demigrate if required
        $shouldMigrate=DAO::getSingleValue($link, "SELECT (SELECT COUNT(*) FROM ilr WHERE LOCATE(\"<CompStatus>1</CompStatus>\",ilr)>0 AND submission = \"W13\" AND contract_id IN (SELECT id FROM contracts WHERE contract_year = 2018) AND ilr.tr_id = '$tr_id')+
...
WHERE LOCATE(\"<CompStatus>6</CompStatus>\",ilr)>0 AND submission = \"W13\" AND ilr.contract_id IN (SELECT id FROM contracts WHERE contract_year = 2018)
AND t2.`start_date` IS NULL AND ilr.tr_id = '$tr_id')+
(SELECT COUNT(*) FROM ilr WHERE LOCATE(\"<Outcome>8</Outcome>\",ilr)>0 AND submission = \"W13\" AND contract_id IN (SELECT id FROM contracts WHERE contract_year = 2018) AND ilr.tr_id = '$tr_id') AS m;");
        if($shouldMigrate==0)
        {
            DAO::execute($link, "delete from ilr where tr_id = '$tr_id' and (select start_date from tr where tr.id = ilr.tr_id) < '2019-08-01' and contract_id in (select id from contracts where contract_year = 2019)");
