<?php
class tr_videos implements IAction
{
	public function execute(PDO $link)
	{
		...
		if($subaction == 'save_assessor_feedback')
		{
			...
			http_redirect('do.php?_action=tr_videos&tr_id='.$_REQUEST['tr_id']);
			exit;
		}

		$tr_id = isset($_REQUEST['tr_id']) ? $_REQUEST['tr_id'] : '';

		$_SESSION['bc']->add($link, "do.php?_action=tr_videos&tr_id={$tr_id}", "Check Video Evidences");
		...
		include_once('tpl_tr_videos.php');
	}
	...
	private function uploadFile(PDO $link)
	{
		...
		http_redirect('do.php?_action=tr_videos&tr_id='.$tr->id.'&unit_ref='.$unit_ref.'&qan='.$qan.'&title='.$title);
	}
}