<?php
class validate_ilr2024 implements IAction
{
    public function execute(PDO $link)
    {
        ...
        error_log('start: ' . print_r($contract_id, true));
        $tr_id = isset($_REQUEST['tr_id']) ? $_REQUEST['tr_id'] : '';

        $ilr = Ilr2021::loadFromXML($xml);
        $validator = new ValidateILR2024();
        try {
        ...
        if ($report != 'No Error') {
            error_log("no: " . print_r($contract_id, true));
            ...
            error_log("yes: " . print_r($contract_id, true));
            ...
            echo '<?xml version="1.0"?><report>
    <success l03="' . htmlspecialchars((string)$ilr->LearnRefNumber) . '" />
</report>';
        }
    }
