<?php
class save_start_training implements IAction
{
    public function execute(PDO $link)
    {
	    ...
	    if( in_array(DB_NAME, ["am_baltic_demo", "am_baltic"]) && $inductee_id != '' )
            ...
            if(in_array(DB_NAME, ["am_baltic_demo","am_baltic"]))
            {
                $ilr_fields = $this->updateFromPreviousTrainingRecord($link, $tr, $course);
            }
            ...
            if($bil_learner)
            {
                ...
                $previous_training_record_id = isset($_REQUEST['previous_training_record_id'])?$_REQUEST['previous_training_record_id']:'';
...
SQL;
            ...
            $PlanLearnHours = "";
            $EPAOrgID = "";
            $p_ilr = DAO::getObject($link, "SELECT  
            CONCAT('<PriorAttain>',SUBSTRING_INDEX(SUBSTRING_INDEX(ilr, '<PriorAttain>', -1), '</PriorAttain>', 1),'</PriorAttain>') AS PriorAttain
            ,CONCAT('<LLDDHealthProb>',SUBSTRING_INDEX(SUBSTRING_INDEX(ilr, '<LLDDHealthProb>', -1), '</LLDDHealthProb>', 1),'</LLDDHealthProb>') AS LLDDHealthProb
            ,CONCAT('<PlanLearnHours>',SUBSTRING_INDEX(SUBSTRING_INDEX(ilr, '<PlanLearnHours>', -1), '</PlanLearnHours>', 1),'</PlanLearnHours>') AS PlanLearnHours
            ,CONCAT('<EPAOrgID>',SUBSTRING_INDEX(SUBSTRING_INDEX(ilr, '<EPAOrgID>', -1), '</EPAOrgID>', 1),'</EPAOrgID>') AS EPAOrgID
            FROM ilr where l03 = '$l03' order by contract_id desc, submission desc limit 1;");
            if($p_ilr!="")
            {
                $LLDDHealthProb = $p_ilr->LLDDHealthProb;
                $PriorAttain = $p_ilr->PriorAttain;
                $PlanLearnHours = $p_ilr->PlanLearnHours;
                $EPAOrgID = $p_ilr->EPAOrgID;
            }
            ...
            if($st)
            {
                while($row = $st->fetch())
                {
                    ...
                    $xml = '<Learner>';
                    $xml .= "<LearnRefNumber>" . $l03 . "</LearnRefNumber>";
                    if($row['l45']!='')
                        $xml .= "<ULN>" . $row['l45'] . "</ULN>";
                    else
                    ...
                    if((DB_NAME=="am_baltic" or DB_NAME=='am_baltic_demo') and $LLDDHealthProb!="")
                        $xml .= $LLDDHealthProb;
                    else
                    ...
                    if((DB_NAME=="am_baltic" or DB_NAME=='am_baltic_demo') and $PriorAttain!="")
                    {
                        $xml .= $PriorAttain;
                    }    

                    if((DB_NAME=="am_baltic" or DB_NAME=='am_baltic_demo') and $PlanLearnHours!="")
                    {
                        $xml .= $PlanLearnHours;
                    }    
                    ...
                    $sta = new Date($sd);
                    $sta->subtractDays(1);
                    /*if((DB_NAME=='am_baltic_demo' or DB_NAME=='am_baltic') and isset($ilr_fields->EmploymentStatus) and $ilr_fields->EmploymentStatus!="")
                    {
                        $xml .= $ilr_fields->EmploymentStatus;
                    }*/
                    if(DB_NAME=='am_baltic' or DB_NAME=='am_baltic_demo')
                    {
                        ...
                        //$xml .= "<LearnerEmploymentStatus><EmpStat>10</EmpStat><DateEmpStatApp>" . $sd . "</DateEmpStatApp><EmpId>" . $row['edrs'] . "</EmpId></LearnerEmploymentStatus>";
                    }
                    elseif(DB_NAME=='am_superdrug' or DB_NAME=='ad_demo')
                    {
                        $xml .= "<LearnerEmploymentStatus><EmpStat>98</EmpStat><DateEmpStatApp>" . $sta->formatMySQL() . "</DateEmpStatApp><EmpId></EmpId></LearnerEmploymentStatus>";
                        $xml .= "<LearnerEmploymentStatus><EmpStat>10</EmpStat><DateEmpStatApp>" . $sd . "</DateEmpStatApp><EmpId>" . $row['edrs'] . "</EmpId></LearnerEmploymentStatus>";
                    }
                    else
                    {
                    ...
                    $AimSeqNumber = 0;
                    if($course->programme_type=='2' || $course->programme_type=='7' || $course->programme_type=='37')
                    {
                        $AimSeqNumber++;
                        ...
                        if($st3)
                        {
                            while($row_sub = $st3->fetch())
                            {
                                ...
                                if((DB_NAME=='am_baltic_demo' or DB_NAME=='am_baltic') and isset($ilr_fields->PlanLearnHours) and $ilr_fields->PlanLearnHours!="")
                                {
                                    $xml.="<PlanLearnHours>" . $ilr_fields->PlanLearnHours . "</PlanLearnHours>"; 
                                }    
                                ...
                                $SOF = $this->getValueFromTemplate2($ilrtemplatetext,str_replace("/" , "", "ZPROG001"),"SOF");
                                ...
                                $xml .= $tr->otj_hours != '' ? "<PHours>" . $tr->otj_hours . "</PHours>" : '';
                                if(DB_NAME=='am_ela')
                                {
                                ...
                                /*elseif((DB_NAME=='am_baltic_demo' or DB_NAME=='am_baltic') and isset($ilr_fields->LDM) and $ilr_fields->LDM!="")
                                {
                                    $xml.=$ilr_fields->LDM; 
                                }*/    
                                else
                                ...
                                /*if((DB_NAME=='am_baltic_demo' or DB_NAME=='am_baltic') and isset($ilr_fields->FinancialDetails) and $ilr_fields->FinancialDetails!="")
                                {
                                    $xml.=$ilr_fields->FinancialDetails; 
                                }*/   
...
HEREDOC;
			    ...
			    $aim_reference = str_replace("/" , "", $row_sub['id']);
                            if( in_array( substr($aim_reference, -2, 2), ["GW", "EP"] ) )
                            {
    ...
    public function updateFromPreviousTrainingRecord(PDO $link, TrainingRecord $tr, Course $course)
    {
        $ilr_fields = new stdClass();
...
SQL;
        ...
        if($bil_tr_id != '')
        {
            $bil_tr = TrainingRecord::loadFromDatabase($link, $bil_tr_id);
            $tr->assessor = $bil_tr->assessor;
            $tr->crm_contact_id = $bil_tr->crm_contact_id;
            $tr->save($link);

            $ilr = DAO::getSingleValue($link, "select ilr from ilr where tr_id = '$bil_tr_id' order by contract_id desc, submission desc limit 1");
            $ilr_fields = new StdClass();
            $xml = @simplexml_load_string($ilr);
            $ilr_fields->PlanLearnHours = @$xml->PlanLearnHours;
            $EmploymentStatus="";
            $ldm="";
            $tnp="";
            /*foreach ($xml->LearnerEmploymentStatus as $LearnerEmploymentStatus)
            {
                $EmploymentStatus.= "<LearnerEmploymentStatus>";
                $EmploymentStatus.= "<EmpStat>" . @(string)$LearnerEmploymentStatus->EmpStat . "</EmpStat>";
                $EmploymentStatus.= "<DateEmpStatApp>" . @(string)$LearnerEmploymentStatus->DateEmpStatApp . "</DateEmpStatApp>";
                $EmploymentStatus.= "<EmpId>" . @(string)$LearnerEmploymentStatus->EmpId . "</EmpId>";
                foreach ($LearnerEmploymentStatus->EmploymentStatusMonitoring as $EmploymentStatusMonitoring)
                {
                    $EmploymentStatus.="<EmploymentStatusMonitoring>";
                    $EmploymentStatus.="<ESMType>" . @(string)$EmploymentStatusMonitoring->ESMType . "</ESMType>";
                    $EmploymentStatus.="<ESMCode>" . @(string)$EmploymentStatusMonitoring->ESMCode . "</ESMCode>";
                    $EmploymentStatus.="</EmploymentStatusMonitoring>";
                }
                $EmploymentStatus.="</LearnerEmploymentStatus>";
            }*/
            ...
            {
                $aimType = @(string)$LearningDelivery->AimType;
                if($aimType==1)
                {
                    /*foreach ($LearningDelivery->LearningDeliveryFAM as $LearningDeliveryFAM)
                    {
                        $ldm.= "<LearningDeliveryFAM>";
                        $ldm.= "<LearnDelFAMType>" . @(string)$LearningDeliveryFAM->LearnDelFAMType . "</LearnDelFAMType>";                    
                        $ldm.= "<LearnDelFAMCode>" . @(string)$LearningDeliveryFAM->LearnDelFAMCode . "</LearnDelFAMCode>";                    
                        $ldm.= "<LearnDelFAMDateFrom>" . @(string)$LearningDeliveryFAM->LearnDelFAMDateFrom . "</LearnDelFAMDateFrom>";
                        $ldm.= "<LearnDelFAMDateTo>" . @(string)$LearningDeliveryFAM->LearnDelFAMDateTo . "</LearnDelFAMDateTo>";
                        $ldm.= "</LearningDeliveryFAM>";
                    }*/

                    /*foreach ($LearningDelivery->TrailblazerApprenticeshipFinancialRecord as $TrailblazerApprenticeshipFinancialRecord)
                    {
                        $tnp.="<TrailblazerApprenticeshipFinancialRecord>";
                        $tnp.="<TBFinType>" . $TrailblazerApprenticeshipFinancialRecord->TBFinType . "</TBFinType>";
                        $tnp.="<TBFinCode>" . $TrailblazerApprenticeshipFinancialRecord->TBFinCode . "</TBFinCode>";
                        $tnp.="<TBFinDate>" . $TrailblazerApprenticeshipFinancialRecord->TBFinDate . "</TBFinDate>";
                        $tnp.="<TBFinAmount>" . $TrailblazerApprenticeshipFinancialRecord->TBFinAmount . "</TBFinAmount>";
                        $tnp.="</TrailblazerApprenticeshipFinancialRecord>";
                    }*/
                }
            }
            ...
            $ilr_fields->LDM = $ldm;
            $ilr_fields->FinancialDetails = $tnp;
            $ilr_fields->EmploymentStatus = $EmploymentStatus;
        }
        return $ilr_fields;
    }
...
SQL;
		...
		elseif($framework->epa_org_id == "EPA0008")
                {
                    $status_id = 11; // C&G
                }

                if(in_array($framework->id, [430, 431, 432]))
                {
                    $status_id = 11; // C&G
                }
            }
            ...
            if(
                in_array($task_id, [6, 7]) && 
                in_array($framework->id, [430, 431, 432])
            )
            {
                $status_id = 43; // Awaiting EPA confirmation
            }
    ...
    private function createFsProgressEntry(PDO $link, TrainingRecord  $tr, $inductee_id)
    {
        ...
        // redundant conditions end

        if($induction->wfd_assessment == "N" && $induction->maths_gcse_elig_met == "N")
        {
            $fs_progress->required = "3"; // Both
            $fs_progress->fs_required = "2"; // Not Required
        }
        elseif($induction->wfd_assessment == "Y" && $induction->maths_gcse_elig_met == "Y")
        {
            $fs_progress->required = "4"; // None
            $fs_progress->fs_required = "2"; // Not Required
        }
        elseif($induction->wfd_assessment == "YP" && $induction->maths_gcse_elig_met == "YP")
        {
            $fs_progress->required = "3"; // Both
            $fs_progress->fs_required = "1"; // In Progress
        }
        elseif($induction->wfd_assessment == "Y" && $induction->maths_gcse_elig_met == "N")
        {
            $fs_progress->required = "1"; // Maths
            $fs_progress->fs_required = "2"; // Not Required
        }
        elseif($induction->wfd_assessment == "YP" && $induction->maths_gcse_elig_met == "N")
        {
            $fs_progress->required = "3"; // Both
            $fs_progress->fs_required = "1"; // In Progress
        }
        elseif($induction->wfd_assessment == "YP" && $induction->maths_gcse_elig_met == "Y")
        {
            $fs_progress->required = "2"; // English
            $fs_progress->fs_required = "1"; // In Progress
        }
        elseif($induction->wfd_assessment == "Y" && $induction->maths_gcse_elig_met == "YP")
        {
            $fs_progress->required = "3"; // Both
            $fs_progress->fs_required = "1"; // In Progress
        }
        elseif($induction->wfd_assessment == "N" && $induction->maths_gcse_elig_met == "YP")
        {
            $fs_progress->required = "3"; // Both
            $fs_progress->fs_required = "1"; // In Progress
        }
        elseif($induction->wfd_assessment == "N" && $induction->maths_gcse_elig_met == "Y")
        {
            $fs_progress->required = "2"; // English
            $fs_progress->fs_required = "2"; // Not Required
        }
        
        // following will not be executed as maths_cert and eng_cert are removed start
        ...
        // following will not be executed as maths_cert and eng_cert are removed end
        //////
