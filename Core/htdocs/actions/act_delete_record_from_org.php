...
class delete_record_from_org implements IAction
{
	...
	private function delete_crm_contact(PDO $link, $record)
	{
		...
		$r = OrganisationCRMContact::loadFromDatabase($link, $record_id);
		$r->delete($link);
	...
	private function delete_system_user(PDO $link, $record)
	{
		...
		$notes = DAO::getSingleValue($link, "SELECT COUNT(*) FROM notes WHERE notes.username = '{$record_username}'");
		if($notes > 0)
		{
			$message = 'This user has audit notes in the system so cannot be deleted.';
			return $message;
		}

		$sql = <<<SQL
SELECT
	SUM(IF (tr.assessor = '{$record_id}', 1, 0)) AS a,
	SUM(IF (tr.tutor = '{$record_id}', 1, 0)) AS b,
	SUM(IF (tr.verifier = '{$record_id}', 1, 0)) AS c,
	SUM(IF (tr.programme = '{$record_id}', 1, 0)) AS d
FROM
	tr
SQL;
		...
		$user = User::loadFromDatabaseById($link, $record_id);
		$identities = "'".addslashes((string)$user->username)."', '".addslashes((string)$user->getFullyQualifiedName())."'";
		$queries[] = "DELETE FROM users WHERE username='$user->username'";
