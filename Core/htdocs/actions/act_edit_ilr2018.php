<?php
class edit_ilr2018 implements IAction
{
    public function execute(PDO $link)
    {
        ...
        $_SESSION['bc']->add($link, "do.php?_action=edit_ilr2018&submission=" . $submission . "&contract_id=" . $contract_id . "&tr_id=" . $tr_id . "&L03=" . $L03, "Add/ Edit ILR Form");
        ...
        {
            $vo = Ilr2018::loadFromDatabase($link, $submission, $contract_id, $tr_id, $L03);
        }
        ...
        $dynamic_field_sql = 'select xml_fieldname, level, historic_al_code, readable_fieldname, fieldname_description, programme_aims, 1 as required from central.lookup_ilr_2018_fields';

        $dynamic_funding_type = strtolower($funding_type);

        $dynamic_table_types = DAO::getTableFields($link, 'central.lookup_ilr_2018_fields');
        if ( isset($dynamic_table_types[$dynamic_funding_type]) ) {
            $dynamic_field_sql = 'select xml_fieldname, level, historic_al_code, readable_fieldname, fieldname_description, programme_aims, '.$dynamic_funding_type.' as required from central.lookup_ilr_2018_fields';
        }
        ...
        require_once('tpl_edit_ilr2018.php');
    }
