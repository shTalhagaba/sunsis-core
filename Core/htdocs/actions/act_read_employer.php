...
class read_employer implements IAction
{
    public function execute(PDO $link)
    {
        ...
        if(SOURCE_LOCAL || 
            in_array(DB_NAME, ["am_demo", "am_city_skills", "am_crackerjack"]) || 
            SystemConfig::getEntityValue($link, 'module_onboarding') || 
            SystemConfig::getEntityValue($link, 'onboarding') || 
            SystemConfig::getEntityValue($link, 'module_crm'))
            http_redirect('do.php?_action=read_employer_v3&id='.$_REQUEST['id']);

	if(in_array(DB_NAME, ["am_superdrug"]))
        {
            http_redirect('do.php?_action=rec_read_employer&id='.$_REQUEST['id']);
        }

	if(in_array(DB_NAME, ["am_baltic", "am_baltic_demo"]))
        {
            http_redirect('do.php?_action=baltic_read_employer&id='.$id);
        }
