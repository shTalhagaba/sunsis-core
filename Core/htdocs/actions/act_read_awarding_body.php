<?php
class read_awarding_body implements IAction
{
	public function execute(PDO $link)
	{
		...
		$_SESSION['bc']->add($link, "do.php?_action=read_awarding_body&id=" . $id, "View Awarding Body");
...
//		$vo5->refresh($link, $_REQUEST);
		...
		$vo6 = ViewAwardingBodyQualifications::getInstance($link, $id);
		$vo6->refresh($link, $_REQUEST);
...
//		$data = $vo4->getStats($link);
		...
		// Presentation
		include('tpl_read_awarding_body.php');
	}

	private function renderLocations(PDO $link, AwardingBody $vo)
	{
	...
	private function renderPersonnel(PDO $link, AwardingBody $vo)
	{
	...
	private function renderLearners(PDO $link, AwardingBody $vo)
	{
