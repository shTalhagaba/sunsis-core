<?php
class add_attendance_module_lesson implements IAction
{
	public function execute(PDO $link)
	{
		...
		$attendance_module = AttendanceModule::loadFromDatabase($link, $_REQUEST['module_id']);
		$vo->qualification = $attendance_module->qualification_title;
		...
		$vo->lesson_title = DAO::getSingleValue($link, "SELECT LEFT(title, 149) FROM attendance_module_groups WHERE id = '{$vo->groups_id}'");
		...
		try
		{
			...
			$grp_dao = new AttendanceModuleGroupDAO($link);
			...
			$attendance_module = AttendanceModule::loadFromDatabase($link, $grp_vo->module_id);
