...
class pdf_from_ilr implements IAction
{
	public function execute(PDO $link)
	{
		...
		$pagecount = $pdf->setSourceFile('ilr0809.pdf');
		...
		$pdf->Text(73, 21, strtoupper($this->spaceout($vo->learnerinformation->L25)));
		...
		$pdf->Text(63, 121, $this->spaceout(str_pad($vo->learnerinformation->L47, 2, '0', STR_PAD_LEFT)));
		...
		if ($vo->learnerinformation->L48 != '' && $vo->learnerinformation->L48 != '00000000') {
			...
			$pdf->Text(112, 121, $this->spaceout($matches[1][0]));
			$pdf->Text(127, 121, $this->spaceout($matches[2][0]));
			$pdf->Text(155, 121, $this->spaceout($matches[4][0]));
		}
		...
		if ($vo->aims[0]->A10 != '45' || $vo->aims[0]->A15 != '99' || ($vo->aims[0]->A18 != '22' && $vo->aims[0]->A18 != '23')) {
			...
			$pdf->Text(191, 47, $this->spaceout(str_pad($vo->programmeaim->A15, 2, '0', STR_PAD_LEFT)));
			$pdf->Text(230, 47, $this->spaceout(str_pad($vo->programmeaim->A16, 2, '0', STR_PAD_LEFT)));
			$pdf->Text(273, 47, $this->spaceout($vo->programmeaim->A26));


			if ($vo->programmeaim->A27 != '' && $vo->programmeaim->A27 != '00000000') {
				preg_match_all("/(^[0-9]{2})\/([0-9]{2})\/([0-9]{2})([0-9]{2})/", $vo->programmeaim->A27, $matches);
				$pdf->Text(49, 59, $this->spaceout($matches[1][0]));
				$pdf->Text(64, 59, $this->spaceout($matches[2][0]));
				$pdf->Text(93, 59, $this->spaceout($matches[4][0]));
			}

			if ($vo->programmeaim->A28 != '' && $vo->programmeaim->A28 != '00000000') {
				preg_match_all("/(^[0-9]{2})\/([0-9]{2})\/([0-9]{2})([0-9]{2})/", $vo->programmeaim->A28, $matches);
				$pdf->Text(142, 59, $this->spaceout($matches[1][0]));
				$pdf->Text(157, 59, $this->spaceout($matches[2][0]));
				$pdf->Text(185, 59, $this->spaceout($matches[4][0]));
			}
			...
			if ($vo->programmeaim->A23 != '') {
				...
				$pdf->Text(245, 59, $this->spaceout($matches[1][0]));
				$pdf->Text(273, 59, $this->spaceout($matches[2][0]));
			}


			$pdf->Text(62, 71, $this->spaceout($vo->programmeaim->A51a));
			$pdf->Text(115, 71, $this->spaceout(str_pad($vo->programmeaim->A14, 2, '0', STR_PAD_LEFT)));

			$pdf->Text(190, 71, $this->spaceout($vo->programmeaim->A46a));
			$pdf->Text(212, 71, $this->spaceout($vo->programmeaim->A46b));

			$pdf->Text(279, 71, $this->spaceout($vo->programmeaim->A02));


			if ($vo->programmeaim->A31 != '' && $vo->programmeaim->A31 != '00000000') {
				preg_match_all("/(^[0-9]{2})\/([0-9]{2})\/([0-9]{2})([0-9]{2})/", $vo->programmeaim->A31, $matches);
				$pdf->Text(48, 100, $this->spaceout($matches[1][0]));
				$pdf->Text(63, 100, $this->spaceout($matches[2][0]));
				$pdf->Text(92, 100, $this->spaceout($matches[4][0]));
			}

			if ($vo->programmeaim->A40 != '' && $vo->programmeaim->A40 != '00000000') {
				preg_match_all("/(^[0-9]{2})\/([0-9]{2})\/([0-9]{2})([0-9]{2})/", $vo->programmeaim->A40, $matches);
				$pdf->Text(152, 100, $this->spaceout($matches[1][0]));
				$pdf->Text(167, 100, $this->spaceout($matches[2][0]));
				$pdf->Text(195, 100, $this->spaceout($matches[4][0]));
			}

			$pdf->Text(48, 110, $vo->programmeaim->A34);
			$pdf->Text(97, 110, $vo->programmeaim->A35);
			$pdf->Text(152, 110, $this->spaceout(str_pad($vo->programmeaim->A50, 2, '0', STR_PAD_LEFT)));
		}
		...
		$pdf->Text(80, 69, $this->spaceout(str_pad($vo->aims[0]->A14, 2, '0', STR_PAD_LEFT)));
		...
		$pdf->Text(41, 80, $this->spaceout($vo->aims[0]->A24));
		...
		$pdf->Text(148, 92, $this->spaceout(str_pad($vo->aims[0]->A18, 2, '0', STR_PAD_LEFT)));
		...
		for ($a = 1; $a <= $vo->subaims; $a++) {
			...
			if ($vo->aims[0]->A10 != '45' || $vo->aims[0]->A15 != '99' || ($vo->aims[0]->A18 != '22' && $vo->aims[0]->A18 != '23')) {
				...
				if (($a % 2) != 0) {
					...
					$pdf->Text(80, 66, $this->spaceout($vo->aims[$a]->A14));
				...
				$pdf->Text(148, 92, $this->spaceout(str_pad($vo->aims[$a]->A18, 2, '0', STR_PAD_LEFT)));
