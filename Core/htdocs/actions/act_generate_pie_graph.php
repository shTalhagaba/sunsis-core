...
class generate_pie_graph
{
	...
	// re 12/07/2011: quick fix for annoying error, more robust fix needs putting in place
	$d = preg_replace('/&/','&amp;', $d);
	...
	foreach( $xml->record as $record ) {	
		...
		$data[$index][2] = $colors[$color_index]; 
		$index++;
		// re 12/07/2011: quick fix for annoying error, more robust fix needs putting in place
		$color_index++;		
		if ( $color_index > 14 ) {
	...
	$total = DAO::getSingleValue($link, "select count(*) from graph_data");
	$ontrack = DAO::getSingleValue($link, "select count(*) from graph_data where description = 'On Track'");
	$behind = DAO::getSingleValue($link, "select count(*) from graph_data where description = 'Behind'");

	if($behind == $total)
		$data[$index-1][2] = $colors[0];
	elseif($ontrack==$total)
		$data[$index-1][2] = $colors[1];
	...
	// #182 - {0000000195} relmes - error on generate_pie_graph if no employers associated with the user
	if ( !isset($data) ) {
		return;
	}
	...
	foreach( $EQpieData as $i => $value ) {
		// #182 - {0000000195} relmes - error on generate_pie_graph
		if ( sizeof($value) <= 1  ) {
			continue;
		}
	  	$total += $value[1];
	...
	// #182 - {0000000195} relmes - additional divsion by zero error if total = 0
	if( $total == 0 ) {
		$total = 1;
	}
	...
	foreach($EQpieData as $i => $value){ 
	  ...
	  // the value for each part is $value[1]
		...
		if ( sizeof($value) <= 1  ) {
	  	...
	  	$piePart = $value[1];
        $piePart100 = sprintf("%.2f",($piePart/$total*100));  // value in percentage, the rounding and * 100 for extra accuracy for pie w/o gaps
