<?php
class save_ilr_2024 implements IAction
{
    ...
    private function process(PDO $link)
    {
        ...
        if($template!=1)
        {
            ...
            if(!empty($xresult))
            {
                foreach($xresult AS $key => $node)
                {
                    ...
                    if( (DB_NAME=='am_baltic' or DB_NAME=='am_baltic_demo') and in_array("'".$a09."'",$aims))
                    {
                        continue;
                    }
                    else
                    ...
                    if($node->LearnActEndDate)
                    {
                        ...
                        $LearnActEndDate = new DateTime($node->LearnActEndDate);
                        $AcademicEndDate = new DateTime("2025-07-31");
                        $a34 = $node->CompStatus;
                        if($node->Outcome=="8" or $node->CompStatus=="6" or $LearnActEndDate > $AcademicEndDate)
                        {
            ...
            if($shoulddemigrate)
            {
                DAO::execute($link, "delete from ilr where tr_id = '$tr_id' and contract_id in (select id from contracts where contract_year = 2025)");
            ...
            {
                $c = DAO::getSingleValue($link, "select count(*) from ilr where tr_id='$tr_id' and contract_id in (select id from contracts where contract_year = 2025)");
                if($c==0)
                {
                    DAO::execute($link, "INSERT INTO ilr SELECT L01,L03,A09,ilr,'W01',contract_type,tr_id,is_complete,is_valid,is_approved,is_active,(SELECT id FROM contracts WHERE parent_id = contract_id) FROM ilr WHERE contract_id in (select id from contracts where contract_year = 2024) and submission = 'W13' AND tr_id = '$tr_id'");
                    DAO::execute($link, "UPDATE tr SET contract_id = (SELECT contract_id FROM ilr WHERE ilr.tr_id = tr.id ORDER BY contract_id DESC LIMIT 0,1) where tr.id = '$tr_id';");
                }
            }
            ...
            $previous_ilr = Ilr2023::loadFromXML($previous_ilr_xml);
            ...
            // Update tr status
            $vo = Ilr2024::loadFromXML($xml);
            $funding_type = Ilr2024::FundingType($vo);
                ...
                foreach($vo->LearningDelivery as $ld)
                {
                    if($ld->AimType=='1')
                    {
                        if($ld_start_date=='')
                        {
                            ...
                            $earliest_start_date = $ld_start_date;
                            $latest_end_date = new Date($ld->LearnActEndDate);
                            $achievement_date = $ld->AchDate;
			...
			$EpaOrgId = (isset($ld->EPAOrgID) && $ld->EPAOrgID->__toString() != '') ? $ld->EPAOrgID->__toString() : '';
                    }
                }
            }
            ...
            elseif(strpos($xml, "<AimType>4</AimType>")!==false)
            {
                ...
                $xpath = $vo->xpath("/Learner/LearningDelivery[AimType='4']/AchDate");
                if(isset($xpath[0]))
                    $achievement_date = $xpath[0];
                else
                    $achievement_date = "";
            ...
            $planned_end_dates = Array();
            foreach($vo->LearningDelivery as $ld)
            {
                $planned_end_dates[] = Date::toMySQL($ld->LearnPlanEndDate);
            }
    		$timestamps = [];
            // Convert each string date to a timestamp
            foreach ($planned_end_dates as $dateString) {
                $timestamp = strtotime($dateString);
                if ($timestamp !== false) { // Check if conversion was successful
                    $timestamps[] = $timestamp;
                }
            }

            // Find the earliest timestamp using min()
            if(isset($timestamps) and sizeof($timestamps)>1)
                $earliestTimestamp = max($timestamps);
            // Convert the earliest timestamp back to a date format
            if(isset($earliestTimestamp))
                $earliestDate = date('Y-m-d', $earliestTimestamp);
            else
            {
                $tr = TrainingRecord::loadFromDatabase($link, $tr_id);
                $earliestDate = $tr->target_date;
            }    

            if(Date::isDate($achievement_date))
                $achievement_date = "'" . Date::toMySQL($achievement_date) . "'";
            else 
                $achievement_date = "NULL";

            if(DB_NAME=='am_ela')
                $link->query("update tr set ni='$ni', achievement_date = $achievement_date, target_date = '$earliestDate', start_date='$start_date',home_email = '$home_email', l42a = '$l42a', l42b = '$l42b', uln = '$uln', ilr_status = '$ilr_status', status_code = '$status_code', outcome='$outcome', closure_date = $closure_date, ethnicity = '$ethnicity' where id = '$tr_id'");
            else
                $link->query("update tr set ni='$ni', target_date = '$earliestDate', start_date='$start_date',home_email = '$home_email', l42a = '$l42a', l42b = '$l42b', uln = '$uln', ilr_status = '$ilr_status', status_code = '$status_code', outcome='$outcome', closure_date = $closure_date, ethnicity = '$ethnicity' where id = '$tr_id'");


            if(DB_NAME=='am_ela' && $status_code!=1)
            {
                DAO::execute($link, "UPDATE student_qualifications INNER JOIN tr ON tr.id = student_qualifications.tr_id AND tr.status_code!=1 AND tr.closure_date IS NOT NULL SET student_qualifications.actual_end_date = tr.closure_date;");                
            }
	    ...
	    if($EpaOrgId != '')
            {
                //$obj->epa_organisation = substr($EpaOrgId, 0, 7);                
            }
	    ...
	    // update in operations
            ...
            {
                $status_id = '';
                if(trim($EpaOrgId) == "EPA0475")
                {
                    $status_id = 50; //AP
                }
                elseif(trim($EpaOrgId) == "EPA0001")
                {
                    $status_id = 10; //BCS
                }
                elseif(trim($EpaOrgId) == "EPA0440")
                {
                    $status_id = 51; //1st for EPA
                }
		elseif(trim($EpaOrgId) == "EPA0008")
                {
                    $status_id = 11; //C&G
                }	
                //if($status_id != '')	
                //    DAO::execute($link, "UPDATE op_epa SET op_epa.task_status = '{$status_id}' WHERE op_epa.task = 5 AND op_epa.tr_id = '{$tr_id}'");
            }
        }
        else
        {
