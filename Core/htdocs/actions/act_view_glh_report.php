<?php
class view_glh_report implements IAction
{
    public function execute(PDO $link)
    {
        $subaction = isset($_REQUEST['subaction'])?$_REQUEST['subaction']:'';

        $view = VoltView::getViewFromSession('ViewGLHReport', 'ViewGLHReport'); /* @var $view VoltView */
        if(is_null($view))
        {
            $view = $_SESSION['ViewGLHReport'] = $this->buildView($link);
        }
        ...
        $_SESSION['bc']->index = 0;
        $_SESSION['bc']->add($link, "do.php?_action=view_glh_report", "View GLH Report");

        include_once('tpl_view_glh_report.php');
    }
...
SELECT
    ...
    courses.glh AS glh_hours_due,
	(SELECT SUM(duration_hours)*60 + SUM(duration_minutes) FROM glh WHERE tr_id = tr.id) AS glh_hours_actual,
	IF
	(
		tr.`otj_hours` = 0, '',
		IF
		(
			(SELECT SUM(duration_hours)*60 + SUM(duration_minutes) FROM glh WHERE tr_id = tr.id) >=
...
WHERE
    tr.id IS NOT NULL and courses.programme_type != 2
ORDER BY
    tr.firstnames
		");
        ...
        $view = new VoltView('ViewGLHReport', $sql->__toString());
        ...
        $options = array(
            ...
            1=>array(1, 'On Track', null, 'HAVING glh_progress = "On Track"'),
            2=>array(2, 'Behind', null, 'HAVING glh_progress = "Behind"'));
        $f = new VoltDropDownViewFilter('filter_glh_progress', $options, 0, false);
        $f->setDescriptionFormat("GLH Progress: %s");
        $view->addFilter($f);
    ...
    private function renderView(PDO $link, VoltView $view)
    {
        ...
        if($st)
        {
            ...
            echo '<div align="center" ><table id="tblGLHReport" class="table table-bordered table-condensed">';
            echo '<thead class="bg-gray"><tr>';
            echo '<th>Firstnames</th><th>Surname</th><th>Employer</th><th>Assessor</th><th>Framework</th><th>Contract</th><th>Start Date</th><th>Planned End Date</th><th>Completed Date</th>';
            echo '<th>Planned GLH</th><th>Actual GLH</th><th>GLH Progress</th>';
            echo '</tr></thead>';
            echo '<tbody>';
            while($row = $st->fetch(DAO::FETCH_ASSOC))
            {
                ...
                $glh_minutes_due = $row['glh_hours_due'] == '' ? 0 : $row['glh_hours_due']*60;
                echo '<td>' . str_replace(' ', '&nbsp;', ViewTrainingRecords::convertToHoursMins($glh_minutes_due, '%02d hours %02d minutes')) . '</td>';
                $glh_minutes_actual = $row['glh_hours_actual'] == '' ? 0 : $row['glh_hours_actual'];
                if(in_array(DB_NAME, ["am_city_skills", "am_demo"]))
                    $glh_minutes_actual = $row['glh_hours_actual'] + ViewTrainingRecords::calculateGLHMinutes($link, $row['tr_id']);

                //if($row['tr_id']==4506)
               // pre(ViewTrainingRecords::convertToHoursMins($glh_minutes_actual, '%02d hours %02d minutes'));
    
                echo '<td>' . str_replace(' ', '&nbsp;', ViewTrainingRecords::convertToHoursMins($glh_minutes_actual, '%02d hours %02d minutes')) . '</td>';
                echo $row['glh_progress'] == 'On Track' ? '<td class="text-green">'.$row['glh_progress'].'</td>' : '<td class="text-red">'.$row['glh_progress'].'</td>';
                echo '</tr>';
            }
    ...
    private function export_csv(PDO $link, VoltView $view)
    {
        ...
        if($st)
        {
            ...
            header('Content-Disposition: attachment; filename=GLH Report.csv');
            if(strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE') !== false)
            {
                header('Pragma: public');
                header('Cache-Control: max-age=0');
            }
            echo "Firstnames,Surname,Assessor/ Coach, Employer,Contract,Start Date,Planned End Date,Completed Date,GLH Hours Due,GLH Hours Actual,GLH Progress";
            echo "\n";
            while($row = $st->fetch(DAO::FETCH_ASSOC))
            {
                ...
                $glh_minutes_due = $row['glh_hours_due'] == '' ? 0 : $row['glh_hours_due']*60;
                echo ViewTrainingRecords::convertToHoursMins($glh_minutes_due, '%02d hours %02d minutes') . ',';
                $glh_minutes_actual = $row['glh_hours_actual'] == '' ? 0 : $row['glh_hours_actual'];
                if(in_array(DB_NAME, ["am_city_skills", "am_demo"]))
                    $glh_minutes_actual = $row['glh_hours_actual'] + ViewTrainingRecords::calculateGLHMinutes($link, $row['tr_id']);
                echo ViewTrainingRecords::convertToHoursMins($glh_minutes_actual, '%02d hours %02d minutes') . ',';
                echo HTML::csvSafe($row['glh_progress']) . ",";
                echo "\n";
            }
        }
        else
