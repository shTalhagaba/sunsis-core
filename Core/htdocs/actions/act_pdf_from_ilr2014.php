...
class pdf_from_ilr2014 implements IAction
{
	public function execute(PDO $link)
	{
		...
		if (DB_NAME == 'am_lead')
			$pagecount = $pdf->setSourceFile('ilr2014v2.pdf');
		else
			$pagecount = $pdf->setSourceFile('ilr2014.pdf');
		...
		elseif ($vo->LLDDHealthProb == 1)
			...
			$pdf->Text(132, 105, $vo->LLDDHealthProb);
		...
		$pdf->Text(218, 105, $this->spaceout(str_pad($ds, 2, '0', STR_PAD_LEFT), 5));
		$pdf->Text(270, 105, $this->spaceout(str_pad($ld, 2, '0', STR_PAD_LEFT), 5));

		$xpath = $vo->xpath("/Learner/LearnerFAM[LearnFAMType='LDA']/LearnFAMCode");
		$lda = (empty($xpath[0])) ? '' : (string)$xpath[0];
		$pdf->Text(63, 116, $this->spaceout(str_pad($lda, 1, ' ', STR_PAD_LEFT), 5));
		$xpath = $vo->xpath("/Learner/LearnerFAM[LearnFAMType='EHC']/LearnFAMCode");
		$ehc = (empty($xpath[0])) ? '' : (string)$xpath[0];
		$pdf->Text(63, 123, $this->spaceout(str_pad($ehc, 1, ' ', STR_PAD_LEFT), 5));
		$xpath = $vo->xpath("/Learner/LearnerFAM[LearnFAMType='DLA']/LearnFAMCode");
		$dla = (empty($xpath[0])) ? '' : (string)$xpath[0];
		$pdf->Text(63, 130, $this->spaceout(str_pad($dla, 1, ' ', STR_PAD_LEFT), 5));
		$xpath = $vo->xpath("/Learner/LearnerFAM[LearnFAMType='HNS']/LearnFAMCode");
		$hns = (empty($xpath[0])) ? '' : (string)$xpath[0];
		$pdf->Text(198, 120, $this->spaceout(str_pad($hns, 1, ' ', STR_PAD_LEFT), 5));

		//		$xpath = $vo->xpath("/Learner/LearnerFAM[LearnFAMType='ALS']/LearnFAMCode"); $als = (empty($xpath[0]))?'':(string)$xpath[0];
		//		$pdf->Text(87,129,$this->spaceout(str_pad($als,1,' ',STR_PAD_LEFT),5));

		$pdf->Text(178, 126, $this->spaceout(str_pad($vo->ALSCost, 1, ' ', STR_PAD_LEFT), 6));
		...
		$pdf->Text(246, 159, $this->spaceout($ppe1, 6));
		//$pdf->Text(271,159,$this->spaceout($ppe2,6));
   ...
   */
		...
		if ($ProvSpecLearnMon1 != '') {
			...
			$pdf->Text(43, 201, $this->spaceout($matches[1][0], 6));
			$pdf->Text(88, 201, $this->spaceout($matches[2][0], 6));
		}
		...
		if ($ProvSpecLearnMon2 != '') {
			...
			$pdf->Text(135, 201, $this->spaceout($matches[1][0], 6));
			$pdf->Text(181, 201, $this->spaceout($matches[2][0], 6));
		}

		$pdf->Text(253, 201, $this->spaceout($vo->Dest, 5));
	...
	*/
				...
				/*                $xpath = $delivery->xpath("./LearningDeliveryFAM[LearnDelFAMType='WPP']/LearnDelFAMCode");
					$wpp = (empty($xpath[0]))?'':$xpath[0];
					$pdf->Text(155,113,$this->spaceout($wpp));
	*/
...
*/
				...
				$pdf->Text(35, 62, $this->spaceout(str_pad($delivery->ProgType, 2, '0', STR_PAD_LEFT), 5));
				$pdf->Text(84, 62, $this->spaceout($delivery->FworkCode, 6));
				$pdf->Text(137, 62, $this->spaceout(str_pad($delivery->PwayCode, 3, '0', STR_PAD_LEFT), 6));
				$pdf->Text(203, 62, $this->spaceout(str_pad($delivery->PriorLearnFundAdj, 2, '0', STR_PAD_LEFT), 6));
				...
				if ($delivery->ESFProjDosNumber != '' && $delivery->ESFProjDosNumber != 'undefined') {
					...
					$pdf->Text(37, 80, $this->spaceout($matches[1][0], 6));
					$pdf->Text(75, 80, $this->spaceout($matches[2][0], 6));
				}
				if ($delivery->ESFLocProjNumber != 'undefined')
					$pdf->Text(167, 80, $this->spaceout($delivery->ESFLocProjNumber, 6));

				$xpath = $delivery->xpath("./LearningDeliveryFAM[LearnDelFAMType='SOF']/LearnDelFAMCode");
				$sof = (empty($xpath[0])) ? '' : $xpath[0];
				if ($sof != 'undefined')
					$pdf->Text(35, 96, $this->spaceout(str_pad($sof, 1, ' ', STR_PAD_LEFT), 5));
				$xpath = $delivery->xpath("./LearningDeliveryFAM[LearnDelFAMType='FFI']/LearnDelFAMCode");
				$ffi = (empty($xpath[0])) ? '' : $xpath[0];
				if ($ffi != 'undefined')
					$pdf->Text(87, 96, $this->spaceout(str_pad($ffi, 1, ' ', STR_PAD_LEFT), 1));
				$xpath = $delivery->xpath("./LearningDeliveryFAM[LearnDelFAMType='WPL']/LearnDelFAMCode");
				$wpl = (empty($xpath[0])) ? '' : $xpath[0];
				if ($wpl != 'undefined')
					$pdf->Text(125, 96, $this->spaceout(str_pad($wpl, 1, ' ', STR_PAD_LEFT), 1));
				$xpath = $delivery->xpath("./LearningDeliveryFAM[LearnDelFAMType='EEF']/LearnDelFAMCode");
				$eef = (empty($xpath[0])) ? '' : $xpath[0];
				if ($eef != 'undefined')
					$pdf->Text(165, 96, $this->spaceout(str_pad($eef, 1, ' ', STR_PAD_LEFT), 1));
				$xpath = $delivery->xpath("./LearningDeliveryFAM[LearnDelFAMType='NSA']/LearnDelFAMCode");
				$nsa = (empty($xpath[0])) ? '' : $xpath[0];
				if ($nsa != 'undefined')
					$pdf->Text(202, 96, $this->spaceout(str_pad($nsa, 2, ' ', STR_PAD_LEFT), 5));
				$xpath = $delivery->xpath("./LearningDeliveryFAM[LearnDelFAMType='ASL']/LearnDelFAMCode");
				$asl = (empty($xpath[0])) ? '' : $xpath[0];
				if ($asl != 'undefined')
					$pdf->Text(246, 96, $this->spaceout($asl));

				$xpath = $delivery->xpath("./LearningDeliveryFAM[LearnDelFAMType='LDM']/LearnDelFAMCode");
				$ldm1 = (empty($xpath[0])) ? '' : $xpath[0];
				if ($ldm1 != 'undefined')
					$pdf->Text(30, 105, $this->spaceout(str_pad($ldm1, 1, '0', STR_PAD_LEFT), 6));
				$ldm2 = (empty($xpath[1])) ? '' : $xpath[1];
				if ($ldm2 != 'undefined')
					$pdf->Text(84, 105, $this->spaceout(str_pad($ldm2, 1, '0', STR_PAD_LEFT), 6));
				$ldm3 = (empty($xpath[2])) ? '' : $xpath[2];
				if ($ldm3 != 'undefined')
					$pdf->Text(136, 105, $this->spaceout(str_pad($ldm3, 1, '0', STR_PAD_LEFT), 6));
				$ldm4 = (empty($xpath[3])) ? '' : $xpath[3];
				if ($ldm4 != 'undefined')
					$pdf->Text(189, 105, $this->spaceout(str_pad($ldm4, 1, '0', STR_PAD_LEFT), 6));

				$xpath = $delivery->xpath("./LearningDeliveryFAM[LearnDelFAMType='SPP']/LearnDelFAMCode");
				$spp = (empty($xpath[0])) ? '' : $xpath[0];
				if ($spp != 'undefined')
					$pdf->Text(258, 105, $this->spaceout(str_pad(substr($spp, 2, 3), 1, ' ', STR_PAD_LEFT), 6));
