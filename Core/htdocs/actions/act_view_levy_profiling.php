...
SQL;
	...
	private function produceFullReport(PDO $link)
	{
		...
		if ($standard_or_framework == 'fwk') {
			...
			$framework_details = explode('-', $framework_details);
		...
		$total_learners = (int)$negotiation->learners_1618 + (int)$negotiation->learners_19;
		...
		for ($i = 1; $i <= $negotiation->expected_duration; $i++) {
			$amount = str_replace(',', '', $negotiation->monthlyLevyAmount);
			$levy_balance = floatval($levy_balance) + floatval($amount);
			...
			if ($monthly_capped_opp_amount < $levy_balance) {
				...
				if ($negotiation->total_number_of_employees < 50) {
					...
					$employer_contribution_levy = $levy_balance;
					($monthly_amount_above_cap * .05) + $monthly_amount_above_cap;
					$employer_contribution_nonlevy = ($coinvest_amount * .05) + $monthly_amount_above_cap;
					$sfa_contribution = ($coinvest_amount * .95);
					$levy_balance = 0;
				}
			}
			...
			if ($i == $negotiation->expected_duration) {
				if ($negotiation->total_number_of_employees < 50) {
					...
					if ($completion_payment > $levy_balance) {
						$obj->employer_contribution_nonlevy += $completion_payment * .05;
						$obj->completion_payment = $completion_payment * .95;
					} else {
						$obj->completion_payment = $completion_payment;
					}
				}
...
HTML;
		foreach ($negotiation->rows as $key => $value) /* @var $value NegotiationRows */ {
			...
			$amount = str_replace(',', '', $value->monthlyLevyAmount);
			$html .= '<td align="center">&pound;' . round((float)$amount) . '</td>';
