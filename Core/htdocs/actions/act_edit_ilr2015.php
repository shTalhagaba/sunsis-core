<?php
class edit_ilr2015 implements IAction
{
	public function execute(PDO $link)
	{
		...
		$_SESSION['bc']->add($link, "do.php?_action=edit_ilr2015&submission=" . $submission . "&contract_id=" . $contract_id . "&tr_id=" . $tr_id . "&L03=" . $L03, "Add/ Edit ILR Form");
		...
		{
			$vo = Ilr2015::loadFromDatabase($link, $submission, $contract_id, $tr_id, $L03);
		}
		...
		$funding_type = Ilr2015::FundingType($vo);

		// --- *
		// dynamic field displays
		// ---

		$dynamic_field_sql = 'select xml_fieldname, level, historic_al_code, readable_fieldname, fieldname_description, programme_aims, 1 as required from central.lookup_ilr_2015_fields';

		$dynamic_funding_type = strtolower($funding_type);

		$dynamic_table_types = DAO::getTableFields($link, 'central.lookup_ilr_2015_fields');
		if ( isset($dynamic_table_types[$dynamic_funding_type]) ) {
			$dynamic_field_sql = 'select xml_fieldname, level, historic_al_code, readable_fieldname, fieldname_description, programme_aims, '.$dynamic_funding_type.' as required from central.lookup_ilr_2015_fields';
		}
		...
		$TBS_dropdown = DAO::getResultset($link,"SELECT DISTINCT StandardCode, CONCAT(StandardCode, ' - ', StandardName) ,LEFT(StandardName, 1) FROM lars201516.Core_LARS_Standard ORDER BY StandardName;", DAO::FETCH_NUM, "ILR2015 TBS dropdown1");
		...
		$LDM_dropdown = DAO::getResultset($link,"SELECT distinct LearnDelFAMCode, CONCAT(LearnDelFAMCode, ' ', LearnDelFAMCode_Desc), null from lis201415.ilr_learndelfamtypefamcode where LearnDelFAMType = 'LDM' order by CAST(LearnDelFAMCode AS SIGNED);", DAO::FETCH_NUM, "ILR2014 LDM dropdown41");
		$EEF_dropdown = DAO::getResultset($link,"SELECT distinct LearnDelFAMCode, CONCAT(LearnDelFAMCode, ' ', LearnDelFAMCode_Desc), null from lis201415.ilr_learndelfamtypefamcode where LearnDelFAMType = 'EEF' order by CAST(LearnDelFAMCode AS SIGNED);", DAO::FETCH_NUM, "ILR2014 EEF dropdown2");
		...
		$SPP_dropdown = DAO::getResultset($link,"SELECT distinct LearnDelFAMCode, CONCAT(LearnDelFAMCode, ' ', LearnDelFAMCode_Desc), null from lis201415.ilr_learndelfamtypefamcode where LearnDelFAMType = 'SPP' order by LearnDelFAMCode ;", DAO::FETCH_NUM, "ILR2014 SPP dropdown2");
		...
		$WithdrawReason_dropdown = DAO::getResultset($link,"SELECT distinct WithdrawReason, CONCAT(WithdrawReason, ' ', WithdrawReason_Desc), null from lis201415.ilr_withdrawreason order by WithdrawReason;", DAO::FETCH_NUM, "ILR2014 WithdrawReason dropdown2");
		...
		$OutGrade_dropdown = DAO::getResultset($link,"SELECT distinct OutGrade, CONCAT(OutGrade, ' ', OutGrade_Desc), null from lis201415.ilr_outgrade order by OutGrade;", DAO::FETCH_NUM, "ILR2014 OutGrade dropdown2");
		$MainDelMeth_dropdown = DAO::getResultset($link,"SELECT distinct MainDelMeth, CONCAT(MainDelMeth, ' ', MainDelMeth_Desc), null from lis201415.ilr_maindelmeth order by MainDelMeth;", DAO::FETCH_NUM, "ILR2014 MainDelMeth dropdown2");
		...
		$QUALENT3_dropdown = DAO::getResultset($link,"SELECT distinct QualEnt3, CONCAT(QualEnt3, ' ', QualEnt3_Desc), null from lis201415.ilr_qualent3 order by QualEnt3;", DAO::FETCH_NUM, "ILR2014 QualEnt3 dropdown2");
		...
		$TTACCOM_dropdown = DAO::getResultset($link,"SELECT distinct TTACCOM, CONCAT(TTACCOM, ' ', TTACCOM_Desc), null from lis201415.ilr_ttaccom order by TTACCOM;", DAO::FETCH_NUM, "ILR2014 TTACCOM dropdown2");
		...
		if(DB_NAME == "am_crackerjack")
			require_once('tpl_new_edit_ilr2015.php');
		else
			require_once('tpl_edit_ilr2015.php');
//		require_once('tpl_edit_ilr2014_new.php');
	}
