<?php
class edit_ilr2010 implements IAction
{
	public function execute(PDO $link)
	{
		...
		$_SESSION['bc']->add($link, "do.php?_action=edit_ilr2010&submission=" . $submission . "&contract_id=" . $contract_id . "&tr_id=" . $tr_id . "&L03=" . $L03, "Add/ Edit ILR Form");
		...
		$linklis = new PDO(
			"mysql:host=" . DB_HOST . ";dbname=lis201011;port=" . DB_PORT . ";charset=utf8mb4",
			DB_USER,
			DB_PASSWORD,
			$options
		);
		...
		$linklad = new PDO(
			"mysql:host=" . DB_HOST . ";dbname=lad201011;port=" . DB_PORT . ";charset=utf8mb4",
			DB_USER,
			DB_PASSWORD,
			$options
		);

		$max_submission = DAO::getSingleValue($link, "SELECT submission FROM ilr INNER JOIN contracts ON contracts.id = ilr.contract_id WHERE contract_year = 2010 AND tr_id = '$tr_id' AND (CONCAT(submission,contract_year) IN (SELECT CONCAT(submission,contract_year) FROM central.lookup_submission_dates WHERE last_submission_date >= CURDATE() AND central.lookup_submission_dates.contract_type = '2' ORDER BY last_submission_date)) ORDER BY contracts.contract_year DESC LIMIT 0,1");
		...
		if ($submission == '' || $contract_id == '' || $tr_id == '') {
			...
			$vo = Ilr2010::loadFromDatabase($link, $submission, $contract_id, $tr_id, $L03);
		}
		...
		if ($submission != 'W01') {
			...
			$previous_vo = Ilr2010::loadFromDatabase($link, $previous_submission, $contract_id, $tr_id, $L03);
		...
		require_once('tpl_edit_ilr2010.php');
	}
