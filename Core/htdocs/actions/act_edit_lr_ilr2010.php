<?php
class edit_lr_ilr2010 implements IAction
{
	public function execute(PDO $link)
	{
		...
		$_SESSION['bc']->add($link, "do.php?_action=edit_lr_ilr2010&submission=" . $submission . "&contract_id=" . $contract_id . "&tr_id=" . $tr_id . "&L03=" . $L03, "Add/ Edit ILR Form");
		...
		$max_submission = DAO::getSingleValue($link, "SELECT submission FROM ilr INNER JOIN contracts ON contracts.id = ilr.contract_id WHERE contract_year = 2010 and tr_id = '$tr_id' AND (CONCAT(submission,contract_year) IN (SELECT CONCAT(submission,contract_year) FROM central.lookup_submission_dates WHERE contract_type = 1 and last_submission_date >= CURDATE() AND central.lookup_submission_dates.contract_type = '1' ORDER BY last_submission_date)) ORDER BY contracts.contract_year DESC LIMIT 0,1");
		...
		require_once('tpl_edit_lr_ilr2010.php');
	}
