...
class view_interviews_summary implements IAction
{
	public function execute(PDO $link)
	{
		$export = isset($_REQUEST['export']) ? $_REQUEST['export'] : '';
		$_SESSION['bc']->index = 0;
		$_SESSION['bc']->add($link, "do.php?_action=view_interviews_summary", "View Interviews Summary");

		$view = ViewInterviewsSummary::getInstance($link);
		$view->refresh($link, $_REQUEST);

		if ($export == 'excel')
			$this->generateXLSXOutput($link, $view);

		require_once('tpl_view_interviews_summary.php');
	}

	private function generateXLSXOutput(PDO $link, ViewInterviewsSummary $view)
	{
		$html = "";
		...
		$content->getActiveSheet()->setTitle('Interviews Summary');
