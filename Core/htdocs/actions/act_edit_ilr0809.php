<?php
class edit_ilr0809 implements IAction
{
	public function execute(PDO $link)
	{
		...
		try
		{
			$linklis = new PDO("mysql:host=$hostname;dbname=lis200809;port=$port",ini_get('mysqli.default_user'), ini_get('mysqli.default_pw'));
		}
		...
		{	
			$vo = Ilr0809::loadFromDatabase($link, $submission, $contract_id, $tr_id, $L03);
		}
		...
		if($submission!='W01')
		{
			...
			$previous_vo = Ilr0809::loadFromDatabase($link, $previous_submission, $contract_id, $tr_id, $L03);
		...
		require_once('tpl_edit_ilr0809.php');
