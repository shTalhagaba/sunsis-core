<?php
class save_ilr_2025 implements IAction
{
    ...
    private function process(PDO $link)
    {
        ...
        if($template!=1)
        {
            ...
            // Update tr status
            $vo = Ilr2025::loadFromXML($xml);
            $funding_type = Ilr2025::FundingType($vo);
            ...
            $ZprogPlanEndDate = null;
            foreach($vo->LearningDelivery as $ld)
            {
                if((string)$ld->AimType=="1")
                    $ZprogPlanEndDate = (string)$ld->LearnPlanEndDate;
                $planned_end_dates[] = Date::toMySQL($ld->LearnPlanEndDate);
            }

            if(isset($ZprogPlanEndDate))
            {
                $earliestDate = $ZprogPlanEndDate;
            }
            else
            {
                $timestamps = [];
