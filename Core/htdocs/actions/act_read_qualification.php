...
class read_qualification  implements IAction
{
    ...
    public function execute(PDO $link)
    {
        ...
        $_SESSION['bc']->add($link, "do.php?_action=read_qualification&id=" . $qualification_id . "&clients" . $clients . "&internaltitle=" . rawurlencode($internaltitle), "View Qualification");
        ...
        foreach($qual_evidences->unit AS $ev_unit)
        {
		    ...
		    $evidence->tags = isset($ev_attributes['delhours']) ? [$ev_attributes['delhours'] . ' delivery hours'] : [];
                ...
                if(count($evidences) > 1)
                    $element->tags = [count($evidences) . ' evidences', 'Element'];
                else
                    $element->tags = [count($evidences) . ' evidence', 'Element'];
