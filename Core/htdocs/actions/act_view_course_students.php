<?php
class view_course_students implements IAction
{
	public function execute(PDO $link)
	{
		...
		$_SESSION['bc']->add($link, "do.php?_action=view_course_students&id=" . $id, "Course Learners");
		...
		$vo3 = ViewCourseStudents::getInstance($link, $id);
		$vo3->refresh($link, $_REQUEST);

		require_once('tpl_view_course_students.php');
	}
	...
	private function buildView(PDO $link, $course_id)
	{
		...
		elseif($_SESSION['user']->isOrgAdmin() || $_SESSION['user']->type==User::TYPE_MANAGER)
		{
...
WHERE
	courses_tr.course_id=$course_id $where
Group By
	student_qualifications.tr_id
HEREDOC;
			
		$view = new VoltView('view_course_students', $sql); /* @var $view View */
	...
	//	{
			...
			$f = new VoltDropDownViewFilter('filter_school', $options, null, true);
			$f->setDescriptionFormat("School: %s");
			$view->addFilter($f);
/*		}
		else
		{
			// One option only -- the user's pupils only
			$options = array(
				0=>array($_SESSION['org']->id, $_SESSION['org']->short_name, null, "WHERE pot.school_id=".$_SESSION['org']->id));
			$f = new DropDownViewFilter('filter_school', $options, $_SESSION['org']->id, false);
			$f->setDescriptionFormat("School: %s");
			$view->addFilter($f);					
		}
*/		
		...
		$options = array(0=>array(1, 'Learner (asc)', null, 'ORDER BY tr.surname ASC, tr.firstnames ASC'),
			...
			2=>array(3, 'School (asc), Learner (asc)', null, 'ORDER BY organisations.short_name ASC, tr.surname ASC, tr.firstnames ASC'));
		...
		$view->setPreference('course_id',$course_id);		
