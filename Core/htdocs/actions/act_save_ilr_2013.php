<?php
class save_ilr_2013 implements IAction
{
	...
	private function process(PDO $link)
	{
		...
		if($template!=1)
		{
			...
			if(!empty($xresult))
			{
				foreach($xresult AS $key => $node)
				{
					...
					if($node->LearnActEndDate)
					{
						...
						$a34 = $node->CompStatus;
                        $a31b = new Date($node->LearnActEndDate);
                        if($a31b->after('31/07/2014'))
			...
			$previous_ilr = Ilr2013::loadFromXML($previous_ilr_xml);
			...
			if(DB_NAME=='am_reed_demo' || DB_NAME=='ams' || DB_NAME=='am_reed')
			{
				if(trim($previous_ilr->PlanLearnHours)!=trim($vo->PlanLearnHours))
				{
					...
					DAO::execute($link, "insert into ilr_audit (id, username, `date`, A09, `changed`, `from`, `to`, user_agent, tr_id, submission, contrat_id) values(NULL,'$username',NULL,'Learner','PlanLearnHours','$pplh','$plh','$user_agent','$tr_id','$sub','$contract_id');");
				}
			}
			...
			$rftitle = DAO::getSingleValue($link, "select title from contracts where id = '$contract_id' and (title LIKE '% RF %'or title LIKE '% Work Routes %')");
			$wpltitle = DAO::getSingleValue($link, "select title from contracts where id = '$contract_id' and (title LIKE '% WPL %' or title LIKE '% Flex Fund %')");
