<?php
class edit_ilr2016 implements IAction
{
    public function execute(PDO $link)
    {
        ...
        $_SESSION['bc']->add($link, "do.php?_action=edit_ilr2016&submission=" . $submission . "&contract_id=" . $contract_id . "&tr_id=" . $tr_id . "&L03=" . $L03, "Add/ Edit ILR Form");
        ...
        {
            $vo = Ilr2016::loadFromDatabase($link, $submission, $contract_id, $tr_id, $L03);
        }
        ...
        $funding_type = Ilr2016::FundingType($vo);

        // --- *
        // dynamic field displays
        // ---

        $dynamic_field_sql = 'select xml_fieldname, level, historic_al_code, readable_fieldname, fieldname_description, programme_aims, 1 as required from central.lookup_ilr_2016_fields';

        $dynamic_funding_type = strtolower($funding_type);

        $dynamic_table_types = DAO::getTableFields($link, 'central.lookup_ilr_2016_fields');
        if ( isset($dynamic_table_types[$dynamic_funding_type]) ) {
            $dynamic_field_sql = 'select xml_fieldname, level, historic_al_code, readable_fieldname, fieldname_description, programme_aims, '.$dynamic_funding_type.' as required from central.lookup_ilr_2016_fields';
        }
        ...
        $StdCode_dropdown = DAO::getResultset($link,"SELECT DISTINCT StandardCode, CONCAT(StandardCode, ' - ', StandardName) ,LEFT(StandardName, 1) FROM lars201617.Core_LARS_Standard ORDER BY StandardName;", DAO::FETCH_NUM, "ILR2015 TBS dropdown1");
        ...
        $LDM_dropdown = DAO::getResultset($link,"SELECT distinct LearnDelFAMCode, CONCAT(LearnDelFAMCode, ' ', LearnDelFAMCode_Desc), null from lis201415.ilr_learndelfamtypefamcode where LearnDelFAMType = 'LDM' order by CAST(LearnDelFAMCode AS SIGNED);", DAO::FETCH_NUM, "ILR2014 LDM dropdown5");
        ...
        $ACT_dropdown = array(
            array('1', '1 Apprenticeship funded through a contract for services with the employer'),
            array('2', '2 Apprenticeship funded through a contract for services with the Skills Funding Agency')
        );
        ...
        if(DB_NAME == "am_crackerjack")
            require_once('tpl_new_edit_ilr2016.php');
        else
            require_once('tpl_edit_ilr2016.php');
    }
