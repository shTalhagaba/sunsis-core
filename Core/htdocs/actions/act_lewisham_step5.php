<?php
class lewisham_step5 implements IAction
{
...
//		$link->query("delete from learner where L03 not in (select A03 from aim)");
		...
		$sql = "SELECT * FROM aim LEFT JOIN learner ON learner.L03 = aim.A03 WHERE A15 != 99 AND A26 != 0 AND A09 = 'ZPROG001'";
		$st = $link->query($sql);
		if($st) 
		{
			while($row = $st->fetch())
			{
				...
				$a15 = $row['A15'];
				$a26 = $row['A26'];
				$count = DAO::getSingleValue($link, "SELECT tr.id FROM tr LEFT JOIN courses_tr ON courses_tr.tr_id = tr.id LEFT JOIN courses ON courses.id = courses_tr.course_id  LEFT JOIN frameworks ON frameworks.id = courses.framework_id LEFT JOIN framework_qualifications ON framework_qualifications.framework_id = frameworks.id WHERE frameworks.framework_type = '$a15' AND frameworks.framework_code = '$a26' AND tr.l03 = '$l03'");
				if($count=='')
				{
					...
					$a26 = $row['A26'];
					$a15 = $row['A15'];
					$course_id = DAO::getSingleValue($link, "SELECT courses.id FROM courses LEFT JOIN frameworks ON frameworks.id = courses.framework_id LEFT JOIN framework_qualifications ON framework_qualifications.framework_id = frameworks.id WHERE frameworks.framework_type='$a15' and frameworks.framework_code='$a26'");
...
/*					$tr->provider_saon_start_number = $provider_location->saon_start_number;
					...
					$aimsquery = "select * from aim where A03 = '$l03' and A15 = '$a15' and A26 = '$a26' and A09!='ZPROG001' limit 0,1";
					$staimsquery = $link->query($aimsquery);
					if($staimsquery) 
					{
						while($rowaims = $staimsquery->fetch())
						{
					...
					$aimsquery = "select * from aim where A03 = '$l03' and A15 = '$a15' and A26 = '$a26' and A09!='ZPROG001' limit 1,100";
					$staimsquery = $link->query($aimsquery);
					if($staimsquery) 
					{
						while($rowaims = $staimsquery->fetch())
						{
							$ilr .= "<subaim>" . $this->createAim($row) . "</subaim>";
						}
					}
					$ilr .= "</ilr>";
				...
				//	DAO::execute($link, "insert into ilr values('$provider->upin','$a03', '$a09', '$ilr', 'W03', 'ER', '$tr->id', '1', '1', '1', '1', $tr->contract_id)");
					...
					$course_id = DAO::getSingleValue($link, "SELECT courses.id FROM courses LEFT JOIN frameworks ON frameworks.id = courses.framework_id LEFT JOIN framework_qualifications ON framework_qualifications.framework_id = frameworks.id WHERE frameworks.framework_type='$a15' and frameworks.framework_code='$a26'");
					$framework_id = DAO::getSingleValue($link, "SELECT frameworks.id FROM courses LEFT JOIN frameworks ON frameworks.id = courses.framework_id LEFT JOIN framework_qualifications ON framework_qualifications.framework_id = frameworks.id WHERE frameworks.framework_type='$a15' and frameworks.framework_code='$a26'");
					$tr_id = $tr->id;
					if($course_id=='')
							pre($l03);
