...
class edit_org_crm_note implements IAction
{
  ...
  ')'
), null FROM organisation_contact WHERE org_id = '{$organisation->id}' ORDER BY contact_name");
  ...
  COALESCE(`contact_mobile`, ''),
  ')'
), null FROM organisation_contact WHERE org_id = '{$organisation_id}' ORDER BY contact_name";
		$st = $link->query($sql);
		if($st)
		{
			echo "<?xml version=\"1.0\" ?>\r\n";
