<?php
class edit_employer_agreement_form implements IUnauthenticatedAction
{
    public function execute(PDO $link)
    {
        $tr_id = isset($_REQUEST['employer_id']) ? $_REQUEST['employer_id'] : '';
        $id = isset($_REQUEST['id']) ? $_REQUEST['id'] : '';
        $key = isset($_REQUEST['key']) ? $_REQUEST['key'] : '';
        $output = isset($_REQUEST['output']) ? $_REQUEST['output'] : '';
        $source = isset($_REQUEST['source']) ? $_REQUEST['source'] : '';

        $employer_name= DAO::getSingleValue($link,"select legal_name from organisations where id = '$tr_id'");

        $employer_address = "";
        $keytoverify = md5("PerspectiveSunesisemployer_id=".$tr_id."source=".$source."id=".$id);
        if(!isset($_SESSION['user']))
            if($key!=$keytoverify)
                pre("Invalid URL");

        $form = EmployerAgreementForm::loadFromDatabase($link,$id);

        if($source==3 and $form->signature_employer_font!='')
            pre("Agreement is complete");
...
<table class="table1" style="width: 900px">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>NI Number:</td>
            <td>'.$form_assessor1->learner_ni.'</td>
        </tr>
        <tr>
            <td>Employer Name:</td>
    ...
    <tr>
        <td>
            <table>
                <tr>
                    <td>Behaviours:</td>
                    <td colspan=3><textarea name="behaviours" rows="4" cols="100">'.$form_employer->behaviours.'</textarea></td>
                </tr>
                <tr>
                    <td>Ability:</td>
                    <td colspan=3><textarea name="ability" rows="4" cols="100">'.$form_employer->ability.'</textarea></td>
                </tr>
                <tr>
                    <td>Skills and Knowledge:</td>
                    <td colspan=3><textarea name="skills_knowledge" rows="4" cols="100">'.$form_employer->skills_knowledge.'</textarea></td>
                </tr>
                <tr>
                    <td>Achievements/ progress at work:</td>
                    <td colspan=3><textarea name="achievements" rows="4" cols="100">'.$form_employer->achievements.'</textarea></td>
                </tr>
            </table>
        </td>
    </tr>
...
//==============================================================
        ...
        include('tpl_edit_employer_agreement_form.php');
