...
class assessment_dashboard2 implements IAction
{
        public function execute(PDO $link)
        {
                ...
                if ($subaction == 'show_cm_leaver') {
                        echo $this->show_cm_leaver($link);
                        exit;
                }

                $_SESSION['bc']->index = 0;
                $_SESSION['bc']->add($link, "do.php?_action=assessment_dashboard2", "Assessment Dashboard");
...
HEREDOC;
        ...
        private function showInductionDashPanels(PDO $link)
        {
                ...
                $where_due = " and GREATEST(sub.due_date,COALESCE(sub.extension_date,'1900-01-01')) between '$start_date' and '$end_date'";
...
AND GREATEST(sub.due_date,COALESCE(sub.extension_date,'1900-01-01')) >= CURDATE() AND tr_id IN (SELECT id FROM tr WHERE status_code = 1)
...
LEFT JOIN project_submissions AS sub ON sub.project_id = tr_projects.id AND
	sub.id = (SELECT MAX(id) FROM project_submissions WHERE project_submissions.project_id = tr_projects.id)
WHERE  sub.iqa_rework_awaiting_marking IS NULL and sub.`completion_date` IS NULL AND COALESCE(iqa_status, 0)!=2 AND sent_iqa_date IS NULL AND submission_date IS NOT NULL
AND tr_id IN (SELECT id FROM tr WHERE status_code = 1) $where_assessorev $where_manager;");
...
(
	...
	AND sub.`completion_date` IS NULL AND iqa_status IS NULL AND sent_iqa_date IS NULL AND submission_date IS NULL AND set_date IS NOT NULL and GREATEST(sub.due_date,COALESCE(sub.extension_date,'1900-01-01')) >= CURDATE()
))
AND tr_id IN (SELECT id FROM tr WHERE status_code = 1)
 $where_assessorev $where_manager;");

                $total_iqa_rejected_evidence_matrix = DAO::getSingleValue($link, "SELECT COUNT(*) FROM tr_projects
LEFT JOIN project_submissions AS sub ON sub.project_id = tr_projects.id AND
	sub.id = (SELECT MAX(id) FROM project_submissions WHERE project_submissions.project_id = tr_projects.id)
WHERE  COALESCE(iqa_status, 0)=2 and completion_date is null
AND tr_id IN (SELECT id FROM tr WHERE status_code = 1)
 $where_assessorev $where_manager;");
...
AND tr_id IN (SELECT id FROM tr WHERE status_code = 1) $where_assessorev $where_manager;");
                ...
                $total_iqa_rejected_trs_array   = $total_iqa_rejected_trs   ? explode(",", $total_iqa_rejected_trs)   : [];
                $total_iqa_rejected_trs_arrayev = $total_iqa_rejected_trsev ? explode(",", $total_iqa_rejected_trsev) : [];
...
LEFT JOIN project_submissions AS sub ON sub.project_id = tr_projects.id AND
	sub.id = (SELECT MAX(id) FROM project_submissions WHERE project_submissions.project_id = tr_projects.id AND project_submissions.submission_date IS NULL)
        WHERE GREATEST(due_date,COALESCE(extension_date,'1900-01-01')) between '$start_date' and '$end_date' $where_assessorev $where_manager;");

                $gt_due_today = $due_today + $due_todayev;

                $iqa_recheck = DAO::getSingleValue($link, "SELECT count(*)
FROM tr_projects
LEFT JOIN project_submissions AS sub ON sub.project_id = tr_projects.id AND
	sub.id = (SELECT MAX(id) FROM project_submissions WHERE project_submissions.project_id = tr_projects.id)
        WHERE iqa_recheck_date is not null and completion_date is null $where_assessorev $where_manager;");

                $iqa_rework_awaiting_marking = DAO::getSingleValue($link, "SELECT count(*)
FROM tr_projects
LEFT JOIN project_submissions AS sub ON sub.project_id = tr_projects.id AND
	sub.id = (SELECT MAX(id) FROM project_submissions WHERE project_submissions.project_id = tr_projects.id)
        WHERE iqa_rework_awaiting_marking is not null and completion_date is null $where_assessorev $where_manager;");
...
LEFT JOIN courses ON courses.id = courses_tr.`course_id`
WHERE
status_code = 1 and courses.programme_type=2 and assessment_evidence = 2
AND tr.id NOT IN (SELECT tr_id FROM tr_operations WHERE tr_operations.tr_id = tr.id AND extractvalue(tr_operations.bil_details, '/Notes/Note[last()]/Type')='F')
$where_assessorev $where_manager;
";
                $ontrackev = [];
	...
	<div class="small-box bg-yellow">
    	...
    	<div>
    	    <b>
		        ...
		        <div class="col-xs-6 bg-success"><a href="do.php?_action=view_training_records_v2&_reset=1&ViewTrainingRecordsV2_filter_tr_ids=$behindiqatrs" class="small-box-footer">View Report <i class="fa fa-arrow-circle-right"></i></a></div>
		        <div class="col-xs-6 bg-success"><a href="do.php?_action=view_training_records_v2&_reset=1&ViewTrainingRecordsV2_filter_tr_ids=$behindiqatrsev" class="small-box-footer">View Report <i class="fa fa-arrow-circle-right"></i></a></div>
		        <!--<div class="col-xs-6 bg-success"><a href="do.php?_action=view_assessment_plan_logs2&_reset=1&ViewAssessmentPlanLogs2_filter_assessment_plans=$behindiqatrs" class="small-box-footer">View Report <i class="fa fa-arrow-circle-right"></i></a></div>
...
<div class="col-lg-6 col-xs-6">
	<div class="small-box bg-green">
		<div class="inner">
			<h1>$total_iqa_rejected_evidence_matrix</h1>
			<p>Total IQA Rejected</p>
		</div>
		<div class="icon"><i class="fa fa-users"></i> </div>
		<div class="col-xs-12 bg-success"><a href="do.php?_action=view_evidence_matrix_projects&_reset=1&ViewEvidenceMatrixProjects_filter_paperwork=8&ViewEvidenceMatrixProjects_filter_assessor=$assessor&ViewEvidenceMatrixProjects_filter_manager=$manager" class="small-box-footer">View Report <i class="fa fa-arrow-circle-right"></i></a></div>
	</div>
</div>
HTML;
...
<div class="col-lg-6 col-xs-6">
	<div class="small-box bg-aqua">
		...
		<div class="icon"><i class="fa fa-users"></i> </div>
		<div class="col-xs-12 bg-success"><a href="do.php?_action=view_evidence_matrix_projects&_reset=1&ViewEvidenceMatrixProjects_filter_paperwork=9&ViewEvidenceMatrixProjects_filter_assessor=$assessor&ViewEvidenceMatrixProjects_filter_manager=$manager" class="small-box-footer">View Report <i class="fa fa-arrow-circle-right"></i></a></div>
	</div>
</div>
HTML;
...
<div class="col-lg-6 col-xs-6">
	<div class="small-box bg-aqua">
		<div class="inner">
			<h1>$iqa_rework_awaiting_marking</h1>
			<p>IQA Rework Awaiting Marking</p>
		</div>
		<div class="icon"><i class="fa fa-users"></i> </div>
		<div class="col-xs-12 bg-success"><a href="do.php?_action=view_evidence_matrix_projects&_reset=1&ViewEvidenceMatrixProjects_filter_paperwork=10&ViewEvidenceMatrixProjects_filter_assessor=$assessor&ViewEvidenceMatrixProjects_filter_manager=$manager" class="small-box-footer">View Report <i class="fa fa-arrow-circle-right"></i></a></div>
	</div>
</div>
HTML;
...
WHERE signature_learner_font IS NULL AND signature_assessor_font IS NOT NULL AND tr.status_code = 1 $assessor_where $where_manager);");
                $total_employer_incompleted = DAO::getSingleValue($link, "SELECT COUNT(DISTINCT assessor_review.id) FROM assessor_review
LEFT JOIN tr ON tr.id = assessor_review.tr_id
LEFT JOIN arf_introduction ON arf_introduction.`review_id` = assessor_review.id
WHERE signature_learner_font IS NOT NULL AND signature_employer_font IS NULL AND tr.status_code = 1 $assessor_where $where_manager;");
                $total_incomplete = DAO::getSingleValue($link,          "SELECT
(SELECT COUNT(DISTINCT assessor_review.id) FROM assessor_review
		LEFT JOIN tr ON tr.id = assessor_review.tr_id
		LEFT JOIN arf_introduction ON arf_introduction.`review_id` = assessor_review.id
		WHERE signature_learner_font IS NULL AND signature_assessor_font IS NOT NULL AND tr.status_code = 1 $assessor_where $where_manager)+
...
HTML;
        ...
        private function show_cm_leaver(PDO $link)
        {
                $from_date = isset($_REQUEST['filter_cm_month']) ? new Date(date($_REQUEST['filter_cm_year'] . '-' . date('m', strtotime($_REQUEST['filter_cm_month'])) . '-01')) : new Date(date('Y-m-d'));
                $leaver_start_date = $from_date->getYear() . '-' . $from_date->getMonth() . '-01';
                $last_day_of_month = cal_days_in_month(CAL_GREGORIAN, $from_date->getMonth(), $from_date->getYear());
                $leaver_end_date = $from_date->getYear() . '-' . $from_date->getMonth() . '-' . $last_day_of_month;
                $tbl_caseload_management = "(SELECT m1.* FROM caseload_management m1 LEFT JOIN caseload_management m2 ON (m1.tr_id = m2.tr_id AND m1.id < m2.id) WHERE m2.id IS NULL) AS caseload_management";
                $count = DAO::getSingleValue($link, "SELECT COUNT(DISTINCT tr_id) FROM {$tbl_caseload_management} WHERE caseload_management.destination IN ('Leaver', 'Direct Leaver - No intervention') AND caseload_management.closed_date BETWEEN '{$leaver_start_date}' AND '{$leaver_end_date}'");
...
<div class="small-box bg-red">
        ...
        <a href="do.php?_action=view_caseload_management_report&filter_destination=1&from_closed_date=$leaver_start_date&to_closed_date=$leaver_end_date&_reset=1" class="small-box-footer">Click to see <i class="fa fa-arrow-circle-right"></i></a>
</div>        
HTML;
