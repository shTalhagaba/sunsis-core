<?php
class validate_ilr2013 implements IAction
{
    public function execute(PDO $link)
    {
        ...
        if($report != 'No Error')
        {
			...
			$sql = "UPDATE tr SET tr.ilr_status = 0 WHERE tr.id = " . $tr_id . " AND tr.contract_id = " . $contract_id;
	        DAO::execute($link, $sql);
        ...
        else
        {
            ...
            DAO::execute($link, $sql);
			$sql = "UPDATE tr SET tr.ilr_status = 1 WHERE tr.id = " . $tr_id . " AND tr.contract_id = " . $contract_id;
	        DAO::execute($link, $sql);
            header("Content-Type: text/xml");
            echo '<?xml version="1.0"?><report><success l03="' . htmlspecialchars((string)$ilr->LearnRefNumber->L03) . '" /></report>';
