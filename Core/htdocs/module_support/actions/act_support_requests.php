...
class support_requests implements IAction {
    ...
    public function execute(PDO $link) {
        ...
        if ( $_SESSION['user']->isAdmin() || (DB_NAME=="am_baltic" && in_array($_SESSION['user']->username, ["pahodgson"])) )
        {
            if($filter_username != '')
            {
                ...
                $case = $this->client->get_entry_list($this->session_id, 'Cases',' `cases`.`created_by` = "ad7f7f13-2dcf-21c8-b931-4c8e1f495088" and `cases`.`name` = "Support request: '.$filter_user_complete_name.' of '.$_SESSION['user']->org->legal_name.'" ', ' `cases`.`status` desc, `cases`.`date_entered` desc ',0,'',3000);
            }
            else
                $case = $this->client->get_entry_list($this->session_id, 'Cases',' `cases`.`created_by` = "ad7f7f13-2dcf-21c8-b931-4c8e1f495088" and `cases`.`name` like "Support request: % of '.$_SESSION['user']->org->legal_name.'" ', ' `cases`.`status` desc, `cases`.`date_entered` desc ',0,'',3000);
        }
        else
            $case = $this->client->get_entry_list($this->session_id, 'Cases',' `cases`.`created_by` = "ad7f7f13-2dcf-21c8-b931-4c8e1f495088" and `cases`.`name` = "Support request: '.$_SESSION['user']->firstnames.' '.$_SESSION['user']->surname.' of '.$_SESSION['user']->org->legal_name.'" ', ' `cases`.`status` desc, `cases`.`date_entered` desc ',0,'',3000);
        ...
        foreach( $case->entry_list as $field ) {
            ...
            if ( array_key_exists($this_client_case['status'], $case_type_groups) ) {
                ...
                $this_case .= "<td colspan='5'>".mb_convert_encoding($this_client_case['description'],'UTF-8')."</td>";
                $this_case .= "</tr>";

                $caseToExport['description'] = preg_replace('/\n/', ' ', mb_convert_encoding($this_client_case['description'],'UTF-8'));
            ...
            if ( strval(intval($_REQUEST['case_number'])) === $_REQUEST['case_number'] && $_REQUEST['case_number'] >= 10000 && $_REQUEST['case_number'] <= 99999 ) {
                ...
                */
                ...
                if ( isset($_REQUEST['case-comment']) && $_REQUEST['case-comment'] != '' ) {
                    $update_text = mb_convert_encoding(htmlspecialchars((string)$_REQUEST['case-comment']),'UTF-8');
