...
class rec_vacancies_home implements IAction
{
	public function execute(PDO $link)
	{
		$_SESSION['bc']->index=0;
		$_SESSION['bc']->add($link, "do.php?_action=rec_vacancies_home", "e-Recruitment Homepage");

		$diary_filter_region = isset($_REQUEST['diary_filter_region'])?$_REQUEST['diary_filter_region']:'';

		require_once('tpl_rec_vacancies_home.php');
	}
...
HEREDOC;
		...
		if( $result = $link->query($sql_request_retrieval) )
		{
			...
			while( $row = $result->fetch() )
			{
				...
				if ( !is_null($row['screening']) AND $row['screening'] == 'R' ) {
				...
				elseif ( !is_null($row['screening']) AND $row['screening'] == 'A' )
				{
				...
				elseif ( !is_null($row['screening']) AND $row['screening'] == 'G' )
				{
